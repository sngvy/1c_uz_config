...
Процедура ЗавершитьСхему(ГрафическаяСхема, ТекущийЭлемент) Экспорт
	...
	СтрЗавершения = "{{{4,"+ Строка(ИндексЗавершения)  +",{1,1,{"+Символ(34) +"#"+ Символ(34)+","+Символ(34)+"Завершение"+Символ(34)+"}}"  +  "," +Символ(34)+  "Завершение" +Символ(34)+",6},4,cf1fff5b-b11b-45bc-aa6d-6e9558df5623,0},2,{{{{6,{3,4,{0}},{3,3,{-22}},{3,3,{-3}},{6,3,0,1},{1,0},1,1,1,5,0,0,0},5," +Строка(МаксПраво)+ ","+ Строка(МаксНиз)+","+Строка(140+МаксПраво)+","+ Строка(80 + МаксНиз)+",5," +Строка(70 + МаксПраво)+ ","+ Строка(МаксНиз)+"," +Строка(139+МаксПраво)+ ","+ Строка(40+МаксНиз)+"," +Строка(139+МаксПраво)+ ","+ Строка(79+МаксНиз)+"," +Строка(МаксПраво)+ ","+ Строка(79+МаксНиз)+"," +Строка(МаксПраво)+ ","+ Строка(40+МаксНиз)+",4,{3,0,{0},"",-1,-1,1,0},{4,0,{0},1,1,0,e45c0cd8-a878-4bcb-8e1a-af934481e1cc,0}},1}},{0}},7}";
	...
	КонецПопытки;
	УдалитьФайлы(пИмяФайла); //!!! поидее надо раскоментить
КонецПроцедуры
...
Функция СоздатьУзел(ТекущийЭлемент, ГрафическаяСхема, ГрафическийЭлемент, Наименование = "",
	...
	Если ТипЗнч(ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") И 
	    ...
	    ИмяНачВариант = ТекущийЭлемент.НачалоВариант.Наименование;
		МассивВыбор = ТекущийЭлемент.НачалоЭлемент.Элементы;
		СчетчВыборки = 0;
		Для каждого Вар Из МассивВыбор Цикл
			Если Вар.Наименование = ИмяНачВариант Тогда
	...
	Если ТипЗнч(ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") и 
	    ...
	    ИндексДляОтрисовки = ПолучитьИндексТекущегоЭлементаПоСтруктура(ТекущийЭлемент.НачалоЭлемент, СтруктураГрафическойСхемы)+1;	 		
	    КоординатыЛинии    = ОпределитьКоординатыЛинии(ДекоративнаяЛиния, Координаты, Направление);
	    УдалитьПредыдЛинию = Истина;	   
	    ВарДляОтрисовки = -1;    
	    Удалять = Ложь; 	
...
Процедура ОтрисовкаЛинийДляТелефонныйЗвонокИсходящий(ГрафическаяСхема, СтруктураГрафическойСхемы, Элемент)
	...
	//	СтруктураЛинии = ДобавитьЛинию(СтруктураГрафическойСхемы, , , ИндексТекущегоЭлемента+1, ИндексТекущегоЭлемента+2+Счетчик, КоординатыЛинии.НачалоСторона, КоординатыЛинии.КонецСторона, , , , Ложь);  
		...
		СтруктураЛинии = ДобавитьЛинию(СтруктураГрафическойСхемы, , ,ИндексТекущегоЭлемента+1, ,ИндексЛинииВарианта , , , , , Ложь,,НачалоВарианта );  

		ПровестиЛиниюМеждуТочками(СтруктураЛинии, 
		КоординатыЛинии.НачалоЛево, 
		СмещениеЭлемента, 
		КоординатыЛинии.НачалоЛево+20,
		СмещениеЭлемента);
...
Функция ПолучитьСтруктуруНеПустойГрафическойСхемы(ГрафическаяСхема) Экспорт
	...
	Для каждого Элемент Из ГрафическаяСхема.ЭлементыГрафическойСхемы Цикл	
		...
		ИначеЕсли ТипЭлемента = Тип("ЭлементГрафическойСхемыДекоративнаяЛиния")
			...
			СвойстваЛинии = ПолучитьСвойстваЛинии(Элемент, ГрафическаяСхема, ТипЭлемента);
...
// Возвращаемое значение:
//   Структура
Функция ПолучитьСвойстваЛинии(Линия, ГрафическаяСхема, ТипЛинии = Неопределено)
	Если ТипЗнч(Линия.НачалоВариант) <> Тип("ВариантЭлементаГрафическойСхемыВыборВарианта") Тогда			
		ТипСоединительная = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния");
		ТипДекоративная   = Тип("ЭлементГрафическойСхемыДекоративнаяЛиния");
		...
		Если ТипЛинии = ТипДекоративная Тогда	
	...
	Иначе	
		Если ТипЗнч(Линия.НачалоЭлемент) = Тип("ЭлементГрафическойСхемыВыборВарианта") Тогда	
			ИмяНачВариант = Линия.НачалоВариант.Наименование;
			МассивВыбор = Линия.НачалоЭлемент.Элементы;
			СчетчВыборки = 0;
			Для каждого Вар Из МассивВыбор Цикл		
				Если Вар.Наименование = ИмяНачВариант Тогда	
					НомерВар = СчетчВыборки;
			...
			НачСтрононана = 7 + 2*НомерВар;	
			СтруктураЛинии = Новый Структура;
		    СтруктураЛинии.Вставить("НачалоЭлемент", ПолучитьИндексТекущегоЭлемента(Линия.НачалоЭлемент, 
					ГрафическаяСхема) + 1);
	    	СтруктураЛинии.Вставить("КонецЭлемент", ПолучитьИндексТекущегоЭлемента(Линия.КонецЭлемент, 
					ГрафическаяСхема) + 1);
...
Функция ДобавитьТочкуЛинии(пЛиния, пЛево, пВерх) Экспорт
...
Функция мПолучитьТекстБлокаИмени(пСтруктураИмени)
	Возврат "{4,"+пСтруктураИмени.Номер+",{1,"
	+?(ПустаяСтрока(пСтруктураИмени.Наименование), "0", "1,{""#"","""+пСтруктураИмени.Наименование+"""}")+"},"
	+""""+пСтруктураИмени.Идентификатор+""","+пСтруктураИмени.Номер+"}";
КонецФункции
...
Функция СохранитьГрафическуюСхему(пСтруктураГрафическойСхемы, пИмяФайла, СтрЗаверш="") Экспорт
	...
	Если СтрЗаверш <> "" Тогда
		...
		КолЭлементов = Число(Лев(Сред(ТекстСхемы,Найти(ТекстСхемы,Символы.ПС) + 1),
...
Функция ОтобразитьГрафическуюСхему(пСтруктураГрафическойСхемы, пПолеГрафическойСхемы, СтрЗаверш="") Экспорт
...
Функция мПолучитьЛексему(пТекст, пПозиция, пКонец)
	...
	Пока Истина Цикл
		...
		Если лСимвол=Символы.ПС Тогда пПозиция=пПозиция+1; Продолжить; КонецЕсли; 
		Если лСимвол="" Тогда Прервать; КонецЕсли;
		Если лВСтроке Тогда
			пПозиция=пПозиция+1; 
			Если лСимвол="""" Тогда Возврат лЛексема; КонецЕсли;
			лЛексема=лЛексема+лСимвол;
			Продолжить;
		КонецЕсли;
		Если Найти("{},", лСимвол)<>0 Тогда
			Если ПустаяСтрока(лЛексема) Тогда пПозиция=пПозиция+1; Возврат лСимвол; КонецЕсли;
			Прервать;
		КонецЕсли;
		пПозиция=пПозиция+1;
		Если лСимвол="""" Тогда лВСтроке=Истина; Продолжить; КонецЕсли;
		лЛексема=лЛексема+лСимвол;
	КонецЦикла;
	Если лЛексема="" Тогда пКонец=Истина; Возврат ""; КонецЕсли; 
	Если СтрДлина(лЛексема)=36 Тогда Возврат Новый УникальныйИдентификатор(лЛексема); КонецЕсли;
	Попытка	Возврат Число(лЛексема); Исключение Возврат 0; КонецПопытки; 
КонецФункции
...
Функция мДобавитьВСтруктуру(пСтруктура, пТекст, пПозиция)
	лОжидаетсяИдентификатор=Истина; лКонец=Ложь;
	Пока Истина Цикл
		лЛексема=мПолучитьЛексему(пТекст, пПозиция, лКонец);
		Если лКонец Тогда Возврат Истина; КонецЕсли; // Завершилось
		Если лЛексема="}" Тогда Возврат Истина; КонецЕсли;
		Если Не лОжидаетсяИдентификатор Тогда 
			Если лЛексема="," Тогда лОжидаетсяИдентификатор=Истина; Продолжить; КонецЕсли; 
...
//!
Процедура РасставитьЭлементыВДереве(Дерево, КореньДерева, Верх, Лево, РазмерСхемыЛево = 0, РазмерСхемыВерх = 0) 
		Экспорт
...
Процедура РасставитьЭлементыВДеревеСУчетомВыбранных2(Дерево, КореньДерева, Знач Верх, Знач Лево) Экспорт
	...
	Если КореньСтрока.Выбран Тогда
		Для Номер = 1 По КореньСтрока.КоличествоДетей Цикл
			Ребенок = Дерево.НайтиСтроки(Новый Структура("Родитель, Номер", КореньСтрока.Идентификатор, 
					Номер))[0];
			Ребенок.НеУдалять = Истина;
			РасставитьЭлементыВДеревеСУчетомВыбранных2(Дерево, Ребенок.НомерСтроки - 1, Верх, 
					Лево + КореньСтрока.Ширина + 40);
		    Верх = Верх + КореньСтрока.Высота + 20;
		КонецЦикла;	
	КонецЕсли;						
...
Процедура СинхронизироватьИменаГрафическойСхемыСИдентификаторамиДерева(ГрафическаяСхема, Дерево) Экспорт
	...
	Для Каждого ЭлементСГС Из СтруктураГрафическойСхемы.Элементы Цикл
		Если ТипЗнч(ЭлементСГС._СвойстваЭлемента) <> Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") И 
			...
			ТекущаяСтрока = Дерево.НайтиСтроки(Новый Структура("Идентификатор", 
					ЭлементСГС._СвойстваЭлемента.Имя))[0];
...
Процедура ДобавитьСписокЭлементов_ДобавитьЭлементСтадияВверх(Дерево, КартаОригинал, СтруктураГрафическойСхемы, 
	...
	СтрокаРодитель = Дерево.НайтиСтроки(Новый Структура("Идентификатор", РодительТекущегоЭлемента.Имя))[0];
