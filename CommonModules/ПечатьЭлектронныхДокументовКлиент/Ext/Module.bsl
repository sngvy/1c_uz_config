...
Процедура ПечатьОбработка(Объект, Печатать = Ложь, ПрикрепитьФайлы) Экспорт
	...
	Таблица = Новый Структура("Объект, ВидПечатногоДокумента, РегистрационныйНомер", Объект.Объект, Объект.Шаблон, РегистрационныйНомер);
	...
	ИначеЕсли КоличествоШаблонов > 1 Тогда	
		...
		Иначе 
			...
			Если Массив_НаПечать<>Неопределено Тогда
				...
				////  ПРИКРЕПЛЕНИЕ ФАЙЛОВ К ОБЪЕКТУ
				ПрикрепитьИЗарегистрировать(Объект, Массив_НаПечать, ПрикрепитьФайлы, ВидПечатногоДокумента.ОднаИсходящаяКорреспонденция)
				//Если ПрикрепитьФайлы Тогда
				//	Массив_НаПрикрепление = ПолучитьМассивНаПрикрепление(Массив_НаПечать);
...
Функция СформироватьИмяФайла(ОписаниеОснования, ИмяПечатнойФормы, Шаблон)
	...
	Возврат Проверки.УбратьЗапрещенныеСимволыВИмениФайла(ИмяОснования + "_" + ИмяШаблона);
