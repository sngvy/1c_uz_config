...
Процедура ПриОткрытииНаСервере()
	Если ЭтаФорма.ДолговоеОбязательство.Пустая() = Ложь Тогда
...
Процедура ОбъектПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
		...
		ПолучитьДниПросрочкиНаСервере();
		ПолучитьВозрастНаСервере();
		ПоказатьОповещениеПользователя("Предупреждение",,"Номер договора был изменен. Проверьте правильность выбранного контакта должника",БиблиотекаКартинок.NC_РозыскДолжника);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьСостояниеБИТФон()
	...
	ИначеЕсли ФормаНабора.Открыта() = Ложь ИЛИ ФормаНабора.СтатусПодключен = Ложь Тогда
		ЭтаФорма.Элементы.ОтключитьМикрофон.ЦветФона = Новый Цвет();
	КонецЕсли;
	// Снять сотрудника с линии при создании мероприятия
	ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
