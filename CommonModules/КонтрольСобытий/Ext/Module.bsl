...
Функция РезультативныеЗвонки(Должник) Экспорт
	...
	Если Константы.КоличествоРезультативныхЗвонков.Получить() Тогда
		...
		|	И Мероприятие.ТипМероприятия В (&СписокТипов)");
		Запрос.УстановитьПараметр("НачалоДня", НачалоДня(ТекущаяДатаСеанса()));
		Запрос.УстановитьПараметр("Должник", Должник);
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Справочники.ТипыМероприятий.НайтиПоНаименованию("Телефонный звонок исходящий"));
		МассивТипов.Добавить(Справочники.ТипыМероприятий.НайтиПоНаименованию("Телефонный звонок входящий"));
		Запрос.УстановитьПараметр("СписокТипов", МассивТипов);
		Результат = Запрос.Выполнить().Выбрать();
		Если Результат.Следующий() Тогда
			Если Результат.ЧислоРезультативных >= КоличествоЗвонковДень Тогда
		...
		Результат = Запрос.Выполнить().Выбрать();
		Если Результат.Следующий() Тогда
			Если Результат.ЧислоРезультативных >= КоличествоЗвонковНеделя Тогда
		...
		Результат = Запрос.Выполнить().Выбрать();
		Если Результат.Следующий() Тогда
			Если Результат.ЧислоРезультативных >= КоличествоЗвонковМесяц Тогда
...
Функция РезультативныеЗвонкиМесяц(Должник) Экспорт
	...
	Результат = Запрос.Выполнить().Выбрать();
	Если Результат.Следующий() Тогда
		Если Результат.ЧислоРезультативных >= Константы.ЗвонкиМесяц.Получить() Тогда
...
Функция КонтрольЧасовогоПояса(Должник) Экспорт
	Если Константы.ЧасовойПоясЗвонка.Получить() Тогда
		...
		ИначеЕсли НЕ ОшибочныйОтрицательныйПояс и Должник.ЧасовойПояс = "EET" Тогда  
			КалининградскоеВремя = МестноеВремя(УниверсальноеВремя, Должник.ЧасовойПояс) - (СмещениеСтандартногоВремени(Должник.ЧасовойПояс, УниверсальноеВремя)/2);
		...
		ЧП = Константы.ЧасовойПояс.Получить();
		ДомашнееВремя = МестноеВремя(УниверсальноеВремя, ЧП);
		ЗаполнитьИнтервал(ДомашнееВремя,Интервал);
		...
		Если МестноеВремя <> Неопределено Тогда // Это МСК+0 
			Если МестноеВремя < Интервал.ВремяОт Или МестноеВремя > Интервал.ВремяДо Тогда
				Возврат Истина;
			Иначе 
				Возврат Ложь;
			КонецЕсли;
		ИначеЕсли КалининградскоеВремя <> Неопределено Тогда // Это МСК-1
			Если КалининградскоеВремя < Интервал.ВремяОт или КалининградскоеВремя > Интервал.ВремяДо Тогда
	...
	Если Константы.КоличествоСМС.Получить() Тогда
		...
		Пока Результат.Следующий() Цикл
			Если Результат.КоличествоСообщений >= КоличествоСМСДень Тогда
				Сообщить("Количество сообщений сегодня: "+КоличествоСМСДень);
				Возврат Истина;
			Иначе
			КонецЕсли;
		КонецЦикла;	
		...
		Пока Результат.Следующий() Цикл
			Если Результат.КоличествоСообщений >= КоличествоСМСНеделя Тогда
				Сообщить("Количество сообщений на этой неделе: "+КоличествоСМСНеделя);
				Возврат Истина;
			Иначе
			КонецЕсли;
		КонецЦикла;
		...
		Пока Результат.Следующий() Цикл
			Если Результат.КоличествоСообщений >= КоличествоСМСМесяц Тогда
				Сообщить("Количество сообщений в этом месяце: "+КоличествоСМСМесяц);
				Возврат Истина;
				Прервать;
			Иначе
				Возврат Ложь;
