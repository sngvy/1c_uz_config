...
Функция СведенияОТелефоне(КонтактнаяИнформация) Экспорт
	...
	Результат.Представление = Строка(ТелефонаПоПолям.Value);
	Результат.КодСтраны     = Строка(ТелефонаПоПолям.CountryCode);
	Результат.КодГорода     = Строка(ТелефонаПоПолям.AreaCode);
	Результат.НомерТелефона = Строка(ТелефонаПоПолям.Number);
	Результат.Добавочный    = Строка(ТелефонаПоПолям.ExtNumber);
	Результат.Комментарий   = Строка(ТелефонаПоПолям.Comment);
