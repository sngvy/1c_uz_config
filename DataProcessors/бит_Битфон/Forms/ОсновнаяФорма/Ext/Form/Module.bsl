...
&НаКлиенте
Процедура ОбработчикСобытияРегистрация(Данные) Экспорт
...
Процедура ОбработчикСобытияВходящийЗвонок(Данные)
	...
	ТекстЗаголовка = "Вас вызывает номер " + НомерВходящий;
	Текст = Строка(контактВходящий);
	Если ЗначениеЗаполнено(контактноеЛицо) Тогда
		Текст = Текст + " (" + Строка(контактноеЛицо) + ")";
	КонецЕсли;
	ПоказатьОповещениеПользователя(ТекстЗаголовка, , Текст,БиблиотекаКартинок.бит_КартинкаБитфон_48);
...
Процедура ОбработкаСостоянияОбычногоКанала(Канал, Состояние, ВходящийВызов, НомерЗвонка, стрНомерДляИстории,
	...
	ИначеЕсли Состояние = "ЗвонокПрерван" Тогда 	//Обработчик завершения разговора
		...
		КонецЕсли;
		Сигнал();
		// ПоказатьОповещениеПользователя("БИТ.Phone",,"Разговор завершен",БиблиотекаКартинок.Информация32);
	...
	ИначеЕсли Состояние = "ЗвонокОтклонён" Тогда 	//На удаленной стороне сбросили вызов не поднимая трубку.
		...
		КонецЕсли;
		Сигнал();
		ПоказатьОповещениеПользователя("БИТ.Phone",,"Не удалось дозвониться",БиблиотекаКартинок.бит_КартинкаБитфон_48);		
...
Процедура ПеревестиЗвонокНаКанале(Канал, стрНомерПеревода)
	...
	Если типПеревода = ПредопределенноеЗначение("Перечисление.бит_ТипПереводаЗвонка.Условный") Тогда
		...
		бит_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Выполняется перевод...", ДлинаНомераПеревода, "Перевод звонка на " + стрНомерПеревода);
	Иначе
...
Процедура ОбновитьСтатусыПодключения()
	Если СтатусПодключен = Истина Тогда
		Если СтатусНеБеспокоить = Истина Тогда
			Элементы.ГруппаБИТфонКоманды.Картинка = БиблиотекаКартинок.бит_НеБеспокоить;
		...
		Иначе
			Элементы.ГруппаБИТфонКоманды.Картинка = БиблиотекаКартинок.бит_ВСети;
	...
	Иначе
		Элементы.ГруппаБИТфонКоманды.Картинка = БиблиотекаКартинок.бит_НеВСети;
...
&НаКлиенте
Процедура ОбновитьСтатусАбонентаSIP(ИдентификаторАбонента) Экспорт
	// Заглушка	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбработатьСостояниеЗвонка" Тогда
		Если ЭтаФорма.СостояниеЗвонка = "ЗвонокОтклонён" Тогда
			ПоказатьОповещениеПользователя("БИТ.Phone",,"Нет ответа",БиблиотекаКартинок.бит_КартинкаБитфон_48);
		...
		ИначеЕсли ЭтаФорма.СостояниеЗвонка = "ЗвонокПрерван" Тогда
			Если ЭтаФорма.ДлительностьРазговора = "0:00:00" Тогда
				ПоказатьОповещениеПользователя("БИТ.Phone",,"Вызов сброшен",БиблиотекаКартинок.бит_КартинкаБитфон_48);
				Оповестить("ОбновитьВремяРаботы");
			...
			ИначеЕсли ЭтаФорма.ДлительностьРазговора = "0:00:01"
				...
				ИЛИ ЭтаФорма.ДлительностьРазговора = "0:00:07" Тогда
				ПоказатьОповещениеПользователя("БИТ.Phone",,"Нет ответа",БиблиотекаКартинок.бит_КартинкаБитфон_48);
				Оповестить("ОбновитьВремяРаботы");
				Оповестить("НеудачныйЗвонок");
				ЭтаФорма.СостояниеЗвонка = "";
			ИначеЕсли ЭтаФорма.ДлительностьРазговора <> "0:00:00"
				...
				ИЛИ ЭтаФорма.ДлительностьРазговора <> "0:00:06"
				ИЛИ ЭтаФорма.ДлительностьРазговора <> "0:00:07" Тогда
				Пояснение = "Разговор завершен. Время в разговоре: "+ЭтаФорма.ДлительностьРазговора;
				ПоказатьОповещениеПользователя("БИТ.Phone",,Пояснение,БиблиотекаКартинок.бит_КартинкаБитфон_48);
				Оповестить("ОбновитьВремяРаботы");
