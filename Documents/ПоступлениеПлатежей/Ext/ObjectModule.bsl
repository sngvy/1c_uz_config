...
Функция ЗагрузитьДанные(НастройкаСоответствий, ПолеИмпорта) Экспорт

	ЗаполнитьДанные(ПолеИмпорта, НастройкаСоответствий);
	
	ВыборСоответствий.Загрузить(
		НастройкаСоответствий.ВыборСоответствий.Выгрузить()
	);
...
Функция Обработать() Экспорт
	...
	Возврат Документы.ПоступлениеПлатежей.ПустаяСсылка();
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Для каждого Стр Из Движения.ЗадолженностьПоОбъектам Цикл
		КредитныеИстории.УдалитьСобытиеСделки(Стр.Объект, Ссылка, "2.3");
		КредитныеИстории.УдалитьСобытиеСделки(Стр.Объект, Ссылка, "2.5");
	КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ВалютаРегл = Справочники.Валюты.ПустаяСсылка() Тогда
		...
		Сообщение.Текст = "Не определена валюта регламентированного учета!";
		Сообщение.Сообщить();
		Отказ = Истина;
	КонецЕсли;
	...
	Документы.ПоступлениеПлатежей.ИнициализироватьДанныеДокумента(
		ЭтотОбъект,
		ТаблицаДвиженийОбъекты,
		ТаблицаДвиженийСобытия);
		
	Если Не Отказ И ТаблицаДвиженийОбъекты.Количество() > 0 Тогда
		Движения.ЗадолженностьПоОбъектам.Загрузить(ТаблицаДвиженийОбъекты);
		Движения.ЗадолженностьПоОбъектам.Записывать = Истина;
	КонецЕсли;
	
	Если Не Отказ И ТаблицаДвиженийСобытия.Количество() > 0 Тогда
		Движения.СобытияПоОбъектам.Загрузить(ТаблицаДвиженийСобытия);
	...
	Для каждого Стр Из Объекты Цикл
		СобытиеЗафиксировано = КредитныеИстории.СобытиеСделкиЗафиксированоИсточником(Стр.Объект, Ссылка, "2.3");
		Если Не СобытиеЗафиксировано Тогда
			КредитныеИстории.ЗаписатьСобытиеСделки(Стр.Объект, Стр.ДатаПлатежа, Ссылка, "2.3");
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДанные(ПолеИмпорта, НастройкаСоответствий) Экспорт
	...
	Для НомерСтрокиТаблицыИмпорта = 2 По ПолеИмпорта.ВысотаТаблицы Цикл
		...
		Если не ЗначениеЗаполнено(НоваяСтрока.ДатаПлатежа) ИЛИ Не ЗначениеЗаполнено(НоваяСтрока.Объект) Тогда
			Объекты.Удалить(НоваяСтрока.НомерСтроки-1);
		КонецЕсли;
	КонецЦикла;
