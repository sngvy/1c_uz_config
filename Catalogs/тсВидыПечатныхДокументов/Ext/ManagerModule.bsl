...
Функция ПолучитьМакетыШаблонов(Шаблоны, ОднаКорреспонденция)
	...
	Для каждого СтрокаВидШаблона Из Шаблоны Цикл
		...
		//Лебедева 28.05.2024
		//- при пакетном создании корреспонденции на универнальной печати попадают шаблоны (комплекты), у которых нет макета
		//Пропускаем такой шаблон
		Если ТипЗнч(ВидШаблона) <> Тип("СправочникСсылка.тсШаблоныПечатныхДокументов") Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЗаполнитьМакетШаблона(ВидШаблона, СтрокаВидШаблона = Неопределено)
	...
	ВидимостьОбластей = ЗадатьОбласти(ОписаниеОбластей, ВидШаблона);
	...
	НастройкиОбластей.Вставить("ВидимостьОбластей", ВидимостьОбластей);
...
Функция СформироватьПечатнуюФорму(Основание, МакетСНастройкой, ОбщиеНастройкиКорреспонденции = Неопределено)
	...
	Попытка
		...
		ВидимостьОбластей = МакетСНастройкой.Области["ВидимостьОбластей"];

		Для каждого ОписаниеОбласти Из ОписаниеОбластей Цикл
		
			Если Не ВыводитьОбласть(
				ОписаниеОбласти.Значение.ИмяОбласти,
				ВидимостьОбластей,
				Основание) Тогда
...
Функция ЗадатьОбласти(ОписаниеОбластей, ВидШаблона)
	...
	ВидимостьОбластей = Новый Соответствие;
	Для каждого Настройка Из НастройкаПоПорядку Цикл
	
		СкриптВидимости = Справочники.ФункцииТекстовыхШаблонов
			.ТекстФункции(Настройка["ФункцияВидимости"]);
		ВидимостьОбластей.Вставить(Настройка["ИмяОбласти"], СкриптВидимости);
	
	КонецЦикла;
	
	Возврат ВидимостьОбластей;

КонецФункции

Функция ВыводитьОбласть(ИмяОбласти, НастройкиВидимости, Основание)

	ФункцияВидимости = НастройкиВидимости.Получить(ИмяОбласти);
	
	Если ФункцияВидимости = Неопределено Тогда
	
		Возврат Истина;
	
	КонецЕсли;
	
	Результат = ВычислитьВидимость(ФункцияВидимости, Основание);
	
	Возврат Результат;

КонецФункции // ()

Функция ВычислитьВидимость(СкриптВидимости, Основание)

	Если ПустаяСтрока(СкриптВидимости) Тогда
	
		Возврат Истина;
	
	КонецЕсли;
	
	Результат = Справочники.ШаблоныТекстаДляАвтоинформирования
		.ВычислитьФункцию(СкриптВидимости, Основание);
