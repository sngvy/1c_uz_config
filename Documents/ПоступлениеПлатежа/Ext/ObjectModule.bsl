...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтотОбъект.Займ, "0182") = ЮД Тогда
		ЭтотОбъект.РаспределитьПоСудебнымРешениям();
		ЭтотОбъект.Записать();
	Иначе ЭтотОбъект.РаспределитьПоВнесудебке();
		ЭтотОбъект.Записать();
	КонецЕсли;
...
Функция РаспределитьПоСудебнымРешениям() Экспорт
	...
	Если НеразнесеннаяСумма > 0 Тогда
		...
		Разнесение.Решение = ИсполнительныйДокумент.СудебноеРешение;
		Разнесение.ЧастьРешения = ИсполнительныйДокумент.ЧастьРешения;
		// Если не найдено решение, то разносить по процентам
