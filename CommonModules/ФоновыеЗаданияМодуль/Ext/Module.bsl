...
Процедура ОбновитьИП() Экспорт
	...
	Выборка = РезультатЗапроса.Выбрать();	
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПроверки = Дата(1,1,1) ИЛИ Выборка.ДатаПроверки <= НачалоНедели(ТекущаяДата()) Тогда
...
Процедура ОбновитьДелаОБанкротстве() Экспорт
	...
	|	ДолговыеОбязательства.Должник КАК Должник,
	|	ДолговыеОбязательства.Должник.Код КАК ДолжникКод,
	|	ЕСТЬNULL(ДелаОБанкротстве.ДатаПроверки, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПроверки
	|ИЗ
	|	Справочник.ДолговыеОбязательства КАК ДолговыеОбязательства
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПроверки = Дата(1,1,1) ИЛИ Выборка.ДатаПроверки <= НачалоНедели(ТекущаяДата()) Тогда
			ЗапросыЕФРСБ.ПолучитьСведения(Выборка.Должник, Выборка.ДолжникКод);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
