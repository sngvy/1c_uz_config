...
Функция ТекущийСеансОбрабатываетЗадания(ЗаданияОбрабатываютсяНормально = Неопределено,
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Состояние.ОкончаниеОбработкиОчередногоЗадания) Тогда
		...
		Если ТекущаяДатаСеанса() - 360 > Состояние.ОкончаниеОбработкиОчередногоЗадания Тогда
			ОписаниеОшибки = УдалитьСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск обработки регламентных заданий задерживается более 1 часа!
	...
	Иначе
		...
		Если ТекущаяДатаСеанса() - 360 > Состояние.НачалоОбработкиОчередногоЗадания Тогда
			ОписаниеОшибки = УдалитьСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обработка регламентного задания выполняется непрерывно уже более 1 часа!
	...
	Пока НЕ ОбработкаЗавершена И Задания.Количество() > 0 Цикл
		...
		Для каждого Задание ИЗ Задания Цикл
			...
			Если (ВремяОбработки = 0 И
			     ...
			     (ВремяОбработки <> 0 И
			      НачалоОбработки + ВремяОбработки <= ТекущаяДатаСеанса()) Тогда
			...
			Состояние.НачалоОбработкиОчередногоЗадания    = ТекущаяДатаСеанса();
			...
			Если Задание.Использование Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Состояние.ОкончаниеОбработкиОчередногоЗадания = ТекущаяДатаСеанса();
...
Процедура УстановитьНастройкиОбработкиРегламентныхЗаданий(Настройки) Экспорт
	...
	ЗаполнитьВосстановитьНастройки(Настройки);
	
	НачатьТранзакцию();
	Попытка
		Состояние = ПолучитьСостояниеОбработкиРегламентныхЗаданий(Истина);
		Если Состояние.Настройки.БлокировкаОбработкиРегламентныхЗаданий <> Настройки.БлокировкаОбработкиРегламентныхЗаданий
			ИЛИ Состояние.Настройки.ИмяПользователяДляЗапуска <> Настройки.ИмяПользователяДляЗапуска
			ИЛИ Состояние.Настройки.ПарольПользователяДляЗапуска <> Настройки.ПарольПользователяДляЗапуска Тогда
			ВызватьИсключениеЕслиНетПраваАдминистрирования();
		КонецЕсли;
...
//
//
Процедура ОбработатьРегламентноеЗаданиеВручную(
	Знач Задание,
	...
	Иначе
		...
		Попытка
			Состояние = ПолучитьСостояниеОбработкиРегламентныхЗаданий(Истина);
			НаименованиеФоновогоЗадания = УдалитьСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск вручную: %1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ПараметрыОткрытияСеансаОбработкиРегламентныхЗаданий(Знач ПоНастройкеАвтоОткрытия = Ложь) Экспорт
	...
	Результат.СтрокаЗапуска = """"
		...
		+ """ ENTERPRISE /Debug /IBConnectionString"""
	...
	Попытка
		...
		Если НЕ УдалитьОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			Пока НомерЗадания >= 0 Цикл
				...
				Строка.ОписаниеИнформацииОбОшибке = ?(Задание.ИнформацияОбОшибке = Неопределено, "", ОбработкаОшибок.ПодробноеПредставлениеОшибки(Задание.ИнформацияОбОшибке));
				НомерЗадания = НомерЗадания - 1;
			КонецЦикла;
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	ИначеЕсли УдалитьОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
...
// Процедура ЗаполнитьВосстановитьНастройки служит для заполенения/восстановления структуры свойств
// настроек, хранимых в структуре "Состояние" свойства Настройки.
//
// Параметры:
//  Настройки - произвольный.
//
Процедура ЗаполнитьВосстановитьНастройки(Настройки)
...
Функция ПолучитьСостояниеОбработкиРегламентныхЗаданий(Заблокировать = Ложь)
	...
	ЗаполнитьВосстановитьНастройки(НоваяСтруктура.Настройки);
...
Процедура ОбработатьРегламентноеЗадание(Знач Задание,
	...
	Попытка
		...
		НаименованиеФоновогоЗадания = ?(ЗапускВручную,
		                                УдалитьСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск вручную: %1'"),
	...
	Исключение
		...
		СвойстваФоновогоЗадания.ОписаниеИнформацииОбОшибке = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	// Зафиксируем окончание выполнения метода.
	СвойстваФоновогоЗадания.Конец = ТекущаяДатаСеанса();
