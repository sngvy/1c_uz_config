...
Процедура ОбновитьИП(Команда)
	ОбновитьИПНаСервере();
	ФормаЗапроса = ПолучитьФорму("ОбщаяФорма.ЗапросВФССП");
	Если ФССПСлужебный.ПолучитьКодРегионаПоДолжнику(ЭтаФорма.Объект.Должник) <> Неопределено Тогда
		ФормаЗапроса.Должник = ЭтаФорма.Объект.Должник;
		ФормаЗапроса.Открыть();
	Иначе
		// Адрес непригоден
		Предупреждение("Не удалось определить регион должника! Проверьте наличие и корректность адреса регистрации",,"Интеграция с ФССП");
	КонецЕсли;	
КонецПроцедуры
