...
Процедура ПолучитьСведения(Должник, Код) Экспорт
	...
	Запрос = Новый HTTPЗапрос("/backend/prsnbankrupts?searchString=" + Должник + "&regionId=all&isActiveLegalCase=null&offset=0&limit=15", Заголовки);
	...
	Если Данные.pageData.Количество() = 0 Тогда
		...
		МенеджерЗаписи.Дело = "Дела о банкротстве отсутствуют";
	...
	ИначеЕсли Данные.pageData.Количество() > 0 Тогда			
		...
		Для Сч = 0 По Данные.pageData.Количество() Цикл
			...
			// Записать дела в регистр сведений
			МенеджерЗаписи = РегистрыСведений.ДелаОБанкротстве.СоздатьМенеджерЗаписи();
