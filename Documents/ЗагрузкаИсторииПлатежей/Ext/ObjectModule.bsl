...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	ЭтотОбъект.Движения.ИсторияПлатежей.Записывать = Истина;
	...
	Для каждого стр из этотОбъект.Движения.ИсторияПлатежей Цикл
		КредитныеИстории.УдалитьСобытиеСделки(Стр.Объект, ЭтотОбъект.Ссылка, "2.3");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого ТекСтрокаОбъекты Из Объекты Цикл
		...
		Движение.Период = ТекСтрокаОбъекты.ДатаПлатежа;
		Движение.Объект = ТекСтрокаОбъекты.Объект;
		Если ЗначениеЗаполнено(ТекСтрокаОбъекты.ВалютаПлатежа) Тогда
			Движение.Валюта = ТекСтрокаОбъекты.ВалютаПлатежа;
		Иначе
			Движение.Валюта = ВалютаРегл;
		КонецЕсли;
		Движение.Задолженность = ТекСтрокаОбъекты.СуммаВВалютеПлатежа;
		СобытиеЗафиксировано = КредитныеИстории.СобытиеСделкиЗафиксированоИсточником(ТекСтрокаОбъекты.Объект, ЭтотОбъект.Ссылка, "2.3");
		Если Не СобытиеЗафиксировано Тогда 
			КредитныеИстории.ЗаписатьСобытиеСделки(ТекСтрокаОбъекты.Объект, ТекСтрокаОбъекты.ДатаПлатежа, ЭтотОбъект.Ссылка, "2.3");
		КонецЕсли;
	КонецЦикла;
	Движения.Записать();
