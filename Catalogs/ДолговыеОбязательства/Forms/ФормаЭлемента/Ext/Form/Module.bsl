...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.РольАБД = Ложь Тогда
		ЭтаФорма.Элементы.ГруппаКредитнаяИстория.Доступность = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Перменная управляет вариантом отчета по ДО. По умолчанию - "Истина"
	НовыйВид = Ложь;
...
Процедура ЗаполнитьСобытияКИ(Команда)
	...
	ПодключитьОбработчикОжидания("ОбновитьСобытияКИ", 5, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСобытияКИ()
	ЭтаФорма.Элементы.СобытияКИ.Обновить();
КонецПроцедуры

&НаСервере
Процедура ПрекратитьОбязательствоНаСервере()
	ИсходноеСобытие = РегистрыСведений.КредитныеИсторииДанныеСделки.СоздатьМенеджерЗаписи();
	
	ИсходноеСобытие.Сделка = ЭтаФорма.Объект.Ссылка;
	ИсходноеСобытие.ДатаСобытия = ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Объект, "0158");
	ИсходноеСобытие.Событие = Справочники.КредитныеИсторииСобытияСделки.НайтиПоНаименованию("Правопреемство от другого лица");
	ИсходноеСобытие.ДокументСобытие = Справочники.НастройкиВыгрузкиНБКИ_RUTDF.НайтиПоКоду("000000001");
	...
	Иначе
		Сообщить("Не найдено событие правопреемства!");
		Возврат;
	КонецЕсли;
	...
	СобытиеПрекращения.Сделка = ЭтаФорма.Объект.Ссылка;
	СобытиеПрекращения.ДатаСобытия = ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Объект, "0158");
	...
	СобытиеПрекращения.ДокументСобытие = Справочники.НастройкиВыгрузкиНБКИ_RUTDF.НайтиПоКоду("000000001");
...
Процедура ПрекратитьОбязательство(Команда)
	ПрекратитьОбязательствоНаСервере();
	ПоказатьОповещениеПользователя("Обмен с НБКИ",,"Обязательство прекращено",БиблиотекаКартинок.NC_ПодсистемаОбменСНБКИ);
	ЭтаФорма.Элементы.СобытияКИ.Обновить();
КонецПроцедуры
...
Процедура СформироватьОтчетВНБКИ(Команда)
	...
	ОбновляемаяЗапись.ОбъектУчета = ЭтаФорма.Объект.Ссылка;
	ОбновляемаяЗапись.Должник = ЭтаФорма.Объект.Должник;
	ПараметрыЗаписи = Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение);
	ФормаДокументаОтчета.Записать(ПараметрыЗаписи);
	ФормаДокументаОтчета.СформироватьОтчет(Неопределено);
	ПоказатьОповещениеПользователя("Обмен с НБКИ",,"Отчет в НБКИ сформирован",БиблиотекаКартинок.NC_ПодсистемаОбменСНБКИ);
	ЭтаФорма.Элементы.ЖурналВыгрузокНБКИ.Обновить();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьИП(Команда)
	ФормаАдаптера = ПолучитьФорму("ОбщаяФорма.АдаптерФССП");
	ФормаАдаптера.Адрес = ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Объект.Должник, "0049");
	ФормаАдаптера.Должник = ЭтаФорма.Объект.Должник;
	ФормаАдаптера.Код = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Должник, "Код");
	ФормаАдаптера.ДатаРождения = ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Объект.Должник, "0043");
