...
Процедура ПриОткрытииНаСервере()
	...
	ЭтаФорма.ВерсияКонфигурацииNC = Константы.ВерсияКонфигурацииNC.Получить();
...
Процедура ПриОткрытии(Отказ)
	...
	Если Дата(ТекущаяДата()) < Дата(НачалоДня(ТекущаяДата()) + 37800) Тогда
		ПоказатьОповещениеПользователя("Обработка обещаний",,"Обработайте обещания не позднее 11:00",БиблиотекаКартинок.NC_ПодсистемаКонтрольЗадолженности,СтатусОповещенияПользователя.Важное);
	КонецЕсли;
	...
	// Проверить обновления клиента WhatsApp
	ПодключитьОбработчикОжидания("ПроверитьОбновленияВЦ", 1, Истина);
КонецПроцедуры
...
Процедура Отбор_Обещания()
	...
	Если Элементы.Дерево.ТекущиеДанные <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЭтаФорма.ОбещанияМесяц, "ДатыИСуммы.Дата", НачалоДня(ДобавитьМесяц(НачалоМесяца(Текущаядата()),-1)), ВидСравненияКомпоновкиДанных.БольшеИлиРавно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
	КонецЕсли;
КонецПроцедуры
...
Процедура Задачи(Команда)
	ОткрытьФорму("РегистрСведений.Задачи.ФормаСписка");
КонецПроцедуры
...
Процедура РасчетАВ(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
		...
		ФормаРасчетАВ.Открыть();
	Иначе
		Предупреждение("Не выбран объект!",,"Расчет АВ");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПортфель(Команда)
	ПоказатьОповещениеПользователя("Обновление портфеля",,"Пожалуйста, подождите...",БиблиотекаКартинок.NC_Портфель);
	Элементы.СписокДО.Обновить();
...
Процедура ПосчитатьДолжников(Команда)
	// Посчитать и сообщить количество должников
...
Процедура ОткрытьВЦ(Команда)
	...
	Если EXE.Существует() Тогда
		...
		КомандаОткрытьВЦ = USERPROFILE + "\AppData\Local\WhatsApp\WhatsApp.exe";
...
Процедура КонтрольСроков(Команда)
	// Передать с ключом
	Если Элементы.СписокДО.ТекущиеДанные <> Неопределено Тогда
		ФормаКонтроль = ПолучитьФорму("ОбщаяФорма.КонтрольСроков");
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ФормаКонтроль.СписокДО, "Ссылка.Ответственный", ЭтаФорма.Элементы.Дерево.ТекущиеДанные.Ссылка, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ФормаКонтроль.СписокДО, "Ссылка.Причина архивации", ЭтаФорма.КатегорияРучнойОбзвон, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
	...
	Иначе
		...
		Иначе
			Предупреждение("Не найдены объекты отбора!",,"Контроль сроков");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПоказатьСписокИзменений(Команда)
	ФормаСписокИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
...
Процедура ОбновитьОстаткиНаСервере()
	ФоновыеЗадания.Выполнить("ФоновыеЗаданияМодуль.ОбновлениеОстатков",,,"Обновление остатков");
КонецПроцедуры
...
Процедура ПроверкаПо230ФЗ(Команда)
	Если ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.СписокДО И Элементы.СписокДО.ТекущиеДанные <> Неопределено Тогда
		...
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.СписокДО.ТекущиеДанные.Объект, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.СписокДО.ТекущиеДанные.Объект, "Должник"));
		...
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
		ФормаРезультаты.ОграничениеПоВЦ = ОграничениеПоВЦ;
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияСегодня И Элементы.ОбещанияСегодня.ТекущиеДанные <> Неопределено Тогда
		...
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ОбещанияСегодня.ТекущиеДанные.Объект, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ОбещанияСегодня.ТекущиеДанные.Объект, "Должник"));
		// Передать результаты в форму
		ФормаРезультаты = ПолучитьФорму("ОбщаяФорма.РезультатыПроверкиПо230ФЗ");
		ФормаРезультаты.ОграничениеПоЗвонкам = ОграничениеПоЗвонкам;
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
		...
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ОбещанияМесяц.ТекущиеДанные.Объект, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ОбещанияМесяц.ТекущиеДанные.Объект, "Должник"));
		// Передать результаты в форму
		ФормаРезультаты = ПолучитьФорму("ОбщаяФорма.РезультатыПроверкиПо230ФЗ");
		ФормаРезультаты.ОграничениеПоЗвонкам = ОграничениеПоЗвонкам;
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
		...
		ФормаРезультаты.Открыть();
	Иначе
		Предупреждение("Не найден объект контроля!",,"Проверка по 230-ФЗ");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗвонокКредитору(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияСегодня И Элементы.ОбещанияСегодня.ТекущиеДанные <> Неопределено Тогда
		...
		НоваяЗадача.Объект.Объект = К;
		НоваяЗадача.Объект.ТипМероприятия = ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокИсходящий");
		НоваяЗадача.Объект.РезультатМероприятия = ЭтаФорма.РезультатЗвонокКредитору;
		// НоваяЗадача.Объект.Контакт = РезультатНомер;
		НоваяЗадача.КонтрагентКЛ = Контрагент;
		НоваяЗадача.Открыть();
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
...
Процедура ПроверитьОбновленияКонфигурации()
	// Показать сообщение об обновлении конфигурации
	НоваяВерсияКонфигурацииNC = "3.6.3.2";
	Если ЭтаФорма.ВерсияКонфигурацииNC <> НоваяВерсияКонфигурацииNC Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ФормаСпискаИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
...
&НаКлиенте
Процедура ПроверитьОбновленияВЦ()
	// Проверять обновления клиента WhatsApp каждые 10 дней
	Если День(ТекущаяДата()) = 1 И Дата(ТекущаяДата()) < Дата(НачалоДня(ТекущаяДата()) + 32400)
		ИЛИ День(ТекущаяДата()) = 11 И Дата(ТекущаяДата()) < Дата(НачалоДня(ТекущаяДата()) + 32400)
		ИЛИ День(ТекущаяДата()) = 21 И Дата(ТекущаяДата()) < Дата(НачалоДня(ТекущаяДата()) + 32400)
		ИЛИ День(ТекущаяДата()) = 31 И Дата(ТекущаяДата()) < Дата(НачалоДня(ТекущаяДата()) + 32400) Тогда
		Ответ = Вопрос("Для корректной работы WhatsApp требуется актуальная версия клиента. Проверить наличие обновлений?",РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да,"Интеграция с WhatsApp");
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ФормаУстановкиИОбновленияВЦ = ПолучитьФорму("ОбщаяФорма.УстановкаИОбновлениеWhatsApp");
			ФормаУстановкиИОбновленияВЦ.Открыть();
		КонецЕсли;
...
&НаКлиенте
Процедура УстановитьВЦ(Команда)
	ФормаУстановкиИОбновленияВЦ = ПолучитьФорму("ОбщаяФорма.УстановкаИОбновлениеWhatsApp");
	ФормаУстановкиИОбновленияВЦ.Открыть();
КонецПроцедуры
