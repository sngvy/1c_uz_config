...
Процедура ЗапросПочтаРФ() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ИЗ
	                      |	Исх КАК Исх
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыШПИРегистрация.СрезПоследних КАК СтатусыШПИСрезПоследних
	                      ...
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыШПИОтслеживание.СрезПоследних КАК ВнешниеСтатусыШПИСрезПоследних
...
Процедура МножественныйЗапросКТрекеру(массивНомеров)
	рЛогин = Константы.УчеткаПочтыРФ.Получить().Логин;
	рПароль = Константы.УчеткаПочтыРФ.Получить().Пароль;
	...
	ЗаписьВРегистрСведений(текБилетОчереди,ДобавленияШтрихКодовСФормыВРегистр(массивНомеров));
КонецПроцедуры
...
Функция ЗаписьВРегистрСведений(пНомерТикета,пШтрихкода)
	...
	Если текНаборЗаписей.Количество() = 0 Тогда     
		...
		ТекНоваяЗапись.ДатаОтправленияЗапроса = ТекущаяДата();
...
КонецФункции // ЗаписьВРегистрСведений()
...
Процедура ПолучитьОтветНаСервере(НомерТикета)
	...
	XMLЗапросПоТикету =	"<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:pos=""http://fclient.russianpost.org/postserver"">
	|<soapenv:Header/>
	|<soapenv:Body>
...
Процедура СтрокуВОбъект(рОтветОтСервера)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.Имя = "ns3:Error" Тогда
			...
			ОперацияСПисьмом.DateOper = Строка(ТекущаяДата());
	...
	//РегистрыСведений.ВнешниеСтатусыШПИ.СрезПоследних(ТекущаяДата(), Новый Структура("ШПИ", Справочники.ШПИ.НайтиПоНаименованию("23443234243243")))
