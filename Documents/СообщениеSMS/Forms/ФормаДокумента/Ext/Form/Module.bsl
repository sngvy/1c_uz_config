...
&НаСервере
Процедура ДатаКогдаОтправитьПриИзмененииНаСервере()
	Если Константы.КоличествоСМС.Получить() И Объект.Адресаты.Количество() > 0 Тогда
		//СтруктураВП = КонтрольСобытий.КонтрольСМС230ФЗ(Объект.Адресаты[0].
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДатаКогдаОтправитьПриИзменении(Элемент)
	ДатаКогдаОтправитьПриИзмененииНаСервере();
КонецПроцедуры
