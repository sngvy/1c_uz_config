...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			КраткоеПредставлениеОшибки = НСтр("ru = 'Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о адресных сведениях.'");
		КонецЕсли;
...
Процедура ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально)
	...
	Если РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) Тогда
		Если СтрСравнить(НаселенныйПунктДетально.AddressType, УправлениеКонтактнойИнформациейКлиентСервер.АдресЕАЭС()) = 0 
			Или СтрСравнить(НаселенныйПунктДетально.AddressType, УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес()) = 0 Тогда
			НаселенныйПунктДетально.AddressType = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес();
		КонецЕсли;
...
Процедура ПослеЗагрузкиАдресногоКлассификатора(Результат, ДополнительныеПараметры) Экспорт
	//СообщениеОНеобходимостиОбновленияКлассификатора(НаселенныйПунктДетально.Регион.Идентификатор); // старый код
	//БИТ поправили так как не было поля Регион в списке
...
Процедура ПредложениеЗагрузкиКлассификатора(Знач Текст = "", Знач ВыбранноеЗначение = Неопределено)
	...
	Регион = ?(ВыбранноеЗначение <> Неопределено, ВыбранноеЗначение.Представление, "");
	...
	Если НеЗагружатьАдресныйКлассификатор = Неопределено ИЛИ НеЗагружатьАдресныйКлассификатор = Ложь Тогда
		...
		ПараметрыЗагрузки.Вставить("ВыбранноеЗначение", ВыбранноеЗначение);
