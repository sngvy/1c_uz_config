...
#Область ВзаимодействиеУЗ

Функция СоздатьФайлВКонфигурации(ДанныеФайла) Экспорт

	ПараметрыФайла = РаботаСФайламиСлужебныйКлиентСервер.ПараметрыДобавленияФайла();
	ПараметрыФайла.ВладелецФайлов = ДанныеФайла["ПапкаВладелец"];
	...
	Попытка
		...
		ФайлСсылка = РаботаСФайламиСлужебныйВызовСервера.ДобавитьФайл(
			ПараметрыФайла,
			ДанныеФайла.АдресХранилища);
	
	Исключение
		ВызватьИсключение
			"Не удалось создать карточку файла: "
			+ ОписаниеОшибки();
	КонецПопытки;
