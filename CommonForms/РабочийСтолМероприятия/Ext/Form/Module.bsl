...
&НаСервере
Процедура АнализПортфеляНаСервере()	
	...
	|	ОбещанныеПлатежи.Автор = &Автор
	|	И ОбещанныеПлатежи.ДатыИСуммы.Дата = &Дата";
	Запрос.УстановитьПараметр("Автор", ЭтаФорма.Элементы.Дерево.ТекущиеДанные.Ссылка);
	Запрос.УстановитьПараметр("Дата", НачалоДня(ТекущаяДата()));
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	...
	Запрос.УстановитьПараметр("НачалоПериода", КонецМесяца(ТекущаяДата()));
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
...
Процедура АнализПортфеля(Команда)
	...
	ФормаАнализаПортфеля.КоличествоОбещанийСегодня = ЭтаФорма.СуммаОбещанийСегодняАнализ;
	...
	ФормаАнализаПортфеля.ПроцентСбора = (ЭтаФорма.КоличествоПлатежейМесяцАнализ / ЭтаФорма.КоличествоВРаботе) * 100;
...
Процедура ПосчитатьДолжников(Команда)
	// Посчитать и сообщить количество должников в портфеле
	ПолучитьКоличествоСтрокВДинамическомСпискеНаСервере();
...
Процедура ПроверитьОбновленияКонфигурации()
	...
	НоваяВерсияКонфигурацииNC = "4.3.3";
