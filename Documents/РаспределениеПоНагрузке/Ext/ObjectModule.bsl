...
Процедура СортироватьОбъектыПоСуммамГрафикИСудебныйРегистр()
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗадолженностьПоСудебнымРешениям.Остатки КАК ЗадолженностьПоСудебнымРешениямОстатки
		|		ПО ТаблицаОбъекты.Объект = ЗадолженностьПоСудебнымРешениямОстатки.Займ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИсковыеТребования.Остатки КАК ИсковыеТребованияОстатки
...
Процедура ЗаполнитьНастройкиРаспределения() Экспорт
	...
	Иначе
		...
		|		ПО ОбъектыВРаботеОстатки.Объект = Судебка.Займ
		|ГДЕ
		|	ОбъектыВРаботеОстатки.Сотрудник В(&Сотрудник)
		|
		|СГРУППИРОВАТЬ ПО
		|	ОбъектыВРаботеОстатки.Сотрудник";
		
		Запрос.УстановитьПараметр("ДатаОстатков", ТекущаяДата());
		Запрос.УстановитьПараметр("Сотрудник",  ЭтотОбъект.Сотрудники.ВыгрузитьКолонку("Сотрудник"));
