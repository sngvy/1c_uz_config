...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Логика при копировании документа
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		Схема = Параметры.ЗначениеКопирования.Схема;	
		Карта = Схема.Получить();
	КонецЕсли;
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если УправлениеЗапрещеннымиСимволами
		.ПроверитьЗапрещенныеСимволыДляБизнесПроцесса(Объект.Наименование) Тогда
	
		Запрещенные = УправлениеЗапрещеннымиСимволами
			.СимволыБизнесПроцессов();
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(
			"В наименовании есть запрещенные символы - %1
			|При сохранении будут удалены,
			|А так же будут убраны пробельные символы, кроме пробела",
			СтрСоединить(Запрещенные, ", ")
		);
		Сообщение.Сообщить();
