...
Функция НастройкаСоответствияТабличнойЧасти(ВидШаблона) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	тсШаблоныПечатныхДокументов.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втШаблон.Ссылка КАК Ссылка,
		...
		|	тсШаблоныПечатныхДокументовТаблицаВыбораТаблиц.UID КАК UID
		|ПОМЕСТИТЬ втНастроеныеТаблицы
		|ИЗ
		|	втШаблон КАК втШаблон
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.тсШаблоныПечатныхДокументов.ТаблицаВыбораТаблиц КАК тсШаблоныПечатныхДокументовТаблицаВыбораТаблиц
		|		ПО втШаблон.Ссылка = тсШаблоныПечатныхДокументовТаблицаВыбораТаблиц.Ссылка
		...
		|	тсШаблоныПечатныхДокументовОбластиШаблона.ТипОбласти КАК ТипОбласти
		|ПОМЕСТИТЬ втОбластиШаблона
		|ИЗ
		|	втШаблон КАК втШаблон
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.тсШаблоныПечатныхДокументов.ОбластиШаблона КАК тсШаблоныПечатныхДокументовОбластиШаблона
		|		ПО втШаблон.Ссылка = тсШаблоныПечатныхДокументовОбластиШаблона.Ссылка
		|			И (тсШаблоныПечатныхДокументовОбластиШаблона.ТипОбласти = ""СтрокаТаблицы"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	втНастроеныеТаблицы.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ втКолонкиТаблицы
		|ИЗ
		|	втНастроеныеТаблицы КАК втНастроеныеТаблицы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.тсШаблоныПечатныхДокументов.ТаблицаТаблиц КАК тсШаблоныПечатныхДокументовТаблицаТаблиц
		|		ПО втНастроеныеТаблицы.Ссылка = тсШаблоныПечатныхДокументовТаблицаТаблиц.Ссылка
		|			И втНастроеныеТаблицы.UID = тсШаблоныПечатныхДокументовТаблицаТаблиц.UID
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втШаблон.НачалоТега КАК НачалоТега,
		|	втШаблон.КонецТега КАК КонецТега,
		|	втКолонкиТаблицы.Поле КАК Поле,
		|	втКолонкиТаблицы.Значение КАК Значение,
		|	втКолонкиТаблицы.Функция КАК Функция,
		|	втНастроеныеТаблицы.ИмяОбласти КАК ИмяОбласти,
		|	втНастроеныеТаблицы.ФункцияОбласти КАК ФункцияОбласти,
		|	втОбластиШаблона.ТипОбласти КАК ТипОбласти
		|ИЗ
		|	втШаблон КАК втШаблон
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втНастроеныеТаблицы КАК втНастроеныеТаблицы
		|		ПО втШаблон.Ссылка = втНастроеныеТаблицы.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втОбластиШаблона КАК втОбластиШаблона
		|		ПО втШаблон.Ссылка = втОбластиШаблона.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКолонкиТаблицы КАК втКолонкиТаблицы
		|		ПО втШаблон.Ссылка = втКолонкиТаблицы.Ссылка";
...
Функция ПорядокОбластей(ВидШаблона) Экспорт
	...
	Запрос.Текст = 
		...
		|	тсШаблоныПечатныхДокументовОбластиШаблона.ИмяОбласти КАК ИмяОбласти,
		|	тсШаблоныПечатныхДокументовОбластиШаблона.ТипОбласти КАК ТипОбласти,
		|	тсШаблоныПечатныхДокументовОбластиШаблона.ФункцияВидимости КАК ФункцияВидимости
		|ИЗ
