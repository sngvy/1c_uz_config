...
Процедура ПриОткрытииНаСервере()
	...
	ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Контрагент) = Истина Тогда
		...
		ЭтаФорма.Элементы.СвязанныйДоговор.Видимость = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	// Запретить управление инструментами пока не будет обработан статус Битфона
	ВыключитьДоступностьИнструментов();
	...
	// Запретить закрытие незаполненного мероприятия, если это входящий звонок
	// Исправить пустое значение реквизита "Автор" при поступлении входящего звонка
	Если ЭтаФорма.Объект.Выполнена = Ложь
		И ЭтаФорма.Объект.ТипМероприятия = ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий") Тогда
		ЭтаФорма.Записать();
		ЭтаФорма.Модифицированность = Истина;
	КонецЕсли;	
	
	// Запретить закрытие незаполненного мероприятия, если оно не требует выбора контакта
	Если ЭтаФорма.Объект.Выполнена = Ложь И ЭтаФорма.Объект.Контакт = ""
		И ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ПустаяСсылка")
		И ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий") Тогда
		ЭтаФорма.Записать();
		ЭтаФорма.Модифицированность = Истина;
	КонецЕсли;
	...
	Если ЭтаФорма.ПредупреждениеАрхив = Истина Тогда		
		ЭтаФорма.Элементы.РаботаСЗаявками.Доступность = Ложь;
	...
	// Запросить информацию из реестров
	Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) Тогда
		Если ЕстьИП() Тогда
			ЭтаФорма.ЕстьИП = Истина;
		КонецЕсли;
		Если ЕстьДоБ() Тогда
			ЭтаФорма.ЕстьДелаОБанкротстве = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЭтаФорма.НеОбрабатыватьСтатусБитФона Тогда
		ПодключитьОбработчикОжидания("УстановитьСтатусНеБеспокоитьФорма", 1, Истина);
		ПодключитьОбработчикОжидания("УстановитьСтатусНеБеспокоитьГлобальный", 5, Истина);
	Иначе
		ВключитьДоступностьИнструментов();
	КонецЕсли;
КонецПроцедуры
...
Функция ЕстьИП()
	Возврат ЭтаФорма.Объект.Объект.Должник.ЕстьИП;
КонецФункции

Функция ЕстьДоБ()
	Возврат ЭтаФорма.Объект.Объект.Должник.ЕстьДоБ;
КонецФункции
		...
		Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
			...
			Пока ЭлементСпискаВыбора < ЭтаФорма.Элементы.Контакт.СписокВыбора.Количество() И Не КонтактНайден Цикл
				...
				Если ЧислоВхождений = 1 Тогда
					ПоказатьОповещениеПользователя("Контакты",,"Выбран контакт по совпадению номера",БиблиотекаКартинок.NC_Выполнено);
		...
		Если ЭтаФорма.Объект.Выполнена = Ложь Тогда
			...
			Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
				...
				Пока ЭлементСпискаВыбора < ЭтаФорма.Элементы.Контакт.СписокВыбора.Количество() И Не КонтактНайден Цикл
					...
					Если ЧислоВхождений = 1 Тогда
						ПоказатьОповещениеПользователя("Контакты",,"Выбран контакт по совпадению имени",БиблиотекаКартинок.NC_Выполнено);
						ЭтаФорма.Объект.Контакт = ЭтаФорма.Элементы.Контакт.СписокВыбора[ЭлементСпискаВыбора].Значение;
						ЭтаФорма.Записать();
						// Вернуть флаг модифицированности
						ЭтаФорма.Модифицированность = Истина;
				...
				Если КонтактНайден = Ложь Тогда
					...
					Иначе
						...
						ПоказатьОповещениеПользователя("Контакты",,"Не удалось найти подходящий контакт",БиблиотекаКартинок.NC_Предупреждение);
	...
	Попытка
		...
		Если Не Объект.Выполнена Тогда
			...
			Иначе
				...
				Если Объект.ВремяРаботы = 0 И ЭтаФорма.ВремяЗавершенияРазговора = Дата(1,1,1) Тогда
					Объект.ВремяРаботы = Объект.ДатаВыполнения - Объект.Дата;
				ИначеЕсли Объект.ВремяРаботы = 0 И ЭтаФорма.ВремяЗавершенияРазговора <> Дата(1,1,1) Тогда
					Объект.ВремяРаботы = Объект.ДатаВыполнения - ЭтаФорма.ВремяЗавершенияРазговора;
				КонецЕсли;
				
				// Исправить отрицательное значение
				Если Объект.ВремяРаботы < 0 Тогда
					Объект.ВремяРаботы = Объект.ВремяРаботы * -1;
				КонецЕсли;
...
Процедура ПроверкаЗаполненияПолейПередВыполнением(Отказ, ТекстСообщения)
	...
	Если Не ОтказПроверкиЗаполнения И Объект.Результат.ЗаполнитьКомментарий И 
		...
		//Возврат;
	КонецЕсли;
	// Запретить проведение, если не заполнен связанный договор
	Если Не ОтказПроверкиЗаполнения И ЭтаФорма.Объект.Результат = Справочники.РезультатыМероприятий.НайтиПоНаименованию("Звонок кредитору")
		И Не ЗначениеЗаполнено(ЭтаФорма.Объект.СвязанныйДоговор) Тогда
		ОтказПроверкиЗаполнения = Истина;
		ТекстСообщения = "Не заполнен связанный договор!";
		//Возврат;
	КонецЕсли;
...
Процедура РезультатПриИзменении(Элемент)
	...
	Если СвязанныйДокумент = ПредопределенноеЗначение("Перечисление.ВидыСвязанныхДокументов.ОбещанныйПлатеж") Тогда
		...
		Если Элемент <> Неопределено Тогда
			// Зарпетить установку обещаний для архивных дел
			Если Не ЭтаФорма.ПредупреждениеАрхив И Не ЭтаФорма.ПредупреждениеЮД Тогда
				...
				КонецЕсли;
			Иначе
				Предупреждение("Создание обещаний для таких договоров запрещено!",,"Предупреждение");
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли СвязанныйДокумент = ПредопределенноеЗначение("Перечисление.ВидыСвязанныхДокументов.РеструктуризацияДолга") Тогда	
		Элементы.ГруппаСвязанныйДокумент.Видимость = Истина;
...
&НаКлиенте
Процедура УстановитьСтатусНеБеспокоитьФорма()
...
&НаКлиенте
Процедура УстановитьСтатусНеБеспокоитьГлобальный()
	ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
	Если ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусНеБеспокоить = Ложь И ФормаНабора.ИдетРазговор = Ложь Тогда
		ТелефонияСлужебный.ПерехватитьУправлениеТелефонией(Ложь);
		ФормаНабора.кнНеБеспокоить(Неопределено);
		ПодключитьОбработчикОжидания("ВключитьДоступностьИнструментов", 3, Истина);
	Иначе
		// Разрешить управление инструментами
		ВключитьДоступностьИнструментов();
	КонецЕсли;
	// Передать управление обработчику ожидания
	ПодключитьОбработчикОжидания("ПроверитьСостояниеБИТФона", 1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьДоступностьИнструментов()
	ЭтаФорма.Элементы.ГруппаИнструменты.Доступность = Ложь;
	ЭтаФорма.Элементы.ГруппаУправлениеЗвонком.Доступность = Ложь;
	ЭтаФорма.Элементы.ГруппаРезультат.Доступность = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ВключитьДоступностьИнструментов()
	ЭтаФорма.Элементы.ГруппаИнструменты.Доступность = Истина;
	ЭтаФорма.Элементы.ГруппаУправлениеЗвонком.Доступность = Истина;
	ЭтаФорма.Элементы.ГруппаРезультат.Доступность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьСостояниеБИТФона()
	ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьВремяРаботы" Тогда
		...
		Если ЭтаФорма.Объект.Выполнена = Истина Тогда
			Возврат;
		Иначе
	...
	Если ИмяСобытия = "СброшенныйЗвонок" Тогда
		Если ЭтаФорма.НеОбрабатыватьСтатусБитФона Тогда
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ЭтаФорма.Объект.Результат) = Истина
				И ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Результат, "АнализируетсяБлокировщиком") = Истина Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Объект.Результат) = Ложь Тогда
				...
				Иначе
					...
					//
				КонецЕсли;
			ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Объект.Результат) = Истина
				И ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Результат, "АнализируетсяБлокировщиком") = Ложь Тогда
				//ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");				
				//ФормаНабора.кнОтключить(Неопределено);
				ТелефонияСлужебный.ПерехватитьУправлениеТелефонией(Ложь);
				//
				ПоказатьОповещениеПользователя("БИТ.Phone",,"Разговор завершен",БиблиотекаКартинок.NC_РазговорЗавершен);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмяСобытия = "ШаблонИзмененСМС" Тогда
		...
		ИначеЕсли ЭтаФорма.Объект.ТипМероприятия = ПредопределенноеЗначение("Справочник.ТипыМероприятий.ОтправкаСМС") Тогда	
			...
			// Пробуем закрыть форму отправки и выполнить мероприятие
			Попытка
				ФормаСМС.Закрыть();
				ВыполнитьИЗакрыть(Неопределено);
			Исключение
...
Процедура ОтправитьСМС(Команда)
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) Тогда
		Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
			...
			ФормаСМС = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОтправкаСМС");
			ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
			Если ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусПодключен = Истина И ФормаНабора.ИдетРазговор = Ложь Тогда
				ТелефонияСлужебный.ПерехватитьУправлениеТелефонией(Ложь);
			ИначеЕсли ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусНеБеспокоить = Истина И ФормаНабора.ИдетРазговор = Ложь Тогда
				ТелефонияСлужебный.ПерехватитьУправлениеТелефонией(Истина);
				//
			КонецЕсли;
			...
			ОткрытьФорму(ФормаСМС,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
...
Процедура ЗакрепитьДолжника(Команда)
	НовоеЗакреплениеНаСервере();
	Если ЭтаФорма.Объект.ТипМероприятия = ЭтаФорма.ТипЗаявкаЗакрепление Тогда
		// Открыть форму для записи ответственного сотрудника, если заявка создана с учетной записи РКЦ
		Если ЭтоРКЦ() Тогда 
			ФормаВыбораОтветственного = ПолучитьФорму("ОбщаяФорма.ВыборОтветственногоПоДО");
			ФормаВыбораОтветственного.ДО = ЭтаФорма.ДолговоеОбязательство;
			ФормаВыбораОтветственного.Открыть();
		Иначе
			ЗакрепитьДолжникаНаСервере();
		КонецЕсли;
...
Процедура ДолжникМероприятия(Команда)
	...
	ФормаМероприятия.Элементы.Список.Автообновление = Ложь;
	ОткрытьФорму(ФормаМероприятия,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ДолжникОбязательства(Команда)
	...
	ОткрытьФорму(ФормаОбязательства,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ДолжникОбещания(Команда)
	...
	ОткрытьФорму(ФормаОбещания,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ДолжникПогашения(Команда)
	...
	ОткрытьФорму(ФормаПогашения,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ДолжникПогашенияСудебные(Команда)
	...
	ОткрытьФорму(ФормаПогашенияСудебные,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ДолжникСМС(Команда)
	...
	ОткрытьФорму(ФормаСпискаСМС,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ВходящийЗвонокПриИзменении(Элемент)
	// Исправить комментарий, если пометка была установлена вручную и результат мероприятия изменился на "Входящий звонок"
	Объект.Комментарий = СтрЗаменить(Объект.Комментарий, "Входящий звонок → ", "");
	Объект.Комментарий = "Входящий звонок → " + Объект.Комментарий;
	Если ЭтаФорма.Элементы.ВходящийЗвонок.Видимость = Истина Тогда
		ЭтаФорма.Элементы.ВходящийЗвонок.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура НаписатьВВЦ(Команда)
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) Тогда
		...
		ОткрытьФорму(ФормаВЦ,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НаписатьВВайбер(Команда)
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) Тогда
		...
		ОткрытьФорму(ФормаВайбер,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ИсполнительныеПроизводства(Команда)
	Отбор = Новый Структура("Должник", ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"));  
	ФормаСпискаИП = ПолучитьФорму("РегистрСведений.ИсполнительныеПроизводства.ФормаСписка", Новый Структура("Отбор", Отбор));
	ФормаСпискаИП.КоманднаяПанель.Видимость = Ложь;
	ОткрытьФорму(ФормаСпискаИП,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ДелаОБанкротстве(Команда)
	Отбор = Новый Структура("Должник", ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"));  
	ФормаСпискаДоБ = ПолучитьФорму("РегистрСведений.ДелаОБанкротстве.ФормаСписка", Новый Структура("Отбор", Отбор));
	ФормаСпискаДоБ.КоманднаяПанель.Видимость = Ложь;
	ОткрытьФорму(ФормаСпискаДоБ,,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
