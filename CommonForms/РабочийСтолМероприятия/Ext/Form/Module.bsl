...
Процедура ПриОткрытии(Отказ)
	...
	НоваяВерсияКонфигурацииNC = "3.5.23.1";
	...
	Если День(ТекущаяДата()) = 1
		ИЛИ День(ТекущаяДата()) = 11
		ИЛИ День(ТекущаяДата()) = 31 Тогда
		Ответ = Вопрос("Для корректной работы WhatsApp требуется актуальная версия клиента. Проверить наличие обновлений сейчас? Операция может занять длительное время.",РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да,"Интеграция с WhatsApp");
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ЭтаФорма.УстановитьВЦ(Неопределено);	
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьПортфель(Команда)
	Предупреждение("Пожалуйста, подождите...",1,"Обновление портфеля");
	Элементы.СписокДО.Обновить();
...
Процедура ОбновитьОстатки(Команда)
	Если ЭтаФорма.РольРКЦ = Истина ИЛИ ЭтаФорма.РольАБД = Истина Тогда
		ПоказатьОповещениеПользователя("Обновление остатков",,"Задание запущено в фоновом режиме",БиблиотекаКартинок.NC_Подсистема_тсАдминистрирование,СтатусОповещенияПользователя.Важное);
...
Процедура УстановитьВЦ(Команда)
	...
	Пока EXE.Размер() <> "162 690 784" Цикл
		Прервать;
	КонецЦикла;
	ПоказатьОповещениеПользователя("Установка WhatsApp",,"Запуск программы установки...",БиблиотекаКартинок.NC_Установка);
