...
&НаСервере
Функция ПолучитьОКАТОПоИндексуФИАС(Индекс) Экспорт 
	Попытка
		ОписаниеТипа = Новый ОписаниеТипов("Число");
		ЧислоИндекс = ОписаниеТипа.ПривестиЗначение(Индекс);
		// ЧислоИндекс = Число(Индекс);
	Исключение
		Возврат 0;
	КонецПопытки;
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      ...
	                      |	ВЫРАЗИТЬ(ДополнительныеАдресныеСведения.ПочтовыйИндекс / 1000 - 0.5 КАК ЧИСЛО(3, 0))
	                      |ИЗ
	                      |	РегистрСведений.ДополнительныеАдресныеСведения КАК ДополнительныеАдресныеСведения
	                      |ГДЕ
	                      |	(ВЫРАЗИТЬ(ДополнительныеАдресныеСведения.ПочтовыйИндекс / 1000 - 0.5 КАК ЧИСЛО(3, 0))) = &Индекс");
	...
	Результат = Запрос.Выполнить().Выбрать();
	Если Результат.Следующий() Тогда
		ОКАТОЗнач = Строка(Результат.OKATO);
		Если СтрДлина(ОКАТОЗнач) = 13 Тогда
			ОКАТОЗнач = "0" + ОКАТОЗнач;
		КонецЕсли;	
		Значение = ОКАТОЗнач;
	Иначе
		Значение = "99 999 999 999";
	КонецЕсли;    
...
&НаСервере
Функция ВычислитьЗначениеСтраныОКСМ(вхЗначение) Экспорт 
	
	
	струкСтрана = Новый Структура("ОКСМ, Наименование");
	струкСтрана.Вставить("ОКСМ","643");
	струкСтрана.Вставить("Наименование","РОССИЯ");  
	
	Если ЗначениеЗаполнено(вхЗначение) Тогда
		
		ЗначениеПоиска = Строка(вхЗначение);
		...
		|ГДЕ
		|	СтраныМира.Код = &ЗначениеПоиска
		|	ИЛИ СтраныМира.Наименование = &ЗначениеПоиска";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			струкСтрана.ОКСМ = ВыборкаДетальныеЗаписи.Код;
			струкСтрана.Наименование = ВыборкаДетальныеЗаписи.Наименование;
		КонецЦикла;
