...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.Ссылка.Пустая() И Объект.ВыборСоответствий.Количество() = 0 Тогда
		...
		Для Каждого Элемент Из СписокРеквизитов Цикл
			...
			Если Элемент.Значение = "Наименование" ИЛИ Элемент.Значение = "НаименованиеКА" ИЛИ Элемент.Значение = "НаименованиеКР" ИЛИ Элемент.Значение = "Идентификатор" ИЛИ 
					Элемент.Значение = "ТипДОЛ" ИЛИ Элемент.Значение = "ИдентификаторКА" ИЛИ Элемент.Значение = "ИдентификаторКР" ИЛИ Элемент.Значение = "ТипКР" ИЛИ Элемент.Значение = "ТипКА" Тогда
...
Процедура ЗаполнитьТаблицуДанных()
	...
	Если тзДанныеХранилища = Неопределено Тогда
		тзДанныеЗагрузки = УправлениеЗагрузкамиДанных.ИзвлечьТабличнуюЧастьВЭлементИзРеквизитаЗД(тзДанные, тзДанныеХранилища);		
	Иначе
...
Процедура ПерезаполнитьЗаголовки(тзДанныеХранилища)
	Для НомерКолонки = 1 По тзДанныеХранилища.Колонки.Количество() Цикл
		...
		ИначеЕсли Имя = "Идентификатор" Тогда  
			Заголовок = "Код должника"; //!
		ИначеЕсли Имя = "ИдентификаторКА" Тогда  
			Заголовок = "Код контрагента"; //!
		ИначеЕсли Имя = "ИдентификаторКР" Тогда  
			Заголовок = "Код кредитора"; //!
		ИначеЕсли Имя = "ИдентификаторУ" Тогда
			Заголовок = "Код услуги";
		ИначеЕсли Имя = "ИдентификаторИД" Тогда
			Заголовок = "Код исполнительного документа";
		ИначеЕсли Имя = "ИдентификаторД" Тогда
			Заголовок = "Код договора";
		...
		ИначеЕсли Имя = "ЗадолженностьВалютаЗагрузки" Тогда
			Заголовок = "Валюта загрузки";
		ИначеЕсли Имя = "ЗадолженностьОсновнойДолг" Тогда
			Заголовок = "Основной долг";	
		ИначеЕсли Имя = "ЗадолженностьПроценты" Тогда
			Заголовок = "Проценты";
		ИначеЕсли Имя = "ЗадолженностьШтрафы" Тогда
			Заголовок = "Штрафы";
		ИначеЕсли Имя = "ЗадолженностьПени" Тогда
			Заголовок = "Пени";
		ИначеЕсли Имя = "ЗадолженностьСоставляющая1" Тогда
			Заголовок = "Составляющая1";
		ИначеЕсли Имя = "ЗадолженностьСоставляющая2" Тогда
			Заголовок = "Составляющая2";
		ИначеЕсли Имя = "ЗадолженностьСоставляющая3" Тогда
			Заголовок = "Составляющая3";
		Иначе
			Число1 = Неопределено;
			Число2 = Неопределено;
			Попытка
				//--Число2 = Число(Сред(Имя, 3));
				//--Число1 = Число(Сред(Имя, 2));				
				Число2 = Число(СтрЗаменить(Сред(Имя, 3),"_",""));
				Число1 = Число(СтрЗаменить(Сред(Имя, 2),"_",""));				
			Исключение	
			КонецПопытки;
			
			Если Число1 <> Неопределено Тогда
				Элемент = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоКоду(СтрЗаменить(Сред(Имя, 2),"_"," "));
				Если Элемент <> Неопределено Тогда
					Заголовок = Элемент.Наименование;
				КонецЕсли;
			ИначеЕсли Число2 <> Неопределено Тогда
				Элемент = ПланыВидовХарактеристик.ТипыСотрудников.НайтиПоКоду(СтрЗаменить(Сред(Имя, 3),"_"," "));
				Если Элемент <> Неопределено Тогда
					Заголовок = Элемент.Наименование;
				КонецЕсли;
...
Процедура ЗагрузитьДанные(Команда)
	...
	тзДанные.Очистить();	
	Мегазагрузка(Объект.Ссылка);	
КонецПроцедуры

&НаСервере
Процедура Мегазагрузка(ОбъектСсылка);
	ЗаполнитьДанные(ОбъектСсылка, "тзДанные");	
	ЗаполнитьДанные1(ОбъектСсылка, "тзДанные");
...
Процедура ЗаполнитьДанные(ДокументОбъект, ИмяТаблицы)
	...
	//	Для НомерСтрокиТаблицыИмпорта = 2 По ПолеИсходнойТаблицы.ВысотаТаблицы Цикл
			...
			КонецЕсли;
	    Таб.ЗагрузитьКолонку(Массив, ТекСтрока.ИмяКолонки);
	КонецЦикла;
...
//  ИмяТаблицы   - имя таблицы. -
// 
Процедура ЗаполнитьДанные1(ДокументОбъект, ИмяТаблицы)
	Для НомерСтрокиТаблицыИмпорта = 2 По ПолеИсходнойТаблицы.ВысотаТаблицы Цикл
		ПустаяСтрокаТаблицы = Истина;
		Для Номер = 1 По ПолеИсходнойТаблицы.ШиринаТаблицы Цикл
			Если ЗначениеЗаполнено(ПолеИсходнойТаблицы.Область("R" + Формат(НомерСтрокиТаблицыИмпорта, "ЧГ=") + "C" + 
					...
					Если Элемент.Представление = Строка(ТекСтрока.ИмяДанных) Тогда	
						...
						Иначе
							...
							Если ТипДома = "дом" Тогда
								...
								//Удалить Владение
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Владение");

							ИначеЕсли ТипДома = "владение" Тогда	
							    СписокПараметров.Добавить(ТипДома, "ТипДома");
								//Удалить Дом
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Дом");
							...
							Если ТипКорпуса = "корпус" Тогда
								...
								//Удалить Строение
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Строение");
	
							ИначеЕсли ТипКорпуса = "строение" Тогда
								СписокПараметров.Добавить(ТипКорпуса, "ТипКорпуса");
								//Удалить Корпус
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Корпус");
							...
							Если ТипКвартиры = "кв." Тогда
								...
								//Удалить Офис
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Офис");

							ИначеЕсли ТипКвартиры = "оф." Тогда
								СписокПараметров.Добавить(ТипКвартиры, "ТипКвартиры");
								//Удалить Квартира
								КоллекцииСервер.УдалитьПараметрИзСписка(СписокПараметров, "Квартира");
			...
			ИначеЕсли ТекСтрока.ТипДанных = "Число" Тогда
				...
				Для Каждого Элемент Из Объект.ВыборСоответствий Цикл
					Если Элемент.ИмяДанных = ТекСтрока.ИмяДанных Тогда
						НоваяСтрока[Элемент.ИмяКолонки] = Результат;
						Прервать;
					КонецЕсли
				КонецЦикла;
...
Функция ВозвратТипЗначенияКолонки(ВыбранноеЗначение)
	...
	ИначеЕсли Переменная = Тип("СправочникСсылка.Пользователи") Тогда
		Возврат "Справочник ссылка: Пользователи";	
	Иначе
...
Процедура ЗаписатьДолжниковФоновымПроцессом(Команда)
	//Чуров
	ПередЗаписьюСправочниковОповещение(Истина);
...
//Чуров++
&НаКлиенте
Процедура ПередЗаписьюСправочниковОповещение(ФоновыйПроцесс = Ложь)
	
	Если ПроверкаПередЗаписьюСправочников() Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗаписьюСправочниковОповещениеЗавершение", ЭтаФорма, новый Структура("ФоновыйПроцесс", ФоновыйПроцесс)),
			"Внимание! Данные должников будут безвозвратно изменены. Продолжить?", РежимДиалогаВопрос.ОКОтмена,,,"Изменение данных дожников");
...
&НаКлиенте
Процедура ПередЗаписьюСправочниковОповещениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Если Не ДополнительныеПараметры.ФоновыйПроцесс Тогда
...
Процедура ЗаписатьДолжников(Команда)
	...
	//Чуров
	Иначе
		ПередЗаписьюСправочниковОповещение();
	КонецЕсли;
...
Функция ЗаписатьДолжниковНаСервере()
	...
	Если Константы.УчетПоДО.Получить() И Объект.КонтрольДО Тогда
...
Функция ЗаписатьСправочникиВФоне()
	...
	Запись.ДатаДобавленияВРегистр = ТекущаяДатаСеанса();
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Модифицированность Тогда
		...
		Отказ = истина;
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтаФорма), "Данные были изменены! Сохранить изменения?", 
...
Процедура ВыборСоответствийПередНачаломДобавленияСервер(Результат)
	Если Объект.ВыборСоответствий.НайтиСтроки(Новый Структура("ИмяДанных", Результат)).Количество() = 0 Тогда	
		...
		КонецЕсли;
		Нов.ИмяКолонки = "_" + СтрЗаменить(Результат.Код, " ", "_");
...
Процедура ВыборСоответствийПараметрыСвязиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Элемент.Родитель.ТекущиеДанные.ТипДанных = "Число" ИЛИ 
...
Процедура УдалитьКолонкуИзТаблицыДанные(Знач Элемент)
	...
	Элементы.Удалить(Элементы.Найти("тзДанные" + "_" + СтрЗаменить(Элемент.Код, " ", "_")));
		
	//1 Удалить из Объект.тзДанные
	КолонкиТаблицы = Новый Массив;
	КолонкиТаблицы.Добавить("тзДанные." + "_" + СтрЗаменить(Элемент.Код, " ", "_"));	
