Процедура СоздатьДоБ(Контрагент, ДатаПоследнейПроверки = Неопределено) Экспорт
	...
	Если ИДДолжника <> Неопределено Тогда
		...
		Иначе		
			...
			|			И ИдентификаторыСообщенийБанкротство.Регистратор.Контрагент = СостояниеДоБ.Контрагент
			|ГДЕ
			...
			Пока ВыборкаПерваяГруппировка.Следующий() Цикл
				...
				Пока ВыборкаНомерСудебногоДела.Следующий() Цикл
					...
					ДелоОБанкротстве.ДатаПоследнейПроверки = ТекущаяДатаСеанса();
...
Функция ПолучитьДанныеМетодом(Метод, Знач ДатаНачала = Неопределено, Знач ДатаОкончания = Неопределено, ТипКода = Неопределено, ЗначениеКода = Неопределено,
	...
	Попытка 
		...
		ИначеЕсли Результат.КодСостояния = 401 Тогда
			Данные = "Код ошибки: 401, проверьте логин и пароль!";
			Возврат Данные;	
		Иначе
...
Процедура Банкротство_ЗагрузкаСообщенийИДоБ() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫРАЗИТЬ(ОбъектыВРаботеОстатки.Объект КАК Справочник.ДолговыеОбязательства) КАК Объект
	|ПОМЕСТИТЬ втДОвРаботе
	|ИЗ
	...
	|ПОМЕСТИТЬ втКотрагентыВРаботе
	|ИЗ
	|	Справочник.ДолговыеОбязательства.Контрагенты КАК ДолговыеОбязательстваКонтрагенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втДОвРаботе КАК втДОвРаботе
	|		ПО ДолговыеОбязательстваКонтрагенты.Ссылка = втДОвРаботе.Объект
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втКотрагентыВРаботе.Значение КАК Контрагент,
	|	ДелоОБанкротстве.ДатаПоследнейПроверки КАК ДатаПоследнейПроверки
	|ИЗ
	|	втКотрагентыВРаботе КАК втКотрагентыВРаботе
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДелоОБанкротстве КАК ДелоОБанкротстве
	|		ПО втКотрагентыВРаботе.Значение = ДелоОБанкротстве.Контрагент";
	
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДатаСеанса());
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.Контрагент)
			ИЛИ Выборка.Контрагент.ПолучитьОбъект() = Неопределено Тогда
