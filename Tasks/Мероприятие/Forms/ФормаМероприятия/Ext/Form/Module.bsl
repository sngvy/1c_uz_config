...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("УстановитьСтатусНеБеспокоить", 2, Истина);
	...
	Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
		...
		Пока ЭлементСпискаВыбора < ЭтаФорма.Элементы.Контакт.СписокВыбора.Количество() И Не КонтактНайден Цикл
			Представление = ЭтаФорма.Элементы.Контакт.СписокВыбора.Получить(ЭлементСпискаВыбора).Представление;
			ЧислоВхождений = СтрЧислоВхождений(ВРег(Представление), ВРег(ОбъектДолжник));
			Если ЧислоВхождений = 1 Тогда
				ЭтаФорма.Объект.Контакт = ЭтаФорма.Элементы.Контакт.СписокВыбора.Получить(ЭлементСпискаВыбора).Значение;
	...
	Если ЭтаФорма.Доступность = Ложь Тогда
		ЭтаФорма.Закрыть();
		Если ЗначениеЗаполнено(ЭтаФорма.ЭтотОбъект.Ответственный) = Истина Тогда
			ПоказатьОповещениеПользователя("Мероприятие заблокировано",,"Мероприятие уже редактирует " + ЭтаФорма.ЭтотОбъект.Ответственный, БиблиотекаКартинок.NC_Заблокировано);
		Иначе
...
Процедура ПроверкаНаБанкротство(Команда)
	...
	Ответ = Вопрос("Какой ресурс использовать для проверки?",СписокКнопок,,КодВозвратаДиалога.Да,"Проверка на банкротство");
	Если Ответ = КодВозвратаДиалога.Да Тогда
