...
Процедура ЗаписатьКонтакты(ТекущаяСтрока, ТекущийОбъект, Назначение, Обновлять) Экспорт
	Если Обновлять Тогда
		...
		Для Каждого Элемент ИЗ Строки Цикл  			
			...
			// Не форматировать почту и ссылки
			Если Не СтрНайти(НомерТел, "@") > 0
				И Не СтрНайти(НомерТел, ".ru") > 0
				И Не СтрНайти(НомерТел, ".com") > 0 Тогда
			...
			Если НайденнаяСтрока = Неопределено Тогда			
				...
				// Не форматировать почту и ссылки
				Если Не СтрНайти(НовСтр.Номер, "@") > 0
					И Не СтрНайти(НовСтр.Номер, ".ru") > 0
					И Не СтрНайти(НомерТел, ".com") > 0 Тогда
