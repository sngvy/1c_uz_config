...
Функция ВернутьОбъектыПоИсточнику(ДокументОбъект, ЭтоПроведение) Экспорт
	...
	Для Каждого СтрДвижение Из Движения Цикл 
		Если СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Задолженность") = 0 И СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Расчеты") = 0  И СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Накопления") = 0 Тогда
			Продолжить; 
