...
Процедура ПодключитьАТС(Команда)
	...
	Иначе
		ПодключитьВК();
	КонецЕсли;
	ПоказатьОповещениеПользователя("БИТ.Phone",,"Подключение к АТС установлено",БиблиотекаКартинок.NC_Подсистема_бтБИТАТС);
КонецПроцедуры

&НаКлиенте
Процедура ОтключитьАТС(Команда)
	Отключить();
	ПоказатьОповещениеПользователя("БИТ.Phone",,"Подключение к АТС разорвано",БиблиотекаКартинок.NC_Подсистема_бтБИТАТС);
КонецПроцедуры
