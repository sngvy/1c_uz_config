...
Процедура кнПодключить(Команда) Экспорт
	ПоказатьОповещениеПользователя("БИТ.Phone",,"На линии",БиблиотекаКартинок.NC_Онлайн);
...
Процедура кнОтключить(Команда) Экспорт
	ПоказатьОповещениеПользователя("БИТ.Phone",,"Не на линии",БиблиотекаКартинок.NC_Оффлайн);
	ОтключитьСофтФон();
КонецПроцедуры
...
&НаКлиенте
Процедура ОтклонитьВходящийВызовНаКанале(НомерКанала) Экспорт
	Если СофтФон <> Неопределено Тогда
		// Заглушка для значения IsIncomingCall
		Попытка
			входящийВызов = СофтФон.IsIncomingCall(НомерКанала);
...
&НаКлиенте
Функция НайтиСвободнуюЛинию(НомерСвободногоКанала) Экспорт
	...
	Если РежимНеБеспокоитьНаАТС Тогда
		...
		Иначе
			...
			Иначе
				...
				вызовУстСтатусаУспешен = НачатьИсходящийВызовНаКанале(ПолучитьКаналТекущейЛинии(), стрКомандаНеБеспокоить);
				// Переподключение в случае неуспеха
				Если Не вызовУстСтатусаУспешен Тогда					
					ЭтаФорма.кнПереподключить(Неопределено);						
					вызовУстСтатусаУспешен = НачатьИсходящийВызовНаКанале(ПолучитьКаналТекущейЛинии(), стрКомандаНеБеспокоить);
				КонецЕсли;
				Если вызовУстСтатусаУспешен Тогда
					Элементы.ГруппаБИТфонПанели.Доступность = Ложь;
	...
	Если РежимНеБеспокоитьНаАТС Тогда
		...
		Иначе
			...
			Иначе
				...
				ТелефонияСлужебный.ПерехватитьУправлениеТелефонией(Ложь);
				вызовОтмСтатусаУспешен = НачатьИсходящийВызовНаКанале(ПолучитьКаналТекущейЛинии(), стрКомандаОтменитьНеБеспокоить);
				// Переподключение в случае неуспеха
				Если Не вызовОтмСтатусаУспешен Тогда
					ЭтаФорма.кнПереподключить(Неопределено);						
					вызовОтмСтатусаУспешен = НачатьИсходящийВызовНаКанале(ПолучитьКаналТекущейЛинии(), стрКомандаОтменитьНеБеспокоить);
				КонецЕсли;
				Если вызовОтмСтатусаУспешен Тогда
