...
Функция ПолучитьНомер(ТипПисьма, Организация, ДО) Экспорт
	...
	МенеджерЗаписи.ПериодНумерации = НачалоПериодаНумерации(ТипПисьма.Нумератор.Периодичность, ТекущаяДата());
	...
	МенеджерЗаписи.Организация = Организация;
	МенеджерЗаписи.ПериодНумерации = НачалоПериодаНумерации(МенеджерЗаписи.Нумератор.Периодичность, ТекущаяДата());
...
Процедура ЗаполнитьТЧФайлы(ИсхКорр,Об,ШаблонПечФормы)
