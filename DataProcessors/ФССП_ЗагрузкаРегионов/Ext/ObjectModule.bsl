...
Процедура ЗагрузитьДанные(ПолеИсходнойТаблицы, ОтправлятьСообщенияОбОшибке) Экспорт
	...
	Для НомерСтроки = 2 По ПолеИсходнойТаблицы.ВысотаТаблицы Цикл
		КодРегионаЗнач = ПолеИсходнойТаблицы.Область(НомерСтроки, ЭтотОбъект.КодРегиона).Текст;
		НаименованиеРегионаЗнач = ПолеИсходнойТаблицы.Область(НомерСтроки, ЭтотОбъект.НаименованиеРегиона).Текст;
...
Процедура ЗаполнитьНомераСтолбцов() Экспорт
	...
	ЭтотОбъект.НаименованиеРегиона = 2;
