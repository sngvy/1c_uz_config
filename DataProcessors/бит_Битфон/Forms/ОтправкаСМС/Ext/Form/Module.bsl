...
Функция ВыполнитьОтправкуСМСНаСервер(списокНомеров)
	...
	Если Не ЭтаФорма.МассоваяОтправка Тогда
		Для Счетчик = 1 По 100 Цикл
			Состояние("Отправка СМС", Счетчик, "Пожалуйста, подождите...", БиблиотекаКартинок.NC_СМС);
		КонецЦикла;
	КонецЕсли;
...
Процедура Отправить(Команда) Экспорт
	...
	Если ВидПолучателя = 0 Тогда
		Если ЗначениеЗаполнено(НомерПолучателя) И ЗначениеЗаполнено(ЭтаФорма.ТекстСМС) Тогда
			номера = Новый СписокЗначений;
			...
			КонецЕсли;
		Иначе
			бит_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не заполнены обязательные поля!",,"Отправка СМС");
			Возврат;
		КонецЕсли;
