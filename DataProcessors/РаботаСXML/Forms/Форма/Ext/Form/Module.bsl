...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Элемент Из Элементы.Группа7.ПодчиненныеЭлементы Цикл
...
Процедура ВыгрузитьДанныеНаСервере(АдресФайлаВоВременномХранилище)
	...
	Настройка = Объект.ВариантЗагрузки; 
	Если Настройка = ПолучитьВариантЗагрузкиЮрСлужба() Тогда
		ЗаменитьСсылки(ИмяВременногоФайла,Ложь);			
	КонецЕсли;
...
Процедура ЗагрузитьДанныеЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	#Иначе	
		...
		Если Результат Тогда
			...
			Данные = Новый ДвоичныеДанные(ВыбранноеИмяФайла);
			...
			Настройка = Объект.ВариантЗагрузки; 
			Если Настройка = ПолучитьВариантЗагрузкиЮрСлужба() Тогда
				ЗаменитьСсылки(ИмяТемпФайла,Истина);			
			КонецЕсли;
			...
			ДанныеТемп = Новый ДвоичныеДанные(ИмяТемпФайла);
			АдресТемп  = ПоместитьВоВременноеХранилище(ДанныеТемп,УникальныйИдентификатор);
			//Данные.Записать(ИмяТемпФайла);
			
			//ЗагрузитьДанныеНаСервере(Адрес, Файл.Расширение);
			ЗагрузитьДанныеНаСервере(АдресТемп,Файл.Расширение);
		КонецЕсли;
	#КонецЕсли
...
Процедура ВариантЗагрузкиПриИзменении(Элемент)
	Если НЕ Объект.ВариантЗагрузки = ПолучитьВариантЗагрузкиЮрСлужба() Тогда
		Для Каждого Элемент Из Элементы.Группа7.ПодчиненныеЭлементы Цикл
			Элемент.Видимость = Ложь; 
		КонецЦикла;
	Иначе
...
&НаСервере
Процедура ЗаменитьСсылки(ИмяФайла, Загрузка)
	Список = Новый СписокЗначений();
	ВариантНастройки = Объект.ВариантЗагрузки;                    
	Если ВариантНастройки = Перечисления.ВариантыНастроек.ЮрСлужба Тогда
		Список.Добавить("d626b5c4-745b-11e8-9935-78e7d1889294", Строка(Объект.ЮрСлужба_Рук.УникальныйИдентификатор()));
		Список.Добавить("d626b5c5-745b-11e8-9935-78e7d1889294", Строка(Объект.ЮрСлужба_Отв.УникальныйИдентификатор()));
		Список.Добавить("d626b5c6-745b-11e8-9935-78e7d1889294", Строка(Объект.ЮрСлужба_Рег.УникальныйИдентификатор()));
		Список.Добавить("ю_Отв_По_Делу", Строка(Объект.ЮрСлужба_Отв.Код));
		Список.Добавить("ю_Сотр_В_Регионе", Строка(Объект.ЮрСлужба_Рег.Код));
	КонецЕсли;
	Если Загрузка Тогда
		ЗаменитьСсылкиВФайле(ИмяФайла, Список);
	ИначеЕсли Не Загрузка Тогда
		ЗаменитьСсылкиВФайлеВыгрузка(ИмяФайла,Список);
	КонецЕсли;	
КонецПроцедуры
...
КонецПроцедуры
&НаСервере
Процедура ЗаменитьСсылкиВФайлеВыгрузка(ИмяФайла, Список)
	Если Список.Количество() > 0 Тогда
	    ТД = Новый ТекстовыйДокумент();
		ТД.Прочитать(ИмяФайла);
		Стр = ТД.ПолучитьТекст();
		Для Каждого Элемент Из Список Цикл 
			Стр = СтрЗаменить(Стр,Элемент.Представление,Элемент.Значение);
		КонецЦикла;
		ТД.УстановитьТекст(Стр);
		ТД.Записать(ИмяФайла);
	КонецЕсли;
...
&НаСервере
Функция ПолучитьВариантЗагрузкиЮрСлужба()
	Возврат Перечисления.ВариантыНастроек.ЮрСлужба;
КонецФункции
