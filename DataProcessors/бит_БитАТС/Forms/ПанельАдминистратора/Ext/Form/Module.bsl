...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "БитфонПанельБИТАТС_ПолучитьПеременнуюКанала" Тогда
		
		Если ЗначениеЗаполнено(Параметр.Результат) Тогда
			стрИмяПеременной = Параметр.Результат;
			ПолучитьПеременнуюКаналаЗавершение(стрИмяПеременной);
		КонецЕсли;
...
&НаКлиенте
Процедура ПолучитьПеременнуюКанала(Команда)
	стрИмяПеременной = "";
	бит_ТелефонияКлиентПереопределяемый.ПоказВводСтроки(стрИмяПеременной, "Имя переменной канала", "БитфонПанельБИТАТС_ПолучитьПеременнуюКанала");
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьПеременнуюКаналаЗавершение(стрИмяПеременнойКанала)
	ТекущаяСтрокаНомеров = Элементы.Номера.ТекущиеДанные;
	Если ТекущаяСтрокаНомеров <> Неопределено Тогда
		ИмяКанала			= ТекущаяСтрокаНомеров.Канал;
		//
		...
		Попытка
			Если ИспользоватьВебСервис Тогда
				стрЗначПеременнойКанала = бит_АТССервер.ВебСервисПолучитьПеременнуюКанала(ТекущаяАТС, ВебСервисИдКлиента, ИмяКанала, стрИмяПеременнойКанала);
			Иначе
				стрЗначПеременнойКанала = обКонтроллерАТС.GetChannelVariable(ИмяКанала, стрИмяПеременнойКанала);
			КонецЕсли;
			бит_ТелефонияКлиент.ВывестиСообщение("Значение переменной " + стрИмяПеременнойКанала + ": " + стрЗначПеременнойКанала, ЭтаФорма);
		Исключение
			бит_ТелефонияКлиент.ВывестиСообщение("Ошибка получения переменной канала - " + ОписаниеОшибки(), ЭтаФорма);
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
