...
Процедура БитОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) 
	...
	ИначеЕсли MandarinОперации Тогда
		ОбработатьОткрытиеMandarinОперации(ДанныеЗаполнения.ЕмаилПлательщика,ДанныеЗаполнения.ВидЗаказа,ДанныеЗаполнения.СсылкаДляПлательщика,ДанныеЗаполнения.ОбъектУчета);
	Иначе
...
Процедура ОбработатьОткрытиеMandarinОперации(Емаил,ВидЗаказа,СсылкаДляПлательщика,ОбъектУчета)
	...
	Текст = Автоинформирование.ПолучитьПодсказку(ОбъектУчета,ШаблонПисьма);
	Текст = Текст + Строка(СсылкаДляПлательщика);
КонецПроцедуры   
