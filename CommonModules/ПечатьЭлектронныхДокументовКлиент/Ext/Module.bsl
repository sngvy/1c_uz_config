...
//
Процедура СформироватьДокументПриРаботеВФормеРегистра(Знач ТекущиеДанные = Неопределено, ПрикрепитьФайлы, 
	...
	ИначеЕсли ОбъектыСервер.ВычислитьФункцию(
		"Значение = Справочники.тсВидыПечатныхДокументов.Количество(ОбъектВх)",
		ТекущиеДанные.ВидПечатногоДокумента,
		Неопределено,
		Неопределено) = 0 Тогда
		
		Если ОбъектыСервер.РазыменоватьСсылку(ТекущиеДанные.ВидПечатногоДокумента, "ЭтоГруппа") = Ложь Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Для данного печатного документа не задан шаблон: " + ТекущиеДанные.ВидПечатногоДокумента;
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	...
	РегистрационныйНомер = ТекущиеДанные.РегистрационныйНомер;
	
	МассивДокументовНаПечать = Новый Массив;
	СтруктураРезультат = ОбработкаПутейИГенерацияДокументов(
		Объект,
		ВидПечатногоДокумента,
		КаталогПоУмолчанию,
		СтрокаТаблициПлатежиЛП,
		МассивДокументовНаПечать,
		ГлобальнаяСклейка,
		ПрикрепитьФайлы,
		НеПечататьПовторно,
...
Процедура ПечатьОбработка(Объект, Печатать = Ложь, ПрикрепитьФайлы) Экспорт
	...
	РегистрационныйНомер = ПечатьЭлектронныхДокументовСервер.РегистрационныйНомерКомплекта(Объект.Объект, Объект.Шаблон);
	Таблица.Вставить("РегистрационныйНомер", РегистрационныйНомер);
	...
	Наборы = ПолучитьШаблоныДляПечати(Объект.Шаблон);
	
	Для каждого Набор Из Наборы Цикл
	
		ДляКонвертации = Новый Массив;
		ДляКонвертации.Добавить(Объект.Объект);
		Основания = ПолучитьОбъектыПоНазначению(ДляКонвертации, Набор);
		Для каждого Основание Из Основания Цикл
		
			Таблица.Вставить("Объект", Основание);
			
			Таблица.Вставить("ВидПечатногоДокумента", Набор);
			
			СформироватьДокументПриРаботеВФормеРегистра(
				Таблица,
				ПрикрепитьФайлы,
				,
				Печатать
...
Процедура ПечатьОбъекты(МассивОбъект, Шаблон, ДокСсылка, Каталог_ПретензионноеПисьмо = "", ПрикрепитьФайлы) Экспорт
	...
	ШаблоныДляПечати = ПолучитьШаблоныДляПечати(Шаблон);
	
	МассивФайловСклейки = Новый Массив;
	Для каждого ТекущийШаблон Из ШаблоныДляПечати Цикл
		
		Основания = ПолучитьОбъектыПоНазначению(МассивОбъект, ТекущийШаблон);
		ФормированиеДокументовПоШаблонам(
			ИмяКаталога,
			ДокСсылка,
			Основания,
			ТекущийШаблон,
...
Функция ПолучитьОбъектыПоНазначению(МассивОбъект, Шаблон)

	Попытка
	
		Основания = ПечатьЭлектронныхДокументовСервер.ОбъектыПоНазначению(
			МассивОбъект,
			Шаблон
		);
...
Функция ПолучитьШаблоныДляПечати(Шаблон)

	Возврат ОбъектыСервер.ВычислитьФункцию(
		"Значение = Справочники.тсВидыПечатныхДокументов.НаборыДляПечати(ОбъектВх)",
		Шаблон,
		Неопределено,
		Неопределено
	);
...
Процедура ФормированиеДокументовПоШаблонам(
	ИмяКаталога,
	ДокСсылка,
	МассивОбъект,
	Шаблон,
	...
	Для Каждого Объект Из МассивОбъект Цикл
		...
		РегистрационныйНомер = ПечатьЭлектронныхДокументовСервер.РегистрационныйНомерКомплекта(Объект, Шаблон);
		
		Таблица = Новый Структура(
			"Объект, ВидПечатногоДокумента, КаталогПоУмолчанию, ИмяФайла, РегистрационныйНомер",
			Объект, Шаблон, ИмяКаталога, "Шаблон.doc", РегистрационныйНомер
		);
...
//   <Строка>   - Полное имя папки в которой созданы документы.
//
Функция СоздатьФайлыВКаталоге_ГлобальнаяСклейка(
			Пакет, ПрикрепитьФайлы, КолСтрокТЧ, ВидПечатногоДокумента, КаталогПоУмолчанию, ШаблонСсылка,
			Объект, КомпонентаСклонения, ВыборкаСвойств, тзТаблицаДляВставки,
			Массив_Файлов = Неопределено, РегистрационныйНомер = Неопределено)
	...
	Для Каждого Эл Из КолСтрокТЧ Цикл		                                                        
		...
		БезРассылки = Истина;
		ИндексСтроки = Неопределено;
		ИмяФайла = СгенерироватьШаблон(
			Эл, Объект, ПутьДляПакетаДокументов, ИндексСтроки,
			БезРассылки, КомпонентаСклонения, Префикс, ВыборкаСвойств,
			тзТаблицаДляВставки, ПрикрепитьФайлы,,, РегистрационныйНомер);
	...
	Если ПрикрепитьФайлы Тогда 
			...
			Для й=0 По КолСтрокТЧ.Количество()-1 Цикл
				...
				Массив_НаПечать.Добавить(Структура);						
			КонецЦикла;			
			ПрикрепитьИЗарегистрировать(
				Объект,
				Массив_НаПечать,
				ПрикрепитьФайлы,
				ОбъектыСервер.РазыменоватьСсылку(ВидПечатногоДокумента, "ОднаИсходящаяКорреспонденция"));
...
Функция СформироватьКаталогДляГрупп(КаталогПоУмолчанию, ВызывающийДокумент, ИмяКаталога)
	...
	Для й = 1 По 999999 Цикл
		...
		Если Не НайденныеФайлы.Количество() Тогда
			ИмяЦелевогоФайла = КаталогПоУмолчанию + ПолучитьРазделительПутиКлиента() + ИмяФайлаДокумента;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция ШаблонИзСправочника(СправочникШаблон, ВызывающийДокумент, КаталогПоУмолчанию, ИндексСтроки, БезРассылки, 
		...
		СтрокаТаблициПлатежиЛП = Неопределено, ИмяФайлаШаблона_dot, ПрикрепитьФайлы, РегистрационныйНомер = Неопределено)
	...
	СправочникШаблон.РегНомер = РегистрационныйНомер;
	Если ОбъектыСервер.ПолучитьЗначениеКонстанты("АвторегистрацияПечатныхФорм")
		И ПрикрепитьФайлы И ОбъектыСервер.РазыменоватьСсылку(СправочникШаблон, "Исходящий") Тогда
		
		Если РегистрационныйНомер = Неопределено Тогда
		...
		НаборШаблоныПечатиРеквизитыИСвойстваЗнач.Добавить(Новый Структура("Поле, Значение, Функция, СтрокаДляПолученияЗначения, КодДопСвойства,
		| ОбъектДопСвойства, ТипПоля, ПустоеЗначениеНеПробел, СтрокаХранилище", ОбъектыСервер.РазыменоватьСсылку(СправочникШаблон.Шаблон, "ТегРегДата"), Формат(ОбщегоНазначенияКлиент.ДатаСеанса(), "ДФ='dd.MM.yyyy'"), ПредопределенноеЗначение("Справочник.ФункцииТекстовыхШаблонов.ПустаяСсылка"),
		, , , 
		ПредопределенноеЗначение("Перечисление.ТипыПолейУЭД.Текст"), , ));
	КонецЕсли;
...
//
Функция ОбработкаПутейИГенерацияДокументов(
			Объект, ВидПечатногоДокумента, КаталогПоУмолчанию,
			СтрокаТаблициПлатежиЛП = Неопределено, Массив_НаПечать = Неопределено,
			ГлобальнаяСклейка = Ложь, ПрикрепитьФайлы, НеПечататьПовторно = Ложь, РегистрационныйНомер = Неопределено)
	...
	Если ГлобальнаяСклейка Тогда
		...
		КомпонентаСклонения = "";
		ШаблонСсылка = "";
		ПутьДляПакетаДокументов = СоздатьФайлыВКаталоге_ГлобальнаяСклейка(
			Истина, ПрикрепитьФайлы, РезультатМассив, ВидПечатногоДокумента,
			КаталогПоУмолчанию, ШаблонСсылка, Объект, КомпонентаСклонения,
			ВыборкаСвойств, Неопределено, МассивФайлов, РегистрационныйНомер);
		...
		Иначе 
			...
			Если Массив_НаПечать<>Неопределено Тогда
				...
				ПрикрепитьИЗарегистрировать(
					...
					Массив_НаПечать,
					ПрикрепитьФайлы,
					ПечатьЭлектронныхДокументовСервер.ОднаИсходящаяКорреспонденция(ВидПечатногоДокумента)
				);
				//Если ПрикрепитьФайлы Тогда
				//	Массив_НаПрикрепление = ПолучитьМассивНаПрикрепление(Массив_НаПечать);
...
Процедура ПрикрепитьИЗарегистрировать(Основание, Массив_НаПечать, ПрикрепитьФайлы, ИсходящаяКомплектом)

	Если НЕ ПрикрепитьФайлы Тогда
	...
	Регистрация = Новый Массив;
	ДатаПрикрепления = ОбщегоНазначенияКлиент.ДатаСеанса();
	Для Каждого Эл Из Массив_НаПрикрепление Цикл
		...
		Наименование = ОбъектыСервер.РазыменоватьСсылку(Основание, "Наименование");
		Попытка
			ДокументПрикрепление = ПрикреплениеФайлов.ПрикрепитьФайлы(Основание, Эл);
			
			НастройкиРегистрации = Новый Структура;
			НастройкиРегистрации.Вставить("ИсходящийНомер", Эл.Исходящий);
			НастройкиРегистрации.Вставить("ШаблонПечатнойФормы", Эл.Шаблон);
			НастройкиРегистрации.Вставить("РегистрационныйНомер", Эл.РегНомер);
			НастройкиРегистрации.Вставить("ДатаРегистрации", ДатаПрикрепления);
			
			ОбщиеСвойства = Новый Структура;
			ОбщиеСвойства.Вставить("Настройки", НастройкиРегистрации);
			ОбщиеСвойства.Вставить("Основание", Основание);
			ОбщиеСвойства.Вставить("Документ", ДокументПрикрепление);
			ОбщиеСвойства.Вставить("ИсходящаяКомплектом", ИсходящаяКомплектом);

			Регистрация.Добавить(ОбщиеСвойства);
	...
	Если Регистрация.Количество() > 0 Тогда
	
		РегистрацияКорреспонденции(Регистрация);
	
	КонецЕсли;

КонецПроцедуры

Процедура РегистрацияКорреспонденции(Регистрация)

	Комплекты = Новый Массив;
	Комплекты.Добавить(Регистрация);
	
	ПечатьЭлектронныхДокументовСервер.СоздатьИсходящуюКорреспонденцию(Комплекты);
...
Функция СгенерироватьШаблон(СправочникШаблон, Документ, КаталогПоУмолчанию, ИндексСтроки, БезРассылки, 
		...
		ПрикрепитьФайлы, НомерСтроки = Неопределено, СтрокаТаблициПлатежиЛП = Неопределено, РегистрационныйНомер = Неопределено)
	ИмяФайлаШаблона_dot = ""; 		
	ИмяСозданногоФайла = ШаблонИзСправочника(СправочникШаблон, Документ, КаталогПоУмолчанию, ИндексСтроки, 
			...
			НомерСтроки, СтрокаТаблициПлатежиЛП, ИмяФайлаШаблона_dot, ПрикрепитьФайлы, РегистрационныйНомер);
...
#Область УниверсальнаяПечать

Функция ИнициализацияПараметровПечати() Экспорт

	НастройкиПечати = Новый Структура;
	НастройкиПечати.Вставить("КаталогВыбран", Ложь);
	НастройкиПечати.Вставить("Каталог", "");
	НастройкиПечати.Вставить("Прикрепить", Ложь);
	НастройкиПечати.Вставить("Распечатать", Ложь);
	НастройкиПечати.Вставить("Склеить", Ложь);
	НастройкиПечати.Вставить("ПерезаписатьСуществующие", Ложь);
	НастройкиПечати.Вставить("ПрикреплятьПовторноСуществующийТип", Истина);
	НастройкиПечати.Вставить("ИмяПечатнойФормы", "");
	НастройкиПечати.Вставить("Комплекты", Неопределено);
	НастройкиПечати.Вставить("ОткрытьПослеФормирования", Ложь);
	
	Возврат НастройкиПечати;

КонецФункции // ()

Функция ВыборКаталогаСохранения(Знач НастройкиПечати) Экспорт
	...
	Если Диалог.Выбрать() Тогда
		...
		НастройкиПечати.Вставить("КаталогВыбран", Истина);
		НастройкиПечати.Вставить("Каталог", Диалог.Каталог);
...
Функция ЗаполнитьНастройкиИзШаблона(НастройкиПечати, ВидШаблона) Экспорт

	НастройкиПечати["ОткрытьПослеФормирования"] = ОбъектыСервер.РазыменоватьСсылку(ВидШаблона, "ОткрыватьПослеФормирования");
	
	Возврат НастройкиПечати;

КонецФункции // ()

Функция СформироватьВременныеФайлы(Комплект)

	ВременныеФайлы = Новый Соответствие;
	Для каждого ПечатнаяФорма Из Комплект.Значение Цикл
		
		БинДанные = ПолучитьИзВременногоХранилища(ПечатнаяФорма.Значение);
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст =
				"Неудалось записать файл!" + Символы.ПС;
			Сообщение.Сообщить();
		КонецПопытки;
		
		ВременныеФайлы.Вставить(ПечатнаяФорма.Ключ, ИмяВременныйФайл);
...
Процедура ОчиститьФайлы(КомплектыФайлов)

	Для каждого КомплектФайлов Из КомплектыФайлов Цикл
	
		Для каждого ДанныеФайла Из КомплектФайлов.Значение Цикл
		
			ТемпФайл = Новый Файл(ДанныеФайла.Значение);
			Если ТемпФайл.Существует() Тогда
				...
				Исключение
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст =
						"Неудалось удалить файл!" + Символы.ПС
						+ "Имя файла " + ДанныеФайла.Значение + Символы.ПС
						+ ОписаниеОшибки();
					Сообщение.Сообщить();
				КонецПопытки;
...
Функция ПереместитьФайлыВПользовательскийКаталог(НастройкиПечати, Комплект)

	Каталог = НастройкиПечати.Каталог;
	ПорядковыйНомер = 0;
	ОписаниеОснования = Комплект.Ключ;
	
	ПеремещенныеФайлы = Новый Соответствие;
	Для каждого ВременныйФайл Из Комплект.Значение Цикл
	
		ПорядковыйНомер = ПорядковыйНомер + 1;
		ИмяШаблона = ВременныйФайл.Ключ.ШаблонПечатнойФормы;
		ПутьФайла = СформироватьПолноеИмяФайла(НастройкиПечати, ИмяШаблона, ОписаниеОснования, ПорядковыйНомер);
		Если НЕ ЗначениеЗаполнено(ПутьФайла) Тогда
		
			Попытка
			
				УдалитьФайлы(ВременныйФайл.Значение);
				Продолжить;
			...
			Исключение
				...
				Сообщение.Текст =
					"Неудалось удалить файл!" + Символы.ПС
					+ "Имя файла " + ВременныйФайл.Значение;
				Сообщение.Сообщить();
			КонецПопытки;
		
		КонецЕсли;
		
		Количество = ОписаниеОснования.Количество;
		Пока Количество > 0 Цикл
		
			Попытка
			
				КопироватьФайл(ВременныйФайл.Значение, ПутьФайла + "_" + Количество + ШаблоныДокументовКлиент.ФорматУниверсальногоФайла());
				ПеремещенныеФайлы.Вставить(ВременныйФайл.Ключ, ПутьФайла + "_" + Количество + ШаблоныДокументовКлиент.ФорматУниверсальногоФайла());
			
			Исключение
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст =
					"Неудалось переместить файл!" + Символы.ПС
					+ "Имя временного файла " + ПутьФайла + Символы.ПС
					+ ОписаниеОшибки();
				Сообщение.Сообщить();
				ОчиститьФайлы(ПеремещенныеФайлы);
	...
	Возврат ПеремещенныеФайлы;

КонецФункции // ()

Функция НеХотимПрикреплятьФайлЕслиЕстьПрикреплениеСТакимТипомФайла(ДанныеОснования, НастройкаФормированияШаблона, ПрикреплятьПовторноСуществующийТип)

	ТипПрикрепляемогоФайла = ОбъектыСервер.РазыменоватьСсылку(
		НастройкаФормированияШаблона.ШаблонПечатнойФормы,
		"ТипПрикрепляемогоФайла");
	КодТипаФайла = ОбъектыСервер.РазыменоватьСсылку(
		ТипПрикрепляемогоФайла,
		"Код");
	
	Возврат НЕ ПрикреплятьПовторноСуществующийТип
		И ЗначениеЗаполнено(ТипПрикрепляемогоФайла)
		И ОбъектыСервер.ПроверитьТипФайлаОбъекта(ДанныеОснования.Основание, КодТипаФайла);

КонецФункции

Функция ПрикрепитьСформированныеФайлы(ДанныеОснования, ПеремещенныеФайлы, ПрикреплятьПовторноСуществующийТип = Истина)

	ДокументыПрикрепления = Новый Массив;
	Для каждого Файл Из ПеремещенныеФайлы Цикл
	
		НастройкаФормированияШаблона = Файл.Ключ;
		Если НЕ НастройкаФормированияШаблона.Прикреплять Тогда
		
			Продолжить;
		
		КонецЕсли;
		Если НеХотимПрикреплятьФайлЕслиЕстьПрикреплениеСТакимТипомФайла(ДанныеОснования, НастройкаФормированияШаблона, ПрикреплятьПовторноСуществующийТип) Тогда
		
			Продолжить;
		
		КонецЕсли;
		
		ДанныеПрикрепления = Новый Структура;
		
		ДанныеПрикрепления.Вставить("Прикреплять", НастройкаФормированияШаблона.Прикреплять);
		ДанныеПрикрепления.Вставить("Путь", Файл.Значение);
		ДанныеПрикрепления.Вставить("Шаблон", НастройкаФормированияШаблона.ШаблонПечатнойФормы);
		ДанныеПрикрепления.Вставить("Загружен", Ложь);
		
		ТемпФайл = Новый Файл(ДанныеПрикрепления.Путь);
		Размер = Неопределено;
		Если ТемпФайл.Существует() Тогда
		
			Размер = ОкруглитьРазмерФайла(ТемпФайл.Размер());
		
		КонецЕсли;
		ДанныеПрикрепления.Вставить("Размер", Размер);
		ДанныеПрикрепления.Вставить("УИД", Неопределено);
		ДанныеПрикрепления.Вставить("Хранилище", Неопределено);
		
		МассивНаПрикрепление = Новый Массив;
		МассивНаПрикрепление.Добавить(ДанныеПрикрепления);
		ЗаписатьДвоичныеДанные(МассивНаПрикрепление);
		
		ДокументПрикрепление = ПрикреплениеФайлов.ПрикрепитьФайлы(ДанныеОснования.Основание, МассивНаПрикрепление[0]);
		Данные = Новый Структура;
		Данные.Вставить("Документ", ДокументПрикрепление);
		Данные.Вставить("Основание", ДанныеОснования.Основание);
		Данные.Вставить("Настройки", НастройкаФормированияШаблона);
		Данные.Вставить("ИсходящаяКомплектом", ДанныеОснования["ИсходящаяКомплектом"]);
		
		ДокументыПрикрепления.Добавить(Данные);
	
	КонецЦикла;
	
	Возврат ДокументыПрикрепления;

КонецФункции

Функция СформироватьПолноеИмяФайла(НастройкиПечати, ИмяШаблона, Основание, ПорядковыйНомер)

	Каталог = НастройкиПечати.Каталог;
	ИмяПечатнойФормы = НастройкиПечати.ИмяПечатнойФормы;

	ПутьФайла =
		Каталог
		+ ПолучитьРазделительПутиКлиента()
		+ СформироватьИмяФайла(Основание, ИмяПечатнойФормы, ИмяШаблона)
		+ "_" + ПорядковыйНомер;
		
	
	Если НЕ НастройкиПечати.ПерезаписатьСуществующие И ФайлСИменемСуществует(ПутьФайла) Тогда
...
Функция ФайлСИменемСуществует(ПутьФайла)
...
Функция РаспечататьСформированныеФайлы(ПеремещенныеФайлы)

	ФайлДляПечати = ПеремещенныеФайлы;
	ПараметрыПечати = Новый Структура;
	ПараметрыПечати.Вставить("ТипФайла", ШаблоныДокументовКлиент.ФорматУниверсальногоФайла());
	
	АдресХранилища = ПоместитьВоВременноеХранилище(ФайлДляПечати, Новый УникальныйИдентификатор);
	ПараметрыПечати.Вставить("АдресХранилища", АдресХранилища);
	
	ОткрытьФорму("Перечисление.ТипыПечати.Форма.ПечатьДокументов", ПараметрыПечати);

КонецФункции // ()

Функция СклеитьСформированныеФайлы(ИмяАрхиваDOCX, Комплект, ПерваяЗапись)

	ПервыйФайл = ПерваяЗапись;
	ВременныеФайлы = Комплект.Значение;
	
	Для каждого ВременныйФайл Из ВременныеФайлы Цикл

		ТемпФайл = Новый Файл(ВременныйФайл.Значение);
		ТекстИзКомплекта = "";
		Если ТемпФайл.Существует() И ТемпФайл.ЭтоФайл() Тогда
		
			ТекстИзКомплекта = СформироватьСклеиныйТекст(
				ТекстИзКомплекта,
				ТемпФайл.Путь + ТемпФайл.ИмяБезРасширения,
				ПервыйФайл);
	...
	Количество = тсВидыПечатныхДокументовКлиент.КоличествоВерсийОснования(Комплект);
	ДобавитьНовыйЛист = Ложь;

	Пока Количество > 0 Цикл
	
		ДописатьТекстВАрхивDOCX(ИмяАрхиваDOCX, ТекстИзКомплекта, ДобавитьНовыйЛист);
		Если ПерваяЗапись Тогда
		
			ДобавитьНовыйЛист = Истина;
...
Функция СформироватьИмяФайла(ОписаниеОснования, ИмяПечатнойФормы, Шаблон)

	Основание = ОписаниеОснования.Основание;

	ИмяОснования = ?(ТипЗнч(Основание) = Тип("Строка"), Основание, Строка(Основание));
	ИмяШаблона = ?(Шаблон = Неопределено, ИмяПечатнойФормы, Строка(Шаблон));
	Возврат УправлениеЗапрещеннымиСимволами.УбратьЗапрещенныеСимволыДляИмениФайлов(
		ИмяОснования + "_" + ИмяШаблона
	);

КонецФункции 

Функция ФормированиеФайлов(КомплектыБинарныйФормат)

	КомплектыВременныеФайлы = Новый Соответствие;
	Для каждого Комплект Из КомплектыБинарныйФормат Цикл
	
		ВременныеФайлы = СформироватьВременныеФайлы(Комплект);
		КомплектыВременныеФайлы.Вставить(Комплект.Ключ, ВременныеФайлы);
	
	КонецЦикла;
	
	Возврат КомплектыВременныеФайлы;

КонецФункции // ()

#Область Преобразования

Функция СформироватьСодержимоеАрхиваDOCX_Шапка(ИмяФайлаБезРасширения)

	Попытка
	
		ЧтениеZIP = Новый ЧтениеZipФайла(ИмяФайлаБезРасширения + ШаблоныДокументовКлиент.ФорматZIPФайла(), "");
		СоздатьКаталог(ИмяФайлаБезРасширения);
		ЧтениеZIP.ИзвлечьВсе(ИмяФайлаБезРасширения);
		ПутьXML = ИмяФайлаБезРасширения + ПолучитьРазделительПути();
		ПутьXML = ШаблоныДокументовКлиент.ИмяФайлаССодержимымВАрхивеDOCX(ПутьXML);
		...
		НовыйТекст = Сред(ТекстXML, 1, НачалоBody - 1);
		//ИзмененныйXML = Новый ЗаписьXML;
		//ИзмененныйXML.ОткрытьФайл(ПутьXML);
		//ИзмененныйXML.ЗаписатьБезОбработки(НовыйТекст);
		
		ЧтениеФайла.УстановитьТекст(НовыйТекст);
		ЧтениеФайла.Записать(ПутьXML);
	
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось начать склейку!" + Символы.ПС + ОписаниеОшибки();
		Сообщение.Сообщить();
		УдалитьФайлы(ИмяФайлаБезРасширения);
		Возврат Ложь;
	КонецПопытки;
...
Функция ЗаписатьШапку(КомплектыВременныеФайлы)

	Если КомплектыВременныеФайлы.Количество() = 0 Тогда
	
		Возврат "";
	
	КонецЕсли;
	ИмяФайла = "";
	Для каждого Комплект Из КомплектыВременныеФайлы Цикл
	
		КомплектСНастройками = Комплект.Значение;
		Для каждого ФайлСНастройками Из КомплектСНастройками Цикл
		
			ИмяФайла = ФайлСНастройками.Значение;
			Прервать;
	...
	ТемпИмяФайла = СтрЗаменить(
		ИмяФайла,
		ШаблоныДокументовКлиент.ФорматУниверсальногоФайла(),
		"_Темп" + ШаблоныДокументовКлиент.ФорматZIPФайла());
	КопироватьФайл(ИмяФайла, ТемпИмяФайла);
	
	ТемпФайл = Новый Файл(ТемпИмяФайла);
	АрхивДляСклейкиСформирован = Ложь;
	Если ТемпФайл.Существует() Тогда
	
		ИмяТемпФайлаБезРасширения = ТемпФайл.Путь + ТемпФайл.ИмяБезРасширения;
		АрхивДляСклейкиСформирован = СформироватьСодержимоеАрхиваDOCX_Шапка(ИмяТемпФайлаБезРасширения);
		УдалитьФайлы(ТемпИмяФайла);
	
	КонецЕсли;
	
	Если НЕ АрхивДляСклейкиСформирован Тогда
	
		Возврат Неопределено;
	
	КонецЕсли;
	
	ИмяАрхива = ИмяТемпФайлаБезРасширения;

	Возврат ИмяАрхива;

КонецФункции // ()

Функция СформироватьСклеиныйТекст(Знач Буфер, ИмяФайла, ПерваяЗапись)

	ТемпИмяФайла = ИмяФайла + "_Врем" + ШаблоныДокументовКлиент.ФорматZIPФайла();
	КопироватьФайл(ИмяФайла + ШаблоныДокументовКлиент.ФорматУниверсальногоФайла(), ТемпИмяФайла);
	ЧтениеZIP = Новый ЧтениеZipФайла(ТемпИмяФайла, "");
	...
	Если НЕ ПерваяЗапись Тогда
		...
		НовыйТекст = НовыйТекст + ШаблоныДокументовКлиент.ТекстНовойСтрокиАрхиваDOCX();
	
	КонецЕсли;
	
	Если НачалоBody <> 0 И КонецBody <> 0 Тогда
	
		НовыйТекст = НовыйТекст + Сред(ТекстXML, НачалоBody, КонецBody - НачалоBody + ДнинаКонецBody);
...
Процедура ДописатьТекстВАрхивDOCX(ИмяАрхиваDOCX, ТекстИзКомплекта, ДобавитьНовыйЛист = Ложь)

	ФайлАрхива = Новый Файл(ИмяАрхиваDOCX);
	ПутьXML = "";
	Если ФайлАрхива.Существует() Тогда
	
		ПутьXML = ШаблоныДокументовКлиент.ИмяФайлаССодержимымВАрхивеDOCX(ФайлАрхива.ПолноеИмя + ПолучитьРазделительПути());
	...
	НовыйТекст = "";
	Если ДобавитьНовыйЛист Тогда
	
		НовыйТекст = ШаблоныДокументовКлиент.ТекстНовойСтрокиАрхиваDOCX();
	
	КонецЕсли;
	
	НовыйТекст = ТекстXML + НовыйТекст + ТекстИзКомплекта;
...
Функция СклеитьФайлыУниверсальныйФормат(ВходныеКомплекты, Склеить)

	Если НЕ Склеить Тогда
	
		Возврат ВходныеКомплекты;
	
	КонецЕсли;
	
	ИмяАрхиваDOCX = ЗаписатьШапку(ВходныеКомплекты);
	Если ИмяАрхиваDOCX = Неопределено Тогда
	
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось склеить файлы!";
		Сообщение.Сообщить();
		
		Возврат ВходныеКомплекты;
	
	КонецЕсли;
	ПервыйКомплект = Истина;
	Для каждого Комплект Из ВходныеКомплекты Цикл
	
		//ВременныеФайлы = тсВидыПечатныхДокументовКлиент.ПолучитьЗначениеКомплекта(Комплект);
		СклеитьСформированныеФайлы(ИмяАрхиваDOCX, Комплект, ПервыйКомплект);
		ПервыйКомплект = Ложь;
	
	КонецЦикла;
	
	ДописатьТекстВАрхивDOCX(ИмяАрхиваDOCX, "</w:document>");
	
	АрхивКаталог = Новый Файл(ИмяАрхиваDOCX);
	ИмяАрхивирования = АрхивКаталог.Путь + АрхивКаталог.ИмяБезРасширения;
	ИмяZIP = ИмяАрхивирования + ШаблоныДокументовКлиент.ФорматZIPФайла();
	АрхивDOCX = Новый ЗаписьZipФайла(ИмяZIP);
	АрхивDOCX.Добавить(
		ИмяАрхиваDOCX + ПолучитьРазделительПути() + "*",
		РежимСохраненияПутейZIP.СохранятьОтносительныеПути,
		РежимОбработкиПодкаталоговZIP.ОбрабатыватьРекурсивно);
	АрхивDOCX.Записать();
	
	ИмяDOCX = ИмяАрхивирования + ШаблоныДокументовКлиент.ФорматУниверсальногоФайла();
	ПереместитьФайл(ИмяZIP, ИмяDOCX);
	
	УдалитьФайлы(ИмяАрхиваDOCX);
	
	НастройкиШаблона = тсВидыПечатныхДокументовКлиент.ИнициализироватьСтруктуруНастройкиШаблонов();
	НастройкиШаблона.Вставить("Прикреплять", Ложь);
	
	ДанныеАрхива = Новый Соответствие;
	ДанныеАрхива.Вставить(НастройкиШаблона, ИмяDOCX);
	
	КомплектыСклееныеФайлы = Новый Соответствие;
	ОснованиеСКоличеством = тсВидыПечатныхДокументовКлиент.ИнициализироватьСтруктуруОписанияОснования();
	ОснованиеСКоличеством.Вставить("Основание", "ОбъединенныйФайл");
	ОснованиеСКоличеством.Вставить("Количество", 1);
	КомплектыСклееныеФайлы.Вставить(ОснованиеСКоличеством, ДанныеАрхива);
	
	Возврат КомплектыСклееныеФайлы;

КонецФункции // ()

Функция ПреобразованиеФайлов(КомплектыВременныеФайлы, Знач НастройкиПечати)

	КомплектыПреобразованныеФайлы = СклеитьФайлыУниверсальныйФормат(КомплектыВременныеФайлы, НастройкиПечати.Склеить);
	
	Возврат КомплектыПреобразованныеФайлы;

КонецФункции // ()

Функция ПостОбработкаФайлов(КомплектыПреобразованныеФайлы, КомплектыВременныеФайлы, Знач НастройкиПечати)

	НаборДокументов = Новый Массив;
	Для каждого Комплект Из КомплектыПреобразованныеФайлы Цикл
	
		ПеремещенныеФайлы = ПереместитьФайлыВПользовательскийКаталог(НастройкиПечати, Комплект);
		
		Если НастройкиПечати.Прикрепить Тогда
		
			ДокументыПрикрепления = ПрикрепитьСформированныеФайлы(Комплект.Ключ, ПеремещенныеФайлы, НастройкиПечати.ПрикреплятьПовторноСуществующийТип);
			НаборДокументов.Добавить(ДокументыПрикрепления);
		
		КонецЕсли;
		
		Если НастройкиПечати.Распечатать Тогда
		
			РаспечататьСформированныеФайлы(ПеремещенныеФайлы);
		
		КонецЕсли;
		
		Если НастройкиПечати["ОткрытьПослеФормирования"] Тогда
		
			ОткрытьПечатныеФормы(ПеремещенныеФайлы);
		
		КонецЕсли;
	
	КонецЦикла;
	
	СоздатьИсходящуюКорреспонденцию(НаборДокументов);
	
	Возврат ПеремещенныеФайлы;

КонецФункции // ()

Процедура ОткрытьПечатныеФормы(ПеремещенныеФайлы)

	Для каждого ПеремещенныйФайл Из ПеремещенныеФайлы Цикл
	
		ЗапуститьПриложение(ПеремещенныйФайл.Значение);
	
	КонецЦикла; 

КонецПроцедуры

Функция СформироватьКомплекты(НастройкиПечати) Экспорт

	КомплектыБинарныйФормат = НастройкиПечати.Комплекты;
	
	КомплектыВременныеФайлы = ФормированиеФайлов(КомплектыБинарныйФормат);

	КомплектыПреобразованныеФайлы = ПреобразованиеФайлов(КомплектыВременныеФайлы, НастройкиПечати);
	
	ФинальныйНаборФайлов = ПостОбработкаФайлов(КомплектыПреобразованныеФайлы, КомплектыВременныеФайлы, НастройкиПечати);
	
	ОчиститьФайлы(КомплектыВременныеФайлы);
	ОчиститьФайлы(КомплектыПреобразованныеФайлы);
	
	Возврат Истина;

КонецФункции // ()

Процедура СоздатьИсходящуюКорреспонденцию(НаборДокументов)

	ПечатьЭлектронныхДокументовСервер.СоздатьИсходящуюКорреспонденцию(НаборДокументов);
