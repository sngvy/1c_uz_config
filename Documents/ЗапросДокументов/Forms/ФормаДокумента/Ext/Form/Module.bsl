...
Процедура ПриОткрытииНаСервере()
	...
	ЭтаФорма.ПлатежноеТребование = Перечисления.ТипыДокументовПоДоговору.ПлатежноеТребование;
	ЭтаФорма.ПлатежноеТребованиеЦессия = Перечисления.ТипыДокументовПоДоговору.ПлатежноеТребованиеЦессия;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.ТипДокумента) = Ложь Тогда
		Предупреждение("Не задан тип документа!",,"Ошибка проведения документа");
		Отказ = Истина;
	ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Объект.Получатель) = Ложь Тогда
		...
		Отказ = Истина;
	ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Объект.СпособОтправки) = Ложь Тогда
		Предупреждение("Не задан способ отправки!",,"Ошибка проведения документа");
		Отказ = Истина;
...
Процедура СоздатьДокумент(Команда)
	...
	Если ЭтаФорма.РольРКЦ = Истина ИЛИ ЭтаФорма.РольАБД = Истина Тогда
		...
		ИначеЕсли ЭтаФорма.Объект.ТипДокумента = ЭтаФорма.СправкаОбОтсутствииЗадолженности Тогда
			...
			ЭтаФорма.Закрыть();
			ФормаДок.Открыть();
		ИначеЕсли ЭтаФорма.Объект.ТипДокумента = ЭтаФорма.ПлатежноеТребование Тогда
			ФормаДок = ПолучитьФорму("Документ.ПлатежноеТребование.Форма.ФормаДокумента");
			ФормаДок.Объект.Договор = ЭтаФорма.Объект.Договор;
			ФормаДок.Объект.Должник = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Договор, "Должник");
			ФормаДок.Объект.Кредитор = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Договор, "Кредитор");
			ФормаДок.Объект.Сумма = ЭтаФорма.ДолгПоТелу + ЭтаФорма.ДолгПоПроцентам + ЭтаФорма.ПениШтрафы;
			ЭтаФорма.Закрыть();
			ФормаДок.Открыть();
		ИначеЕсли ЭтаФорма.Объект.ТипДокумента = ЭтаФорма.ПлатежноеТребованиеЦессия Тогда
			ФормаДок = ПолучитьФорму("Документ.ПлатежноеТребованиеЦессия.Форма.ФормаДокумента");
			ФормаДок.Объект.Договор = ЭтаФорма.Объект.Договор;
			ФормаДок.Объект.Должник = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Договор, "Должник");
			ФормаДок.Объект.Цедент = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Договор, "Кредитор");
			ФормаДок.Объект.Сумма = ЭтаФорма.ДолгПоТелу + ЭтаФорма.ДолгПоПроцентам + ЭтаФорма.ПениШтрафы;
