...
Функция ЗаписатьМероприятие(Данные, Сотрудник)
	...
	Если Данные["Результат"] <> Справочники.РезультатыМероприятий.ПустаяСсылка() Тогда
		Мероприятие.Результат(Данные["Результат"]);
...
Функция ДанныеСотрудника()
	...
	Данные.Вставить("Автор", ТекущийПользователь);
...
Функция ЗаполнитьДанныеДляСозданияМероприятия(Данные)
	...
	Если ЗначениеЗаполнено(Данные.Результат) Тогда
		Данные.Вставить("ФактическаяДата", Данные["ДатаВыполнения"]);
	Иначе
		Данные.Вставить("ФактическаяДата",Дата(1, 1, 1));
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПодобратьРезультат(ВсеРезультаты, Совпадения, ТипМероприятия, Знач Текст, ДанныеИзРеестра) Экспорт	
	...
	Если Результат <> Неопределено
		И Результат.Владелец = ТипМероприятия Тогда
	...
	ДобавленныеРезультаты = Новый Массив();
	МассивЭл = Новый Массив();
	
	Если РезультатыТипа.Количество() <> 0 Тогда 			
		...
		Для каждого Запись Из РезультатыТипа Цикл
			...
			Если СтрНайти(Представление, ВТекст) <> 0 Тогда
				...
				Возврат Результат;
				МассивЭл.Добавить(ВТекст);
				
			КонецЕсли;
			
		КонецЦикла;
		
		Если МассивЭл.Количество() = 0 и ДанныеИзРеестра.РезультатМероприятийИзРеестра = Истина Тогда
			
			НовыйРезультатМероприятий = Справочники.РезультатыМероприятий.СоздатьЭлемент();
			СсылкаНовогоРезультатаМероприятий = Справочники.РезультатыМероприятий.ПолучитьСсылку();
			НовыйРезультатМероприятий.Наименование = Текст;
			НовыйРезультатМероприятий.Владелец = ТипМероприятия;
			НовыйРезультатМероприятий.УстановитьСсылкуНового(СсылкаНовогоРезультатаМероприятий);
			НовыйРезультатМероприятий.Записать();
			Результат = НовыйРезультатМероприятий.Ссылка;
			ДобавленныеРезультаты.Добавить(ВТекст);
			Совпадения.Вставить(Текст, Результат);
	...
	ИначеЕсли Не ПустаяСтрока(Текст) и РезультатыТипа.Количество() = 0 и ДанныеИзРеестра.РезультатМероприятийИзРеестра = Истина Тогда
	...
	Иначе  
		Результат = Справочники.РезультатыМероприятий.ПустаяСсылка();
		Возврат Результат;
...
&НаСервереБезКонтекста
Функция НайтиМероприятие(Мероприятия, Совпадения, Знач Текст, ДанныеИзРеестра)
	...
	Для каждого Запись Из Мероприятия Цикл
		...
		Если Не ПустаяСтрока(Текст)
			...
			И СтрНайти(Представление, ВТекст) <> 0 ИЛИ НЕ ДобавленныеТипы.Найти(ВТекст)= Неопределено Тогда
			
			ТипМероприятия = Запись.Ссылка;
			Совпадения.Вставить(Текст, ТипМероприятия);
			
		ИначеЕсли ДанныеИзРеестра.ТипМероприятияИзРеестра = Истина Тогда
			
			НовыйТипМероприятий = Справочники.ТипыМероприятий.СоздатьЭлемент();
			СсылкаНовогоТипаМероприятий = Справочники.ТипыМероприятий.ПолучитьСсылку();
			НовыйТипМероприятий.Наименование = Текст;
			НовыйТипМероприятий.УстановитьСсылкуНового(СсылкаНовогоТипаМероприятий);
			НовыйТипМероприятий.Записать();
			ТипМероприятия = НовыйТипМероприятий.Ссылка;
			ДобавленныеТипы.Добавить(ВТекст);
			Совпадения.Вставить(Текст, ТипМероприятия);
...
Функция ОбработатьРеестр(Реестр, НачалоРеестра, ВыбраннаяНастройка)
	...
	Для каждого ДанныеСтроки Из Ячейки Цикл
		...
		Если ДанныеСтроки.ТипМероприятия = "" Тогда
			Продолжить;
		Иначе
