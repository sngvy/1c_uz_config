...
Функция СформироватьУИДПоПравиламЦБ(МеткаВремени = Неопределено, АдресСетевойКарты = Неопределено) Экспорт 
	...
	UID = ОбъектыСервер.СформироватьUUIDver1(ТекущаяДата(), ПолучитьMACАдрес());
	UIDБезРазделителей = СтрЗаменить(UID, "-", "");
	UIDНБКИ = UID + "-" + ОбъектыСервер.СгенерироватьКонтрольныйСимвол(UIDБезРазделителей);
	Возврат UIDНБКИ;
