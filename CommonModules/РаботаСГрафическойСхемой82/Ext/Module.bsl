...
Процедура ЗавершитьСхему(ГрафическаяСхема, ТекущийЭлемент) Экспорт
	...
	СтрЗавершения = 
	"{{{4," + Строка(ИндексЗавершения)
	+ ",{1,1,{" + Символ(34) + "#" + Символ(34) + "," + Символ(34)
	+ "Завершение" + Символ(34) + "}}" + "," + Символ(34)
	+ "Завершение" + Символ(34)
	+ ",6},4,cf1fff5b-b11b-45bc-aa6d-6e9558df5623,0},2,{{{{6,{3,4,{0}},{3,3,{-22}},{3,3,{-3}},{6,3,0,1},{1,0},1,1,1,5,0,0,0},5,"
	+ Строка(МаксПраво) + "," + Строка(МаксНиз) + "," + Строка(140 + МаксПраво) + ","
	+ Строка(80 + МаксНиз) + ",5," + Строка(70 + МаксПраво) + "," + Строка(МаксНиз) + "," + Строка(139 + МаксПраво) + ","
	+ Строка(40 + МаксНиз) + "," + Строка(139 + МаксПраво) + "," + Строка(79 + МаксНиз) + "," + Строка(МаксПраво) + ","
	+ Строка(79+МаксНиз) + "," + Строка(МаксПраво) + "," + Строка(40 + МаксНиз)
...
Процедура мДобавитьВСтруктуру(пСтруктура, пТекст, пПозиция)
	...
	Пока Истина Цикл
		...
		Если Не лОжидаетсяИдентификатор Тогда
			...
			Сообщение.Текст = "Неожиданная лексема: " + лЛексема;
			Сообщение.Сообщить();
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьКодИзИмени(пИмя) Экспорт
	...
	Если КоличествоСтрок >= 3 Тогда
		...
		// Проверка кода на число, если нет, то будет исключение
		Попытка
			КодЧислом = Число(Код);
		Исключение
			Код = "";
