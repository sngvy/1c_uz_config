...
Процедура УбратьИзРаботыМ(Отказ, Пользователь, ЭтотОбъект) Экспорт
	...
	Если Таблица.Количество() > 0 Тогда
		...
		ОбъектД.Дата = ТекущаяДатаСеанса();
...
///////////////////////////////////////////////////////////////////////////////
		...
		//	ДокументОбъект.Дата = ТекущаяДатаСеанса() - 1;
		...
		//	КонецПопытки;
		//КонецЕсли;
Процедура ЗапуститьБизнесПроцессБП(Отказ, ЭтотОбъект) Экспорт

	Если Не Константы.БлокироватьСозданиеДокументовИзБП.Получить() Тогда
		
		ОбъектУчетаБП = ЭтотОбъект.Объект;
		
		Если ТипЗнч(ОбъектУчетаБП) = Тип("СправочникСсылка.ДолговыеОбязательства") Тогда
		
			ДанныеУчета = Новый Структура;
			ДанныеУчета.Вставить("Организация", ЭтотОбъект.Организация);
			ДанныеУчета.Вставить("Подразделение", ЭтотОбъект.Подразделение);
			ДанныеУчета.Вставить("Объект", ОбъектУчетаБП);
			ДанныеУчета.Вставить("БизнесПроцесс", ЭтотОбъект.Ссылка);
			ДанныеУчета.Вставить("Комментарий", "Бизнес-Процесс");
			
			Ответ = Справочники.ДолговыеОбязательства.ОрганизацияУчетаДляБизнесПроцесса(ДанныеУчета);
			Если Не Ответ["Результат"] Тогда
			
				Отказ = Истина;
				УправленияСообщениями.ТекстовоеСообщить(Ответ["ТекстОшибки"]);
				Возврат;
...
Процедура ЗапуститьБизнесПроцессСТочкиБП(Отказ, ТСТочка, ТСОбъектРодитель, ТССхема, ТСТипМероприятия,
...
Процедура ЗаполнитьТаблицуВыполненныеСтрелки(Отказ, ОбъектНачало, ВариантНачало = "", Схема,
	...
	Для Каждого Элемент Из Строки Цикл
		...
		Если ТекДата = Неопределено Тогда
			НоваяСтрелка.ДатаВыполнения = ТекущаяДатаСеанса() + Элемент.СуммаЗадержки * 60;
		Иначе
	...
	Если СокрЛП(СтрРезультат) <> "" Тогда
		...
		Для НомСтр = 1 По СтрЧислоСтрок(СтрРезультат) Цикл
			...
			НоваяСтрелка = ЭтотОбъект.ВыполненныеСтрелки.Добавить();
			НоваяСтрелка.ДатаВыполнения = ТекущаяДатаСеанса() + Число(СтрПолучитьСтроку(Стр, 1)) * 60;
			...
			Для Каждого ЭлементСумматор Из Результат.Схема.Сумматоры Цикл
				...
				Если КоличествоВходящих = КоличествоВыполненных Тогда
					...
					Для Каждого ЭлементСтрелка Из Строки Цикл
						Если ЭлементСтрелка.ДатаВыполнения > ТекущаяДатаСеанса() Тогда
							Отказ = Истина;
							Прервать;
						КонецЕсли; 
					КонецЦикла;
...
Процедура ЗапуститьВозможныеСтадии(Отказ, ТекДата = Неопределено, ЭтотОбъект)
	...
	Пока Индекс < ЭтотОбъект.ВыполненныеСтрелки.Количество() Цикл
		Элемент = ЭтотОбъект.ВыполненныеСтрелки[Индекс];
		Если Элемент.ДатаВыполнения > ТекущаяДатаСеанса() Тогда
...
Процедура ПродолжитьВыполнениеБПБП(Отказ, ИсточникСобытия, ЭтотОбъект) Экспорт
