...
Функция СписокПуст()
	...
	|	И ДолговыеОбязательстваДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура РазблокироватьВсе(Команда)
	Если ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.СписокДО И Элементы.СписокДО.ТекущиеДанные <> Неопределено Тогда
		...
		Пока Не СписокПуст() Цикл
			Состояние("Разблокировка дел",, "Пожалуйста, подождите...", БиблиотекаКартинок.NC_Разблокировано);
			Элементы.СписокДО.Обновить();
		КонецЦикла;
