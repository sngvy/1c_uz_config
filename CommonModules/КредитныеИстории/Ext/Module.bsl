...
Функция ТипЗначенияДляВерсииНБКИ() Экспорт 
...
Функция РасчетИсковойДавности(ДатаПлатежа = Неопределено, ДатаОкончанияДоговора = Неопределено) Экспорт 
	ТриГода = 94694400;
	Если Не ЗначениеЗаполнено(ДатаПлатежа) И Не ЗначениеЗаполнено(ДатаОкончанияДоговора) Тогда
		Возврат Ложь;
	Иначе
		Если ЗначениеЗаполнено(ДатаПлатежа) Тогда
			Если ТекущаяДатаСеанса() - ДатаПлатежа >= ТриГода Тогда
				Возврат Истина;
			Иначе
				Возврат Ложь;
			КонецЕсли;	
		...
		Если ЗначениеЗаполнено(ДатаОкончанияДоговора) Тогда
			Если ТекущаяДатаСеанса() - ДатаОкончанияДоговора >= ТриГода Тогда
				Возврат Истина;
			Иначе
				Возврат Ложь;
			КонецЕсли;	
...
Процедура ОбработатьСобытиеПоИсточнику(Запись, ИсточникСобытия) Экспорт
	...
	Если ТипДокументаСобытия = Тип("ДокументСсылка.ПоступлениеПлатежа") Тогда
		...
		Запись.БезнадежныйДолг = ?(КредитныеИстории.РасчетИсковойДавности(ИсточникСобытия.ДатаПлатежа), "1", "0"); 
	КонецЕсли; 
	
	Если Запись.Событие.КодНБКИ = "2.5" Тогда
		Запись.БезнадежныйДолг = ?(КредитныеИстории.РасчетИсковойДавности(,Запись.ДатаОкончанияСделки), "1", "0"); 
...
Функция ВычислитьНаследника(Сделка, ДокументСобытие)
...
Функция СформироватьБлок32_35(Запись, Префикс,  ДопПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Запись.Обеспечительство)  Тогда
		...
		ИначеЕсли Запись.Обеспечительство.ВидОбеспечения = Перечисления.ВидыОбеспечения.ИноеОбеспечение Тогда 
			...
			Если Настройка.ВерсияRUTDF <> Перечисления.ВерсияRUTDF.v3 И Настройка.ВерсияRUTDF <> Перечисления.ВерсияRUTDF.v4 Тогда
				КредитныеИсторииИзменения.СформироватьБлокОбеспечительстваПоручительство(Запись.Обеспечительство, СтрокаОтчета);
			КонецЕсли;
            КредитныеИсторииИзменения.ОчиститьНеиспользуемыеПоля(СтрокаОтчета, Настройка.ВерсияRUTDF);
	...
	Если ТекущаяСтадия = Неопределено ИЛИ ТекущаяСтадия = Перечисления.СтадииЗадолженности.ПустаяСсылка() ИЛИ ТекущаяСтадия = Перечисления.СтадииЗадолженности.Пустая Тогда
		...
		Если ЗаписейПрекращения < 1 Тогда  
			...
			Если НЕ Фиксировать2_3 Тогда
				КредитныеИстории.УдалитьСобытиеСделки(Объект, ДокументРегистратор, "2.3");
			КонецЕсли;	
		КонецЕсли;
...
Функция ФиксироватьИзмененияПриСписаниях() Экспорт
	...
	Пока РезультатЗапроса.Следующий() Цикл
		Фиксировать = РезультатЗапроса.Формировать28ПриСудебномСписании;	
	КонецЦикла;
