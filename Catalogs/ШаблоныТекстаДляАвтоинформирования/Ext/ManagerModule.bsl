...
Процедура ЗарегистрироватьВложения(Шаблон, Объект, ИсходящееПисьмо) Экспорт

	ИнформацияОФайлах = ИнформацияОПрикрепленныхФайлах(Шаблон, Объект);
	
	Для Каждого ВыделеннаяСтрока Из ИнформацияОФайлах Цикл
		
		ПараметрыВложения = Новый Структура;
		ПараметрыВложения.Вставить("ИмяФайла", ВыделеннаяСтрока.ИмяФайла);
		ПараметрыВложения.Вставить("Размер", ВыделеннаяСтрока.РазмерФайла);
		
		ПрикрепленныйФайл = Новый Структура;
		ПрикрепленныйФайл.Вставить("Регистратор", ВыделеннаяСтрока.Регистратор);
		ПрикрепленныйФайл.Вставить("НомерСтроки", ВыделеннаяСтрока.НомерСтрокиРегистратор);
		Вложение = Обработки.Интерфейс_Файлы.ВременныйФайлИзПрикрепленного(ПрикрепленныйФайл);
		
		МодульУправлениеЭлектроннойПочтой = ОбщегоНазначения.ОбщийМодуль("УправлениеЭлектроннойПочтой");
		Если НЕ ПустаяСтрока(Вложение["УИД"]) Тогда
		
			ПараметрыВложения.Вставить("ИДФайлаЭлектронногоПисьма", Вложение["УИД"]);
		
		КонецЕсли;
		
		МодульУправлениеЭлектроннойПочтой.ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(
			ИсходящееПисьмо,
			Вложение["Хранилище"],
			ПараметрыВложения);
...
// Возвращает сведения о прикрепленных файлах для вложения
Функция ИнформацияОПрикрепленныхФайлах(Шаблон, Объект) Экспорт

	Текст = ОбъектыСервер.ПолучитьТекстЗапросаПоискаФайловПоШаблону();
	ТипыФайловШаблона = ПолучитьТипыФайлов(Шаблон);
...
#Область ТипыПрикрепляемыхФайлов

// Возвращает ТипыФайлов из СправочникСсылка.ШаблоныТекстаДляАвтоинформирования без дублей
//   Или пустой массив
Функция ПолучитьТипыФайлов(Шаблон)

	Если ТипЗнч(Шаблон) <> Тип("СправочникСсылка.ШаблоныТекстаДляАвтоинформирования") Тогда
	
		ВызватьИсключение "Входящий параметр должен быть типом - СправочникСсылка.ШаблоныТекстаДляАвтоинформирования";
	
	КонецЕсли;
	
	НаборТиповФайлов = Новый Массив;
	ПроверкаДублей = Новый Соответствие;
	
	Для каждого Настройка Из Шаблон.ТипыПрикрепляемыхФайлов Цикл
	
		Если Не ЗначениеЗаполнено(Настройка.Тип) Тогда
		
			Продолжить;
		
		КонецЕсли;
		Если ПроверкаДублей.Получить(Настройка.Тип) = Неопределено Тогда
		
			НаборТиповФайлов.Добавить(Настройка.Тип);
			ПроверкаДублей.Вставить(Настройка.Тип, Истина);
		
		КонецЕсли;
	
	КонецЦикла;

	Возврат НаборТиповФайлов;
...
Функция ПолучитьЗаполненныйТекст(Шаблон, НаПримереОбъект) Экспорт
	...
	Для Каждого Элемент Из Объект.Настройка Цикл
		Если Элемент.Используется Тогда
			...
			ИначеЕсли Элемент.ТипПоля = 
				...
				Массив = УЭДСервер.ПолучитьМассивИзСтроки(Объект.Настройка[Элемент.НомерСтроки - 1].СтрокаХранилище);
	...
	Для Каждого Элемент Из ШаблонАИ.Таблицы Цикл
		...
		Иначе //Таблица
			Для Каждого ЭлементЭначение Из ПолученноеЗначение Цикл
				...
				Для Каждого ЭлементКолонка Из СписокКолонок Цикл
					Попытка
						ЗначениеКолонки = ЭлементЭначение[ЭлементКолонка.Значение];
					Исключение
...
// Возвращает файлы для вложения
Функция Тест(Шаблон, ук)

	Текст = ОбъектыСервер.ПолучитьТекстЗапросаПоискаФайловПоШаблону();
	ТипыФайловШаблона = ПолучитьТипыФайлов(Шаблон);
	
	Запрос = Новый Запрос(Текст);
	Запрос.УстановитьПараметр("Объект", ук);
	Запрос.УстановитьПараметр("МассивТиповФайлов", ТипыФайловШаблона);
