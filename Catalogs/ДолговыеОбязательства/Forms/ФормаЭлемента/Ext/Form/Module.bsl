...
Процедура ПриОткрытииНаСервере(Отказ)
	...
	ЭтаФорма.АБД = Справочники.Пользователи.НайтиПоНаименованию("Администратор");
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.ПредупреждениеАрхив = Истина Тогда
		...
		Если ЭтаФорма.Авторизованный <> ЭтаФорма.РКЦ И ЭтаФорма.Авторизованный <> ЭтаФорма.ЗРКЦ И ЭтаФорма.Авторизованный <> ЭтаФорма.АБД Тогда
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ИсходящийЗвонокТЧ(Команда)
	...
	Если ВидТелефонии = ПредопределенноеЗначение("Перечисление.ВидыТелефонии.БИТФон") Тогда
		...
		ИначеЕсли Ф.Открыта() = Истина И Ф.СтатусПодключен = Истина Тогда
			...
			ПоказатьОповещениеПользователя("Исходящий звонок",,"Начат вызов на выбранный номер...",БиблиотекаКартинок.NC_ИсходящийЗвонок);
...
Процедура НайтиВВЦ(Команда)
	...
	ПоказатьОповещениеПользователя("Интеграция с WhatsApp",,"Выполняется поиск номера...",БиблиотекаКартинок.NC_ПодсистемаИнтеграцияСWhatsApp);
	ФормаВЦ = ПолучитьФорму("ОбщаяФорма.РассылкаНаWhatsApp",,,Новый УникальныйИдентификатор());
	ФормаВЦ.Номер = СтрСоединить(СтрРазделить(Элементы.ДолжникТелефоны.ТекущиеДанные.Номер, "+( )"), "");
	ФормаВЦ.Договор = ЭтаФорма.Объект.Ссылка;
...
Процедура РозыскДолжника(Команда)
	ПоказатьОповещениеПользователя("Розыск должника",,"Выполняется поиск номера...",БиблиотекаКартинок.NC_РозыскДолжника);
