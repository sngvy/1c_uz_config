...
Процедура ПриЗаписи(Отказ)
	...
	ЕстьПовторы = Справочники.тсВидыПечатныхДокументов.ЕстьПовторы();
	Если ЕстьПовторы Тогда
	
		Отказ = Истина;
		
		Текст = УправленияСообщениями.ПредупреждениеОбПользовательскойОшибке(
			"Есть печатные формы, встречающиеся не один раз"
		);
...
Функция ПолучитьТекущиеШаблоны()
	...
	Шаблоны = ШаблоныПечатныхФорм.Выгрузить();
