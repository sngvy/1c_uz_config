...
Процедура ЗапуститьРассылку() Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Для с = 0 По к - 1 Цикл
				...
				Если Запись.Дата <= ТекущаяДатаСеанса() Тогда
	...
	Пока Результат.Следующий() Цикл
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			ОбновитьСтатусы(СписокСообщений);
...
Функция ВыполнитьМероприятие(Мероприятие, Статус)
	...
	Если Результат.Следующий() Тогда
		...
		МероприятиеОбъект.ДатаВыполнения = ТекущаяДатаСеанса();
		Отказ = Ложь;
...
&НаСервере
Процедура ОбновитьСтатусы(СписокСообщений)
...
Процедура Обработка()
	...
	Для Каждого Элемент Из Результат Цикл
		Если Элемент.Реестр = null Тогда
			...
			Реестр.ДатаСоздания = ТекущаяДатаСеанса();
		...
		Иначе
			...
			Для Каждого Сообщение ИЗ Реестр.Сообщения Цикл
				Если Сообщение.Статус = 1 ИЛИ Сообщение.Статус = 2 ИЛИ Сообщение.Статус = 3 ИЛИ Сообщение.Статус = 4 Тогда 
					Если ТекущаяДатаСеанса() - Реестр.ДатаСоздания > 3600 * 32 Тогда
			...
			Если Определено ИЛИ Доставлено Тогда
				...
				КонецЕсли;
				Мероприятие.ДатаВыполнения = ТекущаяДатаСеанса();
				Отказ = Ложь;
...
Функция ОтправитьСМС(СписокСообщений, Учетка)
	...
	//Результат = БИТ_СМС_Коммуникатор.ОтправитьСообщения(СписокСообщений, Отправитель, "", ТекущаяДатаСеанса(), Актуальность);
	//          
	//Если Результат < 0 Тогда
...
Процедура ОтправитьЕмаилПоДО(УчетнаяЗапись, ЕмаилКуда, Текст, Шаблон, Об) Экспорт
	...
	Отправка = Истина;
	Отправка = Справочники.ШаблоныТекстаДляАвтоинформирования.ОтправитьEMail(
		Почта,
		УчетнаяЗапись,
		Шаблон,
		Текст,
...
&НаСервере
Функция СформироватьАнкетуПользователя() Экспорт
	СтрокаБуфер = "";
	
	СисИнфо = Новый СистемнаяИнформация;
	
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Пользователь: " + ПараметрыСеанса.ТекущийПользователь.Наименование); 
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Организация: " + ПараметрыСеанса.ТекущийПользователь.Организация);
	
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Операционная система: " + СисИнфо.ВерсияОС);
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Версия платформы: " + СисИнфо.ВерсияПриложения);
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Объем оперативной памяти (Мб): " + СисИнфо.ОперативнаяПамять);
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Процессор: " + СисИнфо.Процессор);
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Тип платформы: " + СисИнфо.ТипПлатформы);
	
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Конфигурация: " + Метаданные.Синоним);
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Версия конфигурации: " + Метаданные.Версия); 
	
	КредитныеИстории.ДобавитьКСтроке(СтрокаБуфер, "Вариант размещения: " + ВариантРазмещения());
	
	Возврат СтрокаБуфер;
КонецФункции 

Функция ВариантРазмещения()
...
&НаСервере
Функция СформироватьАтрибутыПисьмавТехподдержку() Экспорт
	АтрибутыПисьма = Новый Структура;
	АтрибутыПисьма.Вставить("АдресТехПоддержки", "uz@1cbit.ru");
	
	СтрокаБуфер = "Здравствуйте! Это форма обратной связи с командой разработки БИТ.Управление Задолженностью." + Символы.ПС
	+ "Благодарим за обратную связь!" + Символы.ПС + "Пожалуйста, укажите свои пожелания, предолжения или отзывы далее в тексте: " + Символы.ПС;
	
	АтрибутыПисьма.Вставить("ТекстПисьмаШаблон", СтрокаБуфер);

	
	Возврат АтрибутыПисьма;
КонецФункции 
