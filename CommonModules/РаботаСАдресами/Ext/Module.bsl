...
//                                     и структура с кодами адреса(идентификаторы, ОКТМО, ОКАТО и др).
...
Функция ПодготовитьАдресДляВвода(Данные) Экспорт
	...
	Если Данные.Свойство("houseNumber") Тогда
		НаселенныйПунктДетально.HouseNumber = Данные.houseNumber; 
	КонецЕсли;
...
Функция СведенияОбАдресеВВидеСтруктуры(Знач АдресСтрокой, ДополнительныеПараметры)
	...
	ПолученныйАдрес = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(АдресСтрокой);
	...
	Если ПустаяСтрока(Адрес.ID) И (Параметры.КодыАдреса Или Параметры.КодыКЛАДР) Тогда
	...
	Если ЗначениеЗаполнено(Адрес.CountryCode) Тогда
	...
	Иначе
		...
		Результат.Вставить("КодСтраны", ?(СведенияОСтране <> Неопределено, СведенияОСтране.Код, ""));
	КонецЕсли;
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
			МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
			Если Параметры.КодыКЛАДР Или Параметры.КодыАдреса Тогда
