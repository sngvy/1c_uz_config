...
Процедура ПриОткрытииНаСервере()
	...
	ЭтаФорма.ПоказыватьСписокИзменений = Константы.ПоказыватьСписокИзменений.Получить();
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.ВерсияКонфигурацииNC <> "3.4.12.1" Тогда
		ФормаСпискаИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
		ФормаСпискаИзменений.Открыть();
	ИначеЕсли ЭтаФорма.ВерсияКонфигурацииNC = "3.4.12.1" И ЭтаФорма.ПоказыватьСписокИзменений = Истина Тогда
		ФормаСпискаИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
		ФормаСпискаИзменений.Открыть();
	КонецЕсли;
...
Процедура ВходНаАТС(Команда)
	// Открыть БИТ.АТС
	ПоказатьОповещениеПользователя("БИТ.АТС",,"Приложение открывается...",БиблиотекаКартинок.NC_Подсистема_бтБИТАТС);
...
Процедура ОбновитьПортфель(Команда)
	Предупреждение("Выполняется обновление портфеля...",1,"Пожалуйста, подождите...");
	Элементы.СписокДО.Обновить();
...
Процедура ПосчитатьДолжников(Команда)
	...
	ПоказатьОповещениеПользователя("Подсчет должников",,"В портфеле "+ЭтаФорма.КоличествоВРаботе+" должника(-ов)",БиблиотекаКартинок.NC_Портфель);
КонецПроцедуры
...
Процедура ОткрытьВЦ(Команда)
	// Открыть WhatsApp
	Попытка
		ПоказатьОповещениеПользователя("Интеграция с WhatsApp",,"Приложение открывается...",БиблиотекаКартинок.NC_ПодсистемаИнтеграцияСWhatsApp);
