...
Процедура НастройкаВыгрузкиПриИзмененииНаСервере() 
	...
	ЭтоЕдиныйФормат = ТипЗнч(Объект.НастройкаВыгрузки) = Тип("СправочникСсылка.НастройкиВыгрузкиКредитныхИсторий");
	...
	Если Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.RUTDF Тогда
		Объект.ИмяФайла = "RUTDF_" + Объект.ИмяФайла;
	КонецЕсли;
	
	
	Элементы.ГруппатчАннулируемыеЗаписиD.Видимость = НЕ ЭтоЕдиныйФормат;
	Элементы.ГруппатчИсправляемыеЗаписиC.Видимость = НЕ ЭтоЕдиныйФормат;
...
Процедура УправлениеВидимостью()  
	Если Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.A Тогда 
		...
		Элементы.ГруппатчЗаписиRUTDF.ТолькоПросмотр = Истина; 
		...
		Элементы.тчЗаписиRUTDFПодбор.Доступность = Ложь;
		Элементы.тчЗаписиRUTDFПодборПоНастройкам.Доступность = Ложь; 
		Элементы.тчЗаписиRUTDFПодборИзФайла.Доступность = Ложь;
	...
	ИначеЕсли Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.RUTDF Тогда
		Элементы.ГруппатчНовыеЗаписиА.ТолькоПросмотр = Истина;
		Элементы.ГруппатчОбновляемыеЗаписиB.ТолькоПросмотр = Истина;
		Элементы.ГруппатчАннулируемыеЗаписиD.ТолькоПросмотр = Истина;  
		ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.тчЗаписиRUTDF;	
		Элементы.ГруппатчИсправляемыеЗаписиC.ТолькоПросмотр = Истина;  
		Элементы.ГруппатчЗаписиTUTDF.ТолькоПросмотр = Истина; 
		Элементы.ГруппатчЗаписиRUTDF.ТолькоПросмотр = Ложь;
		...
		Элементы.тчЗаписиRUTDFПодбор.Доступность = Истина;
		Элементы.тчЗаписиRUTDFПодборПоНастройкам.Доступность = Истина; 
		Элементы.тчЗаписиRUTDFПодборИзФайла.Доступность = Истина;
	...
	Если ТипЗнч(Объект.НастройкаВыгрузки) = Тип("СправочникСсылка.НастройкиВыгрузкиНБКИ_RUTDF") Тогда
		Элементы.ГруппатчАннулируемыеЗаписиD.Видимость = НЕ КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Объект.НастройкаВыгрузки.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4);
		Элементы.ГруппатчИсправляемыеЗаписиC.Видимость = НЕ КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Объект.НастройкаВыгрузки.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4);
	Иначе
		 Элементы.ГруппатчАннулируемыеЗаписиD.Видимость = Ложь;
		 Элементы.ГруппатчИсправляемыеЗаписиC.Видимость = Ложь;
	КонецЕсли;	 
КонецПроцедуры  
...
Процедура ПроверкаВерсии()
	Если Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.C ИЛИ Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.D Тогда 
		...
		Если ЭтоЕдиныйФормат Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Данные операции не совместимы с единым форматом отчета.";
			Сообщение.Сообщить();
		Иначе	
			Если КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Объект.НастройкаВыгрузки.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда 
				Сообщение = Новый СообщениеПользователю;
...
Процедура ОбработатьРезультатПодбораСервер(КоличествоОбъектов) 
	...
	ИначеЕсли Объект.Операция = Перечисления.ВидыОперацийОтчетаRUTDF.RUTDF Тогда
		ИмяВызывающейТаблицы = "тчЗаписиRUTDF"; 	
	Иначе
...
&НаСервере
Процедура ВосстановитьСобытияНаСервере(ПараметрыЗапуска)
	КредитныеИсторииИзменения.ГлавнаяВосстановитьСобытияRUTDF(ПараметрыЗапуска, Неопределено);
	
КонецПроцедуры

&НаКлиенте
Процедура ВосстановитьСобытия(Команда) 
	Если Объект.Ссылка.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Сначала сохраните документ!";
		Сообщение.Сообщить();
	...
	ТекстУведомления = "";
	ПараметрыЗапуска = 	ОпределитьПараметрыЗапуска();
	//ВосстановитьСобытияНаСервере(ПараметрыЗапуска); 
	
	СтруктураФоновогоЗадания  = ВыполнитьФоновоеЗаданиеВосстановитьСобытияНаСервере(ПараметрыЗапуска, УникальныйИдентификатор);
	ИДЗадания = СтруктураФоновогоЗадания.ИдентификаторЗадания;
...
&НаСервере
Функция ВыполнитьФоновоеЗаданиеВосстановитьСобытияНаСервере(ПараметрыЗапуска, УникальныйИдентификатор)
	НаименованиеЗадания = "Восстановление событий RUTDF";

	ВыполняемыйМетод = "КредитныеИсторииИзменения.ГлавнаяВосстановитьСобытияRUTDF";
...
Функция СформироватьОтчетНаСервереЕдиный()
	...
	ДокОбъект = РеквизитФормыВЗначение("Объект");
	ТекстСообщенияФормирования = "";
	ДокументXML = ДокОбъект.СформироватьФайлВыгрузкиXML(ТекстСообщенияФормирования);
	Сообщить(ТекстСообщенияФормирования);  
	Возврат ДокументXML;
КонецФункции
