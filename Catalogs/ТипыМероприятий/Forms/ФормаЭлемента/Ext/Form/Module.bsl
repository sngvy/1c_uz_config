...
Процедура ШаблонСМСПриИзменении(Элемент)
	ШаблонСМСПриИзмененииСервер();
КонецПроцедуры

&НаСервере
Процедура ШаблонСМСПриИзмененииСервер()
	Если ЗначениеЗаполнено(Объект.ШаблонСМС) Тогда
		Объект.ВидКИДляРассылки = Справочники.ВидыКИ.Телефон;
	Иначе
		Объект.ВидКИДляРассылки = "";
	КонецЕсли;	
КонецПроцедуры
