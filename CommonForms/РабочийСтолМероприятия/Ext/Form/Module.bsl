...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.ВерсияКонфигурацииNC <> "3.3.21" Тогда
		ФормаСпискаИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
		ФормаСпискаИзменений.Открыть();
	ИначеЕсли ЭтаФорма.ВерсияКонфигурацииNC = "3.3.21" И ЭтаФорма.ПоказыватьСписокИзменений = Истина Тогда
		ФормаСпискаИзменений = ПолучитьФорму("ОбщаяФорма.ОбновлениеКонфигурации");
		ФормаСпискаИзменений.Открыть();
	КонецЕсли;
...
Процедура ПозвонитьДолжнику(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
		...
		НоваяЗадача.Открыть();
	Иначе
		Предупреждение("Не выбран объект мероприятия!",,"Исходящй звонок");
	КонецЕсли;
КонецПроцедуры
