...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = "") Экспорт
	...
	//Лебедева
	ТексСообщения = "";
	Если ЗначениеЗаполнено(ЗначенияПолей) И ТипЗнч(ЗначенияПолей) = Тип("Структура") Тогда
		Попытка
			ТексСообщения = ЗначенияПолей.ТекстСообщения;
		Исключение
			ТекстСообщения = "";
		КонецПопытки;
	КонецЕсли;	
	...
	МодульОтправкаSMSКлиент.ОтправитьSMS(НомераПолучателей, ТексСообщения, Новый Структура("ПеревестиВТранслит", Ложь));
