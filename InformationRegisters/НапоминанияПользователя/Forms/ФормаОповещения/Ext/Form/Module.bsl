...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Элемент Из Список.Параметры.Элементы Цикл
		...
		ИначеЕсли Строка(Элемент.Параметр) = "ТекущаяДата" Тогда
			...
			Элемент.Использование = Истина; 
		ИначеЕсли Строка(Элемент.Параметр) = "Должность" Тогда
			Элемент.Значение = ПараметрыСеанса.ТекущийПользователь.ТипСотрудника;
			Элемент.Использование = Истина; 
		ИначеЕсли Строка(Элемент.Параметр) = "Объекты" Тогда
			Элемент.Значение = НапоминанияПользователяСервер.ЗаполнитьОбъекты();
			Элемент.Использование = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПеренестиНапоминание(Отказ)
	...
	Набор.Пользователь = ПараметрыСеанса.ТекущийПользователь;	
	Набор.ДатаСледующегоНапоминания = ТекущаяДатаСеанса()
		+ Перечисления.НапоминанияИнтервалыПереноса.ПолучитьВСекундах(НапомнитьЧерез);
