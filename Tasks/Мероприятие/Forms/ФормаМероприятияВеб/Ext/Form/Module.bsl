...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ИначеЕсли Событие = "Коммутация" Тогда
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не удалось послать сообщение";
			Сообщение.Сообщить();
		КонецПопытки;	
...
Процедура ВыполнитьИЗакрыть(Команда)
	...
	Если Не Отказ Тогда
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не удалось закрыть форму!";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Функция ОтменитьВыполнениеЗадачи()
	...
	Исключение
		ОтменитьТранзакцию();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Ошибка блокировки!";
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
...
Процедура ПланируемаяДатаПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.ПланируемаяДата) И Объект.ПланируемаяДата < НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
...
Процедура РезультатНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.РезультатыМероприятий.ФормаВыбора");
	
	ПараметрыВыбораФормы = Новый Структура;
	ПараметрыВыбораФормы.Вставить("Владелец", Объект.ТипМероприятия);
	СписокФункцийДляОтбора = СформироватьСписокРезультатов();
	ПараметрыВыбораФормы.Вставить("ФункцияВидимости", СписокФункцийДляОтбора);
	Оповестить("ВыборДляМероприятияВеб", ПараметрыВыбораФормы);
