...
&НаСервере
Функция ПолучитьШаблон(Документ, Наименование) Экспорт // ЗАЩИЩЕНА!
...
&НаСервере
Функция ПереводТаблицыВМассив(тзТаблицаДляВставки, НаборШаблоныПечатиТаблЗнач, ИспользоватьИтоги) Экспорт // ЗАЩИЩЕНА!
...
Функция ПолучитьТипИзСтроки(Назначение) Экспорт
	...
	Для Каждого ТС Из Метаданные.Документы Цикл
		Если Найти(ТС.Представление(), Назначение) Тогда
			Возврат ТипЗнч(Документы[ТС.Имя].ПустаяСсылка());
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ТС Из Метаданные.Справочники Цикл
		Если Найти(ТС.Представление(), Назначение) Тогда
			Возврат ТипЗнч(Справочники[ТС.Имя].ПустаяСсылка());
		КонецЕсли;
	КонецЦикла;
