...
Процедура ОбновитьДанныеСервисовСопровождения() Экспорт
	...
	// Сервисы поставляемые другими Библиотеками и подсистемами.
	Для каждого Модуль Из МодулиСервисов Цикл
		ОбщийМодуль = ОбщегоНазначения.ОбщийМодуль(Модуль);
		ОбщийМодуль.ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения);
	КонецЦикла;
...
Функция ПроверитьДоступностьСервиса(РезультатОперации, ПараметрыПодключения)
	...
	РезультатПроверки = ИнтернетПоддержкаПользователейКлиентСервер.ПроверитьURLДоступен(
		URLОперацииPing,
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Подключение тестовых периодов.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
