...
Функция ВерсияПоставщикаДанных() Экспорт
	...
	Возврат Результат; // не подключен
...
Функция РаспознатьАдрес(ЧастиАдреса, Представление) Экспорт
	...
	ИначеЕсли ЗагруженныеАдресныеСведения.Получить("КлассификаторДоступен") = Истина Тогда
			Адрес = РаспознатьАдресВебСервис(ЧастиАдреса, Представление);
			Возврат Адрес;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			Для каждого СтрокаРезультатаВебСервис Из РезультатВебСервис.Данные Цикл
				...
				Иначе
					ИмяПоляПодбора = ?(ИмяПоля = "munName", "name", "munName");
					ИмяИдентификатораПодбора =  ?(ИмяПоля = "munName", "id", "munId");
					НоваяСтрока.Идентификатор  = СтрокаРезультатаВебСервис[ИмяИдентификатораПодбора];
					НоваяСтрока.Представление  = СтрокаРезультатаВебСервис[ИмяПоляПодбора];
				КонецЕсли;
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
				...
				Иначе
					ИмяПоляПодбора = ?(ИмяПоля = "munName", "name", "munName");
					ИмяПоляИдентификаторПодбора = ?(ИмяПоля = "munName", "id", "munId");
					Если ЗначениеЗаполнено(СтрокаДанных[ИмяПоляИдентификаторПодбора]) Тогда
						ДанныеВыбора.Данные.Добавить(СтрокаДанных[ИмяПоляИдентификаторПодбора], СтрокаДанных[ИмяПоляПодбора]);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		...
		Если ЗначениеЗаполнено(ИдентификаторАдресногоОбъекта) Тогда
			...
			Если НЕ РезультатВебСервис.Отказ И РезультатВебСервис.Данные.Количество() > 0 Тогда
				...
				Если Не ОсновныеКодыЗаполнены Тогда
					...
					Результат.КодыАдреса.КодКЛАДР         = Формат(Сведения.codeKLADR,"ЧГ=0");
					Результат.КодыАдреса.ОКАТО            = Формат(Сведения.okato,"ЧГ=0");
					Результат.КодыАдреса.ОКТМО            = Формат(Сведения.oktmo,"ЧГ=0");
					Результат.КодыАдреса.КодИФНСФЛ        = Формат(Сведения.ifnsFlCode,"ЧГ=0");
					Результат.КодыАдреса.КодИФНСЮЛ        = Формат(Сведения.ifnsUlCode,"ЧГ=0");
					Результат.КодыАдреса.КодУчасткаИФНСФЛ = Формат(Сведения.ifnsFlAreaCode,"ЧГ=0");
					Результат.КодыАдреса.КодУчасткаИФНСЮЛ = Формат(Сведения.ifnsUlAreaCode,"ЧГ=0");
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			...
			|%1 по причине: %2'"), ПолноеИмяФайла, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	// 1. Необходимость актуализировать адресный классификатор - в классификаторе нет записей о регионах.
	ПоследнееОбновление = ОписаниеПоследнейЗагрузки();
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
		Дело.Идентификатор  = "АктуализацияАдресногоКлассификатора";
		Дело.ЕстьДела       = Не ЗначениеЗаполнено(ПоследнееОбновление.ДатаПоследнейЗагрузки);
		Дело.Важное         = Истина;
...
Функция РаспознатьАдресВебСервис(ЧастиАдреса, Представление)
	...
	Для каждого ЧастьАдреса Из ЧастиАдреса Цикл
		...
		Если СтрСравнить(СтрСоединить(СтрРазделить(ЧастьАдреса.Сокращение, "0123456789")), ЧастьАдреса.Сокращение) <> 0 Тогда
			Если ТипыЗданий.Получить(ВРег(ЧастьАдреса.Наименование)) =Неопределено Тогда
				Помещения.Добавить(АдресныйКлассификаторКлиентСервер.ЗначениеСтроенияИлиПомещения(ТРег(ЧастьАдреса.Наименование), ЧастьАдреса.Сокращение));
	...
	Если РезультатВебСервис.Данные.status = "PARTIALLY_RECOGNIZED" Тогда
		Адрес = АдресДляСохраненияВБазе(РезультатВебСервис.Данные.value, Ложь);
		Адрес.comment = РезультатВебСервис.Данные.unrecognizedPart;
	ИначеЕсли РезультатВебСервис.Данные.status = "COMPLETELY_RECOGNIZED" Тогда
		Адрес = АдресДляСохраненияВБазе(РезультатВебСервис.Данные.value, Ложь);
	Иначе
		Адрес = ОписаниеНовойКонтактнойИнформации();
		Адрес.value = РезультатВебСервис.Данные.unrecognizedPart;
...
Процедура УстановитьИдентификаторыАдресаПоЗагруженнымДанным(Адрес)
	...
	ЭтоМуниципальныйАдрес = АдресныйКлассификаторКлиентСервер.ЭтоМуниципальныйАдрес(Адрес.AddressType);
	...
	// Восстанавливаем недостающие адресные сведения
	ДанныеАдреса = Новый Структура();
	ДанныеАдреса.Вставить("Идентификатор", ИдентификаторАдресногоОбъекта);
	ДанныеАдреса.Вставить("Муниципальный", ЭтоМуниципальныйАдрес);
	Результат = АктуальныеАдресныеСведения(ДанныеАдреса);
	Если Не Результат.Отказ Тогда
		
		Если ЭтоМуниципальныйАдрес Тогда
			ЗаполнитьЗначенияСвойств(Адрес, Результат.Данные, "district,districtType,districtId,city,cityType,cityId");
		Иначе
			ЗаполнитьЗначенияСвойств(Адрес, Результат.Данные, "munDistrict,munDistrictType,munDistrictId,settlement,settlementType,settlementId");
		КонецЕсли;
...
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, Знач АдресаДляПроверки)
	...
	JSONСтрока = СтруктураВСтрокуJSON(ДанныеДляЗапроса);
	...
	Если РезультатПроверки.КодСостояния = 200 Тогда
		...
		АдресПроверен = Истина;
		
	ИначеЕсли РезультатПроверки.КодСостояния = 400 Тогда
		
		ДанныеПоОшибке = СтрокаJSONВСтруктуру(ПолученныеДанные);
	...
	Иначе
		ОбработатьОшибкуОбращенияКВебСервису(Результат, РезультатПроверки, НСтр("ru='Ошибка при проверке адреса через веб-сервис:'"), HTTPЗапрос.АдресРесурса + " X-1C-Request-UID:" + ИдентификаторЗапроса);
		
		КраткоеПредставлениеОшибки = НСтр("ru = 'Проверка адреса не доступна по причине:
	...
	Для Каждого ЭлементПроверки Из Данные Цикл
		...
		РезультатПроверки = Новый Структура();
		РезультатПроверки.Вставить("id", "");
		РезультатПроверки.Вставить("АдресПроверен", АдресПроверен);
		РезультатПроверки.Вставить("Варианты", Новый Массив);
		РезультатПроверки.Вставить("Ошибки", Новый Массив);
		...
		Если ЭлементПроверки.Свойство("errorMessage") И ЗначениеЗаполнено(ЭлементПроверки.errorMessage) Тогда
			...
			Если Не СтрНачинаетсяС(ЭлементПроверки.errorMessage, НСтр("ru='Только часть адреса была найдена в справочнике ФИАС'")) Тогда
				ОписаниеОшибки.Текст = ЭлементПроверки.errorMessage;
			Иначе
				ОписаниеОшибки.Текст = НСтр("ru='Адрес отсутствует в адресном классификаторе'");
			КонецЕсли;
...
|		ПО (ДополнительныеСведения.Идентификатор = АдресныйОбъект.ДополнительныеАдресныеСведения)";
	...
	Результат.Ошибки.Очистить(); // Найден корректный адрес. Ошибок нет.
...
Процедура ОбработатьОшибкуОбращенияКВебСервису(Результат, Знач Ответ, ИнформацияОбОшибке, ТекстURL)
	...
	|сервер вернул  HTTP status code: %3
	|с текстом: %4'");
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстОтветаСервера,
		Результат.ПодробноеПредставлениеОшибки, ТекстURL, Ответ.КодСостояния, ОтветОтСервера, );
