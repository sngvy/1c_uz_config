...
Процедура ПриОткрытии(Отказ)
	...
	Если ФормаКонтрольСроков.ЕстьПросроченные Тогда
		ПоказатьОповещениеПользователя("Контроль сроков",,"Есть необработанные должники",БиблиотекаКартинок.NC_Предупреждение,СтатусОповещенияПользователя.Важное);	
	КонецЕсли;	
...
Процедура ПроверкаПо230ФЗ(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ПогашенияМесяц И Элементы.ПогашенияМесяц.ТекущиеДанные <> Неопределено Тогда
		// Получить нарушения
		ОграничениеПоЗвонкам = КонтрольСобытий.РезультативныеЗвонки(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПогашенияМесяц.ТекущиеДанные.Объект, "Должник"));
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПогашенияМесяц.ТекущиеДанные.Объект, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПогашенияМесяц.ТекущиеДанные.Объект, "Должник"));
		// Передать результаты в форму
		ФормаРезультаты = ПолучитьФорму("ОбщаяФорма.РезультатыПроверкиПо230ФЗ");
		ФормаРезультаты.ОграничениеПоЗвонкам = ОграничениеПоЗвонкам;
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ПерспективныеДолжники И Элементы.ПерспективныеДолжники.ТекущиеДанные <> Неопределено Тогда
		...
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПерспективныеДолжники.ТекущиеДанные.Ссылка, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПерспективныеДолжники.ТекущиеДанные.Ссылка, "Должник"));
		// Передать результаты в форму
		ФормаРезультаты = ПолучитьФорму("ОбщаяФорма.РезультатыПроверкиПо230ФЗ");
		ФормаРезультаты.ОграничениеПоЗвонкам = ОграничениеПоЗвонкам;
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ПроблемныеДолжники И Элементы.ПроблемныеДолжники.ТекущиеДанные <> Неопределено Тогда
		...
		ОграничениеПоСМС = КонтрольСобытий.КонтрольСМС(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПроблемныеДолжники.ТекущиеДанные.Ссылка, "Должник"));
		ОграничениеПоВЦ =  КонтрольСобытий.КонтрольВЦ(ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПроблемныеДолжники.ТекущиеДанные.Ссылка, "Должник"));
		// Передать результаты в форму
		ФормаРезультаты = ПолучитьФорму("ОбщаяФорма.РезультатыПроверкиПо230ФЗ");
		ФормаРезультаты.ОграничениеПоЗвонкам = ОграничениеПоЗвонкам;
		ФормаРезультаты.ОграничениеПоСМС = ОграничениеПоСМС;
...
Процедура ПроверитьОбновленияКонфигурации()
	...
	НоваяВерсияКонфигурацииNC = "4.1.26.1";
...
Процедура ПроверитьОбновленияЖурналаНарушений()
	...
	ИначеЕсли ЭтаФорма.КоличествоНарушенийСлужебное < ЭтаФорма.КоличествоНарушений Тогда
		ПоказатьОповещениеПользователя("Нарушения 230-ФЗ",,"Зафиксировано новое нарушение",БиблиотекаКартинок.NC_ПодсистемаНарушения230ФЗ,СтатусОповещенияПользователя.Важное);
		ЭтаФорма.КоличествоНарушенийСлужебное = ЭтаФорма.КоличествоНарушений;
	КонецЕсли;
КонецПроцедуры
