...
Процедура ФормированиеСписковАвтообзвона() Экспорт
	...
	|	МАКСИМУМ(бит_ИсторияЗвонков.ДлительностьЗвонка) КАК ДлительностьЗвонка
	|ИЗ
	...
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К1
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	...
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К2
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL 
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.Должник.ЧасовойПояс = ""AET""
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К3
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL 
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.Должник.ЧасовойПояс = ""Asia/Chita""
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К4
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL 
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.Должник.ЧасовойПояс = ""Asia/Brunei""
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К5
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL 
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.Должник.ЧасовойПояс = ""ACT""
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К6
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL 
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.Должник.ЧасовойПояс = ""Antarctica/Davis""
	|				И ДолговыеОбязательства.Должник.КодКонтрагента ПОДОБНО &К7
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ЛОЖЬ
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL) 
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(Мероприятие.Дата) < &Д
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДлительностьЗвонка УБЫВ,
	|	Дата";
...
Процедура ВыгрузкаВСписокАвтообзвона(Знач Запрос, Знач СписокАвтообзвона)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ КонтрольСобытий.РезультативныеЗвонки(Договор.Должник) Тогда
			...
			Договор.АвтообзвонЗапрещен = Истина;
			Договор.Записать();
...
Процедура ФормированиеСписковАвтообзвонаПараметры(Знач Запрос)
	...
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К1", СлучайноеЧисло);
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К2", СлучайноеЧисло);
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К3", СлучайноеЧисло);
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К4", СлучайноеЧисло);
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К5", СлучайноеЧисло);
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К6", СлучайноеЧисло);
	
	ГСЧ = Новый ГенераторСлучайныхЧисел();
	СлучайноеЧисло = Строка("%"+ГСЧ.СлучайноеЧисло(0, 12)+"."+"%");
	Запрос.УстановитьПараметр("К7", СлучайноеЧисло);
	
	Дата = НачалоДня(ТекущаяДата())-172800;
	Запрос.УстановитьПараметр("Д", Дата);
...
Процедура ОбновлениеОстатков() Экспорт
	// Выгрузить остатки из регистра накопления
...
&НаСервере
Процедура БлокировкаОбъектовАвтообзвона() Экспорт
	// Выгрузить незаблокированные ДО, где больше 10 нерезультативных контактов
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	РезультативныйКонтакт";
	
	ОбработкаНеконтактныхОбъектовАвтообзвона(Запрос);
	
КонецПроцедуры

Процедура ОбработкаНеконтактныхОбъектовАвтообзвона(Знач Запрос)
...
&НаСервере
Процедура РазблокировкаОбъектовАвтообзвона() Экспорт
	// Выгрузить заблокированные ДО
	
	Запрос = Новый Запрос;
	// Установить параметры в отдельной процедуре
	РазблокировкаОбъектовАвтообзвонаПараметры(Запрос);
    //
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДолговыеОбязательства.Ссылка КАК Ссылка,
	...
	|ГДЕ
	|	(ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""В работе""
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ИСТИНА
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL) 
	|			ИЛИ (ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""Новые""
	|				И ДолговыеОбязательства.АвтообзвонЗапрещен = ИСТИНА
	|				И Мероприятие.Ссылка ЕСТЬ НЕ NULL)
	...
	ОбработкаОбъектовАвтообзвона(Запрос);
	
КонецПроцедуры	
	
Процедура РазблокировкаОбъектовАвтообзвонаПараметры(Знач Запрос)
...
Процедура ОбработкаОбъектовАвтообзвона(Знач Запрос)
	...
	Пока Выборка.Следующий() Цикл
		...
		Договор.АвтообзвонЗапрещен = Ложь;
		Договор.Записать();
	КонецЦикла
