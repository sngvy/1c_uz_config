...
Процедура ТипПисьмаПриИзменении(Элемент)
	СообщениеОтказа = Новый СообщениеПользователю;
	Письмо = Объект.ТипПисьма;
	Если НЕ ЗначениеЗаполнено(Письмо) Тогда
		ТекстСообщенияОтказа = "Необходимо выбрать тип письма!"; 
		СообщениеОтказа.Текст = ТекстСообщенияОтказа;
		СообщениеОтказа.Сообщить();
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ПечатныеФормыПисьма.Количество() = 0 Тогда  
		ТекстСообщенияОтказа = "Для данного типа письма не задана ни одна печатная форма!"; 
		СообщениеОтказа.Текст = ТекстСообщенияОтказа;
		СообщениеОтказа.Сообщить();
		Объект.Шаблон = "";
		Возврат;
	Иначе    
		ТекстСообщенияОтказа = "Произошла ошибка во время работы с типом письма. Проверьте карточку типа письма."; 
		СообщениеОтказа.Текст = ТекстСообщенияОтказа;
		СообщениеОтказа.Сообщить();
   		Объект.Шаблон = "";
		Возврат;
	...
	ЗаполнитьПараметрыПисемПочтаРФ();
...
Процедура ВесРПОПриИзменении(Элемент)  
	...
	Если ЗначениеЗаполнено(Объект.ВидРПО) Тогда
		...
		Если Объект.ВесРПО <= 0 Тогда
			ТекстСообщения = "Для отправкой почтой обязательно укажите Вес РПО!";
		КонецЕсли;	
...
Процедура ВидРПОПриИзменении(Элемент)  
	...
	Если ЗначениеЗаполнено(Объект.ВесРПО) Тогда
		...
		КонецЕсли;  
	Иначе
		Если ЗначениеЗаполнено(Объект.ВидРПО) И Объект.ВесРПО <= 0 Тогда
			ТекстСообщения = "Для отправкой почтой обязательно укажите Вес РПО!";
		КонецЕсли;	
	КонецЕсли;  
...
Процедура ПроверкаВидКатегоряКонверт()
	...
	Если Не ЗначениеЗаполнено(Объект.ТипКонверта) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Сообщить();
	КонецЕсли;	
...
&НаСервере
Процедура ЗаполнитьПараметрыПисемПочтаРФ()
	НастойкаПараметровПисем = бит_ИнтеграцияПочтаРФ.ВернутьПараметрыПисем(Объект.ТипПисьма);
	бит_ИнтеграцияПочтаРФ.ЗаполнитьПараметрыИсходящейКорреспонденции(Объект, НастойкаПараметровПисем);
КонецПроцедуры
