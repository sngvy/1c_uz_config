...
Процедура ПолучитьИП(Должник, ДатаРождения, Код) Экспорт
	...
	ИП.Вставить("fio", Должник);
	ИП.Вставить("region_id", Константы.АхантерРегион.Получить());
	...
	Попытка
		Если Данные[0].accruals = Неопределено Тогда
			...
			МенеджерЗаписи.ИП = "Действующие исполнительные производства отсутствуют";
		...
		ИначеЕсли Данные[0].accruals.Количество() > 0 Тогда
			Сч = 0;
			Для Сч = 0 По Данные[0].accruals.Количество() - 1 Цикл
				ИП = Данные[0].accruals[Сч].account;
				КодОтдела = СтрЗаменить(Данные[0].accruals[Сч].additional_fields.division_code, " ", "");
	...
	Исключение
		МенеджерЗаписи = РегистрыСведений.ИсполнительныеПроизводства.СоздатьМенеджерЗаписи();
			...
			МенеджерЗаписи.Должник = Справочники.Контрагенты.НайтиПоКоду(Код);
			МенеджерЗаписи.ИП = "БД ИП ФССП России перегружена. Повторите запрос позднее";
