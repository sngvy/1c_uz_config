...
Функция НайтиКонтакты(Знач СтрокаПоиска)
	...
	ТаблицаКонтактов = Взаимодействия.БитНайтиКонтактыСАдресами(СтрокаПоиска);
...
&НаСервере
Процедура ПриЗакрытииФормыВыбораШаблон(ШаблонПисьма, ДопПараметры) Экспорт
	Если Объект.ПолучателиПисьма.Количество() = 1 Тогда
		ВладелецИмейл = Объект.ПолучателиПисьма[0].Контакт;
		Если ЗначениеЗаполнено(ВладелецИмейл) Тогда
			//ТекстПисьма = Автоинформирование.ПолучитьПодсказку(ВладелецИмейл,ШаблонПисьма);
			ТипТекста = ?(ЗначениеЗаполнено(ШаблонПисьма.ШаблонHTML), Перечисления.ТипыТекстовЭлектронныхПисем.HTML, Перечисления.ТипыТекстовЭлектронныхПисем.ПростойТекст);
			ТекстПисьма = Справочники.ШаблоныТекстаДляАвтоинформирования.ЗаполнитьHTMLШаблон(ШаблонПисьма, ВладелецИмейл); 
			ТекстHTML = ТекстПисьма;
			Объект.ТипТекста = ТипТекста;
		Иначе
			ТекстПисьма = "";
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ПараметрыОтбора.Вставить("ВидОперации", ПредопределенноеЗначение("Перечисление.ВариантыДействийДляСкоринга.EMailРассылка"));
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Отбор", ПараметрыОтбора);
	ПараметрыФормы.Вставить("РежимВыбора",Истина);
	ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбораШаблон", ЭтаФорма,);
	ОткрытьФорму("Справочник.ШаблоныТекстаДляАвтоинформирования.ФормаВыбора",ПараметрыФормы,
