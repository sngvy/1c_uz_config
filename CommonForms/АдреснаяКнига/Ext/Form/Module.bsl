...
Процедура Подключаемый_СписокСправочникаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Пользователь = Элемент.ТекущиеДанные.Ссылка;
	АдресЭП = Элемент.ТекущиеДанные.Представление;
	//Результат = БитНаименованиеИАдресаЭлектроннойПочтыКонтакта(Пользователь,АдресЭП);
	//Если Результат = Неопределено Тогда
	//	Возврат;
	//КонецЕсли;
	
	//Адрес = Результат[0].Адреса;
	//СписокАдресов = Адрес;
	
	ДобавитьПолучателя(АдресЭП, Пользователь.Наименование, ВыбраннаяСтрока, АдресЭП);
...
Процедура ВключитьВСписокПолучателей(Команда)
	...
	ТЧФормыИмя = Элементы.СтраницыСписки.ТекущаяСтраница.Подсказка;
	ТЧФормы = Вычислить("Элементы." + ТЧФормыИмя);
	ВыделенныеСтроки = ТЧФормы.ВыделенныеСтроки;
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	Соответствие = Новый Соответствие();
	Для Каждого Стр Из ВыделенныеСтроки Цикл
		Соответствие.Вставить(Стр, ТЧФормы.ДанныеСтроки(Стр));
	КонецЦикла;	
	СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Соответствие);
	Текст = ЗаписьXML.Закрыть();	
	БитПеренестиВыделенныеСтроки(ТЧФормыИмя,Текст,ВыделенныеСтроки[0]);
...
Процедура БитПеренестиВыделенныеСтроки(ТЧФормыИмя,Текст,ПерваяСтр)
	тСериализаторXDTO = Новый СериализаторXDTO(ФабрикаXDTO); 
	тЧтениеXML = Новый ЧтениеXML;
	тЧтениеXML.УстановитьСтроку(Текст);
	Соответсвие = тСериализаторXDTO.ПрочитатьXML(тЧтениеXML);
	ТЧФормы = Вычислить("Элементы." + ТЧФормыИмя);
	ПерваяСтрок = Соответсвие.Получить(ПерваяСтр);
	...
	Для Каждого Стр Из Соответсвие Цикл
		НоваяСтрока = Результат.Добавить();	
		Знч = Стр.Значение;
		НоваяСтрока.Адрес = Знч.Представление;
		НоваяСтрока.Группа = "Кому";
		НоваяСтрока.Контакт = Знч.Ссылка;
		НоваяСтрока.Представление = Знч.Представление; 
		НоваяСтрока.СписокАдресов = Знч.Представление;
	КонецЦикла;
...
КонецПроцедуры // БитПеренестиВыделенныеСтроки()
...
Процедура ПеренестиВыделенныеСтроки(Знач ВыделенныеСтроки)
	...
	Результат = Взаимодействия.БитАдресаЭлектроннойПочтыКонтактов(ВыделенныеСтроки, ГруппаПоУмолчанию);
...
&НаСервере
Функция БитНаименованиеИАдресаЭлектроннойПочтыКонтакта(Пользователь,АдресЭП);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	УчетныеЗаписиЭлектроннойПочты.Ссылка КАК Адреса
		|ИЗ
	...
	Запрос.УстановитьПараметр("АдресЭлектроннойПочты", АдресЭП);
	Запрос.УстановитьПараметр("ОтветственныйЗаОбработкуПисем", Пользователь);
...
Процедура КонтрагентыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ДобавитьПолучателя(АдресЭП, ПолучитьНаименование(Контрагент), Контрагент, АдресЭП);
...
Процедура ДолговыеОбязательстваВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ДобавитьПолучателя(АдресЭП, ПолучитьНаименование(ДО), ДО, АдресЭП);
