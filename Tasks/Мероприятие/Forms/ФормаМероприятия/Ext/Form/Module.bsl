...
Процедура УстановитьСтатусНеБеспокоитьГлобальный()
	ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
	Если ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусНеБеспокоить = Ложь И ФормаНабора.ИдетРазговор = Ложь Тогда
		ТелефонияСлужебный.ЗавершитьВсеРазговорыИОтклонитьВходящие(Ложь);
...
&НаКлиенте
Процедура ПроверкаЗавершенияРазговора()
	ФормаНабора = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
	Пока ФормаНабора.СтатусПодключен = Истина Цикл
		ТелефонияСлужебный.ЗавершитьВсеРазговорыИОтклонитьВходящие(Ложь);
		Если ФормаНабора.СтатусПодключен = Ложь Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
