...
Процедура ПозвонитьДолжнику(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
		...
		НоваяЗадача.Открыть();
	Иначе
		Предупреждение("Не выбран объект мероприятия!");
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьПортфель(Команда)
	...
	Предупреждение("Выполняется обновление...",1);
КонецПроцедуры
...
Процедура ОткрытьВЦ(Команда)
	// Открыть WhatsApp
	ПоказатьОповещениеПользователя("Интеграция с WhatsApp",,"Приложение открывается...",БиблиотекаКартинок.NC_WA);
