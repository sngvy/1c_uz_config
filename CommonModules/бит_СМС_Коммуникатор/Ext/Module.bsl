...
Функция Подключиться() ЭКСПОРТ
	...
	Отключиться(); //если вдруг сессия поднята
	...
	// Если нет связи, либо ошибки от -3 до -19, то подключаемся на резервный сервер.
	Если РезультатПодключения = Неопределено ИЛИ (РезультатПодключения <= -3 И РезультатПодключения >= -19) Тогда
...
Функция ОтправитьСообщения(СписокСообщений, НомерОтправителя, ТекстСообщенияПоУмолчанию = "",
	...
	Если НЕ ВебСервис = Неопределено Тогда
		// Получаем фабрику XDTO
		Фабрика  = ВебСервис.ФабрикаXDTO;
		МассивСообщений = Фабрика.Создать(Фабрика.Тип("SMS4B", "ArrayOfGroupSMSList"));
	Иначе
		КодОшибки = -9;
		Возврат КодОшибки;
	КонецЕсли;
	...
	Для Индекс = 1 По КолвоОтправок Цикл
		...
		Для Каждого Сообщение Из СписокСообщений Цикл
			...
			СтрокаМассива = Фабрика.Создать(Фабрика.Тип("SMS4B", "GroupSMSList"));
			...
			Если СчетчикПакетаСообщений = РазмерПакета ИЛИ СчетчикПакетаСообщений = КолвоСообщений Тогда
				...
				Если НЕ РезультатОтправки = Неопределено Тогда
					...
					Если КодОшибки >= -19 И КодОшибки <= -1 Тогда
						...
						// Возможно, что Основной сервер не доступен, пробуем переключиться на резервный.
						КодОшибкиПодкл = 0;
						Если ПоменятьСервер(КодОшибкиПодкл) тогда
							Возврат -100000 + КодОшибки; 
						КонецЕсли;
...
Функция ОбновитьСтатусы(СписокСообщений) ЭКСПОРТ
	...
	Если НЕ ВебСервис = Неопределено Тогда
		// Получаем фабрику XDTO
		Фабрика   = ВебСервис.ФабрикаXDTO;
		МассивСообщений = Фабрика.Создать(Фабрика.Тип("SMS4B", "ArrayOfString"));
	Иначе
		КодОшибки = -9;
		Возврат КодОшибки;
	КонецЕсли;
...
Функция ПодключитьВебСервис()
	...
	Попытка
		Если ОсновнойСервер Тогда
			// Основной сервер
			ВебСервис = WSСсылки.бит_СМС_SMS4B.СоздатьWSПрокси("SMS4B", "WSSM", "WSSMSoap12", , , Новый ЗащищенноеСоединениеOpenSSL);
		Иначе
			// Резервный сервер
			ВебСервис = WSСсылки.бит_СМС_SMS4BРезерв.СоздатьWSПрокси("SMS4B", "WSSM", "WSSMSoap12", , , Новый ЗащищенноеСоединениеOpenSSL);
		КонецЕсли;
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
Функция СтрокуВОбычноеПредставление(СтрокаСимволов, ПризнакКода) // StrHexToStr
	...
	индексПодстроки = 1;
	Результат = "";
	Пока индексПодстроки < СтрДлина(СтрокаСимволов) Цикл
		Строка = Сред(СтрокаСимволов, индексПодстроки, КоличествоСимволов);
		Результат = Результат + Символ(ШестнадцатеричноеЧислоВДесятичное(Строка));
		индексПодстроки = индексПодстроки + КоличествоСимволов;
	КонецЦикла;
	Возврат Результат;
