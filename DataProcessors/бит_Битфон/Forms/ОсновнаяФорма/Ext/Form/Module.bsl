...
Процедура ОбработчикСобытияСостояниеЗвонка(Данные)
	...
	// Исправить ошибку при получении значения Неопределено
	Попытка
		ВходящийВызов = СофтФон.IsIncomingCall(Канал)
	Исключение
		ВходящийВызовОпределен = Ложь;
		Пока Не ВходящийВызовОпределен Цикл
			ВходящийВызов	= СофтФон.IsIncomingCall(Канал);
			Если ВходящийВызов <> Неопределено Тогда
				ВходящийВызовОпределен = Истина;
			КонецЕсли;
		КонецЦикла;
	КонецПопытки;
	НомерЗвонка		= "";
