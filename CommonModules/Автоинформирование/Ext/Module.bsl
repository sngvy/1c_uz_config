...
Функция ПолучитьПодсказку(Объект, Шаблон) Экспорт
	...
	Если Шаблон.НазначениеШаблона = Перечисления.ВариантыДействийДляСкоринга.EMailРассылка И ЗначениеЗаполнено(Шаблон.ШаблонHTML) Тогда
		Текст = Справочники.ШаблоныТекстаДляАвтоинформирования.ЗаполнитьHTMLШаблон(Шаблон, Объект);
	Иначе
...
Функция ПолучитьКонтакт(Объект, ВидКИ) Экспорт
	...
	Если ТипЗнч(Объект) <> Тип("СправочникСсылка.ДолговыеОбязательства")
		ИЛИ НЕ ЗначениеЗаполнено(Объект.Должник) Тогда
	...
	Если ВидКИ.ЭтоДополнительноеСведение Тогда
		...
		НайденныеЗначения = РегистрыСведений.ДополнительныеСведения.ПолучитьЗначениеСвойстваОбъекта(Объект.Должник, ВидКИ);
		Если НайденныеЗначения.Количество() > 0 Тогда
		
			ТелефонАдрес = НайденныеЗначения[0];
