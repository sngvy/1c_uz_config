...
Функция УникальноеИмяФайла(Знач ИмяФайла) Экспорт
	...
	Возврат ФайловаяСистемаСлужебныйКлиентСерверУдалить.УникальноеИмяФайла(ИмяФайла);
...
Функция ЗапуститьПрограмму(Знач КомандаЗапуска, ПараметрыЗапускаПрограммы = Неопределено) Экспорт 
	...
	СтрокаКоманды = ОбщегоНазначенияСлужебныйКлиентСерверУдалить.БезопаснаяСтрокаКоманды(КомандаЗапуска);
	...
	Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда
		...
		СтрокаКоманды = ОбщегоНазначенияСлужебныйКлиентСерверУдалить.СтрокаЗапускаКомандыWindows(
