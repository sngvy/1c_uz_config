...
Процедура СоздатьСписокНаСервере()
	...
	|			ТОГДА КонтрагентыТелефоны.КонтактноеЛицо
	|		ИНАЧЕ ДолговыеОбязательства.Должник
	...
	Если ВидыТелефонов = 1 Тогда
		Для счетчикДО = 0 по Долговые.Количество()-1 Цикл
			...
			Для счетчикРез = 0 по РезультатЗапроса.Количество()-1 Цикл
				...
				НовСтр.КонтактноеЛицо = РезультатЗапроса[счетчикРез].КонтактноеЛицо;
			КонецЦикла;
		КонецЦикла;
	ИначеЕсли ВидыТелефонов = 2 Тогда
		Для счетчикДО = 0 по Долговые.Количество()-1 Цикл
	...
	УдалитьДубли.Свернуть("Контрагент, КонтактноеЛицо, ВидТелефона, НомерТелефона, Обработан");
	Объект.Список.Очистить();
