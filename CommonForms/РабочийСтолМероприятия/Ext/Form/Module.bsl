...
Процедура ПроверитьОбновленияКонфигурации()
	...
	НоваяВерсияКонфигурацииNC = "4.6.20.2";
...
Процедура ОбслуживаниеПослеОбновления();
	Для Счетчик = 1 По 50 Цикл
		Состояние("Обновление...", Счетчик, "", БиблиотекаКартинок.NC_ПересчетИтогов);
	КонецЦикла;
	ПересчетТекущихИтоговРегистровПослеОбновления();
	Для Счетчик = 50 По 100 Цикл
		Состояние("Выполнено", Счетчик, "", БиблиотекаКартинок.NC_Выполнено);
	КонецЦикла;
КонецПроцедуры
