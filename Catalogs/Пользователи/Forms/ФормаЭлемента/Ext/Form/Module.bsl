...
Процедура КраткоеИмяПриИзмененииНаСервере()
	...
	Если ПустаяСтрока(Объект.ФиоДляПечати) Тогда
	
		Объект.ФиоДляПечати = Объект.Наименование;
...
&НаКлиенте
Процедура ЗапросЕсиаПоДоверенностиПриИзменении(Элемент)
	
	Если Объект.ЗапросЕсиаОтЛицаРуководителя
		И НазначенДругойРуководитель() Тогда
	
		Объект.ЗапросЕсиаОтЛицаРуководителя = Ложь;
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Уже назначен руководитель";
		Сообщение.Сообщить();
...
Функция НазначенДругойРуководитель()

	Руководитель = Справочники.Пользователи.РуководительОрганизации();
	
	Возврат Руководитель.Пустая();
