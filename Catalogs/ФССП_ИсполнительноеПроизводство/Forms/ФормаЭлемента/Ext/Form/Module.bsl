...
Процедура ПриОткрытииНаСервере()
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ Различные
		|	ФССП_СостояниеИПСрезПоследних.Предмет1 КАК Предмет
		|ПОМЕСТИТЬ втПоследниеДанныеПредметов
		|ИЗ
		|	РегистрСведений.ФССП_СостояниеИП.СрезПоследних(, ИП = &ИП) КАК ФССП_СостояниеИПСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ФССП_СостояниеИПСрезПоследних.Предмет2
		|ИЗ
		|	РегистрСведений.ФССП_СостояниеИП.СрезПоследних(, ИП = &ИП) КАК ФССП_СостояниеИПСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ФССП_СостояниеИПСрезПоследних.Предмет3
		|ИЗ
		|	РегистрСведений.ФССП_СостояниеИП.СрезПоследних(, ИП = &ИП) КАК ФССП_СостояниеИПСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втОстаткиПоИП.Предмет КАК Предмет,
		|	втОстаткиПоИП.СуммаОстаток КАК СуммаОстаток
		|ИЗ
		|	втОстаткиПоИП КАК втОстаткиПоИП
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втПоследниеДанныеПредметов КАК втПоследниеДанныеПредметов
		|		ПО втОстаткиПоИП.Предмет = втПоследниеДанныеПредметов.Предмет
		|ГДЕ
		|	втПоследниеДанныеПредметов.Предмет <> ЗНАЧЕНИЕ(Справочник.ФССП_ПредметыИП.ПустаяСсылка)";
...
&НаКлиенте
Процедура ЗапросФССП(Команда)
	
	СсылкиНаДокументы = ЗапуститьРобота(Объект.Ссылка);
...
Функция ЗапуститьРобота(ИсполнительноеПроизводство)
	МассивДокументов = Обработки.ФССП_Робот.ЕдиничныйЗапросПоИП(ИсполнительноеПроизводство);
