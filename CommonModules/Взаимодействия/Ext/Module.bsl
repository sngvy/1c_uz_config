...
Функция КонтактыПоEmail(Адрес)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = БитСформироватьТекстЗапросаДляПоискаПоEmail(Ложь);
	
	Запрос.УстановитьПараметр("Адрес", Адрес);
	//++БИТ
	Запрос.УстановитьПараметр("КонстантаИмейл", Константы.EmailАдресДолжника.Получить());
	//--БИТ
...
Функция ПолучитьВсеКонтактыПоСпискуEmail(СписокАдресов) Экспорт
	...
	ТекстЗапроса = БитСформироватьТекстЗапросаДляПоискаПоEmail(Истина,Истина);
...
Функция ТекстЗапросаНаПолучениеИнформацииОКонтактах(ВключатьEmail)
	...
	|		ПО ТаблицаКонтакты.ОтправительКонтакт = СправочникКонтакт.Ссылка
...
Функция БитСформироватьТекстЗапросаДляПоискаПоEmail(ПоискПоСписку, ИтогиПоEmail = Ложь)
	...
	ТекстЗапроса = 
		...
		|ГДЕ
		|	ДополнительныеРеквизитыИСведения.Ссылка = &КонстантаИмейл
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КонтрагентыДополнительныеРеквизиты.Ссылка КАК Контакт,
		|	КонтрагентыДополнительныеРеквизиты.Значение КАК Представление,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Наименование КАК Наименование,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Наименование КАК НаименованиеВладельца
		|ИЗ
		|	Справочник.Контрагенты.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Таб КАК Таб
		|		ПО КонтрагентыДополнительныеРеквизиты.Свойство = Таб.Ссылка
		|ГДЕ
		|	КонтрагентыДополнительныеРеквизиты.Значение = &Адрес
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ДополнительныеСведения.Объект.Наименование,
		|	ДополнительныеСведения.Объект.Наименование
		|ИЗ
		...
		|ГДЕ
		|	ДополнительныеСведения.Значение = &Адрес
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Ссылка,
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Значение,
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Ссылка.Наименование,
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Ссылка.Наименование
		|ИЗ
		|	Справочник.ДолговыеОбязательства.ДополнительныеРеквизиты КАК ДолговыеОбязательстваДополнительныеРеквизиты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Таб КАК Таб
		|		ПО ДолговыеОбязательстваДополнительныеРеквизиты.Свойство = Таб.Ссылка
		|ГДЕ
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Значение = &Адрес";
...
Функция НайтиКонтактыСАдресамиПоНаименованию(Знач СтрокаПоиска, НайденныеКонтакты) Экспорт
	...
	ТаблицаКонтакты = БитНайтиКонтактыСАдресами(СтрокаПоиска);
...
Функция БитНайтиКонтактыСАдресами(СтрокаПоиска) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ ПЕРВЫЕ 20
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты КАК АдресЭлектроннойПочты,
		|	УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем.Ссылка КАК ОтветственныйЗаОбработкуПисемСсылка,
		|	УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем.Наименование КАК ОтветственныйЗаОбработкуПисемНаименование,
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты + "" "" + УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем.Наименование КАК ПредставлениеПоиска
		|ПОМЕСТИТЬ ВТ_СпрАдресаЭП
		|ИЗ
		...
		|	КонтрагентыДополнительныеРеквизиты.Значение КАК Представление,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка КАК Контакт,
		|	(ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Значение КАК СТРОКА(100))) + "" "" + (ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Ссылка.Наименование КАК СТРОКА(100))) КАК ПредставлениеПоиска,
		|	КонтрагентыДополнительныеРеквизиты.Ссылка.Наименование КАК Наименование,
		|	"""" КАК НаименованиеВладельца
		|ИЗ
		|	Справочник.Контрагенты.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
		...
		|	И (ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Значение КАК СТРОКА(100))) + "" "" + (ВЫРАЗИТЬ(КонтрагентыДополнительныеРеквизиты.Ссылка.Наименование КАК СТРОКА(100))) ПОДОБНО ""%"" + &СтрПоиска + ""%""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	ДополнительныеСведения.Значение,
		|	ДополнительныеСведения.Объект,
		|	(ВЫРАЗИТЬ(ДополнительныеСведения.Объект.Наименование КАК СТРОКА(100))) + "" "" + (ВЫРАЗИТЬ(ДополнительныеСведения.Значение КАК СТРОКА(100))),
		|	ДополнительныеСведения.Объект.Наименование,
		|	""""
		|ИЗ
		|	РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
		...
		|	И (ВЫРАЗИТЬ(ДополнительныеСведения.Объект.Наименование КАК СТРОКА(100))) + "" "" + (ВЫРАЗИТЬ(ДополнительныеСведения.Значение КАК СТРОКА(100))) ПОДОБНО ""%"" + &СтрПоиска + ""%""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 20
		|	ВТ_СпрАдресаЭП.АдресЭлектроннойПочты,
		|	ВТ_СпрАдресаЭП.ОтветственныйЗаОбработкуПисемСсылка,
		|	ВТ_СпрАдресаЭП.ПредставлениеПоиска,
		|	ВТ_СпрАдресаЭП.ОтветственныйЗаОбработкуПисемНаименование,
		|	""""
		|ИЗ
		|	ВТ_СпрАдресаЭП КАК ВТ_СпрАдресаЭП
		|ГДЕ
		|	ВТ_СпрАдресаЭП.ПредставлениеПоиска ПОДОБНО ""%"" + &СтрПоиска + ""%""";
	
	Запрос.УстановитьПараметр("Свойство", Константы.EmailАдресДолжника.Получить());
	Запрос.УстановитьПараметр("СтрПоиска", СтрокаПоиска);
	
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();

	Возврат РезультатЗапроса;
КонецФункции // БитНайтиКонтактыСАдресами()
...
//	
//	Запрос.УстановитьПараметр("АдресЭлектроннойПочты", СтрокаПоиска);
//	
//	Выборка = Запрос.Выполнить().Выбрать();
//	
//	Пока Выборка.Следующий() Цикл
...
Функция БитАдресаЭлектроннойПочтыКонтактов(Контакты, ГруппаПолучателей = "") Экспорт
	
	Если Контакты.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ТипЗнч(Контакты[0].Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПользователиГруппыПользователей.Ссылка КАК Контакт,
		|	ПользователиГруппыПользователей.Ссылка.Наименование КАК Представление,
		|	""Кому"" КАК Группа,
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты КАК Адрес,
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты КАК СписокАдресов
		|ИЗ
		|	Справочник.Пользователи.ГруппыПользователей КАК ПользователиГруппыПользователей
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
		|		ПО ПользователиГруппыПользователей.Ссылка = УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем
		|ГДЕ
		|	ПользователиГруппыПользователей.Группа = &Группа
		|	И УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты <> """"";
		
		Запрос.УстановитьПараметр("Группа", Контакты[0].Ссылка);
		
		ТаблицаРезультат = Запрос.Выполнить().Выгрузить();
		
		Возврат ТаблицаРезультат;		
	ИначеЕсли ТипЗнч(Контакты[0].Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	Пользователи.Ссылка КАК Контакт,
		|	Пользователи.Ссылка.Наименование КАК Представление,
		|	""Кому"" КАК Группа,
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты КАК Адрес,
		|	УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты КАК СписокАдресов
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
		|		ПО Пользователи.Ссылка = УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем
		|ГДЕ
		|	Пользователи.Ссылка = &Ссылка";
		
		Запрос.УстановитьПараметр("Ссылка", Контакты[0].Ссылка);
...
Функция ПользовательЯвляетсяОтветственнымЗаВедениеПапок(УчетнаяЗапись) Экспорт
	...
	|		ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем
...
Процедура ПриЗаписиДокумента(ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СообщениеSMS") Тогда
...
Процедура ЗаполнитьКонтактыМассиваВзаимодействий(МассивВзаимодействий, РассчитыватьРассмотрено = Ложь) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	СообщениеSMSАдресаты.Ссылка КАК Взаимодействие,
	               ...
	               |ГДЕ
	               |	СообщениеSMSАдресаты.Ссылка В(&МассивВзаимодействий)
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |ГДЕ
	               |	ЭлектронноеПисьмоВходящее.Ссылка В(&МассивВзаимодействий)
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	РегистрСведений.КонтактыВзаимодействий КАК КонтактыВзаимодействий
	               |ГДЕ
	               |	ЭлектронноеПисьмоИсходящееПолучателиПисьма.Ссылка В(&МассивВзаимодействий)
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	Документ.ЭлектронноеПисьмоИсходящее.ПолучателиКопий КАК ЭлектронноеПисьмоИсходящееПолучателиКопий
	               |ГДЕ
	               |	ЭлектронноеПисьмоИсходящееПолучателиКопий.Ссылка В(&МассивВзаимодействий)
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |ГДЕ
	               |	ЭлектронноеПисьмоИсходящееПолучателиСкрытыхКопий.Ссылка В(&МассивВзаимодействий)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	КОНЕЦ КАК Контакт,
	               |	УникальныеКонтактыВзаимодействияПредставления.ПредставлениеКонтакта КАК ПредставлениеКонтакта
	               |ИЗ
...
Функция ЕстьЗадержкаВыполненияЗаданияПолученияИОтправкиПисем()
	...
	// АПК:143 - Не локализуется.
	Если ТекущаяДатаСеанса() - Расписание.ПериодПовтораВТечениеДня < ФоновоеЗадание.Начало Тогда
		Возврат Ложь;
	КонецЕсли;
	
	// АПК:143 - Не локализуется.
	Возврат Расписание.ТребуетсяВыполнение(
		ТекущаяДатаСеанса() - Расписание.ПериодПовтораВТечениеДня,
