...
Процедура Рассчитать(Команда)
	...
	Если СуммаПлатежа = 0 ИЛИ ДнейНаДатуПередачи = 0 Тогда
		Предупреждение("Не заданы параметры платежа!",,"Расчет АВ");
		Возврат;
	ИначеЕсли КвикМани = Ложь
		И Микроклад = Ложь
		И ПраймЗаймМаниМэн = Ложь
		И ГлавколлектФастМани = Ложь Тогда
		Предупреждение("Не выбран кредитор!",,"Расчет АВ");
	...
	Если ПраймЗаймМаниМэн = Истина и ДнейНаДатуПередачи >= 15 и ДнейНаДатуПередачи <= 30 Тогда
		СуммаАВ = СуммаПлатежа * 0.09
	КонецЕсли;
	Если ПраймЗаймМаниМэн = Истина и ДнейНаДатуПередачи >= 31 и ДнейНаДатуПередачи <= 65 Тогда
		СуммаАВ = СуммаПлатежа * 0.11
	КонецЕсли;
	Если ПраймЗаймМаниМэн = Истина и ДнейНаДатуПередачи >= 66 и ДнейНаДатуПередачи <= 180 Тогда
		СуммаАВ = СуммаПлатежа * 0.13
	КонецЕсли;
	Если ПраймЗаймМаниМэн = Истина и ДнейНаДатуПередачи >= 181 и ДнейНаДатуПередачи <= 360 Тогда
		СуммаАВ = СуммаПлатежа * 0.25
	КонецЕсли;
	Если ПраймЗаймМаниМэн = Истина и ДнейНаДатуПередачи >= 361 Тогда
