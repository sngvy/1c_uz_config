...
&НаКлиенте
Процедура Подбор(ЭтаФорма, ИмяКолонкиЗаполнения = "") Экспорт

	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИмяЗаполняемойКолонки", ИмяКолонкиЗаполнения);
	
	ФормаПодбор = ПолучитьФорму(
		"Обработка.ПодборДО.Форма.ФормаДинамическийСписок",
		ПараметрыФормы,
		ЭтаФорма);
...
Процедура ПодборИзДОК(ЭтаФорма) Экспорт
	ОткрытьФорму("Обработка.ПодборДолговыхОбязательств.Форма", , ЭтаФорма);
КонецПроцедуры
...
Процедура ОбщаяДополнительныеРеквизитыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка, ДополнительныеРеквизиты, ЭтаФорма) Экспорт
	...
	Если СвойствоВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыТипаСтрока.Адрес")
			ИЛИ СвойствоВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыТипаСтрока.Телефон") Тогда
