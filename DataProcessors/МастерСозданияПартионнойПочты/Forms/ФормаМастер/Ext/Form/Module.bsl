...
Процедура ЗаполнитьКорреспонденциюСервер()
	...
	//                |		ПО (СтатусыШПИРегистрацияСрезПоследних.Корреспонденция = ИсходящаяКорреспонденция.Ссылка)
	//                |;
	//                |
	//                |////////////////////////////////////////////////////////////////////////////////
	//                |ВЫБРАТЬ
	...
	//                |		ПО (СтатусыШПИРегистрацияСрезПоследних.Корреспонденция = ИсходящаяКорреспонденция.Ссылка)
	//                |ГДЕ
	//                |	ИсходящаяКорреспонденция.ПометкаУдаления = ЛОЖЬ
	//                |	И ИсходящаяКорреспонденция.НомерЗакреплен = ИСТИНА
	//                |	И ИсходящаяКорреспонденция.ВидРПО <> """"
	//                |	И ИсходящаяКорреспонденция.КатегорияРПО <> """"
	//                |	И ИсходящаяКорреспонденция.ТипКонверта <> """"
	//                |	И ИсходящаяКорреспонденция.ВесРПО <> 0
	//                |
	//                |СГРУППИРОВАТЬ ПО
	...
	//                |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыШПИРегистрация.СрезПоследних(
	//                |				,
	//                |				Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыШПИРегистрация.Удален)
	//                |					ИЛИ Статус ЕСТЬ NULL) КАК СтатусыШПИРегистрацияСрезПоследних
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(СтатусыШПИРегистрацияСрезПоследних.ШПИ, 0) КАК ШПИ,
	               |	ЕСТЬNULL(СтатусыШПИРегистрацияСрезПоследних.Статус, 0) КАК Статус
	               |ПОМЕСТИТЬ ТекущийШПИ
	               |ИЗ
	               |	Справочник.ИсходящаяКорреспонденция КАК СправочникИсходящаяКорреспонденция
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыШПИРегистрация.СрезПоследних(, ) КАК СтатусыШПИРегистрацияСрезПоследних
	               |		ПО (СправочникИсходящаяКорреспонденция.Ссылка = СтатусыШПИРегистрацияСрезПоследних.Корреспонденция
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ТекущийШПИ.Ссылка КАК Ссылка
	               |ИЗ
	               |	ТекущийШПИ КАК ТекущийШПИ
	               |ГДЕ
	               |	ТекущийШПИ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыШПИРегистрация.ПолученаФорма103)
	               |	И ТекущийШПИ.Ссылка.ПометкаУдаления = ЛОЖЬ
	               |	И ТекущийШПИ.Ссылка.НомерЗакреплен = ИСТИНА
	               |	И ТекущийШПИ.Ссылка.ВидРПО <> """"
	               |	И ТекущийШПИ.Ссылка.КатегорияРПО <> """"
	               |	И ТекущийШПИ.Ссылка.ТипКонверта <> """"
	               |	И ТекущийШПИ.Ссылка.ВесРПО <> 0";
...
Функция ПолучитьВалидацию(Валидация)
	...
	Если СтрНайти(СтрокаКодов,Валидация,,,) = 0 Тогда
		Возврат Ложь;
...
Функция ПолучитьКачество(Качество)
	...
	Если СтрНайти(СтрокаКодов,Качество,,,) = 0 Тогда
		Возврат Ложь;
	Иначе 
		Возврат Истина;
	КонецЕсли;	
...
Функция СоздатьМассивЗаказов(ТЗПараметрыПисем)
	...
	Для каждого Стр из Объект.СписокИсходящейКорреспонденции Цикл
		...
		Иначе	
			...
			ИД_Магазина =?(ТипЗнч(Стр.ИсходящаяКорреспонденция) = Тип("СправочникСсылка.ИсходящаяКорреспонденция"), (Строка(Стр.ИсходящаяКорреспонденция.РегистрационныйНомер) + " от " + Строка(Формат(Стр.ИсходящаяКорреспонденция.ДатаРегистрации,"ДЛФ=Д"))),(Строка(Стр.ИсходящаяКорреспонденция.Код) + " от " + Строка(Формат(ТекущаяДата(),"ДЛФ=Д")))); 
			...
			тЗапрос.Вставить("postoffice-code",);
...
Процедура СоздатьПартиюНаСервере()
	...
	//		Продолжить;
	//	КонецЕсли;
	//	ПараметрыПисьма  = ТЗПараметрыПисем.Найти(Корр.ИсходящаяКорреспонденция,"Ссылка");
	//	Если ПараметрыПисьма.КатегорияРПО = "ORDERED" и ПараметрыПисьма.ВидРПО = "LETTER" Тогда
	//		МассивСтрок.Добавить(Корр); 
	//	КонецЕсли;
	//КонецЦикла; 
	//СоздатьДокументПартии(ИмяПартии,КатегорияРПО,ВидРПО,МассивСтрок);
	...
	//	ПараметрыПисьма  = ТЗПараметрыПисем.Найти(Корр.ИсходящаяКорреспонденция,"Ссылка");
	//	Если ПараметрыПисьма.КатегорияРПО = "ORDERED" и ПараметрыПисьма.ВидРПО = "BANDEROL" Тогда
	//		МассивСтрок.Добавить(Корр); 
	//	КонецЕсли;
	//КонецЦикла; 
	//СоздатьДокументПартии(ИмяПартии,КатегорияРПО,ВидРПО,МассивСтрок);
	...
	Иначе
		...
		Для Каждого Корр из объект.СписокИсходящейКорреспонденции Цикл
			Если ЗначениеЗаполнено(Корр.ДокументПартия) тогда
				Продолжить;
			Иначе
				Если ЗначениеЗаполнено(Корр.ИмяПартии) Тогда
		...
		Если ТипЗнч(Корр.Ссылка)  = Тип("СправочникСсылка.ГруппаИсходящейКорреспонденции") Тогда
			Для каждого Исх из Корр.Ссылка.СписокИсходящейКорреспонденции Цикл
				Для Каждого Эл из Исх.ИсходящаяКорреспонденция.ПодобранныеФайлы Цикл
					//ТипФайла = Эл.ПрикрепленныйФайл.Файлы[Эл.НомерСтроки-1].ТипФайла;
					...
					Если ТипФайла.Конверт = Истина тогда
						...
						Для Каждого Конверт из строкиКонверт Цикл
							Данные = Конверт.Хранилище.Получить();
							...
							//C:\Users\ADLebedeva\Desktop\Почта РФ\ТипоКонверт.doc
							//ОбъектWord.Documents.add(СтрЗаменить(ФайлКонверт.ПолноеИмя,"\","/"),0,0);
							ОбъектWord.Documents.add(ФайлКонверт.ПолноеИмя,0,0);
							ActiveDoc = ОбъектWord.ActiveDocument;
							Content = ActiveDoc.Content;				
							Замена = Content.Find;
							ШПИстр = строка(Корр.ШПИ);
							Замена.Execute("{number}",,,,,,,,,ФормированиеСтрокиСпецСимволов(ШПИстр), 2);			
							...
							//Куда = ФайлКонверт.Путь;
							//Имя = "НапечатанныйШаблон.docx";
							ActiveDoc.SaveAs(ИмяВремФайла);
							//ЗапуститьПриложение(Куда+Имя);
							ОбъектWord.Quit();
							
							НачВремя = ТекущаяДата();
							
							Пока ТекущаяДата() <= НачВремя + 3 Цикл
								Попытка
				...
				КонецЦикла;	
			КонецЦикла;
		ИначеЕсли ТипЗнч(Корр)  = Тип("СправочникСсылка.ИсходящаяКорреспонденция") Тогда
...
Процедура УдалитьЗаказShipment(Команда)
	ТекДанные = Элементы.СписокИсходящейКорреспонденцииПартии.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекДанные.ИдентификаторПочты) И ЗначениеЗаполнено(ТекДанные.ИмяПартии) Тогда
		тЗапрос = Новый Массив;
