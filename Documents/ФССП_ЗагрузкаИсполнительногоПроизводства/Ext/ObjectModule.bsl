...
Процедура ОбработкаПроведения(Отказ, РежимПроведения) 
	...
	Для Каждого ИП из тзИП_База Цикл
		...
		Если НайденныеИП.Количество() = 0 И ((Не ЗначениеЗаполнено(ИП.СтатусИП)) ИЛИ ИП.СтатусИП = Перечисления.ФССП_СтатусыИП.Действующее ИЛИ ИП.СтатусИП = Перечисления.ФССП_СтатусыИП.ДругойВзыскатель) И ИП.Завершено = Ложь Тогда
