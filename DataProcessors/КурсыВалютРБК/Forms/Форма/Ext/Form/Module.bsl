...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.НачалоПериодаЗагрузки = НачалоМесяца(ТекущаяДатаСеанса());
	Объект.ОкончаниеПериодаЗагрузки = ТекущаяДатаСеанса();
...
Процедура КомандаПодборВалютВыполнить()
	...
	ПараметрыФормы = Новый Структура("ЗакрыватьПриВыборе, Отбор", Ложь, Новый Структура("ВалютаЗагружается", Истина));
...
#КонецЕсли
	...
	Если Не СтатусОперации Тогда
		УдалитьОбщегоНазначенияКлиентСервер.СообщитьПользователю(ПоясняющееСообщение);
	Иначе
...
Функция ЗагрузитьКурсыСРБК_Сервер(знач СписокВалют, знач НачалоПериодаЗагрузки, знач ОкончаниеПериодаЗагрузки, ПоясняющееСообщение)
	
	Возврат РаботаСКурсамиВалютКлиентСервер.ЗагрузитьКурсыВалютПоПараметрам(
				СписокВалют, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, ПоясняющееСообщение);
