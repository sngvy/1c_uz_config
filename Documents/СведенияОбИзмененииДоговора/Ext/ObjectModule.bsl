...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ЭтотОбъект.ПричиныПрекращенияОбязательства <> 2 Тогда  
		ДатаЗаписи =?(ЗначениеЗаполнено(ДатаФактическогоПрекращенияИзменения), ДатаФактическогоПрекращенияИзменения, ДатаСобытия); 
		КредитныеИстории.ЗаписатьСобытиеСделки(ОбъектУчета, ДатаЗаписи, ЭтотОбъект.Ссылка, "2.1");
	КонецЕсли;    
	...
	Если НастройкаФормироватьСписание И ЭтотОбъект.ПричиныПрекращенияОбязательства = 2 Тогда
		Если ЗначениеЗаполнено(ДатаЗаписи) Тогда 
			КредитныеИстории.ЗаписатьСобытиеСделки(ОбъектУчета, ДатаЗаписи + 1 , ЭтотОбъект.Ссылка, "2.3"); 
		Иначе	
			КредитныеИстории.ЗаписатьСобытиеСделки(ОбъектУчета, ДатаПрекращенияОбязательства + 1 , ЭтотОбъект.Ссылка, "2.3");   
		КонецЕсли;
	КонецЕсли;
