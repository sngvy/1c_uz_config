...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	БитОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
КонецПроцедуры   

Процедура БитОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) 
	// Лебедева 18.11.2022
	// Для отправки писем в техподдержку   
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
		Возврат;
	КонецЕсли;	
	ЕстьОснование = ДанныеЗаполнения.Свойство("Основание");
	MandarinОперации = ДанныеЗаполнения.Свойство("MandarinОперации");
	ОбратнаяСвязь =  ДанныеЗаполнения.Свойство("ОбратнаяСвязь");
	Если ЕстьОснование Тогда
		...
		Если ТипЗнч(Основание) = Тип("СправочникСсылка.ДолговыеОбязательства") Тогда
			Если ЕмаилКонст.СправочникВладелец.ИмяПредопределенныхДанных = "Справочник_Контрагенты" Тогда
				EmailПолучателя = ОбъектыСервер.ПолучитьЗначениеСвойства(Основание.Должник,ЕмаилКонст.Код);
			ИначеЕсли ЕмаилКонст.СправочникВладелец.ИмяПредопределенныхДанных = "Справочник_ДолговыеОбязательства" Тогда
				EmailПолучателя = ОбъектыСервер.ПолучитьЗначениеСвойства(Основание,ЕмаилКонст.Код); 
			КонецЕсли;	
			//Основание = Основание.Должник; 
		Иначе
			EmailПолучателя = Основание.ОтправительАдрес;
		КонецЕсли;	  
		
		СтрПолучатели = ПолучателиПисьма.Добавить();
		СтрПолучатели.Адрес = EmailПолучателя;  
		СтрПолучатели.Контакт = Основание;  
		СтрПолучатели.Представление = Основание; 
		
		Если ТипЗнч(Основание) = Тип("СправочникСсылка.ДолговыеОбязательства") Тогда
			ОтправительПредставление = ОбъектыСервер.ПолучитьЗначениеКонстанты("УчеткаEMailРассылок");
			...
			Если ШаблонПисьма.ОбъектУчета = Перечисления.ОбъектыУчета.ДолговыеОбязательства Тогда
				Текст = Автоинформирование.ПолучитьПодсказку(Основание, ШаблонПисьма);
				Тема = ШаблонПисьма.ЗаголовокEMail;
			ИначеЕсли
				ШаблонПисьма.ОбъектУчета = Перечисления.ОбъектыУчета.Контрагенты Тогда
				Текст = Автоинформирование.ПолучитьПодсказку(Основание.Должник, ШаблонПисьма);
				Тема = ШаблонПисьма.ЗаголовокEMail;
			КонецЕсли;
			Если ШаблонПисьма.НазначениеШаблона = Перечисления.ВариантыДействийДляСкоринга.EMailРассылка И ЗначениеЗаполнено(ШаблонПисьма.ШаблонHTML) Тогда
				ТекстHTML = Текст;
				ТипТекста = Перечисления.ТипыТекстовЭлектронныхПисем.HTML;
				Кодировка = "UTF-8";
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли MandarinОперации Тогда
		ОбработатьОткрытиеMandarinОперации(ДанныеЗаполнения);
	Иначе
		Если ОбратнаяСвязь Тогда 
			ОбработатьОткрытиеОбратнойСвязи();  
		КонецЕсли;	
	КонецЕсли;	
КонецПроцедуры

Процедура ОбработатьОткрытиеMandarinОперации(ДанныеЗаполнения)
	ОтправительПредставление = ОбъектыСервер.ПолучитьЗначениеКонстанты("УчеткаEMailРассылок");
	СтрПолучатели = ПолучателиПисьма.Добавить();
	СтрПолучатели.Адрес = ДанныеЗаполнения.Емаил;
	ШаблонПисьма = ?(ДанныеЗаполнения.ВидЗаказа = Перечисления.Mandarin_Действия.Токен, ОбъектыСервер.ПолучитьЗначениеКонстанты("MandarinШаблонТекстаТокен"),ОбъектыСервер.ПолучитьЗначениеКонстанты("MandarinШаблонТекстаОплата"));
	Текст = Автоинформирование.ПолучитьПодсказку(ДанныеЗаполнения.ОбъектУчета, ШаблонПисьма);
	Текст = Текст + Строка(ДанныеЗаполнения.СсылкаДляПлательщика);
КонецПроцедуры   

Процедура ОбработатьОткрытиеОбратнойСвязи()
	ОтправительПредставление = ОбъектыСервер.ПолучитьЗначениеКонстанты("УчеткаEMailРассылок");
	СтрПолучатели = ПолучателиПисьма.Добавить(); 
	ДанныеПисьма = Рассылка.СформироватьАтрибутыПисьмавТехподдержку();
	ШаблонПисьма = Справочники.ШаблоныТекстаДляАвтоинформирования.ПустаяСсылка();
	СтрПолучатели.Адрес = ДанныеПисьма.АдресТехПоддержки;
	Текст = ДанныеПисьма.ТекстПисьмаШаблон + Символы.ПС + СтрЗаменить(Рассылка.СформироватьАнкетуПользователя(), Символы.Таб, Символы.ПС);
КонецПроцедуры
