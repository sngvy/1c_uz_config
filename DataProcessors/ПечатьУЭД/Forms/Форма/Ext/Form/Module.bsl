...
Процедура Печать(Команда)
	...
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ДолговоеОбязательство) Тогда
	
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не выбрано Долговое обязательство!";
		Сообщение.Сообщить();
		Возврат;
	...
	Если ЭтоУниверсальнаяПечатьНаСервере() Тогда
		...
		НастройкиПечати.Вставить("Распечатать", РаспечататьСформированныеДокументы);
		НастройкиПечати.Вставить("ПерезаписатьСуществующие", ПерезаписатьСуществующие);
		...
		Попытка
			...
			ТекстСообщения = ТекстСообщения + " прошло успешно!";
		
		Исключение
			ТекстСообщения = ТекстСообщения + " не получилось!" + Символы.ПС + ОписаниеОшибки();
		КонецПопытки;
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	ЗаполнениеНачальныхДанных();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнениеНачальныхДанных()
...
Функция СформироватьКомплекты()
	...
	Основания.Добавить(ДолговоеОбязательство);
	
	КомплектыБинарныйФормат = Справочники.тсВидыПечатныхДокументов.СформироватьУниверсальныеШаблоны(Объект.Шаблон, Основания);
	...
	Для каждого Комплект Из КомплектыБинарныйФормат Цикл
		...
		ОснованиеСКоличеством.Вставить("Основание", Комплект.Ключ);
		...
		КомплектыБинарныйФорматСКоличеством.Вставить(ОснованиеСКоличеством, Комплект.Значение);
...
&НаСервере
Функция ПолучитьДвоичныеДанные(ДвоичныйАдрес)

	Значение = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(ДвоичныйАдрес));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПечатьМероприятия" Тогда
	
		Объект.Объект = Параметр["Объект"];
		ЗаполнениеНачальныхДанных();
