	...
	Если СвязанныйДокумент = ПредопределенноеЗначение("Перечисление.ВидыСвязанныхДокументов.ОбещанныйПлатеж") Тогда
		...
		Если Элемент <> Неопределено Тогда
			Если Не ЗначениеЗаполнено(Объект.СвязанныйДокумент) ИЛИ 
				...
				ПоказатьОповещениеПользователя("Обещанные платежи",,"Новое обещание сохранено",БиблиотекаКартинок.NC_ПодсистемаКонтрольЗадолженности);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
		...
		Если КонтактНайден = Ложь Тогда
			ПоказатьОповещениеПользователя("Предупреждение",,"Не удалось найти контакт должника. Укажите его вручную",БиблиотекаКартинок.NC_РозыскДолжника);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) = Истина Тогда
		...
		Если ФормаПоискаНомера.ЕстьСовпадения = Истина Тогда
			ПоказатьОповещениеПользователя("Поиск по номеру в базе",,"Найдены совпадающие должники",БиблиотекаКартинок.NC_РозыскДолжника);
		КонецЕсли;
...
Процедура ПозвонитьДолжнику(Команда)
	...
	Если ВидТелефонии = ПредопределенноеЗначение("Перечисление.ВидыТелефонии.БИТФон") Тогда
		...
		ИначеЕсли Ф.Открыта() = Истина И Ф.СтатусПодключен = Истина Тогда
			...
			ПоказатьОповещениеПользователя("БИТ.Phone",,"Начат вызов на выбранный номер",БиблиотекаКартинок.NC_ИсходящийЗвонок);
...
Процедура ОбъектПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
		...
		ПолучитьДниПросрочкиНаСервере();
		ПолучитьВозрастНаСервере();
		ПоказатьОповещениеПользователя("Предупреждение",,"Номер договора был изменен. Проверьте контакт должника",БиблиотекаКартинок.NC_РозыскДолжника);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтправитьСМС(Команда)
	...
	ФормаСМС = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОтправкаСМС",,,Новый УникальныйИдентификатор());
...
Процедура ПоискНомераВБазе(Команда)
	ПоказатьОповещениеПользователя("Поиск по номеру в базе",,"Выполняется поиск совпадений...",БиблиотекаКартинок.NC_РозыскДолжника);
...
Процедура НаписатьВВЦ(Команда)
	...
	ФормаВЦ = ПолучитьФорму("ОбщаяФорма.РассылкаНаWhatsApp",,,Новый УникальныйИдентификатор());
