...
//
Функция СформироватьИнтернетПрофиль(
	Знач УчетнаяЗапись,
	...
	Если Выборка.Следующий() Тогда
		...
		Результат = Новый ИнтернетПочтовыйПрофиль;  // Добавить условие двух галок. отсутствие двух галок считать отправкой
		
		// Отправка и получение заполняются отдельно
		// Если галки стоят обе и на отправку и на получение то заполнится каждое условие
		// Или если галок нет то отработает условие отправки по умолчанию
		Если Выборка.Протокол = "IMAP" Тогда
			Результат.ПарольIMAP = УчетнаяЗапись.Пароль;
			Если УчетнаяЗапись.ИспользоватьДляПолучения И НЕ УчетнаяЗапись.ИспользоватьДляОтправки  Тогда
				ТребуемыеСвойства = СписокСвойствIMAP;
			КонецЕсли;
			
			Если УчетнаяЗапись.ИспользоватьДляОтправки ИЛИ
				НЕ УчетнаяЗапись.ИспользоватьДляПолучения Тогда // когда галки не стоят
...
Процедура ПроверитьВозможностьОтправкиТестовогоСообщения(знач УчетнаяЗапись,
	...
	РаботаСЭлектроннымиСообщениями.ОтправитьПочтовоеСообщение(УчетнаяЗапись, ПараметрыПисьма);
