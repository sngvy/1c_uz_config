			...
			Для Сч = 0 По Данные.pageData.Количество() Цикл
				Попытка
					...
					Если ДатаРождения = ОбъектыСервер.ПолучитьЗначениеСвойства(Должник, "0043") Тогда
						...
						ДО = Справочники.ДолговыеОбязательства.НайтиПоРеквизиту("Должник", Справочники.Контрагенты.НайтиПоКоду(Код));
						Если ОбъектыСервер.ПолучитьЗначениеСвойства(ДО,"0182") <> Справочники.тсЗначенияСвойствОбъектов.НайтиПоКоду("000000048") Тогда // Проверка на передачу в ЮД 
							...
							ИначеЕсли Данные.pageData[Сч].lastLegalCase.status.description = "Реализация имущества гражданина" Тогда 
