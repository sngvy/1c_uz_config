...
Процедура ЗаписатьXMLТипыМероприятий(ЗаписьXML, Тип) Экспорт
	...
	Если ЕстьРеквизиты Тогда
		...
		ЗаписатьXML(ЗаписьXML, ТаблицаРеквизитовДляМП[0].ТипАдресаВыезда, "ТипАдресаВыезда", НазначениеТипаXML.Явное);
	Иначе
...
Функция ПрочитатьXMLМероприятие(ЧтениеXML,ПользовательУЗ,СсылкаМП) Экспорт
	...
	Если МероприятиеЗадачаСсылка = Неопределено Или МероприятиеЗадачаСсылка = Задачи.Мероприятие.ПустаяСсылка() Тогда
		...
		МероприятиеЗадача.Дата = ТекущаяДата();
	...
	КонецЕсли;
	//
	//Если ТипСвязанногоДокумента = Перечисления.ВидыСвязанныхДокументов.ОбещанныйПлатеж тогда 
	//	Если Не ЗначениеЗаполнено(МероприятиеЗадача.СвязанныйДокумент) ИЛИ Найти(Строка(СвязанныйДокумент),"<Объект не найден>") <> 0  Тогда
	//		МероприятиеЗадача.СвязанныйДокумент = Документы.ОбещанныеПлатежи.НайтиПоНомеру(КодПрикрепленногоДокумента,ТекущаяДата());
	//	КонецЕсли;
	//КонецЕсли;	
...
Функция ПрочитатьXMLТипыМероприятий(ЧтениеXML) Экспорт
	...
	МЗ.ТипАдресаВыезда = ПрочитатьXML(ЧтениеXML);
	МЗ.Записать();
	// Проверяем, что текущим узлом является "КонецЭлемента".
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Тогда
		ВызватьИсключение "Ошибка чтения XML";
...
Функция ПрочитатьXMLФайлы(ЧтениеXML,ПользовательУЗ,СсылкаМП) Экспорт
	...
	Если КолВоФайлов = 0  Тогда
		...
		//ТипФайлаБаза = Справочники.ТипыПрикрепляемыхФайлов.НайтиПоКоду(КодТипаФайла);
		//Если ТипФайлаБаза = Неопределено Или ТипФайлаБаза = Справочники.ТипыПрикрепляемыхФайлов.ПустаяСсылка() Тогда
		//	Стр.ТипФайла = ТипФайла;
		//иначе
		//	Стр.ТипФайла = ТипФайлаБаза;
		//КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресДолжникаСвойство) Тогда
		...
		Если ЗначениеЗаполнено(Адрес) Тогда
			...
			Если АдресДолжникаСвойство.ВидСтроки = Перечисления.ВидыТипаСтрока.АдресФИАС Тогда
				...
				Нстр.ТипАдресаВыезда = Перечисления.ТипыАдресовВыезда.АдресДолжника;
	...
	Если ЗначениеЗаполнено(АдресСудаСвойство) Тогда
		...
		Если ЗначениеЗаполнено(Адрес) Тогда
			...
			Если АдресСудаСвойство.ВидСтроки = Перечисления.ВидыТипаСтрока.АдресФИАС Тогда
				...
				Нстр.КодОбъекта = КодОбъекта;
				Нстр.ТипОбъекта = ТипОбъекта;
				Нстр.ТипАдресаВыезда = Перечисления.ТипыАдресовВыезда.АдресСуда;
				Нстр.АдресПредставление = АдресПредставление;
				Нстр.Страна = "Россия";
				ЗаполнитьАдресФИАС(Адрес,Нстр);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресОСПСвойство) Тогда
		...
		Если ЗначениеЗаполнено(Адрес) Тогда
			...
			Если АдресОСПСвойство.ВидСтроки = Перечисления.ВидыТипаСтрока.АдресФИАС Тогда
				...
				Нстр.КодОбъекта = КодОбъекта;
				Нстр.ТипОбъекта = ТипОбъекта;
				Нстр.ТипАдресаВыезда = Перечисления.ТипыАдресовВыезда.АдресОСП;
				Нстр.АдресПредставление = АдресПредставление;
				Нстр.Страна = "Россия";
				ЗаполнитьАдресФИАС(Адрес,Нстр);
			КонецЕсли;
