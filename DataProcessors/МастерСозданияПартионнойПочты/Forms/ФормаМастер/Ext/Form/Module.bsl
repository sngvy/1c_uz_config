...
Процедура ЗаполнитьКорреспонденциюСервер()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.СтатусыШПИРегистрация.СрезПоследних(, ) КАК СтатусыШПИРегистрацияСрезПоследних
	               ...
	               |ГДЕ
	               |	СтатусыШПИРегистрацияСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыШПИРегистрация.ПолученаФорма103)
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ШПИВладелец
	               |;
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ОтправленнаяКорреспонденцияБезШПИСрезПоследних.Период КАК Период,
	               |	ВЫБОР
	               |		КОГДА ОтправленнаяКорреспонденцияБезШПИСрезПоследних.ИсходящаяКорреспонденция ССЫЛКА Справочник.ГруппаИсходящейКорреспонденции
	               |			ТОГДА ГруппаИсхКорр.ИсходящаяКорреспонденция
	               |		ИНАЧЕ ОтправленнаяКорреспонденцияБезШПИСрезПоследних.ИсходящаяКорреспонденция
	               |	КОНЕЦ КАК ИсходящаяКорреспонденцияБезШпи,
	               |	ОтправленнаяКорреспонденцияБезШПИСрезПоследних.Партия КАК Партия,
	               |	ОтправленнаяКорреспонденцияБезШПИСрезПоследних.Отправлено КАК Отправлено
	               |ПОМЕСТИТЬ КорреспонденцияБезШпи
	               |ИЗ
	               |	РегистрСведений.ОтправленнаяКорреспонденцияБезШПИ.СрезПоследних КАК ОтправленнаяКорреспонденцияБезШПИСрезПоследних
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ГруппаИсхКорр КАК ГруппаИсхКорр
	               |		ПО ОтправленнаяКорреспонденцияБезШПИСрезПоследних.ИсходящаяКорреспонденция = ГруппаИсхКорр.ГруппаСсылка
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	ИсходящаяКорреспонденцияБезШпи
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ИсходящаяКорреспонденция.Ссылка КАК Ссылка,
	               |	КорреспонденцияБезШпи.Отправлено КАК КорреспонденцияБезШпи,
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ КорреспонденцияБезШпи КАК КорреспонденцияБезШпи
	               |		ПО ИсходящаяКорреспонденция.Ссылка = КорреспонденцияБезШпи.ИсходящаяКорреспонденцияБезШпи
	               |ГДЕ
	               ...
	               |	И КорреспонденцияБезШпи.Отправлено ЕСТЬ NULL";
...
Процедура СоздатьЗаказы(Команда)
	СоздатьЗаказНаСервереВЗависимостиОтВерсииAPI();
	УстановитьИзмененияВИнтерфейсе();
КонецПроцедуры

&НаСервере
Процедура СоздатьЗаказНаСервереВЗависимостиОтВерсииAPI()
	
	Ресурс = "";
	
	Если Константы.УчеткаПочтыРФ.Получить().ВерсияAPI = Перечисления.ПочтаРФ_ВерсияAPI.Версия_1_0 Тогда
		
		Ресурс = Обработки.МастерСозданияПартионнойПочты.ПолучитьРесурсДляПервойВерсииЗаказа();
		СоздатьЗаказНаСервере(Ресурс);
		
	ИначеЕсли Константы.УчеткаПочтыРФ.Получить().ВерсияAPI = Перечисления.ПочтаРФ_ВерсияAPI.Версия_2_0 Тогда
		
		Ресурс = Обработки.МастерСозданияПартионнойПочты.ПолучитьРесурсДляВторойВерсииЗаказа();
		СоздатьЗаказНаСервере(Ресурс);
	Иначе
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Версия API не установлена";
		Сообщение.Сообщить();
		Возврат;
...
&НаСервере
Процедура СоздатьЗаказНаСервере(Ресурс)
	...
	strJSON = тЗаписьJSON.Закрыть();
	Сервер = Обработки.МастерСозданияПартионнойПочты.ПолучитьСерверПочтыРФ();      
...
Функция СоздатьМассивЗаказов(ТЗПараметрыПисем)
	...
	Для каждого Стр из Объект.СписокИсходящейКорреспонденции Цикл
		...
		ИД_Магазина = СтрЗаменить(ИД_Магазина, "\", "");
	...
	Для Каждого Стр из Объект.ДокументыПартии Цикл
		Если Стр.Зарегистрирована = Истина Тогда
			...
			Если Стр.получена103 Тогда
				...
				ЗаписатьИсхКорреспонденциюБезШПИ(Объект.СписокИсходящейКорреспонденции);
	...
	Если не ЗавершениеРаботы и ЗавершитьРаботуМастера = Ложь Тогда
		...
		Для Каждого стр из Объект.СписокИсходящейКорреспонденции Цикл
			...
			Иначе
				...
				СтруктураМассива.Вставить("IDПочты",Стр.ИдентификаторПочты);
				СтруктураМассива.Вставить("ИсхКорр",Стр.ИсходящаяКорреспонденция);
				БезШПИНаУдаление.Добавить(СтруктураМассива);	
			КонецЕсли;
		КонецЦикла;
		...
		Если ШПИНаУдаление.Количество() <> 0 Или ПартииНаУдаление.Количество() <> 0 Или БезШПИНаУдаление.Количество() Тогда
			Отказ = Истина;
			...
			ДП.Вставить("МассивБезШПИ",БезШПИНаУдаление);
...
Процедура ПослеВопросаУдалитьОбъекты(Результат, Параметры) Экспорт
	...
	УдалитьБезШПИ = Параметры.МассивБезШПИ;
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого эл из УдалитьБезШПИ Цикл
		тЗапрос.Добавить(эл.IDПочты);
		ОчиститьРСОтправленнаяКорреспонденцияБезШПИ(Эл.ИсхКорр);
	КонецЦикла;
...
Процедура ОбновитьИнформациюШипМент(Партия)
	...
	Для Каждого стр из Объект.СписокИсходящейКорреспонденции Цикл
		...
		тПараметрыJSON = Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет, " ", Истина);  
		тЗаписьJSON.УстановитьСтроку(тПараметрыJSON);
		Если Партия и ЗначениеЗаполнено(Стр.ШПИ) тогда
...
Процедура ИндексОПСОбслуживанияПриИзменении(Элемент)
	Для Каждого Эл из объект.СписокИсходящейКорреспонденции Цикл 
		Если Не Эл.КорректностьАдреса Тогда
			Продолжить;
		КонецЕсли;	
...
&НаСервере
Процедура ЗаписатьИсхКорреспонденциюБезШПИ(Знач Корр)
	Для Каждого СтрБезШПИ из Корр Цикл
		Если СтрБезШПИ.ШПИ = Справочники.ШПИ.ПустаяСсылка() Тогда 
			МенеджерЗаписи = РегистрыСведений.ОтправленнаяКорреспонденцияБезШПИ.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Период = ТекущаяДатаСеанса();
			МенеджерЗаписи.ИсходящаяКорреспонденция = СтрБезШПИ.ИсходящаяКорреспонденция;			
			МенеджерЗаписи.Партия = СтрБезШПИ.ДокументПартия;
...
&НаСервере
Процедура ОчиститьРСОтправленнаяКорреспонденцияБезШПИ(Исходящая)
	НаборЗаписей = РегистрыСведений.ОтправленнаяКорреспонденцияБезШПИ.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ИсходящаяКорреспонденция.Установить(Исходящая);
	НаборЗаписей.Записать();
Конецпроцедуры
