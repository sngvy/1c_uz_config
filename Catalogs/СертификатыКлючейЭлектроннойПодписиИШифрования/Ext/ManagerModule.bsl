...
Функция ДанныеОрганизации(Элемент) Экспорт

	Если ТипЗнч(Элемент) <> ТипЗнч(ПустаяСсылка()) Тогда
	...
	Хранилище = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Элемент, "ДанныеСертификата");
	...
	Данные.Вставить("Имя", СвойстваСертификата["КомуВыдан"]);
	Данные.Вставить("Огранизация", СвойстваСертификата["КемВыдан"]);
	
	Коллекция = ЭлектроннаяПодпись.СвойстваСубъектаСертификата(Сертификат);
	Данные.Вставить("ИНН", Коллекция["ИННЮЛ"]);
	Данные.Вставить("ОГРН", Коллекция["ОГРН"]);
	
	Данные.Вставить("Адрес", Коллекция["НаселенныйПункт"] + ", " + Коллекция["Улица"]);
