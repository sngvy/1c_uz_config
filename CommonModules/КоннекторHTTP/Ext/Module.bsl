...
Функция MultiJsonPost(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт

	Ответ = MultiPost(URL, Данные, ДополнительныеПараметры, Сессия);
	
	ПараметрыПреобразованияJSON =
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыПреобразованияJSON", Неопределено);
	
	Возврат КакJson(Ответ, ПараметрыПреобразованияJSON);

КонецФункции // ()

Функция MultiPost(URL, Данные = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт

	Метод = "POST";
	
	ТекущаяСессия = ТекущаяСессия(Сессия);
	
	ПодготовленныйЗапрос = СформироватьДанныеЗапроса(URL, Метод, ТекущаяСессия, ДополнительныеПараметры);
	
	Запрос = СформироватьЗапрос(ПодготовленныйЗапрос, Данные);
	ПодготовленныйЗапрос["HTTPЗапрос"] = Запрос;
	
	НастройкиПодключения = НастройкиПодключения(Метод, URL, ДополнительныеПараметры);
	
	Возврат ОтправитьЗапрос(ТекущаяСессия, ПодготовленныйЗапрос, НастройкиПодключения);

КонецФункции // ()

Функция СформироватьДанныеЗапроса(URL, Метод, Сессия, ДополнительныеПараметры = Неопределено)
	...
	ПодготовленныйЗапрос.Вставить("HTTPЗапрос", Неопределено);
...
Функция СформироватьЗапрос(ДанныеЗапроса, ПараметрыЗапроса)

	СтруктураURL = РазобратьURL(ДанныеЗапроса.URL);
	АдресРесурса = СобратьАдресРесурса(СтруктураURL, ДанныеЗапроса.ПараметрыЗапроса);
	...
	Данные = Неопределено;
	Если ПараметрыЗапроса.Свойство("Данные", Данные) Тогда
		...
		Для каждого Элемент Из Данные Цикл
			...
			ЗаписьДанных.ЗаписатьСтроку(Контент);
	...
	Файлы = Неопределено;
	Если ПараметрыЗапроса.Свойство("Файлы", Файлы) Тогда
		...
		Для каждого Элемент Из Файлы Цикл
			...
			ЗаписьДанных.Записать(Элемент["Данные"]);
...
Функция ТестПост(URL, HTTPЗапрос, ДополнительныеПараметры = Неопределено) Экспорт

	Сессия = ТекущаяСессия(Неопределено);
	Метод = "POST";
	...
	//ЗаголовкиЗапроса.Вставить("Content-Type", "multipart/form-data; boundary=----WebKitFormBoundary7MA4YWxkTrZu0gW");
	//ЗаголовкиЗапроса.Вставить("Authorization", Заголовки.Получить("Authorization"));
...
Функция ВырезатьТекст(Текст, МаксимальнаяДлинаТекста = 1000)
	//Переход на 8.3.20
	Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(Текст) Тогда
