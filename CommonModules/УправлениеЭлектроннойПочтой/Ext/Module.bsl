...
Функция ЗагрузитьЭлектронныеПисьма(ПолученныеПисьма)
	...
	|	И УчетныеЗаписиЭлектроннойПочты.POP3Сервер <> """"
...
Процедура ОтправитьЭлектронныеПисьма(ПисьмаДляОпределенияПапок, ВсеПолученныеПисьма)
	...
	|	И УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты <> """"
	|	И УчетныеЗаписиЭлектроннойПочты.SMTPСервер <> """"
...
Процедура ПолучитьПисьма(Знач ДанныеУчетнойЗаписи, ЕстьОшибки, Получено, ПолученныеПисьма)
	...
	Профиль = РаботаСЭлектроннымиСообщениями.ИнтернетПочтовыйПрофиль(ДанныеУчетнойЗаписи.Ссылка, Истина);
...
Процедура ЗаполнитьПредметИКонтакты(Письмо,
	...
	СоответствиеКонтактов = БитНайтиАдресаЭлектроннойПочтыВКонтактнойИнформации(НеопределенныеАдреса);
...
Функция БитНайтиАдресаЭлектроннойПочтыВКонтактнойИнформации(МассивАдресов)
	...
	Запрос.Текст = 
		...
		|	КонтрагентыДополнительныеРеквизиты.Значение КАК Имейл
		|ИЗ
		|	Справочник.Контрагенты.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Таб КАК Таб
		|		ПО КонтрагентыДополнительныеРеквизиты.Свойство = Таб.Ссылка
		|ГДЕ
		|	КонтрагентыДополнительныеРеквизиты.Значение В(&МассивАдресов)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ДополнительныеСведения.Значение В(&МассивАдресов)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ДолговыеОбязательстваДополнительныеРеквизиты.Значение В(&МассивАдресов)";
	...
	Пока Выборка.Следующий() Цикл
		СоответствиеКонтактов.Вставить(ВРег(Выборка.Имейл), Выборка.Ссылка);
	КонецЦикла;
...
Функция ТекстЗапросаПоДоступнымУчетнымЗаписям()
	...
	|			ТОГДА 0
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|				И УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем = &ТекущийПользователь
	|			ТОГДА 1
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	...
	|	И НЕ ЕСТЬNULL(НастройкиУчетныхЗаписейЭлектроннойПочты.НеИспользоватьВоВстроенномПочтовомКлиенте, ЛОЖЬ)
	|	И (УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем = &ТекущийПользователь
	|			ИЛИ УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
	|
	|УПОРЯДОЧИТЬ ПО
