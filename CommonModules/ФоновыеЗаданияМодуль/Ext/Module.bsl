...
Процедура ОбновитьДелаОБанкротстве() Экспорт	
	...
	|				И ДелаОБанкротстве.ДатаПроверки ЕСТЬ NULL
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование В (""В работе"", ""Новые"", ""Ручной обзвон"")
	|				И ДелаОБанкротстве.ДатаПроверки < &НачалоМесяца)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДолговыеОбязательства.ДатаСоздания УБЫВ";
	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца(ТекущаяДата()));
...
Процедура СохранитьРезультатыIVR() Экспорт
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяЗапись.Результат = Выборка.state;
			НоваяЗапись.ДО = ТелефонВДО(Выборка.number);
			Если Выборка.state = "answered" Тогда
				НоваяЗапись.Действие = Перечисления.ДействияПоРезультатамIVR.ПереводВРаботу;
				ДО = НоваяЗапись.ДО;
				Код = "0181";
				...
				ОбъектыСервер.ЗаписатьЗначениеСвойства(ДО, Код, Значение);	
			Иначе
				НоваяЗапись.Действие = Перечисления.ДействияПоРезультатамIVR.БезИзменений;
			КонецЕсли;
		КонецЦикла;
