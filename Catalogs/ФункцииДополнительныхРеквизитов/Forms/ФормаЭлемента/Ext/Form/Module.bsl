...
Процедура ТестНаСервере()
	...
	Попытка
		ВычислитьФункцию(Объект.Функция, МероприятиеТекущее, МероприятиеСледующее, Отказ);
		Сообщить("Ок!");
	Исключение
...
&НаСервере
Процедура ВычислитьФункцию(Знач Запрос, МероприятиеТекущее, МероприятиеСледующее, Отказ)
	Выполнить(Запрос);	
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьФормуМероприятия(Команда)
	Если МероприятиеСледующее <> Неопределено Тогда
		Форма = ПолучитьФорму("Задача.Мероприятие.Форма.ФормаМероприятия",, ЭтаФорма);
		Форма.Объект = МероприятиеСледующее;
		//Чуров
		ОткрытьФорму(Форма);
