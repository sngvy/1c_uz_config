...
Функция РассчитатьСписокПлатежей(Договор) Экспорт 
	...
	ИначеЕсли Договор.ВидПлатежа = Перечисления.ВидыПлатежей.Аннуитетный Тогда
		...
		Для к = 1 по Срок Цикл   
			...
			стр.ДатаПлатежа = РасчетЗадолженностиМФО.ДобавитьПериодКДате(НачалоПериода, Договор.ПериодичностьСрокаЗайма, Договор.КоличествоПериодичностей);   
...
Функция РасчитатьГрафикПлатежейИНачислений(Договор, ДатаРасчета, ТаблицаПлатежей, Остатки) Экспорт 
	...
	Для Каждого ПлановыйПлатеж Из ГрафикПлатежей Цикл
		ДлинаПериода = (ПлановыйПлатеж.Дата - НачалоПериода)/86400;
	...
	ДлинаПериода = (ДатаРасчета - НачалоПериода)/86400;
	Для к = 1 По ДлинаПериода Цикл
		Попытка
			Сегодня = График[к+Сдвиг];
		Исключение
...
Функция СформироватьТаблицуЗначенийГрафикНачислений(Договор, ДатаРасчета, ТаблицаПлатежей, Остатки) 
	...
	Если Остатки <> Неопределено И ЗначениеЗаполнено(Остатки.ОстаткиДата) Тогда  
		...
		Для Каждого Колонка Из График.Колонки Цикл   
			...
			КонецПопытки;  
		КонецЦикла;  
		//Вчера.НачисленияЗаВесьПериод = Остатки.ОстаткиНачисленияЗаВесьПериод;  
		Вчера.Дата = Остатки.ОстаткиДата; 
	    Вчера.ОстаткиОсновнойДолг = Остатки.ОстаткиОсновнойДолг;
		Вчера.ОстатокОДФактический = Остатки.ОстаткиОсновнойДолг;//Договор.СуммаВыданногоЗайма;//Остатки.ОстатокОДФактический;
		Вчера.ОстатокПроцентовФактический = Остатки.ОстаткиПроценты + Остатки.ОстаткиПросроченныеПроценты;//СуммаПроцентовКУплате(Договор);//Остатки.ОстатокПроцентовФактический;
		Вчера.ОстаткиПроцентыЗаВесьПериод = Вчера.ОстатокПроцентовФактический; //Остатки.ОстаткиПроцентыЗаВесьПериод;
		//Вчера.ОстаткиНачисленияЗаВесьПериод = Остатки.ОстаткиНачисленияЗаВесьПериод;  
		Вчера.ОстаткиНачисленияЗаВесьПериод = Вчера.ОстатокПроцентовФактический + Вчера.ОстаткиШтрафы + Вчера.ОстаткиПени;  
		Вчера.КУплате = Вчера.ОстатокОДФактический + Вчера.ОстатокПроцентовФактический + Вчера.ОстаткиШтрафы + Вчера.ОстаткиПени;  // Вчера.ОстаткиОсновнойДолг + Вчера.ОстаткиПроценты + Вчера.ОстаткиПросроченныеПроценты + Вчера.ОстаткиШтрафы + Вчера.ОстаткиПени; 
	Иначе  
		Вчера = График.Добавить();  
	...
	//ОстатокОдПоГрафику = Договор.СуммаВыданногоЗайма; 
	//Для Каждого Стр из График Цикл  
	//	Если стр.ПлановыйГрафик_СуммаПлатежа = 0 Тогда   
	//		стр.ПлановыйГрафик_ОстаткиОсновнойДолг = ОстатокОдПоГрафику;  
	//	Иначе   
	//		ОстатокОдПоГрафику = стр.ПлановыйГрафик_ОстаткиОсновнойДолг;  
	//	КонецЕсли; 
	//КонецЦикла;  
	...
	ИначеЕсли Договор.ВидПроцентнойСтавки = Перечисления.ВидыПроцентнойСтавки.ПлавающаяПроцентнаяСтавкаЗависимаяОтСрока Тогда
		Для Каждого Интервал Из Договор.ИнтервалыПроцентнойСтавки Цикл
			ИнтервалНачалоИнтервала = РасчетЗадолженностиМФО.ДобавитьПериодКДате(Договор.ДатаВыдачиЗайма + 86400, Договор.ПериодичностьСрокаЗайма, Интервал.ИнтервалОт);
			ИнтервалКонецИнтервала = ?(Интервал.ИнтервалДо = 0, ДатаРасчета, РасчетЗадолженностиМФО.ДобавитьПериодКДате(Договор.ДатаВыдачиЗайма, Договор.ПериодичностьСрокаЗайма, Интервал.ИнтервалДо));
			Пока ИнтервалНачалоИнтервала < ИнтервалКонецИнтервала + 86400 Цикл
				стр = График.НайтиСтроки(Новый Структура("Дата", ИнтервалНачалоИнтервала))[0];
				стр.ПроцентнаяСтавка = Интервал.ПроцентнаяСтавка;
				ИнтервалНачалоИнтервала = ИнтервалНачалоИнтервала + 86400;
			КонецЦикла;
		КонецЦикла;
...
Функция Штрафы(Договор, Вчера, Сегодня) 
	...
	Если Договор.ДатаДоговора > Дата(2017, 01, 01) И Договор.ДатаДоговора <= Дата(2019, 01, 28) Тогда   
		Штрафы = СуммаШтрафы; 
	Иначе  
...
Функция ДниПросрочкиФИФО(Договор, Вчера, Сегодня) 
	...
	Если ДатаВозникновенияПросрочки = Неопределено Тогда
		Возврат 0;
	Иначе
		Возврат (Сегодня.Дата - ДатаВозникновенияПросрочки)/24/3600;
	КонецЕсли;
КонецФункции
...
Функция ПолучитьКоэффициентОграниченияНачисленияПроцентовНаСуммуЗайма(ДатаДоговора, БанковскаяПрограмма)
	...
	ИначеЕсли ДатаДоговора > Дата(2019, 06, 30) И ДатаДоговора <= Дата(2019, 12, 31) Тогда  
		Возврат 2; 
	ИначеЕсли ДатаДоговора > Дата(2019, 12, 31) Тогда  
		Возврат 1.5;  
	КонецЕсли;
КонецФункции
	...
	Если ПрограммаРасчета.ПрекращатьНачисления Тогда  
		...
		Если ЗначениеЗаполнено(ПрограммаРасчета.ПрекращатьНачисления_Количество) Тогда   
			Если ПрограммаРасчета.ПрекращатьНачисления_Период = "Дней" Тогда    
				ДатаПрекращения = Договор.ДатаВыдачиЗайма + ПрограммаРасчета.ПрекращатьНачисления_Количество*24*3600;   
...
Функция ПроцентыГрафикПогашения(Договор, Вчера, Сегодня) 
	...
	Если Сегодня.Дата <= Договор.ДатаВыдачиЗайма + д*24*3600 Тогда  
		Возврат 0; 
	КонецЕсли;
...
Функция МассивПериодов(Договор, НачалоПериода, КонецПериода)
	...
	НачалоИнтервала = НачалоПериода + 24*3600;
	...
	ИначеЕсли Договор.ВидПроцентнойСтавки = Перечисления.ВидыПроцентнойСтавки.ПлавающаяПроцентнаяСтавкаЗависимаяОтСрока Тогда
		Для Каждого Интервал Из Договор.ИнтервалыПроцентнойСтавки Цикл
			...
			Если НачалоИнтервала >= ИнтервалНачалоИнтервала И НачалоИнтервала <= ИнтервалКонецИнтервала Тогда
				...
				НачалоИнтервала = стр.КонецИнтервала + 24*3600;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли Договор.ВидПроцентнойСтавки = Перечисления.ВидыПроцентнойСтавки.ПлавающаяПроцентнаяСтавкаЗависимаяОтСтавки Тогда
...
Функция КонвертацияПроцентнойСтавки(Договор, НачалоПериода, КонецПериода, ПроцентнаяСтавка)          
	...
	ИначеЕсли ПериодичностьСрокаЗайма = Перечисления.ПериодичностьСрокаЗайма.Неделя Тогда  
		...
		Иначе 
			Возврат ПроцентнаяСтавка*(КонецПериода-НачалоПериода)/86400/КоличествоДнейВГоду(КонецПериода);  
		КонецЕсли;		
...
Функция Ставка(ПроцентнаяСтавка, НачалоПериода, КонецПериода) 
	...
	Если кг1 = кг2 Тогда  
		Ставка = ПроцентнаяСтавка*(КонецПериода-НачалоПериода)/86400/дн1; 
	Иначе  
		Ставка = ПроцентнаяСтавка*(кг1-НачалоПериода)/86400/дн1 + ПроцентнаяСтавка*(КонецПериода - кг1)/86400/дн2; 
	КонецЕсли; 
	Возврат Ставка; 
КонецФункции
