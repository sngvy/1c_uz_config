...
#Область ВзаимодействиеСУЗ

&НаКлиенте
Процедура ОткрытьПапку(Папка)

	Элементы.Папки.ВыделенныеСтроки.Очистить();
	
	//ПапкаПодписание = Папки.Отбор.
	//Элементы.Папки.ВыделенныеСтроки.Добавить(ПапкаПодписание);
...
&НаКлиенте
Процедура ОткрытиеИзУЗ()

	Если ЭтаФорма.ВладелецФормы = "УЗ" Тогда
	
		ОткрытьПапку(ПредопределенноеЗначение("Справочник.ПапкиФайлов.Подписание"));
