...
Процедура ФормированиеСписковАвтообзвона() Экспорт
	...
	|	МАКСИМУМ(Мероприятие.Результат.РезультативныйКонтакт) = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	(ВЫРАЗИТЬ(Результат.Ответ / &ДеньМесяца КАК ЧИСЛО(10, 0))) - (ВЫРАЗИТЬ(Результат.Ответ / &ДеньМесяца КАК ЧИСЛО(10, 5))) КАК Поле1
	|ИЗ
	...
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(Мероприятие.Дата) < &Вчера И
	|	МАКСИМУМ(Мероприятие.Результат.РезультативныйКонтакт) = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ФормированиеСписковАвтообзвонаПараметры(Знач Запрос)
	...
	Дата = День(ТекущаяДата()) * 100;
	Запрос.УстановитьПараметр("ДеньМесяца", Дата);
...
Процедура ОбновлениеОстатков() Экспорт
	...
	|ГДЕ
	|	(ДолговыеОбязательства.Кредитор.Код В (""F00114343"", ""F00000160"", ""F00000236"", ""F00000091"", ""F00114342"")
	|			ИЛИ ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование = ""Цессия"")";
...
Процедура ОбновлениеОстатковЮД() Экспорт
	...
	|ГДЕ
	|	ДолговыеОбязательства.Кредитор.Код В (""F00079066"", ""F00099453"", ""F00108622"", ""F00000172"", ""F00109962"", ""F00000206"")
	|
	|СГРУППИРОВАТЬ ПО
	|	ДолговыеОбязательства.Ссылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(СуммаПроцентыВнесудебка) = Ложь Тогда
			СуммаПроцентыВнесудебка = 0;
		КонецЕсли;
		СуммаДОРегистрНовая = СуммаОДВнесудебка + СуммаПроцентыВнесудебка - СуммаУдержанийПоСудебнымРешениям;
		НаборЗаписей = РегистрыСведений.ЗадолженностьПоОбъектамОстатки.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Объект.Установить(Выборка.Ссылка);
		НаборЗаписей.Отбор.ТипЗадолженности.Установить(ПланыВидовХарактеристик.ТипыЗадолженности.НайтиПоНаименованию("Сумма"));
		НаборЗаписей.Прочитать(); 
		Для каждого ЗаписьНабора из НаборЗаписей Цикл
			СуммаДОРегистрСтарая = ЗаписьНабора.СуммаДО;
			Если СуммаДОРегистрСтарая <> СуммаДОРегистрНовая Тогда
				ЗаписьНабора.СуммаДО = СуммаДОРегистрНовая;
				ЗаписьНабора.СуммаРегл = СуммаДОРегистрНовая;
			КонецЕсли;
			НачатьТранзакцию();
	...
	// Выгрузить суммы удержаний по судебным решениям (только для МФО МИЛИ)
	...
	|		ПО (ЗадолженностьПоСудебнымРешениям.Займ = ДолговыеОбязательства.Ссылка)
	|ГДЕ
	|	ДолговыеОбязательства.Кредитор.Код В (""F00107981"")
	|
	|СГРУППИРОВАТЬ ПО
	|	ДолговыеОбязательства.Ссылка";
