	...
	Для Каждого Элемент Из ТзДанныеЭлементы Цикл	
		...
		Попытка	
			Если Элемент.Заголовок = "" Тогда
				Синоним = Метаданные.Документы.ЗагрузкаЗадолженности.ТабличныеЧасти.Объекты.Реквизиты.Найти(Имя).Синоним;
			Иначе 
...
Процедура ВыборСоответсвийСвязи(Элемент, Форма, ДокументОбъект, СписокРеквизитов)
	...
	//Иначе
	//	
	//	ПараметрыФормы.Вставить("СписокКолонок", ВозвратКоллекцииРеквизитовСправочника(ТекСтр.ТипДанных));
	//	ПараметрыФормы.Вставить("СписокПараметров", РасширитьСписокВыбора(СписокВыбора));
...
Процедура ВыборСоответствийПараметрыСвязиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Лев(Элемент.Родитель.ТекущиеДанные.ТипДанных, СтрДлина("Справочник ссылка:")) = "Справочник ссылка:" Тогда
		Если Элемент.Родитель.ТекущиеДанные.ТипДанных = "Справочник ссылка: Контрагенты" Тогда
			ВыбранноеЗначение = "КодКонтрагента=" + ВыбранноеЗначение + ";";
		Иначе
