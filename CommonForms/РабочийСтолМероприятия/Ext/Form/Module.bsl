...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.Авторизованный <> ЭтаФорма.РКЦ Тогда
	...
	КонецЕсли;
	// Отбор обещаний
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЭтаФорма.ОбещанияСегодня, "Ссылка.Автор", ЭтаФорма.Авторизованный, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЭтаФорма.ОбещанияМесяц, "Ссылка.Автор", ЭтаФорма.Авторизованный, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
	...
	Если День(ТекущаяДата()) = 12 Тогда
		ПоказатьОповещениеПользователя("Ротация портфеля",,"Осталось 3 дня до ротации",БиблиотекаКартинок.ДиалогИнформация,СтатусОповещенияПользователя.Важное);
	ИначеЕсли День(ТекущаяДата()) = 13 Тогда
		ПоказатьОповещениеПользователя("Ротация портфеля",,"Осталось 2 дня до ротации",БиблиотекаКартинок.ДиалогИнформация,СтатусОповещенияПользователя.Важное); 
	ИначеЕсли День(ТекущаяДата()) = 14 Тогда
		ПоказатьОповещениеПользователя("Ротация портфеля",,"Остался 1 день до ротации",БиблиотекаКартинок.ДиалогИнформация,СтатусОповещенияПользователя.Важное); 
	ИначеЕсли День(ТекущаяДата()) = 15 Тогда
		ПоказатьОповещениеПользователя("Ротация портфеля",,"Сегодня день ротации портфеля",БиблиотекаКартинок.ДиалогИнформация,СтатусОповещенияПользователя.Важное);
	...
	Если Формат(ТекущаяДата(), "ДЛФ=T") < "10:30:00" Тогда
	ПоказатьОповещениеПользователя("Обработка обещаний",,"Обработайте обещания до 11:00",БиблиотекаКартинок.ДиалогИнформация,СтатусОповещенияПользователя.Важное);
	КонецЕсли;
...
Процедура ПозвонитьДолжнику(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияСегодня И Элементы.ОбещанияСегодня.ТекущиеДанные <> Неопределено Тогда
		К = ПолучитьОбъектЗвонка(Элементы.ОбещанияСегодня.ТекущиеДанные.Объект);
		Если ТипЗнч(К) = Тип("СправочникСсылка.ДолговыеОбязательства") Тогда
			Контрагент = ПолучитьДолжника(К);
		Иначе
			Контрагент = К;
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ОбещанияМесяц И Элементы.ОбещанияМесяц.ТекущиеДанные <> Неопределено Тогда
		К = ПолучитьОбъектЗвонка(Элементы.ОбещанияМесяц.ТекущиеДанные.Объект);
		Если ТипЗнч(К) = Тип("СправочникСсылка.ДолговыеОбязательства") Тогда
			Контрагент = ПолучитьДолжника(К);
		Иначе
			Контрагент = К;
...
Процедура Задачи(Команда)
	ОткрытьФорму("РегистрСведений.УчетРабочегоВремени.ФормаСписка");
КонецПроцедуры
...
&НаКлиенте
Процедура ВходНаАТС(Команда)
	// Открыть БИТ.АТС
	ПоказатьОповещениеПользователя("БИТ.АТС",,"Приложение открывается...",БиблиотекаКартинок.бит_КартинкаБитфон_48);
	КомандаАТС = "https://10.10.0.20/reports";
    ЗапуститьПриложение(КомандаАТС);
КонецПроцедуры

&НаКлиенте
Процедура РасчетАВ(Команда)
	ФормаРасчетАВ = ПолучитьФорму("ОбщаяФорма.РасчетАВ");
...
Процедура ПосчитатьДолжников(Команда)
	...
	ПоказатьОповещениеПользователя("Подсчет должников",,"В портфеле "+ЭтаФорма.КоличествоВРаботе+" должника(-ов)",БиблиотекаКартинок.ДиалогИнформация);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьВЦ(Команда)
	// Открыть WhatsApp
	ПоказатьОповещениеПользователя("Интеграция с WhatsApp",,"Приложение открывается...",БиблиотекаКартинок.WA_DGFinance);
...
Процедура КонтрольСроков(Команда)
	// Передать с ключом
	ФормаКонтроль = ПолучитьФорму("ОбщаяФорма.КонтрольСроков",,,Новый УникальныйИдентификатор());
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ФормаКонтроль.СписокДО, "Ссылка.Ответственный", ЭтаФорма.Авторизованный, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
