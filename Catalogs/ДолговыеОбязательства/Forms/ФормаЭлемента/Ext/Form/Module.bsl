...
Процедура СформироватьОтчетВНБКИ(Команда)
	...
	// Создать мероприятие обмена
	СоздатьМероприятиеОбменаСНБКИ();
	ЭтаФорма.Элементы.ЖурналВыгрузокНБКИ.Обновить();
КонецПроцедуры

&НаСервере
Процедура СоздатьМероприятиеОбменаСНБКИ()
	...
	НовоеМероприятие.Результат = Справочники.РезультатыМероприятий.НайтиПоНаименованию("Обмен с НБКИ");
...
Процедура ОбновитьИПНаСервере()
	...
	ЗапросыФССП.ПолучитьИП(ЭтаФорма.Объект.Должник, ДатаРождения, ЭтаФорма.Объект.Должник.Код);
