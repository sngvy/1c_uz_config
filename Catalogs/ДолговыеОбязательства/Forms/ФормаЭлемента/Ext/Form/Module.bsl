...
Процедура ПриОткрытииНаСервере(Отказ)
	...
	КонецЕсли;
	Если ЭтаФорма.РольАБД Тогда
		Элементы.ГруппаКонтактностьНомеров.ТолькоПросмотр = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.ПредупреждениеАрхив = Истина Тогда
		...
		Если ЭтаФорма.РольРКЦ = Ложь И ЭтаФорма.РольАБД = Ложь Тогда
			Отказ = Истина;
		Иначе
			Ответ = Вопрос("Все равно открыть?",РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да,"Архивный договор");
