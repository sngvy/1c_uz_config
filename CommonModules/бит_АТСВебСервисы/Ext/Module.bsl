...
Функция ПолучитьВебСервисАТС(стрАдресАТС, стрВебСервис) ЭКСПОРТ
	...
	Определения = Новый WSОпределения(стрАдресАТС + "/" + стрВебСервис + "?wsdl", , , , 10, ссл);
	Сервис = Определения.Сервисы[0];
	вс = Новый WSПрокси(Определения, Сервис.URIПространстваИмен, Сервис.Имя, Сервис.ТочкиПодключения[0].Имя, , 5, ссл);
	Возврат вс;
