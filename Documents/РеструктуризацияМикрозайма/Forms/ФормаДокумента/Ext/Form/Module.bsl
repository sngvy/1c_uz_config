...
Процедура ЗаймПриИзмененииНаСервере()
	...
	ЗначениеОбъект.ЗаполнитьДанныеПоЗайму();
...
Функция ЗагрузитьИсториюПогашений(Займ, ДатаРеструктуризации)
	...
	Запрос.УстановитьПараметр("Период", ДатаРеструктуризации);
...
&НаСервере
Процедура СуммаОстатковПриИзмененииНаСервере()
	ЗначениеОбъект = РеквизитФормыВЗначение("Объект");
	ЗначениеОбъект.РассчитатьСуммуПрощения();
	ЗначениеВРеквизитФормы(ЗначениеОбъект, "Объект");		
КонецПроцедуры

&НаКлиенте
Процедура СуммаОстатковПриИзменении(Элемент)
	СуммаОстатковПриИзмененииНаСервере();	
КонецПроцедуры
...
//	Элементы.ГруппаОстаткиНовые.ТолькоПросмотр = НЕ (Объект.Прощение И Объект.ВидПрощения = Перечисления.ВидыПрощения.НаМоментРеструктуризации);
	Элементы.ВидПрощения.Видимость = Объект.Прощение;
	Элементы.СуммаПрощения.Видимость = Объект.Прощение;
	Элементы.ДатаПрощения.Видимость = Объект.Прощение;	
...
&НаСервере
Процедура УстановитьДатуПрощения()
	Если Объект.ВидПрощения = Перечисления.ВидыПрощения.ПриВыполненииУсловийРеструктуризации Тогда
		Объект.ДатаПрощения = Объект.ДатаПогашения;
	ИначеЕсли Объект.ВидПрощения = Перечисления.ВидыПрощения.НаМоментРеструктуризации Тогда
		Объект.ДатаПрощения = НачалоДня(Объект.ДатаРеструктуризации);
	Иначе
		Объект.ДатаПрощения = Дата(1,1,1,0,0,0);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПрощениеПриИзменении(Элемент)
	УправлениеФормой();
	УстановитьДатуПрощения();
КонецПроцедуры

&НаКлиенте
Процедура ВидПрощенияПриИзменении(Элемент)
	УправлениеФормой();
	УстановитьДатуПрощения();
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Объект.Ссылка.Пустая() Тогда
		Объект.ДатаРеструктуризации = ТекущаяДатаСеанса();
		Объект.ВидРеструктуризации = Перечисления.ВидыРеструктуризацииЗадолженности.Новация;
...
&НаКлиенте
Процедура ВидРеструктуризацииПриИзменении(Элемент)
...
Процедура ЗаполнитьПоПрограмме()
	Если ЗначениеЗаполнено(Объект.ПрограммаРасчета) Тогда
		...
		Если Объект.ВидПроцентнойСтавки = Перечисления.ВидыПроцентнойСтавки.ПлавающаяПроцентнаяСтавкаЗависимаяОтСрока Тогда
			...
			РасчетЗадолженностиМФО.ЗаполнитьПериодыПоИнтервалам(Объект, Объект.ИнтервалыПроцентнойСтавки);
...
Процедура ПересчетСуммы(Элемент)
	Объект.ОстаткиСумма = Объект.ОстаткиОсновнойДолг + Объект.ОстаткиПросроченныеПроценты + Объект.ОстаткиПроценты + Объект.ОстаткиШтрафы + Объект.ОстаткиПени;	
КонецПроцедуры
