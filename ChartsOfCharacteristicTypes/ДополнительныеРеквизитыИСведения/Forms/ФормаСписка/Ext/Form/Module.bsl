...
Процедура КомандаДобавитьВсеПредопределенныеНаСервере()
	
	Создатель = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения;
	Логер = БЛогер_Логирование.Создать();
	
	Родитель = Создатель.ДополнительныеРеквизиты_Контрагенты;
	Попытка
	
		Создатель.ИнициализацияПредопределенныхЭлементов(Логер, Родитель);
	
	Исключение
		Ошибка = УправленияСообщениями.СформироватьСтекОшибки(
			ИнформацияОбОшибке(),
			"Нет возможности создать предопределенные реквизиты"
		);
		Залогировать(Логер, Ошибка);
	КонецПопытки;
	
	БЛогер_Логирование.Вывести(Логер);
...
&НаКлиенте
Процедура КомандаДобавитьВсеПредопределенные(Команда)

	ВывестиСообщение("Добавление предопределенных реквизитов:");
	КомандаДобавитьВсеПредопределенныеНаСервере();
	ВывестиСообщение("Конец операции")
