...
Процедура УправлениеФормой()
	...
	КонецЕсли;
	Элементы.ДатаОтмены.Видимость = Объект.Отменено;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РанееНачисленнаяГоспошлина = ГоспошлинаКНачислениюУОснования(
		Объект.ОснованиеДоначисления
	);
...
Процедура РассчитатьГоспошлину(Команда)
	...
	Если НЕ ЗначениеЗаполнено(объект.ДатаИска) Тогда	
		Сообщение("Поле Дата Иска не заполнено! Госпошлина не рассчитана! Заполните дату иска!");
		Возврат;
	КонецЕсли;
...
Процедура РассчитатьГоспошлинуНаСервере()
	...
	ЗначениеВДанныеФормы(ЗначениеОбъект, Объект);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ГоспошлинаКНачислениюУОснования(Знач ОснованиеДоначисления)
	
	Если Не ЗначениеЗаполнено(ОснованиеДоначисления) Тогда
	
		Возврат 0;
	
	КонецЕсли;
	
	Возврат ОснованиеДоначисления.ГоспошлинаКНачислению;
...
Процедура РешениеПриИзменении(Элемент)
	...
	Если Объект.Ответчики.Количество() = 0 Тогда
		...
		Сообщение.Текст = "Не указан ни один ответчик";
		Сообщение.Сообщить();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ДляСудебногоДелаСозданИД" И ЭтаФорма = Источник.ВладелецФормы Тогда
	
		Элементы.ИД.ТекущиеДанные.ИсполнительныйДокумент = Параметр;
...
Процедура ОтмененоПриИзменении(Элемент)
	Если Объект.Отменено Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Занесите дату отмены и распроведите документ!";
		Сообщение.Сообщить();
	КонецЕсли;
	УправлениеФормой();
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаполнитьРОСП(Команда)
	
	Займ = Объект.Займ;
	Если НЕ ЗначениеЗаполнено(Займ) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Предварительно необходимо заполнить поле ""Займ"""; 
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	
	Свойство = ПолучитьЗначениеКонстанты();
	Если Не ЗначениеЗАполнено(Свойство) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не установлена константа ""Адрес должника подсудности РОСП"". Заполнение РОСП невозможно без заполненного адреса должника в формате ФИАС"; 
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли; 
	
	СвойствоВидСтроки = Неопределено;
	АдресДолжника = ПолучитьАдрес(Займ, Свойство, СвойствоВидСтроки);

	Если НЕ ЗначениеЗаполнено(АдресДолжника) ИЛИ
		 НЕ СвойствоВидСтроки = ПредопределенноеЗначение("Перечисление.ВидыТипаСтрока.АдресФИАС") Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Заполнение РОСП возможно только для адреса в формате ФИАС. Проверьте адресные данные должника"; 
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли; 
	...
	ИнформацияОРОСП = "";
	ПодборРОСПНаСервере(СтрокаJSON, ИнформацияОРОСП);
	
	Если ЗначениеЗаполнено(ИнформацияОРОСП) Тогда
	    Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ИнформацияОРОСП;
		Сообщение.Сообщить(); 
	КонецЕсли;
	
КонецПроцедуры //ЗаполнитьРОСП()

&НаСервере
Функция ПолучитьАдрес(Займ, Свойство, СвойствоВидСтроки)
	
	СвойствоВидСтроки = Свойство.ВидСтроки;
	Возврат ОбъектыСервер.ПолучитьЗначениеСвойства(Займ.Должник, Свойство.Код);
	
КонецФункции //ПолучитьАдрес()

&НаСервере
Процедура ПодборРОСПНаСервере(СтрокаJSON, ИнформацияОРОСП)
	
	Объект.РОСП = Неопределено;
	Объект.УФССП = Неопределено;
	Объект.СписокРОСП.Очистить();
	
	ТаблицаРОСП = Новый ТаблицаЗначений;
	Документы.СудебноеДело.ЗаполнитьРОСП(ТаблицаРОСП, СтрокаJSON, ИнформацияОРОСП);
	
	Если ТаблицаРОСП.Количество() = 0 тогда
		Возврат;
	КонецЕсли;
	
	Объект.СписокРОСП.Загрузить(ТаблицаРОСП);
	
	Если ТаблицаРОСП.Количество() = 1 Тогда
		Для Каждого стрРОСП из ТаблицаРОСП Цикл
			Объект.РОСП = стрРОСП.СсылкаССП;
			Объект.УФССП = стрРОСП.УФССП;
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры //ПодборРОСПНаСервере()

&НаКлиенте
Процедура СписокРОСПВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ТекущаяСтрока = Элемент.ТекущиеДанные;
	Объект.РОСП = ТекущаяСтрока.СсылкаССП;
	Объект.УФССП = ТекущаяСтрока.УФССП;
	
КонецПроцедуры //СписокРОСПВыбор()

Функция ПолучитьЗначениеКонстанты()
	Возврат Константы.АдресДолжникаПодсудностиРОСП.Получить();
...
&НаСервере
Процедура ЗаполнениеКодаДляКИ()
			
	Если Объект.Решение = Перечисления.СтатусыСудебногоДела.Удовлетворено Тогда
		Объект.РешениеКодРешенияДляКИ = 2;
	ИначеЕсли Объект.Решение = Перечисления.СтатусыСудебногоДела.УдовлетвореноЧастично Тогда
		Объект.РешениеКодРешенияДляКИ = 3;
	ИначеЕсли
		Объект.Решение = Перечисления.СтатусыСудебногоДела.Отказ Тогда
		Объект.РешениеКодРешенияДляКИ = 3;
	Иначе
		Объект.РешениеКодРешенияДляКИ = 99;	
	КонецЕсли;
