...
Функция ПроверитьМассивДО(Массив,Настройка)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗадолженностьПоОбъектам.Остатки(, ВЫРАЗИТЬ(Объект КАК Справочник.ДолговыеОбязательства) В (&МассивОбъектов)) КАК ЗадолженностьПоОбъектамОстатки
...
Процедура ПерезаписатьДатуПередачиФайл(ПолеИсходнойТаблицы,Настройка,ТекстФайлаОшибок) Экспорт
	Для НомерСтроки = 2 По ПолеИсходнойТаблицы.ВысотаТаблицы Цикл
		ОбъектУчетаЗнач = ПолеИсходнойТаблицы.Область(НомерСтроки, ЭтотОбъект.ОбъектУчета).Текст;
		ДатаПередачиЗнач = ПолеИсходнойТаблицы.Область(НомерСтроки, ЭтотОбъект.ДатаПоследнейУспешнойПередачиНБКИ).Текст;
