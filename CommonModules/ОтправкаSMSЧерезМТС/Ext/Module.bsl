...
Функция СтатусДоставки(ИдентификаторСообщения, НастройкиОтправкиSMS) Экспорт
	...
	РезультатЗапроса = ВыполнитьЗапрос2(АдресСервераКоммуникатор(), HTTPЗапрос);
...
#Область СлужебныеПроцедурыИФункцииУЗ

// ИЗ БСП 3.1.7
//Переход на сервис Маркетолог

Функция ВыполнитьЗапрос2(Знач АдресСервера, Знач HTTPЗапрос, Знач Пользователь = Неопределено, Знач Пароль = Неопределено)
...
Функция ОтправитьSMSМаркетолог(Знач Получатели, Знач Текст, Знач ИмяОтправителя, Знач Логин, Знач Пароль)
	...
	РезультатЗапроса = ВыполнитьЗапрос2(АдресСервераМаркетолог(), HTTPЗапрос, Логин, Пароль);
	Результат = Новый Структура("ОтправленныеСообщения,ОписаниеОшибки", Новый Массив, "");
...
Функция СтатусДоставкиМаркетолог(ИдентификаторСообщения, НастройкиОтправкиSMS)
	...
	РезультатЗапроса = ВыполнитьЗапрос2(АдресСервераМаркетолог(), HTTPЗапрос, НастройкиОтправкиSMS.Логин, НастройкиОтправкиSMS.Пароль);	
	Если Не РезультатЗапроса.ЗапросВыполнен Тогда
		Возврат Результат;
	КонецЕсли;
