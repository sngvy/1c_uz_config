...
Процедура ФормированиеСписковАвтообзвона() Экспорт
	...
	|	И ДолговыеОбязательства.Кредитор.Код <> ""F00123134""
	|
	|СГРУППИРОВАТЬ ПО
	...
	// Займ-Экспресс (дальние регионы)
	
	СписокАвтообзвона = Справочники.СписокДляАвтообзвона.СоздатьЭлемент();
	СписокАвтообзвона.Наименование = "Займ-Экспресс (дальние регионы) " + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy");
	...
	// Займ-Экспресс (основные регионы)
	
	СписокАвтообзвона = Справочники.СписокДляАвтообзвона.СоздатьЭлемент();
	СписокАвтообзвона.Наименование = "Займ-Экспресс (основные регионы) " + Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy");
...
Процедура ОбработкаНеконтактныхДелДляАвтообзвона2(Знач Запрос)
	...
	Пока Выборка.Следующий() Цикл
		Договор = Выборка.Ссылка.ПолучитьОбъект();
		ВРаботе = Справочники.тсЗначенияСвойствОбъектов.НайтиПоНаименованию("В работе");
		Новые = Справочники.тсЗначенияСвойствОбъектов.НайтиПоНаименованию("Новые");
		Категория = ОбъектыСервер.ПолучитьЗначениеСвойства(Договор, "0181");
		Если Категория = ВРаботе Или Категория = Новые Тогда
			// Разблокировать ДО для автообзвона по причине неконтактности
