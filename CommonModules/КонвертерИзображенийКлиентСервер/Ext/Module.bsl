...
//     Ответ - Произвольный - Возвращаемое значение, при Результат = Истина
...
Функция Обработать(Параметры) Экспорт
	...
	Если Не ИсполняемыйФайл.Существует()
		...
		ВызватьИсключение УправленияСообщениямиКлиентСервер.ИнструкцияКОшибке(
			"Отсутствует файл для конвертации документов",
			СтрШаблон(
				"Проверяемый файл - %1
	...
	Собиратель.ДобавитьКоманду(Инструкция, ИсполняемыйФайл.ПолноеИмя)
		...
		.ДобавитьПараметр(Инструкция, Страницы)
...
Функция ПодготовитьПараметры(Каталоги, НастройкиПарсинга) Экспорт
	...
	КаталогиКонвертера = ОбщийВедатель.КаталогиДляКонвертации(Каталоги);
	Описание.Вставить("Каталоги", КаталогиКонвертера);
	
	ПоследняяСтраница = Настройщик.ПолучитьПоследнююСтраницу(НастройкиПарсинга);
	ОграничениеСтраниц = ОбщийВедатель.СоставитьОграничениеСтраниц(ПоследняяСтраница);
	Описание.Вставить("Страницы", ОграничениеСтраниц);
	...
	ИмяФайла = ОбщийВедатель.ИмяФайлаКонвертера();
