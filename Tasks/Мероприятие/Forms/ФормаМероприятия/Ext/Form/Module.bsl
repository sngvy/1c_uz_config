...
Процедура ОбъектПростойПриИзменении(Элемент)
	...
	Попытка
		...
		ПолучитьДатуВыдачиНаСервере();
		ПолучитьОтветственногоНаСервере();
		ПоказатьОповещениеПользователя("Предупреждение",,"Номер договора был изменен. Пожалуйста, проверьте контакт должника",БиблиотекаКартинок.Предупреждение32);
	Исключение
	КонецПопытки
КонецПроцедуры
