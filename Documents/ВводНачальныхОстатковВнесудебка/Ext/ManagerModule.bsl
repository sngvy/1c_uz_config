...
Процедура ИнициализироватьДанныеДокумента(ЭтотОбъект, ТаблицаДвиженийОбъекты, ТаблицаДвиженийСобытия) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.НомерСтроки,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Объект,
	               |	ВЫБОР
	               |		КОГДА ВводНачальныхОстатковВнесудебкаОбъекты.ВалютаЗагрузки = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	               |			ТОГДА &ВалютаРегл
	               |		ИНАЧЕ ВводНачальныхОстатковВнесудебкаОбъекты.ВалютаЗагрузки
	               |	КОНЕЦ КАК ВалютаЗагрузки,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.ОсновнойДолг,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Проценты,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Штрафы,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Пени,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая1,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая2,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая3,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая4,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая5,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая6,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая7,
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Составляющая8
	               |ПОМЕСТИТЬ Объекты
	               |ИЗ
	               |	Документ.ВводНачальныхОстатковВнесудебка.Объекты КАК ВводНачальныхОстатковВнесудебкаОбъекты
	               |ГДЕ
	               |	ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка = &Ссылка
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.СуммаДООстаток, 0) КАК СуммаДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.ОсновнойДолг + Итог.Проценты + Итог.Штрафы + Итог.Пени + Итог.Составляющая1 + Итог.Составляющая2 + Итог.Составляющая3 + Итог.Составляющая4 + Итог.Составляющая5 + Итог.Составляющая6 + Итог.Составляющая7 + Итог.Составляющая8
	               |		ИНАЧЕ (Итог.ОсновнойДолг + Итог.Проценты + Итог.Штрафы + Итог.Пени + Итог.Составляющая1 + Итог.Составляющая2 + Итог.Составляющая3 + Итог.Составляющая4 + Итог.Составляющая5 + Итог.Составляющая6 + Итог.Составляющая7 + Итог.Составляющая8) * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.СуммаРеглОстаток, 0) КАК СуммаРегл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ОсновнойДолгДООстаток, 0) КАК ОсновнойДолгДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.ОсновнойДолг
	               |		ИНАЧЕ Итог.ОсновнойДолг * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ОсновнойДолгРеглОстаток, 0) КАК ОсновнойДолгРегл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               |			ТОГДА Итог.Проценты
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ПроцентыДООстаток, 0) КАК ПроцентыДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Проценты
	               |		ИНАЧЕ Итог.Проценты * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ПроцентыРеглОстаток, 0) КАК ПроцентыРегл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               |			ТОГДА Итог.Штрафы
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ШтрафыДООстаток, 0) КАК ШтрафыДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Штрафы
	               |		ИНАЧЕ Итог.Штрафы * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ШтрафыРеглОстаток, 0) КАК ШтрафыРегл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               |			ТОГДА Итог.Пени
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ПениДООстаток, 0) КАК ПениДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Пени
	               |		ИНАЧЕ Итог.Пени * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.ПениРеглОстаток, 0) КАК ПениРегл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая1ДООстаток, 0) КАК Составляющая1ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая1
	               |		ИНАЧЕ Итог.Составляющая1 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая1РеглОстаток, 0) КАК Составляющая1Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая2ДООстаток, 0) КАК Составляющая2ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая2
	               |		ИНАЧЕ Итог.Составляющая2 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая2РеглОстаток, 0) КАК Составляющая2Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая3ДООстаток, 0) КАК Составляющая3ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая3
	               |		ИНАЧЕ Итог.Составляющая3 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая3РеглОстаток, 0) КАК Составляющая3Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая4ДООстаток, 0) КАК Составляющая4ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая4
	               |		ИНАЧЕ Итог.Составляющая4 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая4РеглОстаток, 0) КАК Составляющая4Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая5ДООстаток, 0) КАК Составляющая5ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая5
	               |		ИНАЧЕ Итог.Составляющая5 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая5РеглОстаток, 0) КАК Составляющая5Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая6ДООстаток, 0) КАК Составляющая6ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая6
	               |		ИНАЧЕ Итог.Составляющая6 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая6РеглОстаток, 0) КАК Составляющая6Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая7ДООстаток, 0) КАК Составляющая7ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая7
	               |		ИНАЧЕ Итог.Составляющая7 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая7РеглОстаток, 0) КАК Составляющая7Регл,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = Итог.ВалютаДоговора
	               ...
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая8ДООстаток, 0) КАК Составляющая8ДО,
	               |	ВЫБОР
	               |		КОГДА Итог.ВалютаЗагрузки = &ВалютаРегл
	               |			ТОГДА Итог.Составляющая8
	               |		ИНАЧЕ Итог.Составляющая8 * Итог.КурсЗАГ
	               |	КОНЕЦ - ЕСТЬNULL(ЗадолженностьПоОбъектамВнесудебнаяОстатки.Составляющая8РеглОстаток, 0) КАК Составляющая8Регл,
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗадолженностьПоОбъектамВнесудебная.Остатки(&МоментДо) КАК ЗадолженностьПоОбъектамВнесудебнаяОстатки
	               |		ПО Итог.Объект = ЗадолженностьПоОбъектамВнесудебнаяОстатки.Объект
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ Итог
...
Процедура ПровестиПоследовательность(ЭтотОбъект) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка КАК Ссылка
	                      |ИЗ
	                      |	ТаблицаОб КАК ТаблицаОб
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатковВнесудебка.Объекты КАК ВводНачальныхОстатковВнесудебкаОбъекты
	                      |		ПО ТаблицаОб.Объект = ВводНачальныхОстатковВнесудебкаОбъекты.Объект
	                      |			И (ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка.МоментВремени > &МоментВремени)
	                      |			И (ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка.Проведен)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Объект,
	                      |	ВводНачальныхОстатковВнесудебкаОбъекты.Ссылка.МоментВремени");
	...
	ТаблицаАЗ.Колонки.Добавить("Ссылка", Новый ОписаниеТипов("ДокументСсылка.ВводНачальныхОстатковВнесудебка"));
	...
	Пока Результат.Следующий() Цикл
		...
		Документы.ВводНачальныхОстатковВнесудебка.ИнициализироватьДанныеДокумента(Результат.Ссылка, ТаблицаДвиженийОбъекты, 
				ТаблицаДвиженийСобытия);
		
		Набор = РегистрыНакопления.ЗадолженностьПоОбъектамВнесудебная.СоздатьНаборЗаписей();
