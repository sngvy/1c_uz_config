...
Процедура ИсходящийЗвонокТЧ(Команда)
	...
	Если ВидТелефонии = ПредопределенноеЗначение("Перечисление.ВидыТелефонии.БИТФон") Тогда
		...
		ИначеЕсли Ф.Открыта() = Истина И Ф.СтатусПодключен = Истина Тогда
			...
			ПоказатьОповещениеПользователя("Исходящий звонок",,"Начат вызов на выбранный номер...",БиблиотекаКартинок.бит_КартинкаБитфон_48);
...
Процедура НайтиВВЦ(Команда)
	...
	ПоказатьОповещениеПользователя("Интеграция с WhatsApp",,"Выполняется поиск номера...",БиблиотекаКартинок.WA_DGFinance);
	ФормаВЦ = ПолучитьФорму("ОбщаяФорма.РассылкаНаWhatsApp");
	ФормаВЦ.Номер = СтрСоединить(СтрРазделить(Элементы.ДолжникТелефоны.ТекущиеДанные.Номер, "+( )"), "");
	ФормаВЦ.Договор = ЭтаФорма.Объект.Ссылка;
...
Процедура РозыскДолжника(Команда)
	ПоказатьОповещениеПользователя("Розыск должника",,"Выполняется поиск номера...",БиблиотекаКартинок.ДиалогИнформация);
