...
Процедура ПередЗаписью(Отказ)
	...
	Если ВалютаДоговора.Пустая() Тогда
		...
		// Большое количество сообщений замедляет загрузку реестров
...
Функция КонтрольВозможностиИзмененияВалютыДО(Сообщать) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
				   |	РегистрНакопления.ЗадолженностьПоДолговомуОбязательству.Остатки(&ТекущаяДата) КАК АктуальнаяЗадолженностьОстатки
				   |ГДЕ
	...
	Если Сообщать Тогда
		...
		Сообщить("" + Ссылка
				+ ": невозможно изменение валюты ДО, т.к. по данному обязательству присутствует остаток (в валюте РУ): "
				+ Выборка.СуммаРеглОстаток,
