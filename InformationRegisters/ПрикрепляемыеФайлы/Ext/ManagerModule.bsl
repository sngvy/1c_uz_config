...
Функция СсылкаФайлПоРегистратору(Регистратор, НомерСтроки) Экспорт
	...
	Запрос.Текст = 
		...
		|	ПрикрепляемыеФайлы.Регистратор = &Регистратор
		|	И ПрикрепляемыеФайлы.НомерСтроки_ = &НомерСтроки_
		|	И ПрикрепляемыеФайлы.ЭлементФайлы ССЫЛКА Справочник.Файлы";
...
Функция ДанныеФайла(ВыбраннаяСтрока) Экспорт
	...
	// Лебедева 22052024 - убрала, так как мешает работе открытия файлов после смены типа хранения - в базе или в каталогое
...
Функция СохраненныеФайлы(Основание) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПрикрепляемыеФайлыСрезПоследних.ТипФайла КАК Тип,
	|	ПрикрепляемыеФайлыСрезПоследних.ИмяФайла КАК Имя,
	|	ПрикрепляемыеФайлыСрезПоследних.ЭлементФайлы КАК Идентификатор,
	|	ПрикрепляемыеФайлыСрезПоследних.Регистратор КАК СсылкаПрикрепления
	|ИЗ
	|	РегистрСведений.ПрикрепляемыеФайлы.СрезПоследних(
	|			,
	|			Объект = &Основание
	|				И ТИПЗНАЧЕНИЯ(ЭлементФайлы) = &ТипЭлементФайлы) КАК ПрикрепляемыеФайлыСрезПоследних";
	
	Запрос.УстановитьПараметр("Основание", Основание);
	Запрос.УстановитьПараметр("ТипЭлементФайлы", Тип("Строка"));
...
Функция СоздатьВременныйФайл(ВыделеннаяСтрока)
	...
	Если ФайлыНаСервере Тогда
		...
		ИмяСохраненногоФайла = Каталог + ДанныеФайла["УИД"] + ФайлПоИмени.Расширение;
