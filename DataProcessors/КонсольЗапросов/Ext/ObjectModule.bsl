...
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, СпособВыгрузки, СоставРезультатов) Экспорт
	...
	КонецЕсли;
	Попытка
		НачВыполненияПакета_ID = КоличествоМиллисекунд();
		МассивРезультатов = Запрос.ВыполнитьПакет();
		Сообщить(Строка((КоличествоМиллисекунд() - НачВыполненияПакета_ID) / 1000) + " с");
	Исключение
