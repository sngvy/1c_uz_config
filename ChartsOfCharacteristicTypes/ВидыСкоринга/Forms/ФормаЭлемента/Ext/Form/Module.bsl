...
Процедура ИзменитьЗапрос(Команда)
	...
	//	Объект.ЗапросМакета = Форма.ТекстЗапроса;
	//КонецЕсли;
	//
	//ИзмСКДСервер();
...
Процедура ОткрытиеСервер()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		//Элементы.ТабличноеПоле1.ОбновитьСтроки();
	КонецЕсли;
	Источник = Новый ИсточникДоступныхНастроекКомпоновкиДанных(СКД);
	Компоновщик.Инициализировать(Источник);	
КонецПроцедуры
...
Процедура СоздатьПроизвольнойПроцедурой(Команда)
	...
	Иначе
		...
		//	Модифицированность = Истина;
		//	Объект.ЗапросМакета = Форма.ТекстовыйДокумент.ПолучитьТекст();
		//	Объект.Объекты.Очистить();
		//	Для Каждого Элемент Из Форма.ВходОбъект Цикл
		//		Объект.Объекты.Добавить().Объект = ПолучитьКодЭлемента(Элемент.Объект);
		//	КонецЦикла;
		//КонецЕсли;
	КонецЕсли;
КонецПроцедуры
