...
Процедура РассчитатьГоспошлинуНаСервере()
	...
	СуммаДляРасчета = ЗначениеОбъект.ПолучитьСуммуДляРасчетаГоспошлины();
	Госпошлина = Документы.СудебноеДело.РасчитатьСуммуГоспошлины(
		СуммаДляРасчета,
		Объект.ВидТребований
	);
	
	Объект.ИскГоспошлина = Госпошлина;
...
Процедура РешениеПриИзменении(Элемент)
	...
	Если Объект.Ответчики.Количество() = 0 Тогда
		...
		Сообщение.Текст = "Не указан не один ответчик";
		Сообщение.Сообщить();
