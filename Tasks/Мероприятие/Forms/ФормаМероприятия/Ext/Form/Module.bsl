...
Процедура ПриОткрытииНаСервере()
	...
	ИначеЕсли ТипЗнч(ЭтаФорма.Объект.Объект) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если ЗначениеЗаполнено(ЭтаФорма.Объект.СвязанныйДоговор) = Истина Тогда
...
Процедура ВыполнитьИЗакрыть(Команда)
	...
	Если ЭтаФорма.Объект.Результат = ЭтаФорма.РезультатЗвонокКредитору И Не ЗначениеЗаполнено(ЭтаФорма.Объект.СвязанныйДоговор) Тогда
		Предупреждение("Не заполнен связанный договор!",,"Контроль заполнения");
		Возврат;
	КонецЕсли;
	Отказ = Ложь;
...
Процедура ИсполнительныеПроизводства(Команда)
	ФормаАдаптера = ПолучитьФорму("ОбщаяФорма.АдаптерФССП");
	ФормаАдаптера.Адрес = ОбъектыСервер.ПолучитьЗначениеСвойства(ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"), "0049");
	ФормаАдаптера.Должник = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник");
	ФормаАдаптера.Код = ПолучитьЗначениеРеквизитаНаСервере(ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"), "Код");
	ФормаАдаптера.ДатаРождения = ОбъектыСервер.ПолучитьЗначениеСвойства(ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"), "0043");
	ФормаАдаптера.ОткрытьМодально();
