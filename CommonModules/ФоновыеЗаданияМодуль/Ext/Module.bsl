...
Процедура ВыполнитьЗагрузкуРеестров(Организация, Автор, НастройкаСоответствий, КаталогФайловДляОбработки, КаталогПеремещенныхФайлов, КаталогОшибочныхФайлов, ОбновлятьРанееЗаписанные, Маска, ТД) Экспорт
	...
	Для Каждого Файл из НайденныеФайлы Цикл
		...
		Попытка
			...
			ПолеИсходнойТаблицы = УправлениеЗагрузкойСервер.ПрочитатьФайлВТабличныйДокумент(ДанныеФайла);
			
		Исключение
			Прочитано = Ложь;
			Сообщение = Сообщение + "Не удалось прочитать файл: " + Файл.ПолноеИмя + Символы.ПС;
			Сообщение = Сообщение + ОписаниеОшибки();
		...
		Попытка
			ПереместитьФайл(Файл.ПолноеИмя, ПапкаПеремещения + ПолучитьРазделительПути() + Файл.Имя);
		Исключение
			ТекстСообщения = "Не удалось переместить файл: " + Файл.ПолноеИмя;
			Сообщение = Сообщение + ТекстСообщения + Символы.ПС;
			ТД.ДобавитьСтроку(ТекстСообщения);
		КонецПопытки;
...
Процедура ФормированиеСписковАвтообзвона() Экспорт
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	РезультатАнализируетсяБлокировщиком УБЫВ";
...
Процедура ОбновитьИП() Экспорт
	...
	|	ЕСТЬNULL(ИсполнительныеПроизводства.ДатаПроверки, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПроверки
	|ИЗ
	|	Справочник.ДолговыеОбязательства КАК ДолговыеОбязательства
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗапросыФССП.ПолучитьИП(Выборка.Должник, ДатаРождения, Выборка.ДолжникКод);
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбновитьДелаОБанкротстве() Экспорт
	...
	|ГДЕ
	|	ДолговыеОбязательства.ДополнительныеРеквизиты.Значение.Наименование В (""В работе"", ""Новые"", ""Ручной обзвон"")";
	РезультатЗапроса = Запрос.Выполнить();	
	Выборка = РезультатЗапроса.Выбрать();	
	Пока Выборка.Следующий() Цикл
		ЗапросыЕФРСБ.ПолучитьСведения(Выборка.Должник.ФИО[0].ФИО, Выборка.ДолжникКод);
	КонецЦикла;
КонецПроцедуры

Процедура ОстановитьАктивныеЗадачиАвтообзвона() Экспорт
	МассивЗадач = Новый Массив;
	МассивЗадач.Добавить("Новые и необработанные - дальние регионы");
	МассивЗадач.Добавить("Новые и необработанные - основные регионы");
	МассивЗадач.Добавить("Основной список - дальние регионы");
	МассивЗадач.Добавить("Основной список - основные регионы");
	...
	Для Каждого Стр Из МассивЗадач Цикл
		...
		ЗаписиSQL = Новый ComObject("ADODB.RecordSet"); 
		ЗаписиSQL.ActiveConnection = Соединение;
		ЗапросСостояние = "SELECT `id` from `autodialer` WHERE `name` = "+""""+Стр+""""+"";
		ЗаписиSQL.Open(ЗапросСостояние, Соединение);
		ИДЗадачи = ЗаписиSQL.Fields.Item(0).Value;
		Соединение.Close();
		...
		ЗаписиSQL = Новый ComObject("ADODB.RecordSet"); 
		ЗаписиSQL.ActiveConnection = Соединение;
		ЗапросСостояние = "SELECT `active` from `autodialer` WHERE `id` = "+""""+ИДЗадачи+""""+"";
		ЗаписиSQL.Open(ЗапросСостояние, Соединение);
		Состояние = ЗаписиSQL.Fields.Item(0).Value;
		Соединение.Close();
		
		Если Состояние = 1 Тогда
			ОстановитьЗадачу(ИДЗадачи);
		КонецЕсли;
		
	КонецЦикла;
КонецПроцедуры

Функция ОстановитьЗадачу(ИДЗадачи)	
	// Логин
