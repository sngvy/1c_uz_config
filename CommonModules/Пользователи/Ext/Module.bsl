...
Функция ЭтоПолноправныйПользовательИБ(Пользователь = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Пользователь) И Пользователь <> АвторизованныйПользователь() Тогда
		ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(УдалитьОбщегоНазначения.ПолучитьЗначениеРеквизита(Пользователь, "ИдентификаторПользователяИБ"));
	...
	Если ПользовательИБ.УникальныйИдентификатор = ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор Тогда
		...
		Если ЗначениеЗаполнено(ПользовательИБ.Имя) Тогда
			...
			Возврат РольДоступна("тсАдминистрирование");
		Иначе
			...
			Если Метаданные.ОсновныеРоли.Количество() = 0 ИЛИ
				 Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.тсАдминистрирование) ИЛИ  Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		...
		КонецЕсли;
	Иначе
		Возврат ПользовательИБ.Роли.Содержит(Метаданные.Роли.тсАдминистрирование);
	КонецЕсли;
...
//{{MRG[ <-> ]
	...
	КонецЕсли;
	Если ПользовательИБ = Неопределено Тогда
		ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоИмени(ПользовательСтруктура.Имя);
//}}MRG[ <-> ]
//{{MRG[ <-> ]
...
//{{MRG[ <-> ]
	ПользовательИБ.Имя						= ПользовательСтруктура["Имя"]; //имеется в виду "КраткоеИмя"
...
//}}MRG[ <-> ]
	Если СписокРолей <> Неопределено Тогда
		Для Каждого СтрокаТаблицы Из СписокРолей Цикл
			Пометка = СтрокаТаблицы.Пометка;
			Роль = Метаданные.Роли.Найти(СтрокаТаблицы.ИмяРоли);
			Если ПользовательИБ.Роли.Содержит(Роль) И НЕ Пометка Тогда
				ПользовательИБ.Роли.Удалить(Роль);
			ИначеЕсли НЕ ПользовательИБ.Роли.Содержит(Роль) И Пометка Тогда
...
Процедура ОпределитьТекущегоПользователя(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ИмяПараметра = "ТекущееПодразделение" Тогда
		ИмяПараметра = "ТекущийПользователь";
	...
	ИначеЕсли ПользовательНеНайден Тогда
		...
		ТекстСообщенияОбОшибке = УдалитьСтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщенияОбОшибке, ИмяПользователя);
		ВызватьИсключение ТекстСообщенияОбОшибке;
	КонецЕсли;
	
	УстановленныеПараметры.Добавить(ИмяПараметра);	
	
	ПараметрыСеанса.ТекущееПодразделение = ПараметрыСеанса.ТекущийПользователь.Подразделение;
КонецПроцедуры
...
Процедура ОбновитьСоставГруппПользователей(Знач ГруппаПользователей) Экспорт
	...
	Пока НЕ ГруппаПользователей.Пустая() Цикл
		...
		ГруппаПользователей = УдалитьОбщегоНазначения.ПолучитьЗначениеРеквизита(ГруппаПользователей, "Родитель");
	КонецЦикла;
...
Процедура ЗаполнитьИдентификаторыПользователей() Экспорт
	...
	Пока СписокПользователей.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Пользователь.ИдентификаторПользователяИБ)
			И СокрЛП(Пользователь.Наименование) <> "<Не указан>" Тогда
...
//!
Процедура ЗаполнитьРеквизитыПоДаннымПользователя(Объект) Экспорт
