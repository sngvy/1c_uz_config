...
Процедура СоздатьНаСервере(ДОСсылка) Экспорт
	...
	//Договор.ДатаРасчета = ТекущаяДата();
...
Процедура СоздатьДоговорМикрозаймаПриЗаписи(Источник, Отказ) Экспорт
	Если РасчетЗадолженностиМФО.ПолучитьТекущийДоговор(Источник.Ссылка, ТекущаяДата()) = Неопределено Тогда
		СоздатьНаСервере(Источник.Ссылка);
		РасчетЗадолженностиМФО.РассчитатьГрафикПоЗайму(Источник.Ссылка,ТекущаяДата());
	КонецЕсли;
КонецПроцедуры
