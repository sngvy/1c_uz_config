...
&НаСервере
Процедура ОграничитьТипОбъекта(Элемент, ВключитьВДоступные = Неопределено) Экспорт

	ДоступныеТипы = Новый Массив;
	Если ТипЗнч(ВключитьВДоступные) = Тип("Массив") Тогда
	
		ДоступныеТипы = ВключитьВДоступные;
	
	КонецЕсли;
	
	НастройкиДетализации = ПравилаДетализации();
	
	Для каждого НастройкаДетализации Из НастройкиДетализации Цикл
	
		Если НастройкаДетализации.Значение Тогда
		
			ДоступныеТипы.Добавить(Тип(НастройкаДетализации.Ключ));
...
// Расширяется в ЖКХ
&НаСервере
Функция ПравилаДетализации()

	Набор = Новый Соответствие;
	
	Набор.Вставить("СправочникСсылка.Контрагенты", Константы.ДетализацияПоКА.Получить());
	Набор.Вставить("СправочникСсылка.ДоговорыКонтрагентов", Константы.ДетализацияПоДоговорам.Получить());
	Набор.Вставить("СправочникСсылка.УслугиПоДоговору", Константы.ДетализацияПоУслугам.Получить());
	Набор.Вставить("СправочникСсылка.ДолговыеОбязательства", Константы.ДетализацияПоДО.Получить());
	Набор.Вставить("СправочникСсылка.ИсполнительныеДокументы", Константы.ДетализацияПоИД.Получить());
	Набор.Вставить("СправочникСсылка.Залоги", Константы.ДетализацияПоЗ.Получить());
	Набор.Вставить("СправочникСсылка.ФССП_ИсполнительноеПроизводство", Истина);
	Набор.Вставить("ДокументСсылка.СудебноеДело", Истина);
...
Процедура ЗаписатьЗначениеСвойства(Объект, КодСвойства, Значение) Экспорт
	...
	Если СсылкаДопРС.Пустая() Тогда
		...
		Сообщение.Текст = "Такого реквизита или сведения не существует!";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	Если СсылкаДопРС.ТипЗначения.Типы().Найти(ТипЗнч(Значение)) = Неопределено Тогда
	
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Тип значения не соответствует типу реквизита!";
		// Заглушка для проведения актов передачи
		// Сообщение.Сообщить();
		// Возврат;
...
Функция ВычислитьЗадолженностьНаДату(ОбъектУчета, ДатаОстатков) Экспорт
	...
	ВариантУчета = ОбъектыСервер.ОпределитьВариантУчетаЗадолженностиПоРегиструСтадий(ОбъектУчета, ДатаОстатков);
...
Функция ОпределитьВариантУчетаЗадолженности2(Об, ДатаПлатежа) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.ОбъектыВРаботе.Остатки(&ТекущаяДата, Объект = &Об) КАК ОбъектыВРаботеОстатки
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					Иначе
						Если ВыборкаДетальныеЗаписи.СудебкаОстаток < 0 Тогда
							ВариантУчета = Перечисления.СтадииЗадолженности.Внесудебная;
						Иначе
							ВариантУчета = Перечисления.СтадииЗадолженности.ПустаяСсылка();
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция ЗадолженностьПоСудебнымРешениям_ПолучитьОстаткиЗадолженности(ОбъектУчета, ДатаОстатков) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ВЫБРАТЬ
	                      |	ВТ.Займ КАК Займ,
	                      |	ЕСТЬNULL(ВТ.Госпошлина, 0) КАК Госпошлина,
	                      |	ЕСТЬNULL(ВТ.Проценты, 0) КАК Проценты,
	                      |	ЕСТЬNULL(ВТ.ОсновнойДолг, 0) КАК ОсновнойДолг,
	                      |	ЕСТЬNULL(ВТ.Пени, 0) КАК Пени,
	                      |	ЕСТЬNULL(ВТ.Штрафы, 0) КАК Штрафы,
	                      |	ЕСТЬNULL(ВТ.Переплата, 0) КАК Переплата,
	                      |	ЕСТЬNULL(ВТ.Госпошлина, 0) + ЕСТЬNULL(ВТ.Проценты, 0) + ЕСТЬNULL(ВТ.ОсновнойДолг, 0) + ЕСТЬNULL(ВТ.Пени, 0) + ЕСТЬNULL(ВТ.Штрафы, 0) + ЕСТЬNULL(ВТ.Переплата, 0) КАК Сумма
	                      |ИЗ
	                      |	ВТ КАК ВТ");
	Запрос.УстановитьПараметр("ОбъектУчета", ОбъектУчета);
	Запрос.УстановитьПараметр("ДатаОстатков", ДатаОстатков);
...
Функция РасчетыПоДоговорам_ПолучитьОстаткиЗадолженности(ОбъектУчета, ДатаОстатков) Экспорт
	...
	|	РасчетыПоДоговорамОстатки.ОсновнойДолгОстаток + РасчетыПоДоговорамОстатки.ПроцентыНачисленныеВПериодПросрочкиОстаток + РасчетыПоДоговорамОстатки.ШтрафыОстаток + РасчетыПоДоговорамОстатки.ПениОстаток + РасчетыПоДоговорамОстатки.ОсновнойДолгГрафикОстаток + РасчетыПоДоговорамОстатки.ПроцентыГрафикОстаток + РасчетыПоДоговорамОстатки.ШтрафыНачисленияОстаток КАК Сумма
	|ИЗ
...
Функция ОтборОбъектовПоРегистратору(Набор) Экспорт
	...
	ИначеЕсли МетаданныеОбъекта.ТабличныеЧасти.Найти("Объекты") <> Неопределено Тогда
...
&НаСервере
Функция ОтборОбъектовПоДокументу(Регистратор) Экспорт
	 
	МетаданныеОбъекта = Регистратор.Метаданные();
	МассивОбъектов = Новый ТаблицаЗначений;
	МассивОбъектов.Колонки.Добавить("Объект");
	МассивОбъектов.Колонки.Добавить("Период");           
	
	Если ТипЗнч(Регистратор)= тип("ДокументСсылка.ПоступлениеПлатежа") Тогда  
		НСтрока = МассивОбъектов.Добавить();
		НСтрока.Период = Регистратор.ДатаПлатежа;
		НСтрока.Объект = Регистратор.Займ; 
	ИначеЕсли ТипЗнч(Регистратор)= тип("ДокументСсылка.СудебноеДело") Тогда
		НСтрока = МассивОбъектов.Добавить();
		НСтрока.Период = ?(ЗначениеЗаполнено(Регистратор.ДатаРешения), Регистратор.ДатаРешения, Регистратор.ДатаИска);
		НСтрока.Объект = Регистратор.Займ; 
	ИначеЕсли МетаданныеОбъекта.ТабличныеЧасти.Найти("Объекты") <> Неопределено 
		И( ТипЗнч(Регистратор)= тип("ДокументСсылка.АктуализацияЗадолженности") ИЛИ ТипЗнч(Регистратор)= тип("ДокументСсылка.ВводНачальныхОстатковВнесудебка")) Тогда
		Для Каждого Стр из Регистратор.Объекты Цикл
			НСтрока = МассивОбъектов.Добавить();
			НСтрока.Период = НачалоДня(Регистратор.Дата);
			НСтрока.Объект = Стр.Объект; 
		КонецЦикла; 
	ИначеЕсли МетаданныеОбъекта.ТабличныеЧасти.Найти("Объекты") <> Неопределено 
		И( ТипЗнч(Регистратор)= тип("ДокументСсылка.НачислениеЗадолженностиВнесудебка") ИЛИ ТипЗнч(Регистратор)= тип("ДокументСсылка.ЗагрузкаЗадолженности")) Тогда
		Для Каждого Стр из Регистратор.Объекты Цикл
			НСтрока = МассивОбъектов.Добавить();
			НСтрока.Период = НачалоДня(Стр.ДатаЗадолженности);
			НСтрока.Объект = Стр.Объект; 
		КонецЦикла;	  
	ИначеЕсли МетаданныеОбъекта.ТабличныеЧасти.Найти("Объекты") <> Неопределено 
		И(ТипЗнч(Регистратор)= тип("ДокументСсылка.ПоступлениеПлатежей") ИЛИ ТипЗнч(Регистратор)= тип("ДокументСсылка.ЗагрузкаИсторииПлатежей")) Тогда
		Для Каждого Стр из Регистратор.Объекты Цикл
			НСтрока = МассивОбъектов.Добавить();
			НСтрока.Период = НачалоДня(Стр.ДатаПлатежа);
			НСтрока.Объект = Стр.Объект; 
		КонецЦикла;	 
	ИначеЕсли ТипЗнч(Регистратор)= тип("ДокументСсылка.РеструктуризацияМикрозайма") ИЛИ ТипЗнч(Регистратор)= тип("ДокументСсылка.ДоговорМикрозайма") Тогда
			НСтрока = МассивОбъектов.Добавить();
			НСтрока.Период = НачалоДня(Регистратор.ДатаДоговора);
			НСтрока.Объект = Регистратор.Займ; 
	Иначе
		 Для Каждого Стр из Регистратор.Объекты Цикл
			НСтрока = МассивОбъектов.Добавить();
			НСтрока.Период = НачалоДня(ТекущаяДата());
			НСтрока.Объект = Стр.Объект; 
		КонецЦикла;	 
...
&НаСервере
Функция ПараметрТекущийПользователь() Экспорт
	Возврат ПараметрыСеанса.ТекущийПользователь;
КонецФункции

Функция ОпределитьВариантУчетаЗадолженностиПоРегиструСтадий(Об, ДатаПлатежа) Экспорт
		
	ВариантУчета = РегистрыСведений.СтадииЗадолженности.ПолучитьПоследнее(НачалоДня(ДатаПлатежа), Новый Структура("ОбъектУчета", Об)).СтадияЗадолженности;
	
	Возврат ВариантУчета;	
		
КонецФункции

// Обновление РС Стадии и РС Задолженность по объектам остатки

Процедура ЗаписатьТекущиеПараметрыЗадолженностиОбработчик(ДокументОбъект) Экспорт   
	ЭтоПроведение = Истина;
	
	МассивОбъектов = ВернутьОбъектыПоИсточнику(ДокументОбъект, ЭтоПроведение);
	
	ЗаписатьСтадиииОстаткиОбъектов(МассивОбъектов, ДокументОбъект.Ссылка, ЭтоПроведение);
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ДоговорМикрозайма") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеструктуризацияМикрозайма") Тогда
		РасчетЗадолженностиМФО.АктуализацияХарактеристикМикрозайма(ДокументОбъект.Займ);
	КонецЕсли;	
		
	 
КонецПроцедуры 

Процедура ЗаписатьСтадиииОстаткиОбъектов(МассивОбъектов, ДокументРегистратор, ЭтоПроведение) Экспорт      
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗадолженностьПоСудебнымРешениямОстатки.Займ КАК Объект,
	|	ВЫБОР
	|		КОГДА ЗадолженностьПоСудебнымРешениямОстатки.ТипЗадолженности = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ТипыЗадолженностиПоРешению.Госпошлина)
	|			ТОГДА ЗадолженностьПоСудебнымРешениямОстатки.СуммаОстаток
	|	КОНЕЦ КАК Госпошлина,
	...
	|	ВЫБОР
	|		КОГДА ЗадолженностьПоСудебнымРешениямОстатки.СуммаОстаток < 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Пустая)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Судебная)
	|	КОНЕЦ КАК СтадияРегистра
	|ПОМЕСТИТЬ ЗадолженностьОст
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоСудебнымРешениям.Остатки(&МоментВремени, Займ В (&ОбъектУчета)) КАК ЗадолженностьПоСудебнымРешениямОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗадолженностьПоОбъектамОстатки.Объект,
	|	0,
	|	ЗадолженностьПоОбъектамОстатки.ПроцентыДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.ОсновнойДолгДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.ПениДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.ШтрафыДООстаток,
	|	0,
	|	ЗадолженностьПоОбъектамОстатки.СуммаДООстаток,
	|	ВЫБОР
	|		КОГДА ЗадолженностьПоОбъектамОстатки.СуммаДООстаток < 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Пустая)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.СтарыйУчет)
	|	КОНЕЦ
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоОбъектам.Остатки(&МоментВремени, Объект В (&ОбъектУчета)) КАК ЗадолженностьПоОбъектамОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ЗадолженностьПоОбъектамВнесудебнаяОстатки.СуммаДООстаток < 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Пустая)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Внесудебная)
	|	КОНЕЦ
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоОбъектамВнесудебная.Остатки(&МоментВремени, Объект В (&ОбъектУчета)) КАК ЗадолженностьПоОбъектамВнесудебнаяОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РасчетыПоДоговорамОстатки.ПениОстаток,
	|	РасчетыПоДоговорамОстатки.ШтрафыОстаток,
	|	РасчетыПоДоговорамОстатки.РасчетныйСчетОстаток,
	|	РасчетыПоДоговорамОстатки.ПроцентыОстаток + РасчетыПоДоговорамОстатки.ПроцентыНачисленныеВПериодПросрочкиОстаток + РасчетыПоДоговорамОстатки.ОсновнойДолгОстаток + РасчетыПоДоговорамОстатки.ШтрафыОстаток + РасчетыПоДоговорамОстатки.ПениОстаток,
	|	ВЫБОР
	|		КОГДА РасчетыПоДоговорамОстатки.ПроцентыОстаток + РасчетыПоДоговорамОстатки.ПроцентыНачисленныеВПериодПросрочкиОстаток + РасчетыПоДоговорамОстатки.ОсновнойДолгОстаток + РасчетыПоДоговорамОстатки.ШтрафыОстаток + РасчетыПоДоговорамОстатки.ПениОстаток < 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Пустая)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Расчетная)
	|	КОНЕЦ
	|ИЗ
	|	РегистрНакопления.РасчетыПоДоговорам.Остатки(&МоментВремени, Займ В (&ОбъектУчета)) КАК РасчетыПоДоговорамОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗадолженностьОст.Объект КАК ОбъектУчета,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Госпошлина, 0)) КАК Госпошлина,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Проценты, 0)) КАК Проценты,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.ОсновнойДолг, 0)) КАК ОсновнойДолг,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Пени, 0)) КАК Пени,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Штрафы, 0)) КАК Штрафы,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Переплата, 0)) КАК Переплата,
	|	СУММА(ЕСТЬNULL(ЗадолженностьОст.Сумма, 0)) КАК Сумма,
	|	ЗадолженностьОст.СтадияРегистра КАК СтадияЗадолженности
	|ИЗ
	|	ЗадолженностьОст КАК ЗадолженностьОст
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗадолженностьОст.Объект,
	|	ЗадолженностьОст.СтадияРегистра
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗадолженностьОст.Объект
	|ИТОГИ
	...
	Запрос.УстановитьПараметр("ОбъектУчета", МассивОбъектов.ВыгрузитьКолонку("Объект"));
	ВидГраницыПроведение = ?(ЭтоПроведение = Истина, ВидГраницы.Включая, ВидГраницы.Исключая); 
	
	Если ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.СудебноеДело") Тогда
		ДатаСреза = ПолучитьДатуНачисленияСудебнойЗадолженности(ДокументРегистратор);
		Если ЗначениеЗаполнено(ДатаСреза) Тогда
			Запрос.УстановитьПараметр("МоментВремени", Новый Граница(ДатаСреза, ВидГраницыПроведение));	
		Иначе
			Запрос.УстановитьПараметр("МоментВремени", Новый Граница(ДокументРегистратор.МоментВремени(), ВидГраницыПроведение));	
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.ПоступлениеПлатежа") Тогда
		ДатаСреза = ДокументРегистратор.ДатаПлатежа;
		Если ЗначениеЗаполнено(ДатаСреза) Тогда
			Запрос.УстановитьПараметр("МоментВремени", Новый Граница(ДатаСреза, ВидГраницыПроведение));	
		Иначе
			Запрос.УстановитьПараметр("МоментВремени", Новый Граница(ДокументРегистратор.МоментВремени(), ВидГраницыПроведение));	
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Для каждого Эл из МассивОбъектов Цикл 
			НоваяЗаписьСтадия = РегистрыСведений.СтадииЗадолженности.СоздатьМенеджерЗаписи(); 
			НоваяЗаписьСтадия.Период = Эл.Период;
			НоваяЗаписьСтадия.СтадияЗадолженности = Перечисления.СтадииЗадолженности.Пустая;
			НоваяЗаписьСтадия.ОбъектУчета = Эл.Объект;
			НоваяЗаписьСтадия.Документ = ДокументРегистратор;
			НоваяЗаписьСтадия.Записать();   
			
			
			КредитныеИстории.НБКИ_ЗаписатьОбязательствоСубъектаПрекратилосьОбработчик(Эл.Объект, НоваяЗаписьСтадия.Период, ДокументРегистратор, НоваяЗаписьСтадия.СтадияЗадолженности);
			
			Нов = РегистрыСведений.ЗадолженностьПоОбъектамОстатки.СоздатьНаборЗаписей();
			Нов.Отбор.Объект.Установить(Эл.Объект);						
			Нов.Записать();             
		КонецЦикла;
	Иначе
		...
		Пока ВыборкаОбъект.Следующий() Цикл;
			Нов = РегистрыСведений.ЗадолженностьПоОбъектамОстатки.СоздатьНаборЗаписей();
			Нов.Отбор.Объект.Установить(ВыборкаОбъект.ОбъектУчета);						
			Нов.Записать(); 
			
			Для каждого НазваниеСоставляющей Из МассивНазванияСоставляющих Цикл	
				Нов = РегистрыСведений.ЗадолженностьПоОбъектамОстатки.СоздатьМенеджерЗаписи();
				Нов.Объект = ВыборкаОбъект.ОбъектУчета;
				Нов.ТипЗадолженности = ПланыВидовХарактеристик.ТипыЗадолженности[НазваниеСоставляющей];
				Нов.СуммаДО   = ВыборкаОбъект[НазваниеСоставляющей]; 
				Нов.СуммаРегл   = ВыборкаОбъект[НазваниеСоставляющей]; 
				Нов.Записать();                                                     
			КонецЦикла;
			...
			ЭтоПолноеПогашение = (ВыборкаОбъект.Сумма <= 0); 
			ЭтоПлатеж = ТипПлатеж(ДокументРегистратор);
			Если ЭтоПолноеПогашение И ЭтоПлатеж Тогда
				Период = МассивОбъектов.Найти(ВыборкаОбъект.ОбъектУчета).Период;
				ОбъектУчета = ВыборкаОбъект.ОбъектУчета;
				НоваяЗаписьСтадия = РегистрыСведений.СтадииЗадолженности.СоздатьМенеджерЗаписи(); 
				НоваяЗаписьСтадия.Период = Период;
				НоваяЗаписьСтадия.СтадияЗадолженности = Перечисления.СтадииЗадолженности.Пустая;
				НоваяЗаписьСтадия.ОбъектУчета = ОбъектУчета;
				НоваяЗаписьСтадия.Документ = ДокументРегистратор;
				НоваяЗаписьСтадия.Записать();   	
				КредитныеИстории.НБКИ_ЗаписатьОбязательствоСубъектаПрекратилосьОбработчик(ОбъектУчета, НоваяЗаписьСтадия.Период, ДокументРегистратор, НоваяЗаписьСтадия.СтадияЗадолженности);				
				
			КонецЕсли;
			
			Если НЕ ЭтоПолноеПогашение И ЭтоПлатеж Тогда
				Период = МассивОбъектов.Найти(ВыборкаОбъект.ОбъектУчета).Период;
				ОбъектУчета = ВыборкаОбъект.ОбъектУчета;
				ОбъектыСервер.УдалитьСтадииОтменаПроведения(ДокументРегистратор);
				КредитныеИстории.УдалитьСобытиеСделки(ОбъектУчета, ДокументРегистратор, "2.5");				
			...
			ВыборкаСтадии = ВыборкаОбъект.Выбрать();
			ЕстьРасчетнаяСтадия = Ложь;
			ЕстьСудебнаяСтадия = Ложь; 
			ЕстьВнесудебка = Ложь;
			СтадияЗамена = Неопределено;
			
			Если ВыборкаСтадии.НайтиСледующий(Новый Структура("СтадияЗадолженности", Перечисления.СтадииЗадолженности.Внесудебная)) Тогда
				ЕстьВнесудебка = Истина;  
				СтадияЗамена = Перечисления.СтадииЗадолженности.Внесудебная;
			КонецЕсли;
            ВыборкаСтадии.Сбросить();
			
			Если ВыборкаСтадии.НайтиСледующий(Новый Структура("СтадияЗадолженности", Перечисления.СтадииЗадолженности.Расчетная)) Тогда
				СтадияЗамена = Перечисления.СтадииЗадолженности.Расчетная;
				ЕстьРасчетнаяСтадия = Истина; 
			КонецЕсли;
			ВыборкаСтадии.Сбросить();
			
			Если ВыборкаСтадии.НайтиСледующий(Новый Структура("СтадияЗадолженности", Перечисления.СтадииЗадолженности.Судебная)) Тогда 
				СтадияЗамена = Перечисления.СтадииЗадолженности.Судебная;
				ЕстьСудебнаяСтадия = Истина; 
			КонецЕсли;
			ВыборкаСтадии.Сбросить();   
					
			ТекущаяСтадия = ВенутьСтадиюПоРегистратору(ДокументРегистратор); 
			
			Если (ТекущаяСтадия = Перечисления.СтадииЗадолженности.Расчетная 
				ИЛИ ТекущаяСтадия = Перечисления.СтадииЗадолженности.Внесудебная) И ЕстьСудебнаяСтадия Тогда
				Продолжить;
			ИначеЕсли ТекущаяСтадия = Перечисления.СтадииЗадолженности.Внесудебная И ЕстьРасчетнаяСтадия Тогда 
				Продолжить; 
			ИначеЕсли ТекущаяСтадия = Перечисления.СтадииЗадолженности.Пустая Тогда 
				Продолжить;	
			ИначеЕсли ТекущаяСтадия = Перечисления.СтадииЗадолженности.СтарыйУчет И ЗначениеЗаполнено(СтадияЗамена) Тогда 
				ТекущаяСтадия = СтадияЗамена;
			Иначе			
				Период = МассивОбъектов.Найти(ВыборкаОбъект.ОбъектУчета).Период;
				ЗаписьСтадия = РегистрыСведений.СтадииЗадолженности.СоздатьМенеджерЗаписи();
				ЗаписьСтадия.Период = Период;
				ЗаписьСтадия.ОбъектУчета = ВыборкаОбъект.ОбъектУчета;
				ЗаписьСтадия.Документ = ДокументРегистратор;
				
				ЗаписьСтадия.Прочитать();
				ЗаписьСуществует = ЗаписьСтадия.Выбран();
				
				Если НЕ ЗаписьСуществует Тогда
					НоваяЗаписьСтадия = РегистрыСведений.СтадииЗадолженности.СоздатьМенеджерЗаписи(); 
					//ЗаполнитьЗначенияСвойств(НоваяЗаписьСтадия, ВыборкаСтадии); 
					НоваяЗаписьСтадия.ОбъектУчета  = ВыборкаОбъект.ОбъектУчета;
					НоваяЗаписьСтадия.Период = Период;   
					НоваяЗаписьСтадия.Документ = ДокументРегистратор;
					НоваяЗаписьСтадия.СтадияЗадолженности = ТекущаяСтадия;
					НоваяЗаписьСтадия.Записать();
					КредитныеИстории.НБКИ_ЗаписатьОбязательствоСубъектаПрекратилосьОбработчик(ВыборкаОбъект.ОбъектУчета, НоваяЗаписьСтадия.Период, ДокументРегистратор, НоваяЗаписьСтадия.СтадияЗадолженности);
				Иначе
					ЗаписьСтадия.СтадияЗадолженности = ТекущаяСтадия;
					ЗаписьСтадия.Записать();
					КредитныеИстории.НБКИ_ЗаписатьОбязательствоСубъектаПрекратилосьОбработчик(ВыборкаОбъект.ОбъектУчета, ЗаписьСтадия.Период, ДокументРегистратор, ЗаписьСтадия.СтадияЗадолженности);
				КонецЕсли;
...
&НаСервере
Функция ВернутьОбъектыПоИсточнику(ДокументОбъект, ЭтоПроведение) Экспорт
	МассивОбъектов = Новый ТаблицаЗначений;
	МассивОбъектов.Колонки.Добавить("Объект");
	МассивОбъектов.Колонки.Добавить("Период"); 
	...
	Для Каждого СтрДвижение Из Движения Цикл 
		Если ТипЗнч(СтрДвижение) = Тип("РегистрНакопленияНаборЗаписей.ИсторияПлатежей") Тогда
			 МассивОбъектов = ОтборОбъектовПоДокументу(ДокументОбъект.Ссылка);  
			Продолжить;
		ИначеЕсли СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Задолженность") = 0  И СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Расчеты") = 0  И СтрНайти(Строка(ТипЗнч(СтрДвижение)), "Накопления") = 0 Тогда
			Продолжить; 
		ИначеЕсли ТипЗнч(СтрДвижение) = Тип("РегистрНакопленияНаборЗаписей.РасчетыПоДоговорам") Тогда
			МассивОбъектов = ОтборОбъектовПоДокументу(ДокументОбъект.Ссылка);  
			Продолжить;
		Иначе	
			Если СтрДвижение.Количество() > 0 Тогда   
				Для Каждого Движение Из СтрДвижение Цикл
					НоваяСтрока = МассивОбъектов.Добавить();  
					ИмяОбъектаУчета = ?((ТипЗнч(Движение) = Тип("РегистрНакопленияЗапись.ЗадолженностьПоСудебнымРешениям") ИЛИ ТипЗнч(Движение) = Тип("РегистрНакопленияЗапись.РасчетыПоДоговорам")), "Займ", "Объект");
					НоваяСтрока.Объект = Движение[ИмяОбъектаУчета];						
	...
	Если Не ЗначениеЗаполнено(МассивОбъектов) Тогда
		...
		МассивОбъектов = ОтборОбъектовПоДокументу(ДокументОбъект.Ссылка); 
	КонецЕсли;		  
	МассивОбъектов.Свернуть("Период, Объект");
	Возврат МассивОбъектов;
КонецФункции      

Функция ПолучитьМассивНазванияСоставляющих() Экспорт
...
Процедура ЗаполнитьРССтадииЗадолженности() Экспорт      
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗадолженностьПоОбъектамОбороты.Период КАК Период,
	|	ЗадолженностьПоОбъектамОбороты.Объект КАК Объект,
	|	ЗадолженностьПоОбъектамОбороты.СуммаДОПриход КАК СуммаДОПриход,
	|	ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.СтарыйУчет) КАК Стадия,
	|	ЗадолженностьПоОбъектамОбороты.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ Регистры
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоОбъектам.Обороты(, , Регистратор, ) КАК ЗадолженностьПоОбъектамОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗадолженностьПоОбъектамВнесудебнаяОбороты.Период,
	|	ЗадолженностьПоОбъектамВнесудебнаяОбороты.Объект,
	|	ЗадолженностьПоОбъектамВнесудебнаяОбороты.СуммаДОПриход,
	|	ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Внесудебная),
	|	ЗадолженностьПоОбъектамВнесудебнаяОбороты.Регистратор
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоОбъектамВнесудебная.Обороты(, , Регистратор, ) КАК ЗадолженностьПоОбъектамВнесудебнаяОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗадолженностьПоСудебнымРешениямОбороты.Период,
	|	ЗадолженностьПоСудебнымРешениямОбороты.Займ,
	|	ЗадолженностьПоСудебнымРешениямОбороты.СуммаПриход,
	|	ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Судебная),
	|	ЗадолженностьПоСудебнымРешениямОбороты.Регистратор
	|ИЗ
	|	РегистрНакопления.ЗадолженностьПоСудебнымРешениям.Обороты(, , Регистратор, ) КАК ЗадолженностьПоСудебнымРешениямОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	РасчетыПоДоговорамОбороты.Период,
	|	РасчетыПоДоговорамОбороты.Займ,
	|	РасчетыПоДоговорамОбороты.ОсновнойДолгПриход,
	|	ЗНАЧЕНИЕ(Перечисление.СтадииЗадолженности.Расчетная),
	|	РасчетыПоДоговорамОбороты.Регистратор
	|ИЗ
	|	РегистрНакопления.РасчетыПоДоговорам.Обороты(, , Регистратор, ) КАК РасчетыПоДоговорамОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Регистры.Объект КАК ОбъектУчета,
	|	Регистры.Период КАК Период,
	|	Регистры.Стадия КАК СтадияЗадолженности,
	|	Регистры.Регистратор КАК Документ
	|ИЗ
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтадияЗадолженности),
	...
	НаборЗаписей = РегистрыСведений.СтадииЗадолженности.СоздатьНаборЗаписей();
	...
	Пока ВыборкаОбъект.Следующий() Цикл
		...
		Пока ВыборкаСтадии.Следующий() Цикл 
			Если ВыборкаСтадии.СтадияЗадолженности = Перечисления.СтадииЗадолженности.Внесудебная И ЕстьРасчетнаяСтадия Тогда 
				Продолжить;
			ИначеЕсли ВыборкаСтадии.СтадияЗадолженности = Перечисления.СтадииЗадолженности.СтарыйУчет И Константы.ИспользоватьСудебныйБлок.Получить() Тогда 
				Продолжить;
			Иначе		
				НоваяЗапись = НаборЗаписей.Добавить(); 
				ЗаполнитьЗначенияСвойств(НоваяЗапись, ВыборкаСтадии); 
			КонецЕсли;	
		КонецЦикла; 
...
Процедура УдалитьСтадииОтменаПроведения(ДокументОбъект) Экспорт      
		
	НаборСтадия = РегистрыСведений.СтадииЗадолженности.СоздатьНаборЗаписей();
	НаборСтадия.Отбор.Документ.Установить(ДокументОбъект.Ссылка); 	
	НаборСтадия.Записать();
	
КонецПроцедуры    

Функция ТипПлатеж(ДокументОбъект) Экспорт  
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ПоступлениеПлатежа") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ПоступлениеПлатежей") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ЗагрузкаИсторииПлатежей") Тогда
		Возврат Истина;  
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ВенутьСтадиюПоРегистратору(ДокументОбъект) Экспорт 
	
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.НачислениеЗадолженностиВнесудебка") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ВводНачальныхОстатковВнесудебка") Тогда
		Стадия = Перечисления.СтадииЗадолженности.Внесудебная;
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.РеструктуризацияМикрозайма") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ДоговорМикрозайма") Тогда
		Стадия = Перечисления.СтадииЗадолженности.Расчетная;
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.АктуализацияЗадолженности") тогда
		Если Константы.ИспользоватьСтандарныйУчетЗадолженности.Получить() Тогда
			Стадия = Перечисления.СтадииЗадолженности.СтарыйУчет;
		Иначе
			Стадия = Перечисления.СтадииЗадолженности.Внесудебная;
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.ЗагрузкаЗадолженности") Тогда
		Стадия = Перечисления.СтадииЗадолженности.СтарыйУчет; 	
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.СудебноеДело") Тогда
		Стадия = Перечисления.СтадииЗадолженности.Судебная;	
	Иначе
		Стадия = Перечисления.СтадииЗадолженности.Пустая;
	КонецЕсли;
...
Процедура ПрограммнаяКорректировкаСтарыхРегистров() Экспорт      
	
	//Техническая	     
	//Константа старый учет вкл
	//после Корректировки ВЫКЛ
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗадолженностьПоОбъектамОстатки.Объект КАК Объект,
	...
	|	ЗадолженностьПоОбъектамОстатки.ПениДООстаток КАК ПениДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.ПениРеглОстаток КАК ПениРеглОстаток,
	...
	|	ЗадолженностьПоОбъектамОстатки.Составляющая1ДООстаток КАК Составляющая1ДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.Составляющая1РеглОстаток КАК Составляющая1РеглОстаток,
	|	ЗадолженностьПоОбъектамОстатки.Составляющая2ДООстаток КАК Составляющая2ДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.Составляющая2РеглОстаток КАК Составляющая2РеглОстаток,
	|	ЗадолженностьПоОбъектамОстатки.Составляющая3ДООстаток КАК Составляющая3ДООстаток,
	|	ЗадолженностьПоОбъектамОстатки.Составляющая3РеглОстаток КАК Составляющая3РеглОстаток,
	...
	ДокКорректировка = Документы.АктуализацияЗадолженности.СоздатьДокумент();
	ДокКорректировка.Автор = ПараметрыСеанса.ТекущийПользователь;
	ДокКорректировка.Дата = НачалоДня(НачалоДня(ТекущаяДатаСеанса()) - 1);
	ДокКорректировка.Организация = ПараметрыСеанса.ТекущийПользователь.Организация;
	
	Пока РезультатЗапроса.Следующий() Цикл 
		
		НСтр = ДокКорректировка.Объекты.Добавить();
		Нстр.Объект = РезультатЗапроса.Объект;
		Нстр.ОсновнойДолг = 0;
		Нстр.Пени = 0;
		Нстр.Проценты = 0;
		Нстр.Составляющая1 = 0;
	    Нстр.Составляющая2 = 0;
		Нстр.Составляющая3 = 0;
		Нстр.Составляющая4 = 0;
		Нстр.Составляющая5 = 0;
		Нстр.Составляющая6 = 0;
		Нстр.Составляющая7 = 0;
		Нстр.Составляющая8 = 0;
		Нстр.Штрафы = 0;
	КонецЦикла;
...
Функция ПолучитьДатуНачисленияСудебнойЗадолженности(СудДело) Экспорт  
	
		
	ЗначениеУчета = Константы.ДатаНачисленияПросуженнойЗадолженности.Получить();
	
	Если ЗначениеУчета = Перечисления.ДатыНачисленияПросуженнойЗадолженности.НаДатуРешения Тогда 
		ДатаНачисления = ?(ЗначениеЗаполнено(СудДело.ДатаРешения), СудДело.ДатаРешения, ТекущаяДатаСеанса());
	ИначеЕсли ЗначениеУчета = Перечисления.ДатыНачисленияПросуженнойЗадолженности.НаДатуВступленияВЗаконнуюСилу Тогда
		ДатаНачисления = ?(ЗначениеЗаполнено(СудДело.ДатаВступленияВЗаконнуюСилу), СудДело.ДатаВступленияВЗаконнуюСилу, ТекущаяДатаСеанса());
	Иначе
		ДатаНачисления = ?(ЗначениеЗаполнено(СудДело.ДатаИска), СудДело.ДатаИска, ТекущаяДатаСеанса());
    КонецЕсли;
...
Функция ПреобразоватьСоответствиеВСтруктуру(Соответствие) Экспорт
	...
	Для Каждого Элемент Из Соответствие Цикл
		Ключ = СтрЗаменить(Элемент.Ключ, "-", "");
		Результат.Вставить(Ключ, Элемент.Значение);
	КонецЦикла;
...
Функция ВернутьЗначениеСтруктурыСПроверкойКлюча(ПеремСтруктура, ИмяКлюча) Экспорт
	
	ИскомоеЗначение = "";
	ПеремСтруктура.Свойство(ИмяКлюча, ИскомоеЗначение);
