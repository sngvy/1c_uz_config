...
Функция ПолучитьИсториюПлатежей(Договор) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	РеструктуризацияМикрозаймаИсторияПлатежей.НомерСтроки КАК НомерСтроки,
	                      |	РеструктуризацияМикрозаймаИсторияПлатежей.СуммаПлатежа КАК СуммаПлатежа,
	                      |	РеструктуризацияМикрозаймаИсторияПлатежей.ДатаПлатежа КАК ДатаПлатежа,
	                      |	РеструктуризацияМикрозаймаИсторияПлатежей.Регистратор КАК Регистратор
	                      |ИЗ
	                      |	Документ.РеструктуризацияМикрозайма.ИсторияПлатежей КАК РеструктуризацияМикрозаймаИсторияПлатежей
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеструктуризацияМикрозайма КАК РеструктуризацияМикрозайма
	                      |		ПО РеструктуризацияМикрозаймаИсторияПлатежей.Ссылка.Ссылка = РеструктуризацияМикрозайма.Ссылка
	                      |ГДЕ
	                      |	РеструктуризацияМикрозайма.Ссылка = &ТекущийДоговор
	                      |	И РеструктуризацияМикрозайма.Проведен = ИСТИНА
	                      |	И РеструктуризацияМикрозайма.ПометкаУдаления = ЛОЖЬ");
...
// Новая функциональность не добавлена, так как данная функция не используется 
Функция РассчитатьГоспошлину(СуммаИска) Экспорт
	
	Сумма = РасчетЗадолженностиСудопроизводство.РассчитатьГоспошлинуФЛ_ДоИндексации(СуммаИска);
	Возврат Сумма;
...
Функция РассчитатьГоспошлинуЮр(СуммаИска) Экспорт
	...
	Сумма = РасчетЗадолженностиСудопроизводство.РассчитатьГоспошлинуЮр_ДоИндексации(СуммаИска);
	Возврат Сумма;
