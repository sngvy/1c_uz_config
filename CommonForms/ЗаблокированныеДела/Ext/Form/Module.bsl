...
Процедура СформироватьСписокНаСервере()
	ФоновыеЗадания.Выполнить("ФоновыеЗаданияМодуль.БлокировкаДелДляАвтообзвона",,,"Блокировка дел для автообзвона");
КонецПроцедуры

&НаКлиенте
Процедура СформироватьСписок(Команда)
	Если ЭтаФорма.РольАБД = Истина Тогда
		ПоказатьОповещениеПользователя("Блокировка дел для автообзвона",,"Задание запущено в фоновом режиме",БиблиотекаКартинок.NC_Подсистема_тсАдминистрирование);
