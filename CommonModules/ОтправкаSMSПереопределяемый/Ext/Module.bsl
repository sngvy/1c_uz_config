...
//++БИТ
&НаСервере
Процедура ЗаполнитьНовыеНастройкиSMS4B() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	УчетныеЗаписиSMS4B.Логин КАК Логин,
	                      |	УчетныеЗаписиSMS4B.Пароль КАК Пароль,
	                      |	УчетныеЗаписиSMS4B.КорректировкаUTC КАК КорректировкаUTC,
	                      |	УчетныеЗаписиSMS4B.Отправитель КАК ИмяОтправителя
	                      |ИЗ
	                      |	Константа.УчеткаSMSРассылок КАК УчеткаSMSРассылок
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УчетныеЗаписиSMS4B КАК УчетныеЗаписиSMS4B
	                      |		ПО УчеткаSMSРассылок.Значение = УчетныеЗаписиSMS4B.Ссылка
	                      |ГДЕ
	                      |	УчетныеЗаписиSMS4B.Ссылка.ПометкаУдаления = ЛОЖЬ");
	...
	Если Выборка.Следующий() Тогда
		...
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Владелец, Выборка.Логин, "Логин");
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Владелец, Выборка.ИмяОтправителя, "ИмяОтправителя");
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Владелец, Выборка.КорректировкаUTC, "КорректировкаUTC"); 
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Владелец, Выборка.Пароль);
