...
// Первичное заполнение - псевдокод НБКИ УЗ 0
	...
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","1.7");  
	СтруктураСобытия.Вставить("Наименование","Смена сведений основной части КИ субъекта");
	МассивСобытий.Добавить(СтруктураСобытия); 
	...
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","1.12"); 
	СтруктураСобытия.Вставить("Наименование","Внесудебное банкротство Субъекта");
	МассивСобытий.Добавить(СтруктураСобытия);
...
Процедура ЗаполнитьСобытияДляИсправленияКИ() 
	...
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","3.3");  
	СтруктураСобытия.Вставить("Наименование","Исключение отстутствующей у Источника информации");
	МассивСобытий.Добавить(СтруктураСобытия);    
...
Процедура ЗаполнитьПрекращениеОбеспечения() 
	...
	// 4	Обеспечивающее обязательство прекращено в связи с прекращением основного обязательства
	...
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","4");  
	СтруктураСобытия.Вставить("Наименование","Обеспечивающее обязательство прекращено в связи с прекращением основного обязательства");
	МассивСобытий.Добавить(СтруктураСобытия); 
...
//99	Иная цель
	...
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","21");  
	СтруктураСобытия.Вставить("Наименование","Приобретение авто- и мототранспортного средства");
	МассивСобытий.Добавить(СтруктураСобытия);  
	
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","21.1");  
	СтруктураСобытия.Вставить("Наименование","Приобретение авто- и мототранспортного средства с пробегом менее 1000 километров включительно");
	МассивСобытий.Добавить(СтруктураСобытия); 
	
	СтруктураСобытия = Новый Структура("КодНБКИ","Наименование");
	СтруктураСобытия.Вставить("КодНБКИ","21.2");  
	СтруктураСобытия.Вставить("Наименование","Приобретение авто- и (или) мототранспортного средства с пробегом свыше 1000 километров");
	МассивСобытий.Добавить(СтруктураСобытия);  
...
Функция ВычислитьПараметрПоУмолчанию(КодНБКИ, СтруктураОб,  Об) Экспорт 	
	...
	Если КодНБКИ = "C27.7" ИЛИ  КодНБКИ = "C27.1" Тогда
		// Просроченная   + Дата выхода на просрочку
		Параметр = ТекущаяДатаСеанса(); 
		Если ЗначениеЗаполнено(СтруктураОб.ДоговорМКЗ) ИЛИ ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ) Тогда
			ТекущийДоговор =?(ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ), СтруктураОб.РеструктуризацияМКЗ, СтруктураОб.ДоговорМКЗ);
			ВидПогашения = ТекущийДоговор.СпособПогашения;
			...
			Если ВидПогашения = Перечисления.СпособыПогашенияЗаймов.НаДатуОкончанияДоговора Тогда
				Для каждого стр из ТаблицаГрафика Цикл
					Если стр.ФлагНаличияПросрочки Тогда
						Параметр = стр.Дата;
						Прервать;
					КонецЕсли;
				КонецЦикла;	
			Иначе
				Для каждого стр из ТаблицаГрафика Цикл
					Если стр.ФлагНаличияПросрочки И НачалоМесяца(стр.Дата) = НачалоМесяца(ТекущаяДатаСеанса()) Тогда
						Параметр = стр.Дата;
						Прервать;
					КонецЕсли;
				КонецЦикла;			
	...
	Если КодНБКИ = "C18.11" Тогда
		Если ЗначениеЗаполнено(СтруктураОб.ДоговорМКЗ) ИЛИ ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ) Тогда
			ТекущийДоговор =?(ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ), СтруктураОб.РеструктуризацияМКЗ, СтруктураОб.ДоговорМКЗ);
			Параметр = ТекущийДоговор.ДатаПогашения;
		КонецЕсли;
	КонецЕсли;  
	
	Если КодНБКИ = "C18.19" Тогда
		Если ЗначениеЗаполнено(СтруктураОб.ДоговорМКЗ) ИЛИ ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ) Тогда
			ТекущийДоговор =?(ЗначениеЗаполнено(СтруктураОб.РеструктуризацияМКЗ), СтруктураОб.РеструктуризацияМКЗ, СтруктураОб.ДоговорМКЗ);
			Параметр = ТекущийДоговор.ДатаДоговора;
		КонецЕсли;
	КонецЕсли;     
	
	Если КодНБКИ = "C4.11" Тогда 
		Параметр = "3";
	КонецЕсли; 
...
Процедура ЗаписатьСобытиеСущности(ОбъектВх, ДатаСобытия, ИсточникСобытия, КодСобытия) Экспорт 
	Если Константы.ИспользоватьНБКИRUTDF.Получить() = Ложь И Константы.ПередачаДанныхВБКИ.Получить() = Ложь Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьСобытиеСделки(ОбъектВх, ДатаСобытия, ИсточникСобытия, КодСобытия) Экспорт 
	...
	Если КодСобытия <> "2.11.2" и КодСобытия <> "0.0" Тогда
		Если ДатаСобытия < КредитныеИстории.ДатаСобытияПравоприемства(ОбъектВх) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;	
	...
	|	И КредитныеИсторииДанныеСделки.ДатаСобытия <= &ДатаСобытия
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ТипЗнч(ИсточникСобытия) = Тип("ДокументСсылка.СведенияОбИзмененииДоговора") Тогда 
			ДатаПлановогоПрекращенияИзменения = ?(ЗначениеЗаполнено(ИсточникСобытия.ДатаПлановогоПрекращенияИзменения), ИсточникСобытия.ДатаПлановогоПрекращенияИзменения, ИсточникСобытия.ДатаФактическогоПрекращенияИзменения);
			Если ВыборкаДетальныеЗаписи.ДатаОкончанияСделки < ДатаПлановогоПрекращенияИзменения Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
...
Функция ПустыеНастройки() Экспорт 
	...
	Запрос.Текст = 
		...
		|
		|ВЫБРАТЬ
		|	НастройкиВыгрузкиКредитныхИсторий.Наименование
		|ИЗ
		|	Справочник.НастройкиВыгрузкиКредитныхИсторий КАК НастройкиВыгрузкиКредитныхИсторий
		|ГДЕ
		|	НастройкиВыгрузкиКредитныхИсторий.Используется
		|	И НЕ НастройкиВыгрузкиКредитныхИсторий.ПометкаУдаления";
...
Процедура ЗаписатьСобытиеФЛ(ОбъектВх, ДатаСобытия, ИсточникСобытия, КодСобытия) Экспорт
	...
	Запрос.Текст = 
		...
		|	КредитныеИсторииДанныеФизЛица.СведенияОГражданствеФЛ КАК СведенияОГражданствеФЛ,
		|	КредитныеИсторииДанныеФизЛица.ВнесудебноеБанкротство КАК ВнесудебноеБанкротство
		|ИЗ
		|	ДатаПоследнегоСобытия КАК ДатаПоследнегоСобытия
		...
		Для каждого Элемент из МассивСтруктурАдрес Цикл
			Если Элемент.НБКИФЛ = "C8." Тогда
				...
				Если НЕ ЕстьСтроковыеАдреса Тогда
					// Использовать заполнение по дополнительным реквизитам
	...
	Если ФизЛицо = Истина Тогда
		...
		Для каждого Элемент из МассивСтруктурАдрес Цикл
			Если Элемент.НБКИФЛ = "C9." Тогда  
				...
				Если НЕ ЕстьСтроковыеАдреса Тогда
					// Не использовать заполнение по классификатору
					// ЗаполнитьАдресПоКлассификатору(ОбъектВх, Элемент.Значение, НоваяЗапись);
					НоваяЗапись.Записать();
					Прервать;
				Иначе
	...
	Если ФизЛицо = Истина Тогда
		...
		Для Каждого структура Из МассивСтруктурАдрес Цикл   
			...
			Если Структура.НБКИФЛ = "C" + ЦифраБлока + ".6.7" Тогда 
				...
				ОКАТО = Константы.АхантерОКАТО.Получить();
...
Процедура ЗаполнитьАдресПоКлассификатору(ОбъектВх, ЗначениеКлассификатор, ЗаписьРегистраСведений) Экспорт
	...
	Если СтрНайти(ЗначениеКлассификатор, "КонтактнаяИнформация") > 0 Тогда
		...
		ИдентификаторАдресногоОбъекта = АдресныйКлассификаторСлужебный.КонтактнаяИнформацияВСтруктуруJSON(ЗначениеКлассификатор, "");	
		ЗаписьРегистраСведений.НомерАдресаФИАС = ?(ЗначениеЗаполнено(ИдентификаторАдресногоОбъекта.houseId), ИдентификаторАдресногоОбъекта.houseId, ИдентификаторАдресногоОбъекта.id);
		ЗаписьРегистраСведений.НаселенныйПунктОКАТО = АдресныйКлассификатор.КодыАдреса(ЗначениеКлассификатор).ОКАТО;   
...
Процедура ОбработатьСобытиеПоИсточнику(Запись, ИсточникСобытия) Экспорт
	...
	Если ТипДокументаСобытия = Тип("ДокументСсылка.ДоговорМикрозайма") Тогда
		...
		Запись.КодНБКИЦельКредита = ИсточникСобытия.ЦельКредита.КодНБКИ;  
		...
		Запись.МаксимальнаяПроцентнаяСтавкаПоУсловиям = ИсточникСобытия.МаксимальнаяПроцентнаяСтавкаПоУсловиям;
		Запись.ПризнакПредусмотренногоГрафика = ?((ИсточникСобытия.СпособПогашения = Перечисления.СпособыПогашенияЗаймов.Равномерно), "1", "0");
	КонецЕсли;
	...
	Если ТипДокументаСобытия = Тип("ДокументСсылка.СудебноеДело") Тогда
		...
		Если Не ЗначениеЗаполнено(Запись.ДатаРасчетаПросроченнойЗадолженности) Тогда
	...
	Если ТипДокументаСобытия = Тип("ДокументСсылка.СведенияОбИзмененииДоговора") Тогда
		...
		Если ТипЗнч(ИсточникСобытия.ОснованиеПрекращенияОбязательства) = Тип("СправочникСсылка.ДелоОБанкротстве")Тогда
			Запись.Банкротство = ИсточникСобытия.ОснованиеПрекращенияОбязательства;
		КонецЕсли;

		Запись.ДатаРасчетаСрочнойЗадолженности = ?(Запись.ДатаСобытия <= Запись.ДатаОкончанияСделки, Запись.ДатаСобытия, Дата(1,1,1)); 
		Запись.ДатаРасчетаПросроченнойЗадолженности = ?(Запись.ДатаСобытия > Запись.ДатаОкончанияСделки, Запись.ДатаСобытия, Дата(1,1,1));
	КонецЕсли;  
	...
	Если ТипДокументаСобытия = Тип("ДокументСсылка.ВнесудебноеБанкротствоДолжника") ИЛИ ТипДокументаСобытия = Тип("ДокументСсылка.СообщениеБанкротства") Тогда
		Запись.ВнесудебноеБанкротство = ИсточникСобытия;
		Возврат;
	КонецЕсли; 
...
Процедура ЗаполнитьПервичныеПараметрыДоговоровИСубъектов(Параметры, АдресРезультата) Экспорт   
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ДатаИзРеквизита = "";
		НоваяЗапись = РегистрыСведений.КредитныеИсторииДанныеСделки.СоздатьМенеджерЗаписи();
		Если ЗначениеЗаполнено(КодРеквизита) Тогда
		...
		ЗаписьПСК.Сделка = ВыборкаДетальныеЗаписи.Обязательство;
...
Функция ВыбратьПервоеСобытиеСделки(Сделка, Настройка) Экспорт 
	ЭтоЕдиныйФормат =  ТипЗнч(Настройка) = Тип("СправочникСсылка.НастройкиВыгрузкиКредитныхИсторий");
...
Функция СобытиеСубъектаЗафиксированоИсточником(Субъект, Источник, КодСобытие) Экспорт 
	...
	Если ТипЗнч(Источник) = Тип("ДокументСсылка.ВнесудебноеБанкротствоДолжника") Тогда
		Зафиксировано = (НаборЗаписей.Количество() >= Источник.Публикации.Количество());
	КонецЕсли;	
...
Процедура ЗаписатьСобытиеСменаКИ(ОбъектВх, ИмяРегистра, ДатаСобытия, ПараметрыЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.ДатаВыдачиДокумента) Тогда
...
Функция СформироватьТекстЗаголовкаHEADER(Настройка, ИмяФайла, ДатаОтчета, НепринятыйОтчет) Экспорт
	...
	Если КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Настройка.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда
...
Функция СформироватьСтрокуДаты(Дата) Экспорт
	...
	ДатаСтрока = Формат(Дата, "Л=ru; ДФ=ггггММдд_ЧЧммсс");
	...
	Иначе	 
		...
		Если ИзмененныеЗаписи[2] <> 0 Тогда
			Если ИзмененныеЗаписи[2].Количество() > 0 Тогда
				тзПредыдущихЗаписей = ПолучитьПредыдущиеПоказателиЮЛ(ИзмененныеЗаписи[2].ВыгрузитьКолонку("Должник"));
...
Процедура СформироватьКорректирующиеЗаписи(КоличествоОбъектовУчета, КоличествоГруппБлоков, тТелоОтчета, ТитульныеЗаписиДолжника, ТитульныеЗаписиПоручителя, ТитульныеЗаписиНаследника, Настройка, Операция, ДатаОтчета, ДокументОтчет) Экспорт
	...
	Пока Запись.Следующий() Цикл
		...
		ЗанестиЗаписьВЖурналRUTDF(Запись, ДатаОтчета, Операция, ДокументОтчет, Запись.UID, ЗаголовокГрупХедер);	
...
Процедура СформироватьКорректировкуВсейКИ(КоличествоОбъектовУчета, КоличествоГруппБлоков, тТелоОтчета, ТитульныеЗаписиДолжника, Настройка, Операция, ДатаОтчета, ДокументОтчет) Экспорт
	...
	Для каждого Запись из  тчОбъектовДляПравки Цикл
		...
		ЗанестиЗаписьВЖурналRUTDF(Запись, ДатаОтчета, Операция, ДокументОтчет, Запись.Идентификатор, ЗаголовокГрупХедер);	
...
Процедура ДобавитьИзмененияФЛ(КоличествоОбъектовУчета, КоличествоГруппБлоков, тТелоОтчета, ИзмененныеЗаписиФЛ, ПредыдущиеЗаписи, Настройка, Операция, ДатаОтчета, ДокументОтчет) Экспорт  
	...
	Для каждого Запись из ИзмененныеЗаписиФЛ Цикл 
		...
		СформированБлокПредыдущийДУЛ = Ложь;
		...
		Иначе
			...
			Если Запись.Событие.КодНБКИ = "1.9" Тогда 
				...
				Если Запись.Должник.Телефоны.Количество() > 0 Тогда
					...
					ТелНомер = НБКИОбработатьНомерТелефона(ТелНомер);
			...
			КонецЕсли;
		КонецЕсли;
		ЗанестиЗаписьВЖурналRUTDF(Запись, ДатаОтчета, Операция, ДокументОтчет, АЙДИ, ЗаголовокГрупХедер);  
		
		Если  ЗначениеЗаполнено(ФИОПредыдущее) И НЕ СформированБлокПредыдущийДУЛ Тогда
			// НОВОЕ для 6.01 и старше: Если был занесено ФИО, но не занесен ДУЛ - формируется запись склейки - 3.1 С.2   
...
Функция ПолучитьИзмененияПоОбъектамУчета(ТаблицаОбъектов, ДатаОтчета) Экспорт  
	...
	//Лебедева 28.12.2023 - Дату срез взяла строго меньше даты отчета
	...
	|	СобытияИСтатусы.Событие.КодНБКИ КАК СобытиеКодНБКИ,
	...
	|	СобытияИСтатусы.Сделка.Должник КАК Должник
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	СобытияИСтатусы.Сделка.Должник,
	|	Сделка,
	|	ДатаСобытия,
	|	СобытиеКодНБКИ";
	...
	// Вернуть возможность выгрузки в день прекращения обязательства
	ЗапросДанныеСделки.УстановитьПараметр("ДатаОтчета", КонецДня(ДатаОтчета));
	...
	|	СобытияИСтатусы.СведенияОГражданствеФЛ КАК СведенияОГражданствеФЛ,
	|	СобытияИСтатусы.ВнесудебноеБанкротство КАК ВнесудебноеБанкротство
	|ИЗ
	|	СобытияИСтатусы КАК СобытияИСтатусы
	|ГДЕ
	|	СобытияИСтатусы.ДатаПоследнейВыгрузки ЕСТЬ NULL
	...
	|	ЕСТЬNULL(ЖурналВыгрузокНБКИRUTDF.Принято, ""БезСтатуса"") КАК Принято,
	|	ЖурналВыгрузокНБКИRUTDF.ДатаПоследнейВыгрузки КАК ДатаПоследнейВыгрузки,
	|	КредитныеИсторииДанныеЮрЛица.ПризнакИностранногоЮЛ КАК ПризнакИностранногоЮЛ
	|ПОМЕСТИТЬ СобытияИСтатусы
	|ИЗ
	|	РегистрСведений.КредитныеИсторииДанныеЮрЛица КАК КредитныеИсторииДанныеЮрЛица,
	|	РегистрСведений.ЖурналВыгрузокНБКИRUTDF КАК ЖурналВыгрузокНБКИRUTDF
	...
	|	СобытияИСтатусы.Принято КАК Принято,
	|	СобытияИСтатусы.ДатаПоследнейВыгрузки КАК ДатаПоследнейВыгрузки,
	|	СобытияИСтатусы.ПризнакИностранногоЮЛ КАК ПризнакИностранногоЮЛ
	|ИЗ
	|	СобытияИСтатусы КАК СобытияИСтатусы
	|ГДЕ
	|	СобытияИСтатусы.ДатаПоследнейВыгрузки ЕСТЬ NULL
...
Функция ВернутьКодСудебнойДееспособности(ЗначениеРешенияДокумент) Экспорт
...
Функция ВернутьСвязанныйДокументТипа(СсылкаИсточник, Тип) Экспорт
...
Функция ЗаполнитьАдресаДолжникаФЛ(Должник, ПараметрыРегистрации) Экспорт
	...
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать(); 
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		// Формировать блок только по адресу регистрации
...
Процедура ДобавитьИзмененияЮЛ(КоличествоОбъектовУчета, КоличествоГруппБлоков, тТелоОтчета, ИзмененныеЗаписиЮЛ, ПредыдущиеЗаписи, Настройка, Операция, ДатаОтчета, ДокументОтчет) Экспорт  
	...
	Для каждого Запись из ИзмененныеЗаписиЮЛ Цикл
		...
		ИначеЕсли Запись.Событие.КодНБКИ = "1.9"  Тогда 
			...
			Если НЕ КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Настройка.ВерисяRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда 
		...
		ЗанестиЗаписьВЖурналRUTDF(Запись, ДатаОтчета, Операция, ДокументОтчет, Запись.ОГРН + " " + Запись.ИдентификаторLEI, ЗаголовокГрупХедер); 
	КонецЦикла;	   
...
Процедура ДобавитьИзмененияСделки(КоличествоОбъектовУчета, КоличествоГруппБлоков, тТелоОтчета, ИзмененныеЗаписиСделки, ТитульныеЗаписиДолжника, ТитульныеЗаписиПоручителя, ТитульныеЗаписиНаследника, Настройка, Операция, ДатаОтчета, ДокументОтчет) Экспорт 
	...
	ПризнакиПринудительногоВзыскания = ПолучитьПризнакиПринудительногоВзыскания(ИзмененныеЗаписиСделки);
...
Функция ВычислитьНаследника(Сделка, ДокументСобытие) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСобытие) = Тип("ДокументСсылка.ПоступлениеПлатежа") Тогда	
		...
		КонецЕсли;	
	ИначеЕсли ТипЗнч(ДокументСобытие) = Тип("СправочникСсылка.НастройкиВыгрузкиКредитныхИсторий") ИЛИ ТипЗнч(ДокументСобытие) = Тип("СправочникСсылка.НастройкиВыгрузкиНБКИ_RUTDF") Тогда	
		Наследник = Неопределено;	
	Иначе
		Наследник = Неопределено;
		Запрос = Новый Запрос;
...
Процедура ЗанестиЗаписьВЖурналRUTDF(Запись, ДатаОтчета, Операция, ДокументОтчет, Идентификатор, ЗаголовокГрупХедер) Экспорт
	
	Если СтрНайти(ДокументОтчет.Комментарий, "Массовая Выгрузка ") > 0 Тогда
		ЗаписьЖурнала = РегистрыСведений.ВременныйЖурналМассовыхВыгрузокКИ.СоздатьМенеджерЗаписи();
	Иначе
	...
	ЗаписьЖурнала.ЗаголовокGroupHeader = ?(ТипЗнч(ЗаголовокГрупХедер) = Тип("Строка"), ЗаголовокГрупХедер, ЗаголовокГрупХедер.Строка); 
...
Функция СформироватьИнфоДолжникСобытие1_4(ЗаписьДолжник, Префикс, ДопПараметры = Неопределено) Экспорт 
	...
	Если Префикс = "C" Тогда  
			...
			Если ЗаписьДолжник.Должник.Телефоны.Количество() > 0 Тогда
				ИмяСтрокиОтчета = "C10_CONTACTINFO"; 
				ТелНомер = ЗаписьДолжник.Должник.Телефоны[0].Номер;
				ТелНомер = НБКИОбработатьНомерТелефона(ТелНомер);
...
Функция СформироватьБлок17_10(Запись, Префикс, ДопПараметры = Неопределено) Экспорт
	...
	Если КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(ДопПараметры.Настройка.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда
...
Функция СформироватьБлок22_Х(Запись, Префикс, ДопПараметры = Неопределено) Экспорт   
	...
	Иначе	 
		...
		Иначе
			...
			Если НЕ КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(Настройка.ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда 
				ПСКПроцент = стрЗаменить(ПСКПроцент, ",", ".");
			КонецЕсли;	
		КонецЕсли;
...
Функция СформироватьБлок252627_171819(Запись, Префикс, ДопПараметры = Неопределено) Экспорт 
	...
	Если ЭтоПрекращение Тогда 
		МассивСтрок = СформироватьБлок252627_171819Пустое(Префикс, киФорматДаты(ДатаВозникновенияПросроченнойЗадолженности), киФорматДаты(Запись.ДатаСобытия)); 
	...
	Иначе	 
		...
		Иначе
			ЕстьСрочнаяЗадолженность = Истина; 
		КонецЕсли;	
		...
		Если тзЗадолженности <> Неопределено И НЕ ЕстьСрочнаяЗадолженность Тогда  
			Для Каждого Стр из тзЗадолженности Цикл
				Строка2719 = СформироватьБлок27_19(Запись.ДатаСобытия, ИмяСтрокиОтчета, Запись.ДатаОкончанияСделки, ДатаВозникновенияПросроченнойЗадолженности, ДатаРасчета, Стр, ПризнакрасчетаПоПоследнемуПлатежу, Запись.Событие.КодНБКИ);
			КонецЦикла;
		Иначе
			Строка2719 = ИмяСтрокиОтчета;
			ДобавитьКСтроке(Строка2719, киФорматДаты(ДатаВозникновенияПросроченнойЗадолженности));
...
Функция СформироватьБлок25_17(Префикс, ИмяСтрокиОтчета, ИсходнаяСумма, ДатаРасчета, СтрокаЗадолженность, ПризнакрасчетаПоПоследнемуПлатежу, КодСобытия)
	...
	Иначе	
		...
		ДобавитьКСтроке(Строка2517, ?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", ПризнакрасчетаПоПоследнемуПлатежу, ""));
		...
		ДобавитьКСтроке(Строка2517, ?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", киФорматДаты(ДатаРасчета), ""));
		Если Префикс = "C" Тогда
			ДобавитьКСтроке(Строка2517, "0");
		КонецЕсли;	
	КонецЕсли;
...
Функция СформироватьБлок26_18(ИмяСтрокиОтчета, ДатаОбразованияСрочнойЗадолженности, ДатаРасчета, СтрокаЗадолженность, ПризнакрасчетаПоПоследнемуПлатежу, КодСобытия)
	...
	Иначе	
		...
		ДобавитьКСтроке(Строка2618, ?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", ПризнакрасчетаПоПоследнемуПлатежу, ""));
		...
		ДобавитьКСтроке(Строка2618, ?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", киФорматДаты(ДатаРасчета), ""));
	КонецЕсли;
...
Функция СформироватьБлок27_19(ДатаСобытия, ИмяСтрокиОтчета, ДатаОкончанияСделки, ДатаОбразованияПросроченнойЗадолженности, ДатаРасчета, СтрокаЗадолженность, ПризнакрасчетаПоПоследнемуПлатежу, КодСобытия)
	Строка2719 = ИмяСтрокиОтчета; 
	ДатаПросроченногоПлатежа = ?(ДатаОкончанияСделки > ДатаСобытия, ДатаОбразованияПросроченнойЗадолженности, ДатаОкончанияСделки); 
	Если СтрокаЗадолженность.СуммаДООстаток <= 0 Тогда
	...
	Иначе	
		ДобавитьКСтроке(Строка2719, киФорматДаты(ДатаОбразованияПросроченнойЗадолженности));
		ДобавитьКСтроке(Строка2719, ?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", ПризнакрасчетаПоПоследнемуПлатежу, ""));
		...
		ДобавитьКСтроке(Строка2719,	?(КодСобытия = "2.3" ИЛИ КодСобытия = "2.11.2", киФорматДаты(ДатаРасчета), ""));
		ДобавитьКСтроке(Строка2719, киФорматДаты(ДатаПросроченногоПлатежа));
		ДобавитьКСтроке(Строка2719, киФорматДаты(ДатаПросроченногоПлатежа));
	КонецЕсли;
...
Функция СформироватьБлок252627_171819Пустое(Префикс, ДатаПросроченнойЗадолженности, ДатаСобытия)
	...
	ИмяСтрокиОтчета = Префикс + ?(Префикс = "C", "27_PASTDUEARREAR", "B19_PASTDUEARREAR");
	Строка2719 = ИмяСтрокиОтчета;
	ДобавитьКСтроке(Строка2719, ДатаПросроченнойЗадолженности);
	ДобавитьКСтроке(Строка2719, "");
	ДобавитьКСтроке(Строка2719, "0,00");
	ДобавитьКСтроке(Строка2719, "");  
	ДобавитьКСтроке(Строка2719, "");
...
Функция СформироватьБлок28_20(Запись, Префикс, ДопПараметры = Неопределено) Экспорт
	...
	КодСобытия = Запись.Событие.КодНБКИ;
	...
	//ДниПросрочки = ?(КодСобытия = "2.5", "0", ДниПросрочки);
	...
	Если мПлатежи <> Неопределено Тогда 
		Если НЕ ЭтоФактическийПлатеж Тогда
			...
			ДобавитьКСтроке(Строка2820, ?(ЕстьЗадолженность, "3", "2"));
		...
		Иначе	
			...
			ДобавитьКСтроке(Строка2820, ?(ЕстьЗадолженность, "3", "2"));
			ДниПросрочки = ?(Запись.ДатаСобытия = Стр.ДатаПлатежа, ДниПросрочки, Окр((НачалоДня(Стр.ДатаПлатежа) - НачалоДня(Запись.ДатаВыходаНаПросрочку)) / 86400));
			ДниПросрочки = СтрЗаменить(Строка(ДниПросрочки), Символы.НПП, "");  
			//ДниПросрочки = ?(КодСобытия = "2.5", "0", ДниПросрочки);
			ДниПросрочки = ?(ЕстьЗадолженность, ДниПросрочки, "0");
			ДобавитьКСтроке(Строка2820, ДниПросрочки);	
...
Функция СформироватьБлок29(Запись, Префикс, ДопПараметры = Неопределено) Экспорт 
	...
	фТекущаяЗадолженность = СтрЗаменить(Строка(ТекущаяЗадолженность),  Символы.НПП, "");
	Если НЕ ЗначениеЗаполнено(Запись.СуммаСделки) Тогда
		Возврат "";
	КонецЕсли;
	Настройка = ДопПараметры.Настройка;
	Если Префикс = "C" Тогда	
		...
		СуммаСреднемесячногоПлатежа = ?(Запись.Событие.КодНБКИ = "2.5", "0", фТекущаяЗадолженность);
		Строка29 = Строка29 + Символы.Таб + СуммаСреднемесячногоПлатежа + Символы.Таб + киФорматДаты(Запись.ДатаСобытия);
		Возврат Строка29;
...
Функция СформироватьБлок38_29(Запись, Префикс, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Запись.Банкротство <> "0" ИЛИ ТипЗнч(Запись.ДокументСобытие) = Тип("СправочникСсылка.ДелоОБанкротстве") И НЕ Заполнено Тогда
		Если ЗначениеЗаполнено(Запись.Банкротство) Тогда
			Строка3829 = Строка3829 + СформироватьБлок38_29Банкротство(Запись.ДатаСобытия); 
		Иначе        
			Строка3829 = Строка3829 + СформироватьБлок38_29Прекращение(Запись, ПринудительноеВзыскание);
		КонецЕсли;
		Заполнено = Истина;
...
Функция СформироватьБлок38_29Банкротство(ДатаСобытия) Экспорт 
	СтрокаОтчета = ""; 		
	ДобавитьКСтроке(СтрокаОтчета, "16"); 
	ДобавитьКСтроке(СтрокаОтчета, киФорматДаты(ДатаСобытия));	
	Возврат СтрокаОтчета;	

КонецФункции

Функция СформироватьБлок38_29Прекращение(Запись, ПринудительноеВзыскание) Экспорт  
	...
	Если ТипЗнч(Запись.ДокументСобытие) = Тип("ДокументСсылка.ПоступлениеПлатежа") Тогда
		//Установка 1 и 2
		КодПрекращения = ПолучитьКодПрекращенияПоВидуПлательщика(Запись.ДокументСобытие, ПринудительноеВзыскание);
...
Функция ПолучитьКодПрекращенияПоВидуПлательщика(Платеж, ПринудительноеВзыскание) Экспорт 
	
	Если Платеж.ВидПлательщика = Перечисления.ВидыПлательщика.ПФР ИЛИ Платеж.ВидПлательщика = Перечисления.ВидыПлательщика.Работодатель 
		ИЛИ Платеж.ВидПлательщика = Перечисления.ВидыПлательщика.СудебныйПристав Тогда
		КодПрекращения = "2";
	Иначе      
		Если ПринудительноеВзыскание Тогда
			КодПрекращения = "2";
		Иначе
			КодПрекращения = "1";
		КонецЕсли;
...
Функция СформироватьБлок52_42(ИмяСтрокиОтчета, Сделка, Настройка, ДокПродажа) 
	СтрокаОтчета = "";
	ПриобретательПраваТребования = ДокПродажа.ПриобретательПраваТребования;
	Если ЗначениеЗаполнено(ПриобретательПраваТребования) Тогда
		к = ПриобретательПраваТребования.ФИО.Количество() - 1;
		Если к >= 0 Тогда
			ФамилияФЛ = ПриобретательПраваТребования.ФИО[к].Фамилия;
			ИмяФЛ = ПриобретательПраваТребования.ФИО[к].Имя;
			ОтчествоФЛ = ПриобретательПраваТребования.ФИО[к].Отчество;
		Иначе
			ФИО = ПриобретательПраваТребования.Наименование;
			ФИО = СтрЗаменить(ФИО, " ", " ");
			ФамилияФЛ = Лев(ФИО, СтрНайти(ФИО, " ") - 1);
			ИмяФЛ = Сред(ФИО, СтрНайти(ФИО, " ") + 1, СтрНайти(ФИО, " ", ,,2) - СтрНайти(ФИО, " ") );
			ОтчествоФЛ = Сред(ФИО, СтрНайти(ФИО, " ", ,,2)+1, стрДлина(ФИО) - СтрНайти(ФИО, " ", ,,2) + 1);
		КонецЕсли;
		
		СтрокаОтчета = ИмяСтрокиОтчета + Символы.Таб + ФамилияФЛ + Символы.Таб + ИмяФЛ + Символы.Таб + ОтчествоФЛ;								
		СтрокаОтчета = СтрокаОтчета + СформироватьБлокПоНастройкам(ПриобретательПраваТребования, Настройка, ИмяСтрокиОтчета, "ПриобретательПраваТребования");
		СтрокаОтчета = СокрЛП(СтрокаОтчета);
		ДобавитьКСтроке(СтрокаОтчета, киФорматДаты(ДокПродажа.Дата));
	КонецЕсли;
	Возврат СтрокаОтчета; 
...
Функция СформироватьБлок56_46(Запись, Префикс, ДопПараметры = Неопределено) Экспорт
	...
	ТипОтношенияКСчету = "1";
	Если ДопПараметры.Поручитель <> "1" Тогда
		ТипОтношенияКСчету  =  ДопПараметры.Поручитель;
	КонецЕсли;	
	
	Если ДопПараметры.Наследник <> "1" Тогда
		ТипОтношенияКСчету  =  ДопПараметры.Наследник;
	КонецЕсли;	 
	
  	ДобавитьКСтроке(СтрокаОтчета, ТипОтношенияКСчету);
	ДобавитьКСтроке(СтрокаОтчета, Запись.КодНБКИВидКредита);
	ДобавитьКСтроке(СтрокаОтчета, Запись.UID); 
	ДобавитьКСтроке(СтрокаОтчета, киФорматДаты(Запись.ДатаПередачиФинансирования)); 
	...
	ДобавитьКСтроке(СтрокаОтчета, ?(ЕстьБлокПрекращение, "0", Просрочка90плюс));
	ДобавитьКСтроке(СтрокаОтчета, ?(ЕстьБлокПрекращение, "1", ПризнакПрекращения));
...
Функция ВернутьПоследнийПлатежПоОбъекту(ОбъектУчета, ДатаРасчета, РеквизитПризнакНаличияРанееОсуществленныхПлатежей, ФормироватьПринудительныйПлатеж, ВерсияRUTDF) Экспорт 
	...
	|	И ЗадолженностьПоОбъектам.Объект = &ОбъектВх
	...
	|	И РасчетыПоДоговорам.Займ = &ОбъектВх
	...
	|	И ЗадолженностьПоОбъектамВнесудебная.Объект = &ОбъектВх
	...
	|	И ЗадолженностьПоСудебнымРешениям.Займ = &ОбъектВх
	|	И ЗадолженностьПоСудебнымРешениям.Период <= &ДатаПлатежа
	...
	Иначе 
		Если КредитныеИсторииИзменения.ЭтоСтаршаяВерсияРУТДФ(ВерсияRUTDF, Перечисления.ВерсияRUTDF.v4) Тогда	
			Возврат Неопределено; 
		Иначе
...
Функция ПолучитьНепринятыеЗаписиПоОбъектамУчета(ТаблицаОбъектов, НепринятыйДокумент) Экспорт  
	...
	|	КредитныеИсторииДанныеСделки.МаксимальнаяПроцентнаяСтавкаПоУсловиям КАК МаксимальнаяПроцентнаяСтавкаПоУсловиям,
	|	КредитныеИсторииДанныеСделки.ПризнакПредусмотренногоГрафика КАК ПризнакПредусмотренногоГрафика,
	|	КредитныеИсторииДанныеСделки.Событие.КодНБКИ КАК СобытиеКодНБКИ
	|ИЗ
	...
	|	КредитныеИсторииДанныеФизЛица.СпециальныйНалоговыйРежим КАК СпециальныйНалоговыйРежим,
	|	КредитныеИсторииДанныеФизЛица.Событие.КодНБКИ КАК СобытиеКодНБКИ
	|ИЗ
	...
	|	КредитныеИсторииДанныеЮрЛица.ОГРНПравопредшественника КАК ОГРНПравопредшественника,
	|	НепринятыеЗаписи.ДатаПоследнейВыгрузки КАК ДатаПоследнейВыгрузки,
	|	КредитныеИсторииДанныеЮрЛица.Событие.КодНБКИ КАК СобытиеКодНБКИ
	|ИЗ
	|	НепринятыеЗаписи КАК НепринятыеЗаписи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КредитныеИсторииДанныеЮрЛица КАК КредитныеИсторииДанныеЮрЛица
...
Процедура НБКИ_ЗаписатьОбязательствоСубъектаПрекратилосьОбработчик(Объект, ДатаСобытия, ДокументРегистратор, ТекущаяСтадия) Экспорт 
	Если Не Константы.ИспользоватьНБКИRUTDF.Получить() И Константы.ПередачаДанныхВБКИ.Получить() = Ложь Тогда
		Возврат;
	КонецЕсли;
...
Процедура ФиксированиеОбязательнойВыгрузки(ОбъектыОтчета, ДатаОтчета) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.КредитныеИсторииДанныеСделки КАК КредитныеИсторииДанныеСделки
		|ГДЕ
		|	КредитныеИсторииДанныеСделки.Событие.КодНБКИ <> ""2.6""
		|
		|СГРУППИРОВАТЬ ПО
		|	КредитныеИсторииДанныеСделки.Сделка
		|
...
Функция ПроверкаФорматаКодаПодразделения(СтрокаВх) Экспорт
	Если ЗначениеЗаполнено(СтрокаВх) И СтрНайти(СтрокаВх, "-") = 0 Тогда
		Пока СтрНайти(СтрокаВх, "  ") > 0 Цикл
			СтрокаВх = СтрЗаменить(СтрокаВх, "  ", " ");
		КонецЦикла;
...
Функция ВосстановитьКредитнуюИсторию(Сделка) Экспорт 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ Первые 1
	|	НастройкиВыгрузкиКредитныхИсторий.Ссылка КАК Настройка
	|ИЗ
	|	Справочник.НастройкиВыгрузкиКредитныхИсторий КАК НастройкиВыгрузкиКредитныхИсторий
	|ГДЕ
	|	НастройкиВыгрузкиКредитныхИсторий.Используется
	...
	|
	|ВЫБРАТЬ Первые 1
	|	НастройкиВыгрузкиНБКИ_RUTDF.Ссылка
	|ИЗ
	|	Справочник.НастройкиВыгрузкиНБКИ_RUTDF КАК НастройкиВыгрузкиНБКИ_RUTDF
	|ГДЕ
	|	НЕ НастройкиВыгрузкиНБКИ_RUTDF.ПометкаУдаления
	...
	КредитныеИсторииИзменения.ВосстановитьФактическиеСобытия(Сделка, Настройка);
	
	ЗаписатьПлатежи(Сделка);	
	
	Возврат РезультатЗаполнения;
	
КонецФункции

Процедура ЗаписатьПлатежи(Сделка) Экспорт 
	...
	|	ДанныеДокументов.ДатаСобытия КАК ДатаСобытия
	|ИЗ
	|	ДанныеДокументов КАК ДанныеДокументов
	|ГДЕ
	|	ДанныеДокументов.ДатаСобытия >= &ДатаСобытия
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаСобытия"; 
	
	
	ДатаРутдФ = КонецДня(Дата("20221031"));
	Запрос.УстановитьПараметр("Объект", Сделка);
	Запрос.УстановитьПараметр("ДатаСобытия", ДатаРутДФ);
	РезультатЗапроса = Запрос.Выполнить(); 
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выгрузить(); 
		
	Сч = 0;
	
	ДатаПредыдущийПлатеж = ДатаРутдФ; 
	
	Для Каждого стр из ВыборкаДетальныеЗаписи Цикл 
		
		
		Если ДатаПредыдущийПлатеж = стр.ДатаСобытия Тогда
			ДатаПредыдущийПлатеж = стр.ДатаСобытия;  
			Сч = Сч + 1;
			Продолжить;
		КонецЕсли;	
		КредитныеИстории.ЗаписатьСобытиеСделки(Сделка, стр.ДатаСобытия, стр.Ссылка, "2.3");
		ДатаПредыдущийПлатеж = стр.ДатаСобытия;
		
		Если Сч = ВыборкаДетальныеЗаписи.Количество() - 1 Тогда 
			Если ОбъектыСервер.ОпределитьВариантУчетаЗадолженностиПоРегиструСтадий(Сделка, НачалоДня(ТекущаяДатаСеанса())) = Перечисления.СтадииЗадолженности.Пустая Тогда
				КредитныеИстории.ЗаписатьСобытиеСделки(Сделка, стр.ДатаСобытия, стр.Ссылка, "2.5");
			КонецЕсли;	
		КонецЕсли;	 
...
Функция ФиксироватьИзмененияПриСписаниях() Экспорт
	Фиксировать = Истина;
	// На текущий момент морально устарела, всегда надо платежем писать 2 события
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция Фиксировать2_3ПриПолномПогашении() Экспорт 
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	НастройкиВыгрузкиНБКИ_RUTDF.Ссылка УБЫВ";
...
Функция ПолучитьВерсиюRUTDF(ПеречислениеВерсия)
	Если Не ЗначениеЗаполнено(ПеречислениеВерсия) Тогда
		ПеречислениеВерсия = Перечисления.ВерсияRUTDF.v6_03;
	КонецЕсли;	
...
Функция НБКИОбработатьНомерТелефона(ТелНомер) Экспорт
	
	//+A(BBB)CCCCC» или «ABBBCCCCC»,
	
	ТелНомер = СтрЗаменить(ТелНомер, " ", ""); 
	МинимальноЗнаков = 8;
	
	Если СтрНайти(ТелНомер, "(", НаправлениеПоиска.СНачала) = 1 Тогда 
		 ТелНомер = "+7" + ТелНомер;
	 КонецЕсли;
	 
	 Если СтрНайти(ТелНомер, "(") > 0 И СтрНайти(ТелНомер, "+") = 0 Тогда 
		 ТелНомер = "+" + ТелНомер;  
	 КонецЕсли;  
	 ...
	 Если СтрДлина(ТелНомер) < МинимальноЗнаков Тогда 
		  Возврат "";
	КонецЕсли;	  
...
Функция ВернутьКодыСобытийсРасчетомЗадолженности() Экспорт    
	Массив = Новый Массив;  
	Массив.Добавить("2.1");
	Массив.Добавить("2.2");
	Массив.Добавить("2.3");  
	Массив.Добавить("2.5");
	Массив.Добавить("2.11.2");
	Массив.Добавить("2.11.1");

	Возврат Массив;
КонецФункции    


Функция ДатаСобытияПравоприемства(Сделка) Экспорт
	
	НаборЗаписей = РегистрыСведений.КредитныеИсторииДанныеСделки.СоздатьНаборЗаписей();
	СобытиеПравоприемства = Справочники.КредитныеИсторииСобытияСделки.НайтиПоРеквизиту("КодНБКИ", "2.11.2");
	НаборЗаписей.Отбор.Сделка.Установить(Сделка); 
	НаборЗаписей.Отбор.Событие.Установить(СобытиеПравоприемства);	
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() > 0 Тогда
		ДатаПравоприемства = НаборЗаписей[0].ДатаСобытия;
	Иначе
		СобытиеСтарта = Справочники.КредитныеИсторииСобытияСделки.НайтиПоРеквизиту("КодНБКИ", "0.0");
		НаборЗаписей.Отбор.Сделка.Установить(Сделка); 
		НаборЗаписей.Отбор.Событие.Установить(СобытиеСтарта);	
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() > 0 Тогда 
			ДатаПравоприемства = НаборЗаписей[0].ДатаСобытия;
		Иначе
			ДатаПравоприемства = Дата(1, 1, 1);
		КонецЕсли;
	КонецЕсли;
	
	Возврат ДатаПравоприемства;
КонецФункции


Процедура ЗаполнитьСтруктуруАдресаИзНормализации(Параметры, АдресРезультата) Экспорт
	
	тзДанныеНомализации = Параметры.МасстзДанныеНомализации;
	ВидАдреса = Параметры.ВидАдреса;
	Перезаписывать = Параметры.Перезаписывать;
	
	Если тзДанныеНомализации.Количество() = Неопределено Тогда
		Возврат;
	КонецЕсли;

	
	Если тзДанныеНомализации.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	
	Если Параметры.ТипСервиса = "ПочтаРФ" Тогда
		ЗаполнитьКИДляПочтыРФ(тзДанныеНомализации, ВидАдреса, Перезаписывать);
	ИначеЕсли Параметры.ТипСервиса = "Ahunter" Тогда
		ЗаполнитьКИДляAhunter(тзДанныеНомализации, ВидАдреса, Перезаписывать);
	КонецЕсли;
	
КонецПроцедуры    

Процедура ЗаполнитьКИДляПочтыРФ(тзДанныеНомализации, ВидАдреса, Перезаписывать)

	НаборЗаписей = РегистрыСведений.КредитныеИсторииСтруктураАдреса.СоздатьНаборЗаписей();
	
	ПрогрессВыполнения = 1;
	ВсегоЗаписей = тзДанныеНомализации.Количество(); 
	  	
	Для Каждого РезультатНормализации Из тзДанныеНомализации Цикл
		Если НЕ МодульНормализацияПочтаРФ.ПроверитьРезультатПоКодамКачества(РезультатНормализации.ДополнительныеПараметрыСервиса) Тогда
			Продолжить;
		КонецЕсли;	
		НаборЗаписей.Отбор.ВидАдреса.Установить(ВидАдреса); 
		НаборЗаписей.Отбор.Должник.Установить(РезультатНормализации.Должник);
		НаборЗаписей.Прочитать(); 
		ЭтоНоваяЗапись = Ложь;
		Если НаборЗаписей.Количество() > 0 Тогда
			ЗаписьСтруктура = НаборЗаписей[0];
		Иначе 
			ЗаписьСтруктура = НаборЗаписей.Добавить(); 
			ЭтоНоваяЗапись = Истина;
		КонецЕсли;  
		
		Если НЕ Перезаписывать И НЕ ЭтоНоваяЗапись Тогда
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.КодСтраныОКСМ) Тогда
				ЗаписьСтруктура.КодСтраныОКСМ = РезультатНормализации.КодСтраныОКСМ;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НаименованиеИнойСтраны) Тогда
				ЗаписьСтруктура.НаименованиеИнойСтраны = РезультатНормализации.Страна;
			КонецЕсли;	

			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Индекс) Тогда
				ЗаписьСтруктура.Индекс = РезультатНормализации.Индекс;
			КонецЕсли;	

			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НаселенныйПунктНазвание) Тогда
				ЗаписьСтруктура.НаселенныйПунктНазвание = РезультатНормализации.НаселенныйПункт;
			КонецЕсли;	

			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НаселенныйПунктОКАТО) И ЗначениеЗаполнено(РезультатНормализации.Индекс) Тогда
				ЗаписьСтруктура.НаселенныйПунктОКАТО = ФЗ_230.ПолучитьОКАТОПоИндексуФИАС(РезультатНормализации.Индекс);
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Улица) Тогда
				ЗаписьСтруктура.Улица = РезультатНормализации.Улица;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Дом) Тогда
				ЗаписьСтруктура.Дом = РезультатНормализации.Дом;
			КонецЕсли;

            Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Владение) Тогда
				ЗаписьСтруктура.Владение = РезультатНормализации.Владение;
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Корпус) Тогда
				ЗаписьСтруктура.Корпус = РезультатНормализации.Корпус;
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Строение) Тогда
				ЗаписьСтруктура.Строение = РезультатНормализации.Строение;
			КонецЕсли;  
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.КвартираПомещениеОфис) Тогда
				ЗаписьСтруктура.Строение = РезультатНормализации.Квартира;
			КонецЕсли;
			
		Иначе  
			ЗаполнитьЗначенияСвойств(ЗаписьСтруктура, РезультатНормализации);
			Если ЗначениеЗаполнено(РезультатНормализации.Индекс) Тогда 
				ЗаписьСтруктура.НаселенныйПунктОКАТО = ФЗ_230.ПолучитьОКАТОПоИндексуФИАС(РезультатНормализации.Индекс);   
			КонецЕсли;
			
			Если ЗначениеЗаполнено(РезультатНормализации.НаселенныйПункт) Тогда  
				ЗаписьСтруктура.НаселенныйПунктНазвание = РезультатНормализации.НаселенныйПункт;   
			КонецЕсли;	
						
			Если ЗначениеЗаполнено(РезультатНормализации.Квартира) Тогда  
				ЗаписьСтруктура.КвартираПомещениеОфис = РезультатНормализации.Квартира;  
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(РезультатНормализации.Комната) Тогда  
				ЗаписьСтруктура.КвартираПомещениеОфис = РезультатНормализации.Комната;  
			КонецЕсли; 
       		
			Если ЗначениеЗаполнено(РезультатНормализации.Страна) Тогда  
				ЗаписьСтруктура.НаименованиеИнойСтраны = РезультатНормализации.Страна;   
			КонецЕсли; 
			
		КонецЕсли;  
		
		ЗаписьСтруктура.ВидАдреса = ВидАдреса;  
		НаборЗаписей.Записать();
		
		ПроцентВыполнения = (ПрогрессВыполнения / ВсегоЗаписей)*100;
		ПроцентВыполнения = Окр(ПроцентВыполнения,0);    
		
		ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения, СокрЛП(РезультатНормализации.Должник));
...
Процедура ЗаполнитьКИДляAhunter(тзДанныеНомализации, ВидАдреса, Перезаписывать)
	...
	Для Каждого РезультатНормализации Из тзДанныеНомализации Цикл
		...
		Если НЕ РезультатНормализации.Свойство("FiasQuality") Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Для записи КИ включите настройку Получить Quality и перезагрузите таблицу";
			Сообщение.Поле = "Quality";
			Сообщение.Сообщить();	
			Возврат;
		КонецЕсли;
		
		ПараметрКачества = ?(РезультатНормализации.Свойство("FiasQuality"), РезультатНормализации.FiasQuality, Ложь);

		Если НЕ МодульНормализацияAhunter.ПроверитьКачествоАдреса(РезультатНормализации.Страна, ПараметрКачества) Тогда
			Продолжить;
		КонецЕсли;	
		НаборЗаписей.Отбор.ВидАдреса.Установить(ВидАдреса); 
		НаборЗаписей.Отбор.Должник.Установить(РезультатНормализации.Договор.Должник);
		НаборЗаписей.Прочитать(); 
		ЭтоНоваяЗапись = Ложь;
		Если НаборЗаписей.Количество() > 0 Тогда
			ЗаписьСтруктура = НаборЗаписей[0];
		...
		Если НЕ Перезаписывать И НЕ ЭтоНоваяЗапись Тогда
			...
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.КодСтраныОКСМ) Тогда
				ЗаписьСтруктура.КодСтраныОКСМ = РезультатНормализации.СтранаКод;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Индекс) Тогда
				ЗаписьСтруктура.Индекс = РезультатНормализации.Zip;
			КонецЕсли;	
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НаселенныйПунктНазвание) Тогда
				ЗаписьСтруктура.НаселенныйПунктНазвание = РезультатНормализации.City;
			КонецЕсли;	
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НаселенныйПунктОКАТО) Тогда
				ЗаписьСтруктура.НаселенныйПунктОКАТО = РезультатНормализации.Okato;
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Улица) Тогда		
				Если ЗначениеЗаполнено(РезультатНормализации.Street) Тогда
					ЗаписьСтруктура.Улица = РезультатНормализации.Street;
				Иначе
					ЗаписьСтруктура.Улица = РезультатНормализации.Site;
				КонецЕсли;	
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Дом) Тогда
				ЗаписьСтруктура.Дом = РезультатНормализации.House;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Владение) Тогда
				ЗаписьСтруктура.Владение = РезультатНормализации.House;
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Корпус) Тогда
				ЗаписьСтруктура.Корпус = РезультатНормализации.Building;
			КонецЕсли; 
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.Строение) Тогда
				ЗаписьСтруктура.Строение = РезультатНормализации.Structure;
			КонецЕсли;  
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.КвартираПомещениеОфис) Тогда
				ЗаписьСтруктура.КвартираПомещениеОфис = РезультатНормализации.Flat;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаписьСтруктура.НомерАдресаФИАС) Тогда	
				fias_flat_заполнено = Ложь;
				Если РезультатНормализации.Свойство("fias_flat") Тогда
					Если ЗначениеЗаполнено(РезультатНормализации.fias_flat) Тогда
						ЗаписьСтруктура.НомерАдресаФИАС = РезультатНормализации.fias_flat;
						fias_flat_заполнено = Истина;
					КонецЕсли;
				КонецЕсли;
				
				Если РезультатНормализации.Свойство("fias_house") И НЕ fias_flat_заполнено Тогда
					Если ЗначениеЗаполнено(РезультатНормализации.fias_house) Тогда
						ЗаписьСтруктура.НомерАдресаФИАС = РезультатНормализации.fias_house;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;	
			
		Иначе  
			Если ЗначениеЗаполнено(РезультатНормализации.Okato) Тогда 
				ЗаписьСтруктура.НаселенныйПунктОКАТО = РезультатНормализации.Okato;   
			КонецЕсли;
			
			Если ЗначениеЗаполнено(РезультатНормализации.City) Тогда  
				ЗаписьСтруктура.НаселенныйПунктНазвание = РезультатНормализации.City;   
			КонецЕсли;	
			
			Если ЗначениеЗаполнено(РезультатНормализации.Flat) Тогда  
				ЗаписьСтруктура.КвартираПомещениеОфис = РезультатНормализации.Flat;  
			КонецЕсли; 
			...
			Если ЗначениеЗаполнено(РезультатНормализации.СтранаКод) Тогда
				ЗаписьСтруктура.КодСтраныОКСМ = РезультатНормализации.СтранаКод;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(РезультатНормализации.Zip) Тогда
				ЗаписьСтруктура.Индекс = РезультатНормализации.Zip;
			КонецЕсли;	
			...
			Если ЗначениеЗаполнено(РезультатНормализации.House) Тогда
				
				Если СтрНайти(РезультатНормализации.House, "дом") <> 0 Тогда		
					ЗаписьСтруктура.Дом = РезультатНормализации.House;
				КонецЕсли;
				
				Если СтрНайти(РезультатНормализации.House, "владение") <> 0 Тогда
					ЗаписьСтруктура.Владение = РезультатНормализации.House;
				КонецЕсли;
				
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(РезультатНормализации.Building) Тогда
				ЗаписьСтруктура.Корпус = РезультатНормализации.Building;
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(РезультатНормализации.Structure) Тогда
				ЗаписьСтруктура.Строение = РезультатНормализации.Structure;
			КонецЕсли;  
		...
		ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения, СокрЛП(РезультатНормализации.Договор.ВладелецДолжник));
...
Функция ПолучитьПризнакиПринудительногоВзыскания(ЗаписиСделки) Экспорт    
	...
	ПризнакиПринудительногоВзыскания = Новый ТаблицаЗначений;	
	
	Запрос = Новый Запрос;	
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОбъектыУчета.Сделка КАК Сделка
		|ПОМЕСТИТЬ Сделки
		|ИЗ
		|	&Сделка КАК ОбъектыУчета
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Сделка
		|;
		...
		|	ФССП_ИсполнительноеПроизводство.НомерИД.Владелец КАК Сделка
		|ПОМЕСТИТЬ ВозбужденныеИП
		|ИЗ
		|	Справочник.ФССП_ИсполнительноеПроизводство КАК ФССП_ИсполнительноеПроизводство
		|ГДЕ
		|	ФССП_ИсполнительноеПроизводство.НомерИД.Владелец В(&Владелец)
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПоступлениеПлатежа.ВидПлательщика КАК ВидПлательщика,
		|	ПоступлениеПлатежа.Займ КАК Займ
		|ПОМЕСТИТЬ ПринудительныеПлатежи
		|ИЗ
		|	Документ.ПоступлениеПлатежа КАК ПоступлениеПлатежа
		|ГДЕ
		|	ПоступлениеПлатежа.Займ В(&Займ)
		|	И НЕ ПоступлениеПлатежа.ПометкаУдаления
		|	И ПоступлениеПлатежа.Проведен
		|	И ПоступлениеПлатежа.ВидПлательщика В(&ВидПлательщика)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Займ
		|;
		...
		|	ВозбужденныеИП.Ссылка КАК СуществующееИП,
		|	ПринудительныеПлатежи.ВидПлательщика КАК ВидПлательщика
		|ПОМЕСТИТЬ ВычислениеПризнака
		|ИЗ
		|	Сделки КАК Сделки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВозбужденныеИП КАК ВозбужденныеИП
		|		ПО Сделки.Сделка = ВозбужденныеИП.Сделка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПринудительныеПлатежи КАК ПринудительныеПлатежи
		|		ПО Сделки.Сделка = ПринудительныеПлатежи.Займ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВычислениеПризнака.Сделка КАК Сделка,
		|	ВЫБОР
		|		КОГДА ВычислениеПризнака.ВидПлательщика ЕСТЬ NULL
		|				И ВычислениеПризнака.СуществующееИП ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК ПризнакПринудительногоВзыскания
		|ИЗ
		|	ВычислениеПризнака КАК ВычислениеПризнака"; 
	
	МассивСделок = ЗаписиСделки.ВыгрузитьКолонку("Сделка");
		
	тКопия = Новый ТаблицаЗначений;
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.ДолговыеОбязательства"));
	тКопия.Колонки.Добавить("Сделка", Новый ОписаниеТипов(Массив));
	
	Для каждого Эл из МассивСделок Цикл 
		нстр = тКопия.Добавить();
		нстр.Сделка = Эл;
	КонецЦикла;	
	
	Запрос.УстановитьПараметр("Займ", МассивСделок); 
	Запрос.УстановитьПараметр("Владелец", МассивСделок);
	Запрос.УстановитьПараметр("Сделка", тКопия);  
	
	МассивВидовПлательщиковПринудительное = Новый Массив;
	МассивВидовПлательщиковПринудительное.Добавить(Перечисления.ВидыПлательщика.СудебныйПристав);
	МассивВидовПлательщиковПринудительное.Добавить(Перечисления.ВидыПлательщика.ПФР);
	МассивВидовПлательщиковПринудительное.Добавить(Перечисления.ВидыПлательщика.Работодатель);
    Запрос.УстановитьПараметр("ВидПлательщика", МассивВидовПлательщиковПринудительное);
	
	ПризнакиПринудительногоВзыскания = Запрос.Выполнить().Выгрузить();
	
	
	Возврат ПризнакиПринудительногоВзыскания;
	
КонецФункции

Процедура ЗаполнитьКодыИсточника() 
		
	МассивКодов = Новый Массив;
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","1");  
	СтруктураКодов.Вставить("Наименование", "Заимодавец (кредитор) – кредитная организация");
	МассивКодов.Добавить(СтруктураКодов); 
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","2"); 
	СтруктураКодов.Вставить("Наименование","Заимодавец – микрофинансовая организация");
	МассивКодов.Добавить(СтруктураКодов);
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","3");  
	СтруктураКодов.Вставить("Наименование","Заимодавец – кредитный потребительский кооператив");
	МассивКодов.Добавить(СтруктураКодов);    
	
		
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","5");  
	СтруктураКодов.Вставить("Наименование","Заимодавец-ломбард");
	МассивКодов.Добавить(СтруктураКодов); 
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","6");  
	СтруктураКодов.Вставить("Наименование","Заимодавец – иная организация");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","7");  
	СтруктураКодов.Вставить("Наименование","Организация-лизингодатель");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","8");  
	СтруктураКодов.Вставить("Наименование","Оператор инвестиционной платформы");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","9");  
	СтруктураКодов.Вставить("Наименование","Организация-гарант");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","10");  
	СтруктураКодов.Вставить("Наименование","Организация – взыскатель платы за жилое помещение, коммунальные услуги или услуги связи");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","11");  
	СтруктураКодов.Вставить("Наименование","Федеральный орган исполнительной власти, уполномоченный на осуществление функций по обеспечению установленного порядка деятельности судов и исполнению судебных актов и актов других органов");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","12");  
	СтруктураКодов.Вставить("Наименование","Арбитражный управляющий");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","13");                      
	СтруктураКодов.Вставить("Наименование","Ликвидационная комиссия или ликвидатор");
	МассивКодов.Добавить(СтруктураКодов); 

	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","14");  
	СтруктураКодов.Вставить("Наименование","Специализированное финансовое общество");
	МассивКодов.Добавить(СтруктураКодов);  
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","15");  
	СтруктураКодов.Вставить("Наименование","Ипотечный агент");
	МассивКодов.Добавить(СтруктураКодов);  
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","16");  
	СтруктураКодов.Вставить("Наименование","Кредитный управляющий синдиката кредиторов");
	МассивКодов.Добавить(СтруктураКодов);
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","17");  
	СтруктураКодов.Вставить("Наименование","Лицо, осуществляющее деятельность по возврату просроченной задолженности (коллекторская организация)");
	МассивКодов.Добавить(СтруктураКодов);
	
	СтруктураКодов = Новый Структура("КодНБКИ","Наименование");
	СтруктураКодов.Вставить("КодНБКИ","99");  
	СтруктураКодов.Вставить("Наименование","Иное лицо");
	МассивКодов.Добавить(СтруктураКодов);
	
			
	Для каждого эл из МассивКодов Цикл
		СуществующееСобытие = Справочники.БКИ_КодыИсточника.НайтиПоРеквизиту("КодНБКИ", эл.КодНБКИ);
		Если Не ЗначениеЗаполнено(СуществующееСобытие) Тогда
			
			НовСобытие = Справочники.БКИ_КодыИсточника.СоздатьЭлемент();
			НовСобытие.КодНБКИ = эл.КодНБКИ;
			НовСобытие.Наименование = эл.Наименование;
			НовСобытие.Записать(); 
		КонецЕсли;
