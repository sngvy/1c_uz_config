...
Процедура ПолучитьСведения(Должник, Код) Экспорт
	...
	Попытка
		...
		// Удалить старые дела по должнику
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		ИначеЕсли Данные.pageData.Количество() > 0 Тогда			
			Сч = 0;
			Для Сч = 0 По Данные.pageData.Количество() Цикл
				Попытка
					...
					Если ДатаРождения = ОбъектыСервер.ПолучитьЗначениеСвойства(Должник, "0043") Тогда
						...
						Исключение
							УИДЗапроса = Новый УникальныйИдентификатор();
							Дело = "Заявление о банкротстве возвращено гражданину " + "(" + УИДЗапроса + ")";
							//ДО = Справочники.ДолговыеОбязательства.НайтиПоРеквизиту("Должник", Справочники.Контрагенты.НайтиПоКоду(Код));
							//ОбъектыСервер.ЗаписатьЗначениеСвойства(ДО, "0181", Справочники.тсЗначенияСвойствОбъектов.ПустаяСсылка());
							//ОбъектыСервер.ЗаписатьЗначениеСвойства(ДО, "0182", Справочники.тсЗначенияСвойствОбъектов.НайтиПоКоду("000000029"));
						КонецПопытки;
						Попытка
						...
						// Записать дело в регистр сведений
						МенеджерЗаписи = РегистрыСведений.ДелаОБанкротстве.СоздатьМенеджерЗаписи();
						...
						ДО = Справочники.ДолговыеОбязательства.НайтиПоРеквизиту("Должник", Справочники.Контрагенты.НайтиПоКоду(Код));
						Если Данные.pageData[Сч].lastLegalCase.status.description = "Производство по делу завершено" Тогда
						...
						ИначеЕсли Данные.pageData[Сч].lastLegalCase.status.description <> "Производство по делу завершено" Тогда 
