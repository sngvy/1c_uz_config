...
Процедура ПоместитьМероприятиеВСтроку(НомерСтроки, НомерКолонки, Мероприятие, //остальные параметры последствие переноса
	...
	//++КазанцевЯА-БП
	ИначеЕсли КонтрольСроков И ЗначениеЗаполнено(Мероприятие.СрокВыполнения) И Мероприятие.СрокВыполнения < ТекущаяДата() Тогда
		ЖирныйШрифт = Новый Шрифт(Ячейка.Шрифт,,,Истина);
		Ячейка.Шрифт = ЖирныйШрифт;
		Ячейка.ЦветФона = WebЦвета.Красный;
	ИначеЕсли КонтрольСроков И ЗначениеЗаполнено(Мероприятие.СрокВыполнения) И НачалоДня(Мероприятие.СрокВыполнения) < ТекущаяДата() Тогда
		ЖирныйШрифт = Новый Шрифт(Ячейка.Шрифт,,,Истина);
		Ячейка.Шрифт = ЖирныйШрифт;
		Ячейка.ЦветФона = WebЦвета.ГолубойСКраснымОттенком;
	ИначеЕсли НЕ КонтрольСроков И (НачалоДня(Мероприятие.ПланируемаяДата) + (Мероприятие.ПланируемоеВремя - Дата(1,1,1)) < ТекущаяДата() - 10
				И Мероприятие.ПланируемоеВремя <> Дата(1,1,1))
			ИЛИ НЕ КонтрольСроков И Мероприятие.ПланируемаяДата < НачалоДня(ТекущаяДата()) Тогда
		Ячейка.ЦветФона = WebЦвета.Красный;
	//--КазанцевЯА-БП
	Иначе
...
Процедура ЗаполнитьТДДень(ТабличныйДокумент, Объект, СписокВремен, Расписание, КоличествоКолонок, // ЗАЩИЩЕНА!
	...
	Для Каждого Элемент Из СписокМероприятий Цикл
		...
		//++КазанцевЯА-БП
		КонтрольСроков = Константы.КонтрольСроковМероприятий.Получить();
		ИначеЕсли КонтрольСроков И ЗначениеЗаполнено(Элемент.Мероприятие.СрокВыполнения) И Элемент.Мероприятие.СрокВыполнения < ТекущаяДата() Тогда
			ЖирныйШрифт = Новый Шрифт(Область.Шрифт,,,Истина);
			Область.Шрифт = ЖирныйШрифт;
			Область.ЦветФона = WebЦвета.Красный;
		ИначеЕсли КонтрольСроков И ЗначениеЗаполнено(Элемент.Мероприятие.СрокВыполнения) И НачалоДня(Элемент.Мероприятие.СрокВыполнения) < ТекущаяДата() Тогда
			ЖирныйШрифт = Новый Шрифт(Область.Шрифт,,,Истина);
			Область.Шрифт = ЖирныйШрифт;
		...
		ИначеЕсли НЕ КонтрольСроков И(НачалоДня(Элемент.Мероприятие.ПланируемаяДата) + (Элемент.Мероприятие.ПланируемоеВремя - Дата(1,1,1)) 
				...
				ИЛИ НЕ КонтрольСроков И Элемент.Мероприятие.ПланируемаяДата < НачалоДня(ТекущаяДата()) Тогда
			Область.ЦветФона = WebЦвета.Красный;
		//--КазанцевЯА-БП
		Иначе
			Область.ЦветФона = WebЦвета.БледноМиндальный;
...
&НаСервере
Процедура ЗаполнитьТДПроизвольный(ТабличныйДокумент, Объект, СписокПользователей, ОтображатьВыполненные, // ЗАЩИЩЕНА!
...
//		ТабличныйДокумент.Область("КолонкаДни").ШиринаКолонки = 0.5;
	...
	Если ФлагДО И Не (ФлагИД или ФлагКА или ФлагПоДоговорам или ФлагПоУслугам) Тогда
		...
		РаспределитьМероприятияИД_На_До(ТабличныйДокумент, Объект, СписокПользователей, ОтображатьВыполненные, 
	...
	ИначеЕсли ФлагДО И ФлагИД И Не (ФлагКА или ФлагПоДоговорам или ФлагПоУслугам) Тогда
		...
		НеРаспределитьМероприятияИД_На_До(ТабличныйДокумент, Объект, СписокПользователей, ОтображатьВыполненные, 
				ГруппироватьМероприятия);
	...
	ИначеЕсли ФлагДО И ФлагКА И ФлагПоДоговорам И Не (ФлагИД или ФлагПоУслугам) Тогда
		...
		РаспределитьМероприятияИД_На_До_УслПоДог_На_ДогКА(ТабличныйДокумент, Объект, СписокПользователей, 
		...
		ЗапланированныеТЗ_Распределять_ИД_На_До_УслПоДог_На_ДогКА(СписокПользователей, ЗапланированныеТЗ, 
	...
	ИначеЕсли ФлагДО И ФлагКА И Не (ФлагПоДоговорам Или ФлагИД или ФлагПоУслугам) Тогда			
		...
		Распределить_ИД_на_ДО_УслПоДог_и_ДогКА_на_КА(ТабличныйДокумент, Объект, СписокПользователей, ОтображатьВыполненные, 
				ГруппироватьМероприятия);
		// Распределение мероприятий ИД на До, УслугПоДоговору и ДоговорыКонтрагентов на Контрагентов (ЗапланированныеТЗ).
		ЗапланированныеТЗ_ИД_на_ДО_УслПоДог_и_ДогКА_на_КА(СписокПользователей, ЗапланированныеТЗ, 
				НеОбновляемЗапланированныеТЗ);  		
	ИначеЕсли ФлагДО И ФлагКА И ФлагИД И Не (ФлагПоДоговорам Или ФлагПоУслугам) Тогда 	
		...
		Распределить_УслПоДог_и_ДогКА_на_КА(ТабличныйДокумент, Объект, СписокПользователей, ОтображатьВыполненные, 
				ГруппироватьМероприятия);
		// Распределение мероприятий УслугПоДоговору и ДоговорыКонтрагентов на Контрагентов (ЗапланированныеТЗ).		
		ЗапланированныеТЗ_УслПоДог_и_ДогКА_на_КА(СписокПользователей, ЗапланированныеТЗ, 
				НеОбновляемЗапланированныеТЗ); 		
	Иначе
