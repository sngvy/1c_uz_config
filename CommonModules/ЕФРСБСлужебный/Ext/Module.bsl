			...
			Для Сч = 0 По Данные.pageData.ВГраница() Цикл
				Попытка
					...
					Если ДатаРождения = ОбъектыСервер.ПолучитьЗначениеСвойства(Справочники.Контрагенты.НайтиПоКоду(Код), "0043") Тогда
						...
						КонецПопытки;
						Попытка
							Управляющий = Данные.pageData[Сч].lastLegalCase.arbitrManagerFio;
						Исключение
							// Выбор управляющего по номеру дела
							Если СтрНайти(Дело, "А",,,) > 0 Тогда
								Управляющий = "Арбитражный суд";
							ИначеЕсли СтрНайти(Дело, "https",,,) > 0 Тогда
								Управляющий = "МФЦ";
							Иначе
								Управляющий = "—";
							КонецЕсли;
