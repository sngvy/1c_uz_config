...
Процедура ЗаполнитьТаблЧастьПоКА(МассивКА)
	...
	Если Результат.Количество() = 0 Тогда
		...
		|ИЗ
		|	РегистрНакопления.ОбъектыВРаботе.Остатки(, Объект.Должник В (&МассивКА)) КАК ОбъектыВРаботеОстатки
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВЫРАЗИТЬ(ОбъектыВРаботеОстатки.Объект КАК Справочник.Контрагенты)
		|ИЗ
		|	РегистрНакопления.ОбъектыВРаботе.Остатки(, Объект В (&МассивКА)) КАК ОбъектыВРаботеОстатки
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Объект
		|;
		...
		Если Эл.Пометка Тогда
			...
			Пока Сч <= ВариантКод.Количество() Цикл
				Если Вычислить("Эл.Вариант" + Строка(Сч)) Тогда
					...
					ЭлОбъект.ФактическаяДата = ТекущаяДатаСеанса();
					Отказ = Ложь;
...
Процедура СоздатьЗадачуДляАвтоинформирвоания(Команда)
	...
	Ф.Объект.Дата = ОбщегоНазначенияКлиент.ДатаСеанса();
