...
Процедура ОбновитьИПНаСервере()
	ДатаРождения = СтрЗаменить(Формат(ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Объект.Объект.Должник, "0043"), "ДФ=yyyy-MM-dd"), " 0:00:00", "");
	ЗапросыФССП.ПолучитьИП(ЭтаФорма.Объект.Объект.Должник, ДатаРождения, ЭтаФорма.Объект.Объект.Должник.Код);
	...
	НовоеМероприятие.Объект = ЭтаФорма.ДолговоеОбязательство.Ссылка;
...
Процедура ИсполнительныеПроизводства(Команда)
	ОбновитьИПНаСервере();
	ПоказатьОповещениеПользователя("Интеграция с ФССП",,"Исполнительные производства обновлены",БиблиотекаКартинок.NC_ПодсистемаФССП);
...
&НаСервере
Процедура ДелаОБанкротствеНаСервере()
	ЗапросыЕФРСБ.ПолучитьСведения(ЭтаФорма.Объект.Объект.Должник, ЭтаФорма.Объект.Объект.Должник.Код);
	// Создать мероприятие розыска
	НовоеМероприятие = Задачи.Мероприятие.СоздатьЗадачу();	
	НовоеМероприятие.Автор = ПараметрыСеанса.ТекущийПользователь;
	НовоеМероприятие.Исполнитель = ПараметрыСеанса.ТекущийПользователь;
...
Процедура ДелаОБанкротстве(Команда)
	ДелаОБанкротствеНаСервере();
	ПоказатьОповещениеПользователя("Интеграция с Федресурсом",,"Дела о банкротстве обновлены",БиблиотекаКартинок.NC_ПодсистемаБанкротство);
	Отбор = Новый Структура("Должник", ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник"));  
