...
Процедура РассчитатьГрафикПоЗайму(Займ, ДатаНачала) Экспорт
	...
	//	ДатаДопСоглашения = ПолучитьДатуДопСоглашения(Договор);
	//	Если ДатаДопСоглашения = Неопределено Тогда
	//		РасчетнаяДата = Макс(ДатаРасчета, Договор.ДатаПогашения);
	//	Иначе
	//		РасчетнаяДата = ДатаДопСоглашения;
	//	КонецЕсли;
	//	Остатки = ПолучитьОстаткиПоДоговору(Договор, ДатаНачала);
	//	РассчитатьГрафикПоДоговору(Договор, ДатаРасчета, Остатки);
	//КонецЦикла;
	
	//Список = Новый ТаблицаЗначений;
	//Список.Колонки.Добавить("Займ", Новый ОписаниеТипов("СправочникСсылка.ДолговыеОбязательства"));
	//стр = Список.Добавить();
	//стр.Займ = Займ;
	//АктуализацияХарактеристикМикрозайма(Займ);
КонецПроцедуры
...
Процедура ПерерасчитатьСУчетомПлатежа(Займ, ДатаПлатежа, СуммаПлатежа, Проведение) Экспорт
	ТекущийДоговор = ПолучитьТекущийДоговор(Займ, НачалоДня(ТекущаяДата()));
	...
	Иначе
		...
		Если Остатки.ОстаткиПроценты = 0 И Проведение Тогда
			...
			//кументОбъект.СуммаВыданногоЗайма = Остатки.ОстаткиОсновнойДолг;
...
Функция ПолучитьОстаткиНаДату(Займ, ДатаОстатков) Экспорт
	...
	Если Договор = Неопределено Тогда
		Возврат Неопределено;
		Сообщить("На данную дату не определен договор микрозайма!");
	КонецЕсли;
...
Процедура РегламентноеЗаданиеПерерасчет() Экспорт
	...
	Если  Результат.Количество() > 0 Тогда
		Для к = 0 по Результат.Количество() - 1 Цикл
			РасчетЗадолженностиМФО.РассчитатьГрафикПоЗайму(Результат[к].Займ, НачалоДня(ТекущаяДата()));	
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПересчетСпискаДО(СписокДО) Экспорт
	Для Каждого Элемент Из СписокДО Цикл
		РасчетЗадолженностиМФО.РассчитатьГрафикПоЗайму(Элемент, НачалоДня(ТекущаяДата()));
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаполнитьДекорацииОбъекта(ФормаОбъекта) Экспорт
	...
	Если ПлановыеПлатежи <> Неопределено Тогда
		Для каждого Платеж из ПлановыеПлатежи Цикл
			Если Платеж.Дата >= ТекущаяДата() И Платеж.СуммаПлатежа <> 0 тогда
	...
	//Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	//                      |	ГрафикПлатежейИНачислений.Договор КАК Договор,
	//                      |	ГрафикПлатежейИНачислений.Займ КАК Займ,
	//                      |	ГрафикПлатежейИНачислений.ОплатаОсновнойДолг КАК ОплатаОсновнойДолг,
	//                      |	ГрафикПлатежейИНачислений.ОплатаПроценты КАК ОплатаПроценты,
	//                      |	ГрафикПлатежейИНачислений.ОплатаПросроченныеПроценты КАК ОплатаПросроченныеПроценты,
	//                      |	ГрафикПлатежейИНачислений.ОплатаШтрафы КАК ОплатаШтрафы,
	//                      |	ГрафикПлатежейИНачислений.ОплатаПени КАК ОплатаПени,
	//                      |	ГрафикПлатежейИНачислений.СуммаПлатежа КАК СуммаПлатежа,
	//                      |	ГрафикПлатежейИНачислений.Дата КАК Дата
	//                      |ИЗ
	//                      |	РегистрСведений.ГрафикПлатежейИНачислений КАК ГрафикПлатежейИНачислений
	//                      |ГДЕ
	//                      |	ГрафикПлатежейИНачислений.Займ = &Займ
	//                      |	И (ГрафикПлатежейИНачислений.ОплатаОсновнойДолг <> 0
	//                      |			ИЛИ ГрафикПлатежейИНачислений.ОплатаПроценты <> 0
	//                      |			ИЛИ ГрафикПлатежейИНачислений.ОплатаПросроченныеПроценты <> 0
	//                      |			ИЛИ ГрафикПлатежейИНачислений.ОплатаШтрафы <> 0
	//                      |			ИЛИ ГрафикПлатежейИНачислений.ОплатаПени <> 0)
	//                      |	И ГрафикПлатежейИНачислений.Дата <= &Дата
	//                      |
	//                      |УПОРЯДОЧИТЬ ПО
	//                      |	Дата УБЫВ");
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.УдалитьИсковыеТребования.СрезПоследних КАК ИсковыеТребованияСрезПоследних
		|ГДЕ
...
Процедура ПечатнаяФормаРачетИсковыхТребованийГрафик(ТабДок, Договор) Экспорт
	...
	ОбластьШапка.Параметры.ТекущаяДата = Формат(ТекущаяДата(), "ДЛФ=Д");
