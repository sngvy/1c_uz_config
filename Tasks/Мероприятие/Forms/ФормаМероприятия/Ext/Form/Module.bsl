			...
			Иначе
				...
				Если ((Объект.Результат = Справочники.РезультатыМероприятий.НайтиПоКоду("000000001")) ИЛИ (Объект.Результат = Справочники.РезультатыМероприятий.НайтиПоКоду("000000002"))) Тогда
					...
					Если ТекущийОтветственный <> Сотрудник Тогда
						...
						Акт.Организация = Справочники.Организации.НайтиПоНаименованию("ООО «Норд Коллект»", Истина);
						Акт.Автор = Сотрудник;
	...
	Если СвязанныйДокумент = ПредопределенноеЗначение("Перечисление.ВидыСвязанныхДокументов.ОбещанныйПлатеж") Тогда
		...
		Если Элемент <> Неопределено Тогда
			Если Не ЗначениеЗаполнено(Объект.СвязанныйДокумент) ИЛИ 
				...
				ПоказатьОповещениеПользователя("Обещанные платежи",,"Новое обещание сохранено",БиблиотекаКартинок.ДиалогИнформация);
...
Процедура ПриОткрытииНаСервере(Отказ)
	...
	Иначе
		...
		ИначеЕсли Причина = ЮД И ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий") Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Объект) = Ложь Тогда
		...
		ЭтаФорма.Объект.Объект = ЭтаФорма.НеопознанныйОбъект;
	КонецЕсли;
	...
	Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
		...
		Если КонтактНайден = Ложь Тогда
			ПоказатьОповещениеПользователя("Предупреждение",,"Не удалось найти контакт должника. Укажите его вручную",БиблиотекаКартинок.Предупреждение32);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) = Истина Тогда
		...
		Если ФормаПоискаНомера.ЕстьСовпадения = Истина Тогда
			ПоказатьОповещениеПользователя("Поиск по номеру в базе",,"Найдены совпадающие должники",БиблиотекаКартинок.Предупреждение32);
		КонецЕсли;
...
Процедура НеопознанныйОбъектНаСервере()
	// Получим непредопределенное значение с сервера
	ЭтаФорма.НеопознанныйОбъект = Справочники.ДолговыеОбязательства.НайтиПоНаименованию("Неопознанный");
КонецПроцедуры
...
Процедура ОбъектПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
		...
		ПолучитьДатуВыдачиНаСервере();
		ПолучитьДниПросрочкиНаСервере();
		ПоказатьОповещениеПользователя("Предупреждение",,"Номер договора был изменен. Проверьте контакт должника",БиблиотекаКартинок.Предупреждение32);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьДолжника" Тогда
		...
		Предупреждение("Контактные данные обновлены. Проверьте контакт должника",1);
	КонецЕсли;
...
Процедура ПрошлоВремя30()
	ПоказатьОповещениеПользователя("Время закрытия",,"Прошло 30 секунд",БиблиотекаКартинок.ДиалогИнформация);
КонецПроцедуры	

&НаКлиенте
Процедура ПрошлоВремя60()
	ПоказатьОповещениеПользователя("Время закрытия",,"Прошло 60 секунд",БиблиотекаКартинок.ДиалогИнформация);
КонецПроцедуры
...
Процедура ЗакрепитьДолжника(Команда)
	НовоеЗакреплениеНаСервере();
	Если ЭтаФорма.Объект.ТипМероприятия = ЭтаФорма.ТипЗаявкаЗакрепление Тогда
		ПоказатьОповещениеПользователя("Закрепление должника",,"Должник закреплен",БиблиотекаКартинок.ДиалогИнформация);	
		ЗакрепитьДолжникаНаСервере();
...
Процедура ОткрепитьДолжника(Команда)
	...
	Если ЭтаФорма.Объект.ТипМероприятия = ЭтаФорма.ТипЗаявкаОткрепление Тогда
		ПоказатьОповещениеПользователя("Открепление должника",,"Должник откреплен",БиблиотекаКартинок.ДиалогИнформация);
...
Процедура ПоискНомераВБазе(Команда)
	ПоказатьОповещениеПользователя("Поиск по номеру в базе",,"Выполняется поиск совпадений...",БиблиотекаКартинок.ДиалогИнформация);
	ФормаПоискаНомера = ПолучитьФорму("ОбщаяФорма.ПоискНомераВБазе");
    // Обрезать неинформативную часть и передать с ключом
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ФормаПоискаНомера.СписокДО, "Ссылка.Должник.Телефоны.Номер", Строка(Прав(Объект.Контакт, 7)), ВидСравненияКомпоновкиДанных.Содержит, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, );
...
Процедура НовоеЗакрепление(Команда)
	Если ЭтаФорма.ДолговоеОбязательство.Пустая() Тогда
		Предупреждение("Не найден договор для закрепления!");
		Возврат;
	Иначе
		НовоеЗакреплениеНаСервере();
...
Процедура ЗапланироватьИЗакрыть(Команда)
	...
	ПоказатьОповещениеПользователя("Планирование мероприятий",,"Мероприятие добавлено в планировщик",БиблиотекаКартинок.ДиалогИнформация);
	ЭтаФорма.Записать();	
	ЭтаФорма.Закрыть();
КонецПроцедуры
