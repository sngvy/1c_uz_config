...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ТипПисьмаКорректен() Тогда
...
Процедура ОграничитьРедактирование()
	...
	ЕстьРегНомер = ЗначениеЗаполнено(Объект.РегистрационныйНомер);
	Элементы.ТипПисьма.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.Объект.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.ГруппаДополнительныеРеквизиты.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.ПрикрепитьФайлы.Доступность = НЕ ЕстьРегНомер;
	Элементы.ПрикрепленныеФайлы1.Доступность = НЕ ЕстьРегНомер;
	Элементы.Организация.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.Подписант.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.Наименование.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.ГруппаПодборФайлов.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.ПодобранныеФайлы.ТолькоПросмотр = ЕстьРегНомер;
	Элементы.ПодборФайлов.Доступность = НЕ ЕстьРегНомер;
	Элементы.ПоШаблону.Доступность = НЕ ЕстьРегНомер;
	Элементы.КатегорияРПО.Доступность = НЕ ЕстьРегНомер;
	Элементы.ВидРПО.Доступность = НЕ ЕстьРегНомер;
	Элементы.ТипКонверта.Доступность = НЕ ЕстьРегНомер;
	Элементы.ГруппаПараметры.Доступность = НЕ ЕстьРегНомер;
	
	Элементы.ЗакрепитьНомер.Доступность = Ложь;
	Если НЕ Объект.НомерЗакреплен И ЗначениеЗаполнено(Объект.РегистрационныйНомер) Тогда
...
#Область КонтрольИзмененияОбъектов
...
&НаСервере
Функция ТипПисьмаКорректен()

	Если НЕ Константы.ПечататьШПИ.Получить() Тогда
	...
	Попытка
		...
		Конверт = Справочники.ТипыИсходящихПисем.НайтиШаблонКонверта(Объект.ТипПисьма);
