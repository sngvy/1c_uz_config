...
Процедура ПреобразоватьОкно(Читатель, Писатель, ЛокальноеИмяПроцедуры)
	...
	Если СозданныеВОкнеИФормеПеременные.Получить(ОбрабатываемоеОкно) = Неопределено Тогда
		...
		Иначе
			...
			ДобавитьСтроку(Писатель, ИмяПеременной + ? (ВариантВстроенногоЯзыка = "ru", " = ТестовоеПриложение.НайтиОбъект(Тип(""ТестируемоеОкноКлиентскогоПриложения""), """, " = TestedApplication.FindIbject(Type(""TestedClientApplicationWindow""), """) + УдвоитьКавычки(ЗаголовокОбъекта) + """, , 30);");
...
Процедура ПреобразоватьКомандныйИнтерфейсОкна(Читатель, Писатель, РодительскаяПеременная)
	...
	ПоследнийОбработанныйЭлементУправления = ПреобразоватьЗаголовокВИмяПеременной(ТипОбъекта, ? (ЗаголовокОбъекта = "", ИмяОбъекта, ЗаголовокОбъекта));
...
Процедура ПреобразоватьКнопкуКомандногоИнтерфейса(Читатель, Писатель, РодительскаяПеременная)
	...
	Если КомандаБылаПреобразована Или СтроитьПолнуюИерархию Тогда
		ВставитьСтроку(Писатель, КоличествоСтрокПередОбработкой + 1, ИмяПеременной + " = " + РодительскаяПеременная + 
				? (ВариантВстроенногоЯзыка = "ru", ".НайтиОбъект(Тип(""ТестируемаяКнопкаКомандногоИнтерфейса""), """, ".FindObject(Type(""TestedCommandInterfaceButton""), """) + 
				УдвоитьКавычки(ЗаголовокОбъекта) + """, """ + ИмяОбъекта + """);");
	КонецЕсли;
...
Процедура ПреобразоватьГруппуКомандногоИнтерфейса(Читатель, Писатель, РодительскаяПеременная)
	...
	Если СтроитьПолнуюИерархию Тогда
		ВставитьСтроку(Писатель, КоличествоСтрокПередОбработкой + 1, ИмяПеременной + " = " + РодительскаяПеременная + 
				? (ВариантВстроенногоЯзыка = "ru", ".НайтиОбъект(Тип(""ТестируемаяГруппаКомандногоИнтерфейса""), """, ".FindObject(Type(""CommandInterfaceGroup""), """) + 
				УдвоитьКавычки(ЗаголовокОбъекта) + """, """ + ИмяОбъекта + """);");
	КонецЕсли;
...
Процедура ПреобразоватьФорму(Читатель, Писатель, РодительскаяПеременная)
	...
	Если ФормыОкна.Получить(ОбрабатываемаяФорма) = Неопределено Тогда
		...
		ДобавитьСтроку(Писатель, ИмяПеременной + " = " + РодительскаяПеременная + 
				? (ВариантВстроенногоЯзыка = "ru", ".НайтиОбъект(Тип(""ТестируемаяФорма""), """, ".FindObject(Type(""TestedForm""), """) + 
				УдвоитьКавычки(ЗаголовокОбъекта) + """, """ + ИмяОбъекта + """);");
...
Функция ПреобразоватьКоманду(Читатель, Писатель, ИмяПеременной)
	...
	ИначеЕсли Читатель.Имя = "finishEditingCurrentArea" Тогда
		...
		Пока Читатель.ПрочитатьАтрибут() Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ДобавитьСтроку(Писатель, ИмяПеременной + ? (ВариантВстроенногоЯзыка = "ru", ".ЗакончитьРедактированиеТекущейОбласти(", ".EndEditingCurrentArea(") + ФлагОтмены + ");", Истина);
		ПоследняяОбработаннаяКоманда = ? (ВариантВстроенногоЯзыка = "ru", "ЗакончитьРедактированиеТекущейОбласти", "EndEditingCurrentArea");
		Читатель.Прочитать();
