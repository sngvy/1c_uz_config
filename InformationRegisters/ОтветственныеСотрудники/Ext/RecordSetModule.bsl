...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Запрос.УстановитьПараметр("Период", ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("Объект", Объект);
	Запрос.УстановитьПараметр("Сотрудник", СтарыйСотрудник);
	Если Запрос.Выполнить().Пустой() Тогда
		Возврат;
	КонецЕсли;
	/////////////////////////////////////////////////////////
	...
	Если Не СтарыйСотрудник.Пустая() Тогда
		...
		Если Запрос.Выполнить().Пустой() Тогда
			...
			ОбъектДок.Дата = ТекущаяДатаСеанса() + 1;
...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Исключение
		Попытка
			Набор.Объект = ЭтотОбъект.Отбор.Объект.Значение;
			Набор.ТипСотрудника = ЭтотОбъект.Отбор.ТипСотрудника.Значение;
			Сотрудник = Справочники.Пользователи.ПустаяСсылка();
		Исключение
			Возврат;
		КонецПопытки;
	КонецПопытки;
	...
	Если Набор.Пользователь <> Сотрудник Тогда
