...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// СообщенияЧата.Параметры.УстановитьЗначениеПараметра("Сотрудник", ПараметрыСеанса.ТекущийПользователь);
	//
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("ПолучитьЗаголовокНачальнойСтраницы", 1, Ложь);
	// Запустить БитФон
	ЗапуститьБитФон();
	...
	ИначеЕсли День(ТекущаяДата()) = 15 Тогда
		ПоказатьОповещениеПользователя("Ротация портфеля",,"Сегодня день ротации",БиблиотекаКартинок.NC_Портфель,СтатусОповещенияПользователя.Важное);
	...
	Если ФормаКонтрольСроков.КоличествоПросроченных > 0 Тогда
		ПоказатьОповещениеПользователя("Контроль сроков",,"Необработанных должников: " + ФормаКонтрольСроков.КоличествоПросроченных + "." + " Обработайте их, чтобы избежать открепления во время ротации",БиблиотекаКартинок.NC_Предупреждение,СтатусОповещенияПользователя.Важное);	
	КонецЕсли;	
	...
	КонецЕсли;
	// Установить доступность настройки оповещений
	Если Не ЭтаФорма.РольРКЦ = Истина И Не ЭтаФорма.РольАБД = Истина Тогда
		ЭтаФорма.Элементы.ГруппаОповещения.ТолькоПросмотр = Истина;
	КонецЕсли;
	// Проверить новые записи в журнале банкротств
	Если ЭтаФорма.РольРКЦ = Истина ИЛИ ЭтаФорма.РольАБД = Истина Тогда
		ПодключитьОбработчикОжидания("ПроверитьОбновленияЖурналаБанкротств", 60, Ложь);
	КонецЕсли;
	// Проверить новые записи в журнале ИП
	Если ЭтаФорма.РольРКЦ = Истина ИЛИ ЭтаФорма.РольАБД = Истина Тогда
		ПодключитьОбработчикОжидания("ПроверитьОбновленияЖурналаИП", 900, Ложь);
	КонецЕсли;
	// Обновить статус софтфона
	ПодключитьОбработчикОжидания("ОбновитьСтатусСофтфона", 30, Ложь);
КонецПроцедуры
...
Процедура EmailНаСервере()
	// Получить значения с сервера
	ЭтаФорма.ТипEmail = Справочники.ТипыМероприятий.НайтиПоНаименованию("Email");
	ЭтаФорма.РезультатОтправлено = Справочники.РезультатыМероприятий.НайтиПоНаименованию("Отправлено");
КонецПроцедуры
...
&НаКлиенте
Процедура СообщенияЧатаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ПараметрыДиалога = Новый Структура;
	ПараметрыДиалога.Вставить(
	"Клиент",
	Элемент.ДанныеСтроки(ВыбраннаяСтрока).ДолговоеОбязательство);
...
Процедура ЗапросКредитору(Команда)
	Если ЭтаФорма.РольРКЦ = Истина Тогда
		...
		КонецЕсли;
	Иначе
		Предупреждение("Недостаточно прав",,"Предупреждение");
	КонецЕсли;
КонецПроцедуры
...
Процедура Email(Команда)
	...
	Если ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.СписокДО И Элементы.СписокДО.ТекущиеДанные <> Неопределено Тогда
		...
		НоваяЗадача = ПолучитьФорму("Задача.Мероприятие.Форма.ФормаМероприятия", Новый Структура("Объект", К),,Новый УникальныйИдентификатор());
		НоваяЗадача.Объект.Объект = К;
		НоваяЗадача.Объект.ТипМероприятия = ТипEmail;
		НоваяЗадача.Объект.Результат = РезультатОтправлено; 
		// НоваяЗадача.Объект.Контакт = РезультатНомер;
		НоваяЗадача.КонтрагентКЛ = Контрагент;
		НоваяЗадача.Открыть();
	...
	Иначе
		Предупреждение("Не выбран объект мероприятия!",,"Email");
	КонецЕсли;
КонецПроцедуры
...
Процедура РасчетАВ(Команда)
	...
	ИначеЕсли ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ПогашенияМесяц И Элементы.ПогашенияМесяц.ТекущиеДанные <> Неопределено Тогда
		ФормаРасчетАВ = ПолучитьФорму("ОбщаяФорма.РасчетАВ",,,Новый УникальныйИдентификатор());
		ФормаРасчетАВ.Кредитор = ПолучитьЗначениеРеквизитаНаСервере(Элементы.ПогашенияМесяц.ТекущиеДанные.Займ, "Кредитор");
...
Процедура АнализПортфеля(Команда)
	...
	Исключение
		Предупреждение("Не найдены объекты для расчета показателей!",,"Анализ портфеля");
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПортфель(Команда)
	Состояние("Обновление портфеля",, "Пожалуйста, подождите...", БиблиотекаКартинок.NC_Портфель);	
	Элементы.СписокДО.Обновить();
	...
	Элементы.ПерспективныеДолжники.Обновить();
	Элементы.ПроблемныеДолжники.Обновить();
...
Процедура ПосчитатьДолжников(Команда)
	...
	Если ЭтаФорма.РольРКЦ = Истина И ЭтаФорма.РольАБД = Истина Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(ЭтаФорма.СписокДО, "Объект.Категория");
	КонецЕсли;
	ПоказатьОповещениеПользователя("Подсчет должников",,"Должников в портфеле: "+ЭтаФорма.КоличествоВРаботе,БиблиотекаКартинок.NC_Портфель);
КонецПроцедуры
...
&НаКлиенте
Процедура ИнформацияОВерсии(Команда)
...
Процедура ПроверитьОбновленияКонфигурации()
	...
	НоваяВерсияКонфигурацииNC = "6.6.26.1";
	Если ЭтаФорма.ВерсияКонфигурацииNC <> НоваяВерсияКонфигурацииNC Тогда
		...
		ОбслуживаниеПослеОбновления();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбслуживаниеПослеОбновления();
	ФормаОбслуживания = ПолучитьФорму("ОбщаяФорма.ОбслуживаниеПослеОбновления");
	ФормаОбслуживания.Открыть();
КонецПроцедуры
...
&НаКлиенте
Процедура ПроверитьОбновленияЖурналаБанкротств()
	Если ЭтаФорма.ОповещенияЕФРСБ = Истина Тогда
		ПроверитьОбновленияЖурналаБанкротствНаСервере();
		Если ЭтаФорма.КоличествоЗаписейВЖурналеБанкротствСлужебное < ЭтаФорма.КоличествоЗаписейВЖурналеБанкротств Тогда
			ПоказатьОповещениеПользователя("Интеграция с Федресурсом",,"Есть новые записи в журнале банкротств",БиблиотекаКартинок.NC_ПодсистемаБанкротство,СтатусОповещенияПользователя.Важное);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПроверитьОбновленияЖурналаБанкротствНаСервере()
	ЭтаФорма.КоличествоЗаписейВЖурналеБанкротствСлужебное = Константы.КоличествоЗаписейВЖурналеБанкротств.Получить();
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	ЭтаФорма.КоличествоЗаписейВЖурналеБанкротств = Выборка.Количество();
	Константы.КоличествоЗаписейВЖурналеБанкротств.Установить(Выборка.Количество());
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьОбновленияЖурналаИП()
	Если ЭтаФорма.ОповещенияФССП = Истина Тогда
		ПроверитьОбновленияЖурналаИПНаСервере();
		Если ЭтаФорма.КоличествоЗаписейВЖурналеИПСлужебное < ЭтаФорма.КоличествоЗаписейВЖурналеИП Тогда
			ПоказатьОповещениеПользователя("Интеграция с ФССП",,"Есть новые ИП по должникам",БиблиотекаКартинок.NC_ПодсистемаФССП,СтатусОповещенияПользователя.Важное);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПроверитьОбновленияЖурналаИПНаСервере()
	ЭтаФорма.КоличествоЗаписейВЖурналеИПСлужебное = Константы.КоличествоЗаписейВЖурналеИП.Получить();
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ИсполнительныеПроизводства.ИП ПОДОБНО ""%-ИП%""";
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	ЭтаФорма.КоличествоЗаписейВЖурналеИП = Выборка.Количество();
	Константы.КоличествоЗаписейВЖурналеИП.Установить(Выборка.Количество());
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСтатусСофтфона()
	Попытка
		ЭтаФорма.Телефон = СвязанныйНомер() + " - " + СтатусСофтфона();
	Исключение
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьЗаголовокНачальнойСтраницы()
	ЭтаФорма.Заголовок = "Текущие дела — " + ПользователиКлиентСервер.ТекущийПользователь() + " — " + Формат(ТекущаяДата(), "ДФ='dd.MM.yyyy, dddd, HH:mm'");
КонецПроцедуры

&НаКлиенте
Процедура ЗапуститьБитФон()
	Если ЭтаФорма.ЗапускатьБИТPhone = Истина Тогда
		ФормаБитФона = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
		ФормаБитФона.Открыть();
		ИнтеграцииСлужебный.Пауза(1);
		ФормаБитфона.кнПодключить(Неопределено);
	КонецЕсли;
КонецПроцедуры

Функция СвязанныйНомер()
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	бит_БитАТСНастройки.Пользователь КАК Пользователь,
	|	бит_БитАТСНастройки.НомерСвязанногоТелефона КАК НомерСвязанногоТелефона
	|ИЗ
	|	РегистрСведений.бит_БитАТСНастройки КАК бит_БитАТСНастройки
	|ГДЕ
	|	бит_БитАТСНастройки.Пользователь = &Пользователь";
	Запрос.УстановитьПараметр("Пользователь", ПараметрыСеанса.ТекущийПользователь);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Выборка.Следующий();
	Возврат Выборка.НомерСвязанногоТелефона;
КонецФункции

&НаКлиенте
Функция СтатусСофтфона()
	ФормаСофтфона = ПолучитьФорму("Обработка.бит_Битфон.Форма.ОсновнаяФорма");
	Если ФормаСофтфона.ВызовУдержан Тогда
		Возврат "Удержание";
	ИначеЕсли ФормаСофтфона.ИдетРазговор Тогда
		Возврат "В разговоре";
	ИначеЕсли ФормаСофтфона.СтатусНеБеспокоить Тогда
		Возврат "Не беспокоить";
	ИначеЕсли ФормаСофтфона.СтатусПодключен Тогда
		Возврат "На линии";
	Иначе
		Возврат "Отключен";
	КонецЕсли;
...
&НаКлиенте
Процедура УправлениеОстатками(Команда)
	Если ЭтаФорма.РольРКЦ Или ЭтаФорма.РольАБД Тогда
		ФормаУправление = ПолучитьФорму("ОбщаяФорма.УправлениеОстатками");
		ФормаУправление.Открыть();
	Иначе
		Предупреждение("Недостаточно прав",,"Управление остатками");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьДО(Команда)
	Если ЭтаФорма.НомерДО = "" Тогда
		Предупреждение("Не заполнен номер ДО!",,"Предупреждение");
		Возврат;
	ИначеЕсли ПолучитьДО(ЭтаФорма.НомерДО) <> ПустаяСсылкаНаДО() Тогда
		ОткрытьЗначение(ПолучитьДО(ЭтаФорма.НомерДО));
	Иначе
		Предупреждение("Объект не найден!",,"Предупреждение");
	КонецЕсли;
КонецПроцедуры

Функция ПолучитьДО(НомерДО)
	Возврат Справочники.ДолговыеОбязательства.НайтиПоНаименованию(НомерДО);
КонецФункции

Функция ПустаяСсылкаНаДО()
	Возврат Справочники.ДолговыеОбязательства.ПустаяСсылка();
КонецФункции

&НаСервере
Процедура ЗапуститьIVRНаСервере()
	ФоновыеЗадания.Выполнить("IVRСлужебный.НастройкаИЗапускIVR",,,"Запуск IVR");
КонецПроцедуры

&НаКлиенте
Процедура ЗапуститьIVR(Команда)
	Если ЭтаФорма.РольРКЦ Или ЭтаФорма.РольАБД Тогда
		ПоказатьОповещениеПользователя("Запуск IVR",,"Задание запущено в фоновом режиме",БиблиотекаКартинок.NC_ПодсистемаНастройкиАвтообзвона);
		ЗапуститьIVRНаСервере();
	Иначе
		Предупреждение("Недостаточно прав",,"Запуск IVR");
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ПропущенныеВызовы(Команда)
	Если ЭтаФорма.РольРКЦ Или ЭтаФорма.РольАБД Тогда
		ФормаПропущенных = ПолучитьФорму("ОбщаяФорма.ПропущенныеВызовы");
		ФормаПропущенных.Открыть();
	Иначе
		Предупреждение("Недостаточно прав",,"Пропущенные вызовы");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СменитьРежимМероприятий(Команда)
	Если ЭтаФорма.РольРКЦ Или ЭтаФорма.РольАБД Тогда
		Если ПолучитьРежимМероприятий() Тогда
			СменитьРежимМероприятийНаСервере(Ложь);		
			ПоказатьОповещениеПользователя("Режим мероприятий",,"Автозакрытие мероприятий выключено",БиблиотекаКартинок.NC_Выполнено);
		Иначе
			СменитьРежимМероприятийНаСервере(Истина);		
			ПоказатьОповещениеПользователя("Режим мероприятий",,"Автозакрытие мероприятий включено",БиблиотекаКартинок.NC_Выполнено);
		КонецЕсли;
	Иначе
		Предупреждение("Недостаточно прав",,"Режим мероприятий");
	КонецЕсли;
КонецПроцедуры

Функция СменитьРежимМероприятийНаСервере(Значение)
	Константы.АвтоведениеМероприятий.Установить(Значение);
КонецФункции

Функция ПолучитьРежимМероприятий()
	Возврат Константы.АвтоведениеМероприятий.Получить();
КонецФункции
