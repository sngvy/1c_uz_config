...
Функция ПроконтролироватьНастройки()
	...
	ТекущийОбъект = ЭтотОбъект();
	Если УзелОбменаСсылка <> Неопределено И УзелОбменаСсылка <> ПланыОбмена.бит_ОбменАТС.ПустаяСсылка() И ПланыОбмена.ТипВсеСсылки().СодержитТип(ТипЗнч(УзелОбменаСсылка)) Тогда
