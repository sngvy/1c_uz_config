...
Процедура Отправить(Команда)
	Если Не ЭтаФорма.Шаблон.Пустая() Тогда
		...
		Для Каждого Стр Из СписокРассылки Цикл
			...
			ФормаОтправки.Отправитель = ПолучитьОтправителя();
			...
			Ном = Ном + 1;
		КонецЦикла;
	ПоказатьОповещениеПользователя("Отправка СМС",,"Сообщения отправлены",БиблиотекаКартинок.NC_СМС);
	Иначе
		Предупреждение("Не выбран шаблон!",,"Отправка СМС");
	КонецЕсли;
КонецПроцедуры

Функция ПолучитьОтправителя()
	Возврат Справочники.УчетныеЗаписиSMS4B.НайтиПоКоду("000000001").Отправитель;
КонецФункции
