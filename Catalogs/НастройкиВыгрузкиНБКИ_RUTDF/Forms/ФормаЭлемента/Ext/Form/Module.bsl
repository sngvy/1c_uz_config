...
Процедура УстановитьОформление()
	...
	Элементы.ГруппаПравопреемство.Видимость = Истина;
...
Процедура ПервичноеЗаполнениеПоОдномуДО(Команда) Экспорт
	...
	ПараметрыЗапуска.Вставить("ИсточникСобытия", Объект.Ссылка);
	ПараметрыЗапуска.Вставить("ДатаСобытия", ДатаСтарта);
	ПараметрыЗапуска.Вставить("Договор", ЭтаФорма.Договор);
	...
	Если ЗначениеЗаполнено(ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Должник, "0049")) = Ложь Тогда
		Предупреждение("Не найден адрес регистрации должника! Формирование событий КИ невозможно",,"Формирование событий КИ");
		Возврат;
	КонецЕсли;
	ПолучитьОКАТОАхантер();
	//ПервичноеЗаполнениеСерв(ПараметрыЗапуска); 
	...
	// ДлительныеОперацииКлиент.ОжидатьЗавершение(СтруктураФоновогоЗадания, Новый ОписаниеОповещения("ОбработатьДанные", ЭтотОбъект), ПараметрыОжидания);
	ПоказатьОповещениеПользователя("Обмен с НБКИ",,"События КИ сформированы",БиблиотекаКартинок.NC_ПодсистемаОбменСНБКИ);
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьОКАТОАхантер()
	ФормаАхантер = ПолучитьФорму("ОбщаяФорма.Ахантер");
	ФормаАхантер.Адрес = ОбъектыСервер.ПолучитьЗначениеСвойства(ЭтаФорма.Должник, "0049");
	ФормаАхантер.ОткрытьМодально();
КонецПроцедуры
...
Процедура ДоговорПриИзменении(Элемент)
	ЭтаФорма.Должник = ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Договор, "Должник");
КонецПроцедуры
