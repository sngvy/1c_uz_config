...
Процедура ОбновитьСобытияКИ()
	ПоказатьОповещениеПользователя("Обмен с НБКИ",,"События КИ обновлены",БиблиотекаКартинок.NC_ПодсистемаОбменСНБКИ);
...
Процедура ОбновитьДелаОБанкротствеНаСервере()
	ЗапросыЕФРСБ.ПолучитьСведения(ЭтаФорма.Объект.Должник, ЭтаФорма.Объект.Должник.Код);
КонецПроцедуры
