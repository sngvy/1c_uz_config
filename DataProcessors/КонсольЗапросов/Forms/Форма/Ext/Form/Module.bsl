...
Процедура ПередЗакрытием(Отказ,ЗавершениеРаботы,ТекстПредупреждения,СтандартнаяОбработка)
	Если Модифицированность = Истина Тогда
		...
		Иначе
			...
			ПодключитьОбработчикОжидания("СохранитьНаработки", 1, Истина);
		КонецЕсли;
	КонецЕсли;
	//СохранитьНаработки();
КонецПроцедуры
...
Процедура СкопироватьЗапрос(Команда)
	...
	Если ТекущийЗапрос <> Неопределено Тогда
		Если ТекущийЗапрос.ПолучитьРодителя() <> Неопределено Тогда
			ГридЗапросов.СкопироватьСтроку();
		Иначе
...
Процедура СохранитьКак(Команда)
	...
	Если Диалог.Выбрать() Тогда
		...
		Если Проверка.Существует() Тогда
			Если Вопрос("Файл '" + ПутьКФайлу + "' существует. Заменить?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
				ПреобразоватьИСохранитьДерево(ПутьКФайлу);
				ПутьКФайлу = Путь;
			КонецЕсли;
		Иначе
...
Процедура ПолеРезультатаЗапросаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	Если ТипЗнч(Расшифровка) = Тип("ТабличныйДокумент") Тогда
		...
		//Чуров
		ОткрытьФорму(ФормаВложеннойТаблицы);
...
&НаКлиенте
Процедура СохранитьНаработки() Экспорт
...
Процедура ПреобразоватьИСохранитьДерево(ПутьКФайлу)
	...
	НачатьПолучениеФайлаССервера(АдресХранилища,ПутьКФайлу);
...
Функция ВызовВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, СпособВыгрузки)
	Возврат ПолучитьОбработку().ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, СпособВыгрузки, СоставРезультатов);
КонецФункции
