...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	// Заблокировать мероприятие для редактирования другими сорудниками
	Попытка
	...
	Попытка
		...
		Если Не Объект.Выполнена Тогда
			...
			Иначе
				...
				//
				//
				// Добавить результат с кодом 000000031 для обещаний с WhatsApp
				Если Объект.Результат = Справочники.РезультатыМероприятий.НайтиПоКоду("000000001")
					...
					ИЛИ Объект.Результат = Справочники.РезультатыМероприятий.НайтиПоКоду("000000031") Тогда
...
Процедура ПриОткрытииНаСервере()
	...
	Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
		...
		Если ЭтаФорма.Элементы.ГруппаИнструменты.Видимость = Ложь Тогда
			...
			ЭтаФорма.Элементы.СвязанныйДокумент.Видимость = Истина;
		КонецЕсли;
		//
	...
	ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Ложь И ЗначениеЗаполнено(ЭтаФорма.Контрагент) = Ложь Тогда
		...
		ЭтаФорма.Элементы.СвязанныйДокумент.Видимость = Ложь;
	КонецЕсли;
	//
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтаФорма.Доступность = Ложь И ЭтаФорма.Модифицированность = Ложь Тогда
		ЭтаФорма.Закрыть();
	...
	// Проверить категорию и причину архивации
	...
	Если ЭтаФорма.Объект.ТипМероприятия <> ПредопределенноеЗначение("Справочник.ТипыМероприятий.ТелефонныйЗвонокВходящий")
		...
		КонтактНайден = Ложь;
		Пока НачалоСпискаВыбора < ЭтаФорма.Элементы.Контакт.СписокВыбора.Количество() И КонтактНайден = Ложь Цикл
		...
		Если КонтактНайден = Ложь Тогда
			ПоказатьОповещениеПользователя("Предупреждение",,"Не удалось найти подходящий контакт. Укажите его вручную",БиблиотекаКартинок.NC_Предупреждение);
		КонецЕсли;
	КонецЕсли;
	...
	ЭтаФорма.Элементы.ГруппаКонтакт.Доступность = Ложь;
	
	// Передать управление обработчику
	ПодключитьОбработчикОжидания("УстановитьСтатусНеБеспокоить", 1, Истина);
...
Процедура УстановитьСтатусНеБеспокоить()
	...
	Если ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусНеБеспокоить = Ложь И ФормаНабора.ИдетРазговор = Ложь Тогда
		...
		ПодключитьОбработчикОжидания("ВключитьДоступностьГруппыКонтакт", 3, Истина);
	Иначе
		// Разрешить управление звонком
		ЭтаФорма.Элементы.ГруппаКонтакт.Доступность = Истина;
	КонецЕсли;
	// Передать управление обработчику
...
&НаКлиенте
Процедура ВключитьДоступностьГруппыКонтакт()
	ЭтаФорма.Элементы.ГруппаКонтакт.Доступность = Истина;
КонецПроцедуры
...
Процедура ТипМероприятияПриИзменении(Элемент)
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ЭтаФорма.Объект.ТипМероприятия) = Ложь
			ИЛИ ЭтаФорма.Объект.ТипМероприятия <> ТипЗаявкаОткрепление
			ИЛИ ЭтаФорма.Объект.ТипМероприятия <> ТипЗаявкаЗакрепление Тогда
			ЭтаФорма.Элементы.ОткрепитьДолжника.Видимость = Ложь;
			ЭтаФорма.Элементы.ЗакрепитьДолжника.Видимость = Ложь;
		КонецЕсли;
			...
			Иначе
				...
				Если ТипЗнч(Результат.Свойство) = Тип("СправочникСсылка.ВидыТелефонов") Тогда
					...
					Если ЗначениеЗаполнено(Результат.Комментарий) Тогда
						// Выгрузить тип и комментарий
						Абонент = Строка(Результат.Свойство) + " " + "(" + Результат.Комментарий + ")";
						//
					КонецЕсли;
				Иначе
	...
	Если Не Отказ Тогда
		Попытка
			...
			Если ФормаНабора.Открыта() = Истина И ФормаНабора.СтатусПодключен = Ложь И ФормаНабора.ИдетРазговор = Ложь
				...
				ПоказатьОповещениеПользователя("БИТ.Phone",,"Вы на линии",БиблиотекаКартинок.NC_ОператорНаЛинии);
...
Процедура НаписатьВВЦ(Команда)
	Если ЗначениеЗаполнено(ЭтаФорма.Объект.Контакт) Тогда
		Если ЗначениеЗаполнено(ЭтаФорма.ДолговоеОбязательство) = Истина Тогда
			...
			Если КонтрольСобытий.КонтрольВЦ(ОбъектДолжник) Тогда
...
Процедура ПроверкаНаБанкротство(Команда)
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
		ПроверкаПоФедресурсу = "https://bankrot.fedresurs.ru/bankrupts?searchString=" + ПолучитьЗначениеРеквизитаНаСервере(ЭтаФорма.Объект.Объект, "Должник");
		ПоказатьОповещениеПользователя("Проверка на банкротство",,"Запуск приложения...",БиблиотекаКартинок.NC_ПодсистемаБанкротство);
