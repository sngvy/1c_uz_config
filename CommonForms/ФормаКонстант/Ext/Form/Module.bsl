...
Процедура ПриОткрытии(Отказ)
	...
	ИспользоватьПодключениеМессенджеровПриИзменении(Неопределено);
КонецПроцедуры
...
&НаКлиенте
Процедура АдресИсполняемогоФайлаJavaНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	Если Диалог.Выбрать() Тогда
		НаборКонстант.АдресИсполняемогоФайлаJava = Диалог.ПолноеИмяФайла;
	КонецЕсли;
КонецПроцедуры
...
Процедура ИспользоватьНБКИRUTDFПриИзмененииНаСервере()
	Если КредитныеИстории.ПустыеНастройки() Тогда
		...
		Сообщение.Сообщить();
	КонецЕсли;
	Если Константы.ИспользоватьНБКИRUTDF.Получить() Тогда
		КредитныеИстории.ЗаполнитьПервичныеСправочникиНБКИ();
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Технические настройки применились к БД.";
		Сообщение.Сообщить();
...
&НаКлиенте
Процедура ПередачаДанныхВБКИПриИзменении(Элемент)
	ИспользоватьНБКИRUTDFПриИзмененииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьПодключениеМессенджеровПриИзменении(Элемент)
	
	Элементы.ГруппаМессенджеры.Видимость = НаборКонстант.ИспользоватьПодключениеМессенджеров;
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкиБотов(Команда)
	
	ОткрытьЗначение(
		ПредопределенноеЗначение("Справочник.НастройкиЧатовТелеграм.Телеграм"));
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкиСохраненияИстории(Команда)
	
	ОткрытьФорму("Справочник.НастройкиИсторииЧатовФайловаяСистема.Форма.ФормаЭлемента");
	
КонецПроцедуры

&НаКлиенте
Процедура ШаблонСообщенияЧатаПоТЗПриИзменении(Элемент)
	
	Текст = ПроверкаНаСоответствиеТЗ(НаборКонстант.ШаблонСообщенияЧатаПоТЗ);
	
	Если Не ПустаяСтрока(Текст) Тогда
	
		НаборКонстант.ШаблонСообщенияЧатаПоТЗ = Неопределено;
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(
			"В Шаблоне должны быть поля %1",
			Текст);
		Сообщение.Поле = "НаборКонстант.ШаблонСообщенияЧатаПоТЗ";
		Сообщение.Сообщить();
...
&НаСервереБезКонтекста
Функция ПроверкаНаСоответствиеТЗ(Шаблон)

	Результат = Константы.ШаблонСообщенияЧатаПоТЗ.ШаблонСоответствуетТЗ(Шаблон);
	
	Если Не Результат Тогда
		
		Возврат СтрСоединить(
			Константы.ШаблонСообщенияЧатаПоТЗ.ПроверяемыеТеги(),
			", ");
