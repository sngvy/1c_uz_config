...
Процедура ВыполнитьОбновлениеИнформационнойБазы() Экспорт
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.5.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.4.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
		
		// Действия по переносу
		КредитныеИсторииИзменения.ИсторияКонтроляВерсий();
		КредитныеИстории.ТипЗначенияДляВерсииНБКИ();
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.7.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.6.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.8.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.7.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.9.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.8.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.10.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.9.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.11.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.10.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.12.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.11.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.7.13.18";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.12.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии       
		РаботаСОтчетами371318();
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
		ЗаписьЖурналаРегистрации("ИнформационнаяБаза.ОбновлениеИБ", УровеньЖурналаРегистрации.Информация, , , "Выполнено " + СтрСообщения);
	КонецЕсли;
	
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.0.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.7.13.18" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);		
		// Обновление версии       
		Константы.НомерВерсииКонфигурации.Установить(ВерсияМетаданных);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.1.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.8.0.20" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
		//1 Заполнение Версии АПИ почты РФ  
		УстановитьВерсиюAPIДляВсехУчетныхЗаписейПочтыРФ();

		//2 процедуру заполнения порядка версий КредитныеИсторииИзменения.ЗаполнитьПорядокВерсийРУТДФ 
		КредитныеИсторииИзменения.ЗаполнитьПорядокВерсийРУТДФ();
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.2.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.8.1.20" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
		
		РаботаСОтчетами38220();
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.3.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.8.2.20" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.4.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.8.3.20" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
	...
	ВерсияТекущая = Константы.НомерВерсииКонфигурации.Получить();
	ВерсияМетаданных = "3.8.5.20";
	Если ВерсияТекущая <> "" И ВерсияТекущая = "3.8.4.20" Тогда
		СтрСообщения = "обновление информационной базы с версии " + ВерсияТекущая + " на версию " + ВерсияМетаданных;
		Сообщить("Выполняется " + СтрСообщения);
...
&НаСервере
Процедура РаботаСОтчетами371318() Экспорт  
	мНерабочиеОтчеты = ОбновлениеИБ.ПолучитьСписокНерабочихОтчетов();  	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ХранилищеВнешнихОтчетов.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ХранилищеВнешнихОтчетов КАК ХранилищеВнешнихОтчетов
		|ГДЕ
		|	ХранилищеВнешнихОтчетов.ИмяОтчета В (&ИменаОтчетов)";
	
	Запрос.УстановитьПараметр("ИменаОтчетов", мНерабочиеОтчеты);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
		ЭлОтчет = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ЭлОтчет.СкрытьОтПользователей = Истина;
		ЭлОтчет.Записать();
	КонецЦикла;
	
	мТехническиеОтчеты = ОбновлениеИБ.ПолучитьСписокТехническихОтчетов();
	Запрос.УстановитьПараметр("ИменаОтчетов", мТехническиеОтчеты);  
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();  
	
	ГруппаТехническиеОтчеты = Справочники.ХранилищеВнешнихОтчетов.СоздатьГруппу();
	ГруппаТехническиеОтчеты.Родитель = Справочники.ХранилищеВнешнихОтчетов.Предопределенные;
	ГруппаТехническиеОтчеты.Наименование = "Технические отчеты";  
	ГруппаТехническиеОтчеты.Записать();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
		ЭлОтчет = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		ЭлОтчет.Родитель = ГруппаТехническиеОтчеты.Ссылка;
		ЭлОтчет.Записать();
	КонецЦикла;
...
&НаСервере
Функция ПолучитьСписокНерабочихОтчетов() Экспорт 
	МассивИменМетаданных  = Новый Массив;
	МассивИменМетаданных.Добавить("РезультатыАвтоинформирования");
	МассивИменМетаданных.Добавить("Отчет_ЮрСлужба");
	МассивИменМетаданных.Добавить("СведенияОПользователях");
	МассивИменМетаданных.Добавить("ОтчетДляПланировщика");
	МассивИменМетаданных.Добавить("ОтчетДляПланировщикаМФО"); 
	МассивИменМетаданных.Добавить("ОтчетПоИсковому");
	МассивИменМетаданных.Добавить("ОтчетПоИсполнительному");
	Возврат МассивИменМетаданных; 
КонецФункции    

&НаСервере
Функция ПолучитьСписокТехническихОтчетов() Экспорт 
	МассивИменМетаданных  = Новый Массив;
	МассивИменМетаданных.Добавить("ОтчетЧасовыеПояса");
	МассивИменМетаданных.Добавить("ПрогрессОтложенногоОбновления");
	МассивИменМетаданных.Добавить("ТриСуммы");
	МассивИменМетаданных.Добавить("ПлатежиПоДО");
	Возврат МассивИменМетаданных; 
КонецФункции

&НаСервере
Процедура УстановитьВерсиюAPIДляВсехУчетныхЗаписейПочтыРФ() Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	НЕ УчетныеЗаписиПочтыРФ.ПометкаУдаления");	
	Результат = Запрос.Выполнить().Выбрать();
	
	Пока Результат.Следующий() Цикл
		СправочникОбъект = Результат.Ссылка.ПолучитьОбъект();
		СправочникОбъект.ВерсияAPI = Перечисления.ПочтаРФ_ВерсияAPI.Версия_1_0;
		СправочникОбъект.Записать();
	КонецЦикла;
...
&НаСервере
Процедура РаботаСОтчетами38220() Экспорт 
	
	СпрОтчеты = Справочники.ХранилищеВнешнихОтчетов; 
	
	ИПОтчет = СпрОтчеты.НайтиПоРеквизиту("ИмяОтчета", "ОтчетПоИсковому");
	
	Если ИПОтчет <> СпрОтчеты.ПустаяСсылка() Тогда
		ИПОтчетОбъект = ИПОтчет.ПолучитьОбъект(); 
		ИПОтчетОбъект.СкрытьОтПользователей = Ложь;
		ИПОтчетОбъект.Записать();
	КонецЕсли;	
	
	ИПОтчет = СпрОтчеты.НайтиПоРеквизиту("ИмяОтчета", "ОтчетПоИсполнительному");
